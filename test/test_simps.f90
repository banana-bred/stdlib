...
contains
    ...
    subroutine collect_simps(testsuite)
        ...
        testsuite = [ &
            ...
            , new_unittest("simps_xdp", test_simps_xdp) &
            ...
            , new_unittest("simps_weights_xdp", test_simps_weights_xdp) &
            ...
            , new_unittest("simps_zero_xdp", test_simps_zero_xdp) &
            ...
            , new_unittest("simps_even_xdp", test_simps_even_xdp) &
            ...
            , new_unittest("simps_weights_even_xdp", test_simps_weights_even_xdp) &
            ...
            , new_unittest("simps_six_xdp", test_simps_six_xdp) &
    ...
    subroutine test_simps_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        val = simps(y, 1.0_xdp)
        ans = 576.0_xdp
        call check(error, val, ans, thr=tol_xdp)
        if (allocated(error)) return
        ...
        x = [(0.25_xdp*(i-1), i = 1, n)]
        val = simps(y, x)
        ans = 144.0_xdp
        call check(error, val, ans, thr=tol_xdp)
    end subroutine test_simps_xdp

    subroutine test_simps_weights_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        ans = simps(y, x)
        call check(error, val, ans, thr=tol_xdp)
    end subroutine test_simps_weights_xdp
    
    subroutine test_simps_zero_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        real(xdp), dimension(0) :: a


        call check(error, abs(simps(a, 1.0_xdp)) < epsilon(0.0_xdp))
        if (allocated(error)) return
        call check(error, abs(simps([1.0_xdp], 1.0_xdp)) < epsilon(0.0_xdp))
        if (allocated(error)) return
        call check(error, abs(simps(a, a)) < epsilon(0.0_xdp))
        if (allocated(error)) return
        call check(error, abs(simps([1.0_xdp], [1.0_xdp])) < epsilon(0.0_xdp))
    end subroutine test_simps_zero_xdp

    subroutine test_simps_even_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
    ...
    end subroutine test_simps_even_xdp

    subroutine test_simps_weights_even_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
    ...
    end subroutine test_simps_weights_even_xdp

    subroutine test_simps_six_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
    ...
    end subroutine test_simps_six_xdp
