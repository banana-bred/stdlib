...
module test_stats_mean
    ...
    real(xdp) :: d2_xdp(3, 6) = reshape(d1_xdp, [3, 6])
    real(xdp) :: d3_xdp(3, 2, 3) = reshape(d1_xdp, [3, 2, 3])
    real(xdp) :: d4_xdp(3, 2, 3, 2) = reshape(d1_xdp, [3, 2, 3, 2], [real(xdp) :: 3])
    real(qp) , parameter :: d1_qp(18) = [-10, 2, 3, 4, -6, 6, -7, 8, 9, 4, 1, -20, 9, 10, 14, 15, 40, 30]
    real(qp) :: d2_qp(3, 6) = reshape(d1_qp, [3, 6])
    real(qp) :: d3_qp(3, 2, 3) = reshape(d1_qp, [3, 2, 3])
    real(qp) :: d4_qp(3, 2, 3, 2) = reshape(d1_qp, [3, 2, 3, 2], [real(qp) :: 3])
    ...
    complex(xdp) :: d2_cxdp(3, 6) = reshape(d1_cxdp, [3, 6])
    complex(xdp) :: d3_cxdp(3, 2, 3) = reshape(d1_cxdp, [3, 2, 3])
    complex(xdp) :: d4_cxdp(3, 2, 3, 2) = reshape(d1_cxdp, [3, 2, 3, 2], [complex(xdp) :: (3, -2)] )
    complex(qp) , parameter :: d1_cqp(18) = d1_qp
    complex(qp) :: d2_cqp(3, 6) = reshape(d1_cqp, [3, 6])
    complex(qp) :: d3_cqp(3, 2, 3) = reshape(d1_cqp, [3, 2, 3])
    complex(qp) :: d4_cqp(3, 2, 3, 2) = reshape(d1_cqp, [3, 2, 3, 2], [complex(qp) :: (3, -2)] )
...
contains
    ...
    subroutine collect_stats_mean(testsuite)
        ...
        testsuite = [ &
            ...
            ,new_unittest("test_stats_mean_all_xdp", test_stats_mean_all_xdp) &
            , new_unittest("test_stats_mean_all_optmask_xdp", test_stats_mean_all_optmask_xdp) &
            , new_unittest("test_stats_mean_xdp", test_stats_mean_xdp) &
            , new_unittest("test_stats_mean_optmask_xdp", test_stats_mean_optmask_xdp) &
            , new_unittest("test_stats_mean_mask_all_xdp", test_stats_mean_mask_all_xdp) &
            , new_unittest("test_stats_mean_mask_xdp", test_stats_mean_mask_xdp) &
            ,new_unittest("test_stats_mean_all_qp", test_stats_mean_all_qp) &
            , new_unittest("test_stats_mean_all_optmask_qp", test_stats_mean_all_optmask_qp) &
            , new_unittest("test_stats_mean_qp", test_stats_mean_qp) &
            , new_unittest("test_stats_mean_optmask_qp", test_stats_mean_optmask_qp) &
            , new_unittest("test_stats_mean_mask_all_qp", test_stats_mean_mask_all_qp) &
            , new_unittest("test_stats_mean_mask_qp", test_stats_mean_mask_qp) &
            ...
            ,new_unittest("test_stats_mean_all_cxdp", test_stats_mean_all_cxdp) &
            , new_unittest("test_stats_mean_all_optmask_cxdp", test_stats_mean_all_optmask_cxdp) &
            , new_unittest("test_stats_mean_cxdp", test_stats_mean_cxdp) &
            , new_unittest("test_stats_mean_optmask_cxdp", test_stats_mean_optmask_cxdp) &
            , new_unittest("test_stats_mean_mask_all_cxdp", test_stats_mean_mask_all_cxdp) &
            , new_unittest("test_stats_mean_mask_cxdp", test_stats_mean_mask_cxdp) &
            ,new_unittest("test_stats_mean_all_cqp", test_stats_mean_all_cqp) &
            , new_unittest("test_stats_mean_all_optmask_cqp", test_stats_mean_all_optmask_cqp) &
            , new_unittest("test_stats_mean_cqp", test_stats_mean_cqp) &
            , new_unittest("test_stats_mean_optmask_cqp", test_stats_mean_optmask_cqp) &
            , new_unittest("test_stats_mean_mask_all_cqp", test_stats_mean_mask_all_cqp) &
            , new_unittest("test_stats_mean_mask_cqp", test_stats_mean_mask_cqp) &
            ]
    ...
    subroutine test_stats_mean_all_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_xdp), sum(d1_xdp)/real(size(d1_xdp), xdp)&
                    , 'mean(d1_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d2_xdp), sum(d2_xdp)/real(size(d2_xdp), xdp)&
                    , 'mean(d2_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d3_xdp), sum(d3_xdp)/real(size(d3_xdp), xdp)&
                    , 'mean(d3_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d4_xdp), sum(d4_xdp)/real(size(d4_xdp), xdp)&
                    , 'mean(d4_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_all_optmask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(mean(d1_xdp, .false.))&
                    , 'mean(d1_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d2_xdp, .false.))&
                    , 'mean(d2_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d3_xdp, .false.))&
                    , 'mean(d3_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d4_xdp, .false.))&
                    , 'mean(d4_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_xdp, 1) - sum(d1_xdp, 1)/real(size(d1_xdp, 1), xdp)) <xdptol&
                    , 'mean(d1_xdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_xdp, 1) -&
                    sum(d2_xdp, 1)/real(size(d2_xdp, 1), xdp))) < xdptol&
                    , 'mean(d2_xdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_xdp, 2) -&
                    sum(d2_xdp, 2)/real(size(d2_xdp, 2), xdp))) < xdptol&
                    , 'mean(d2_xdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 1) -&
                    sum(d3_xdp, 1)/real(size(d3_xdp, 1), xdp))) < xdptol&
                    , 'mean(d3_xdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 2) -&
                    sum(d3_xdp, 2)/real(size(d3_xdp, 2), xdp))) < xdptol&
                    , 'mean(d3_xdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 3) -&
                    sum(d3_xdp, 3)/real(size(d3_xdp, 3), xdp))) < xdptol&
                    , 'mean(d3_xdp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 1) -&
                    sum(d4_xdp, 1)/real(size(d4_xdp, 1), xdp))) < xdptol&
                    , 'mean(d4_xdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 2) -&
                    sum(d4_xdp, 2)/real(size(d4_xdp, 2), xdp))) < xdptol&
                    , 'mean(d4_xdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 3) -&
                    sum(d4_xdp, 3)/real(size(d4_xdp, 3), xdp))) < xdptol&
                    , 'mean(d4_xdp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 4) -&
                    sum(d4_xdp, 4)/real(size(d4_xdp, 4), xdp))) < xdptol&
                    , 'mean(d4_xdp, 4): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_optmask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(mean(d1_xdp, 1, .false.))&
                    , 'mean(d1_xdp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(mean(d2_xdp, 1, .false.)))&
                    , 'mean(d2_xdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d2_xdp, 2, .false.)))&
                    , 'mean(d2_xdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_xdp, 1, .false.)))&
                    , 'mean(d3_xdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_xdp, 2, .false.)))&
                    , 'mean(d3_xdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_xdp, 3, .false.)))&
                    , 'mean(d3_xdp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_xdp, 1, .false.)))&
                    , 'mean(d4_xdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_xdp, 2, .false.)))&
                    , 'mean(d4_xdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_xdp, 3, .false.)))&
                    , 'mean(d4_xdp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_xdp, 4, .false.)))&
                    , 'mean(d4_xdp, 4, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_all_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_xdp, d1_xdp > 0)&
                    , sum(d1_xdp, d1_xdp > 0)/real(count(d1_xdp > 0), xdp)&
                    , 'mean(d1_xdp, d1_xdp > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d2_xdp, d2_xdp > 0)&
                    , sum(d2_xdp, d2_xdp > 0)/real(count(d2_xdp > 0), xdp)&
                    , 'mean(d2_xdp, d2_xdp > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d3_xdp, d3_xdp > 0)&
                    , sum(d3_xdp, d3_xdp > 0)/real(count(d3_xdp > 0), xdp)&
                    , 'mean(d3_xdp, d3_xdp > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d4_xdp, d4_xdp > 0)&
                    , sum(d4_xdp, d4_xdp > 0)/real(count(d4_xdp > 0), xdp)&
                    , 'mean(d4_xdp, d4_xdp > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_xdp, 1, d1_xdp > 0) -&
                    sum(d1_xdp, 1, d1_xdp > 0)/real(count(d1_xdp > 0, 1), xdp)) < xdptol&
                    , 'mean(d1_xdp, 1, d1_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_xdp, 1, d2_xdp > 0) -&
                    sum(d2_xdp, 1, d2_xdp > 0)/real(count(d2_xdp > 0, 1), xdp))) < xdptol&
                    , 'mean(d2_xdp, 1, d2_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_xdp, 2, d2_xdp > 0) -&
                    sum(d2_xdp, 2, d2_xdp > 0)/real(count(d2_xdp > 0, 2), xdp))) < xdptol&
                    , 'mean(d2_xdp, 2, d2_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 1, d3_xdp > 0) -&
                    sum(d3_xdp, 1, d3_xdp > 0)/real(count(d3_xdp > 0, 1), xdp))) < xdptol&
                    , 'mean(d3_xdp, 1, d3_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 2, d3_xdp > 0) -&
                    sum(d3_xdp, 2, d3_xdp > 0)/real(count(d3_xdp > 0, 2), xdp))) < xdptol&
                    , 'mean(d3_xdp, 2, d3_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_xdp, 3, d3_xdp > 0) -&
                    sum(d3_xdp, 3, d3_xdp > 0)/real(count(d3_xdp > 0, 3), xdp))) < xdptol&
                    , 'mean(d3_xdp, 3, d3_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 1, d4_xdp > 0) -&
                    sum(d4_xdp, 1, d4_xdp > 0)/real(count(d4_xdp > 0, 1), xdp))) < xdptol&
                    , 'mean(d4_xdp, 1, d4_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 2, d4_xdp > 0) -&
                    sum(d4_xdp, 2, d4_xdp > 0)/real(count(d4_xdp > 0, 2), xdp))) < xdptol&
                    , 'mean(d4_xdp, 2, d4_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 3, d4_xdp > 0) -&
                    sum(d4_xdp, 3, d4_xdp > 0)/real(count(d4_xdp > 0, 3), xdp))) < xdptol&
                    , 'mean(d4_xdp, 3, d4_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_xdp, 4, d4_xdp > 0) -&
                    sum(d4_xdp, 4, d4_xdp > 0)/real(count(d4_xdp > 0, 4), xdp))) < xdptol&
                    , 'mean(d4_xdp, 4, d4_xdp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine
    subroutine test_stats_mean_all_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_qp), sum(d1_qp)/real(size(d1_qp), qp)&
                    , 'mean(d1_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d2_qp), sum(d2_qp)/real(size(d2_qp), qp)&
                    , 'mean(d2_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d3_qp), sum(d3_qp)/real(size(d3_qp), qp)&
                    , 'mean(d3_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d4_qp), sum(d4_qp)/real(size(d4_qp), qp)&
                    , 'mean(d4_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_all_optmask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(mean(d1_qp, .false.))&
                    , 'mean(d1_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d2_qp, .false.))&
                    , 'mean(d2_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d3_qp, .false.))&
                    , 'mean(d3_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(mean(d4_qp, .false.))&
                    , 'mean(d4_qp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_qp, 1) - sum(d1_qp, 1)/real(size(d1_qp, 1), qp)) <qptol&
                    , 'mean(d1_qp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_qp, 1) -&
                    sum(d2_qp, 1)/real(size(d2_qp, 1), qp))) < qptol&
                    , 'mean(d2_qp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_qp, 2) -&
                    sum(d2_qp, 2)/real(size(d2_qp, 2), qp))) < qptol&
                    , 'mean(d2_qp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 1) -&
                    sum(d3_qp, 1)/real(size(d3_qp, 1), qp))) < qptol&
                    , 'mean(d3_qp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 2) -&
                    sum(d3_qp, 2)/real(size(d3_qp, 2), qp))) < qptol&
                    , 'mean(d3_qp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 3) -&
                    sum(d3_qp, 3)/real(size(d3_qp, 3), qp))) < qptol&
                    , 'mean(d3_qp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 1) -&
                    sum(d4_qp, 1)/real(size(d4_qp, 1), qp))) < qptol&
                    , 'mean(d4_qp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 2) -&
                    sum(d4_qp, 2)/real(size(d4_qp, 2), qp))) < qptol&
                    , 'mean(d4_qp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 3) -&
                    sum(d4_qp, 3)/real(size(d4_qp, 3), qp))) < qptol&
                    , 'mean(d4_qp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 4) -&
                    sum(d4_qp, 4)/real(size(d4_qp, 4), qp))) < qptol&
                    , 'mean(d4_qp, 4): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_optmask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(mean(d1_qp, 1, .false.))&
                    , 'mean(d1_qp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(mean(d2_qp, 1, .false.)))&
                    , 'mean(d2_qp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d2_qp, 2, .false.)))&
                    , 'mean(d2_qp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_qp, 1, .false.)))&
                    , 'mean(d3_qp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_qp, 2, .false.)))&
                    , 'mean(d3_qp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d3_qp, 3, .false.)))&
                    , 'mean(d3_qp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_qp, 1, .false.)))&
                    , 'mean(d4_qp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_qp, 2, .false.)))&
                    , 'mean(d4_qp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_qp, 3, .false.)))&
                    , 'mean(d4_qp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(mean(d4_qp, 4, .false.)))&
                    , 'mean(d4_qp, 4, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_all_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_qp, d1_qp > 0)&
                    , sum(d1_qp, d1_qp > 0)/real(count(d1_qp > 0), qp)&
                    , 'mean(d1_qp, d1_qp > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d2_qp, d2_qp > 0)&
                    , sum(d2_qp, d2_qp > 0)/real(count(d2_qp > 0), qp)&
                    , 'mean(d2_qp, d2_qp > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d3_qp, d3_qp > 0)&
                    , sum(d3_qp, d3_qp > 0)/real(count(d3_qp > 0), qp)&
                    , 'mean(d3_qp, d3_qp > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d4_qp, d4_qp > 0)&
                    , sum(d4_qp, d4_qp > 0)/real(count(d4_qp > 0), qp)&
                    , 'mean(d4_qp, d4_qp > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_qp, 1, d1_qp > 0) -&
                    sum(d1_qp, 1, d1_qp > 0)/real(count(d1_qp > 0, 1), qp)) < qptol&
                    , 'mean(d1_qp, 1, d1_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_qp, 1, d2_qp > 0) -&
                    sum(d2_qp, 1, d2_qp > 0)/real(count(d2_qp > 0, 1), qp))) < qptol&
                    , 'mean(d2_qp, 1, d2_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_qp, 2, d2_qp > 0) -&
                    sum(d2_qp, 2, d2_qp > 0)/real(count(d2_qp > 0, 2), qp))) < qptol&
                    , 'mean(d2_qp, 2, d2_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 1, d3_qp > 0) -&
                    sum(d3_qp, 1, d3_qp > 0)/real(count(d3_qp > 0, 1), qp))) < qptol&
                    , 'mean(d3_qp, 1, d3_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 2, d3_qp > 0) -&
                    sum(d3_qp, 2, d3_qp > 0)/real(count(d3_qp > 0, 2), qp))) < qptol&
                    , 'mean(d3_qp, 2, d3_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_qp, 3, d3_qp > 0) -&
                    sum(d3_qp, 3, d3_qp > 0)/real(count(d3_qp > 0, 3), qp))) < qptol&
                    , 'mean(d3_qp, 3, d3_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 1, d4_qp > 0) -&
                    sum(d4_qp, 1, d4_qp > 0)/real(count(d4_qp > 0, 1), qp))) < qptol&
                    , 'mean(d4_qp, 1, d4_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 2, d4_qp > 0) -&
                    sum(d4_qp, 2, d4_qp > 0)/real(count(d4_qp > 0, 2), qp))) < qptol&
                    , 'mean(d4_qp, 2, d4_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 3, d4_qp > 0) -&
                    sum(d4_qp, 3, d4_qp > 0)/real(count(d4_qp > 0, 3), qp))) < qptol&
                    , 'mean(d4_qp, 3, d4_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_qp, 4, d4_qp > 0) -&
                    sum(d4_qp, 4, d4_qp > 0)/real(count(d4_qp > 0, 4), qp))) < qptol&
                    , 'mean(d4_qp, 4, d4_qp > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine
    ...
    subroutine test_stats_mean_all_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_cxdp), sum(d1_cxdp)/real(size(d1_cxdp), xdp)&
                    , 'mean(d1_cxdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d2_cxdp), sum(d2_cxdp)/real(size(d2_cxdp), xdp)&
                    , 'mean(d2_cxdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d3_cxdp), sum(d3_cxdp)/real(size(d3_cxdp), xdp)&
                    , 'mean(d3_cxdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d4_cxdp), sum(d4_cxdp)/real(size(d4_cxdp), xdp)&
                    , 'mean(d4_cxdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_all_optmask_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(real(mean(d1_cxdp, .false.)))&
                    , 'mean(d1_cxdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d2_cxdp, .false.)))&
                    , 'mean(d2_cxdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d3_cxdp, .false.)))&
                    , 'mean(d3_cxdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d4_cxdp, .false.)))&
                    , 'mean(d4_cxdp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_cxdp, 1) - sum(d1_cxdp, 1)/real(size(d1_cxdp, 1), xdp)) <xdptol&
                    , 'mean(d1_cxdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_cxdp, 1) -&
                    sum(d2_cxdp, 1)/real(size(d2_cxdp, 1), xdp))) < xdptol&
                    , 'mean(d2_cxdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_cxdp, 2) -&
                    sum(d2_cxdp, 2)/real(size(d2_cxdp, 2), xdp))) < xdptol&
                    , 'mean(d2_cxdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 1) -&
                    sum(d3_cxdp, 1)/real(size(d3_cxdp, 1), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 2) -&
                    sum(d3_cxdp, 2)/real(size(d3_cxdp, 2), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 3) -&
                    sum(d3_cxdp, 3)/real(size(d3_cxdp, 3), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 1) -&
                    sum(d4_cxdp, 1)/real(size(d4_cxdp, 1), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 2) -&
                    sum(d4_cxdp, 2)/real(size(d4_cxdp, 2), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 3) -&
                    sum(d4_cxdp, 3)/real(size(d4_cxdp, 3), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 4) -&
                    sum(d4_cxdp, 4)/real(size(d4_cxdp, 4), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 4): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_optmask_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(real(mean(d1_cxdp, 1, .false.)))&
                    , 'mean(d1_cxdp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(real(mean(d2_cxdp, 1, .false.))))&
                    , 'mean(d2_cxdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d2_cxdp, 2, .false.))))&
                    , 'mean(d2_cxdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cxdp, 1, .false.))))&
                    , 'mean(d3_cxdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cxdp, 2, .false.))))&
                    , 'mean(d3_cxdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cxdp, 3, .false.))))&
                    , 'mean(d3_cxdp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cxdp, 1, .false.))))&
                    , 'mean(d4_cxdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cxdp, 2, .false.))))&
                    , 'mean(d4_cxdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cxdp, 3, .false.))))&
                    , 'mean(d4_cxdp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cxdp, 4, .false.))))&
                    , 'mean(d4_cxdp, 4, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_all_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_cxdp, d1_cxdp%re > 0)&
                    , sum(d1_cxdp, d1_cxdp%re > 0)/real(count(d1_cxdp%re > 0), xdp)&
                    , 'mean(d1_cxdp, d1_cxdp%re > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d2_cxdp, d2_cxdp%re > 0)&
                    , sum(d2_cxdp, d2_cxdp%re > 0)/real(count(d2_cxdp%re > 0), xdp)&
                    , 'mean(d2_cxdp, d2_cxdp%re > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d3_cxdp, d3_cxdp%re > 0)&
                    , sum(d3_cxdp, d3_cxdp%re > 0)/real(count(d3_cxdp%re > 0), xdp)&
                    , 'mean(d3_cxdp, d3_cxdp%re > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, mean(d4_cxdp, d4_cxdp%re > 0)&
                    , sum(d4_cxdp, d4_cxdp%re > 0)/real(count(d4_cxdp%re > 0), xdp)&
                    , 'mean(d4_cxdp, d4_cxdp%re > 0): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_cxdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_cxdp, 1, d1_cxdp%re > 0) -&
                    sum(d1_cxdp, 1, d1_cxdp%re > 0)/real(count(d1_cxdp%re > 0, 1), xdp)) < xdptol&
                    , 'mean(d1_cxdp, 1, d1_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_cxdp, 1, d2_cxdp%re > 0) -&
                    sum(d2_cxdp, 1, d2_cxdp%re > 0)/real(count(d2_cxdp%re > 0, 1), xdp))) < xdptol&
                    , 'mean(d2_cxdp, 1, d2_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_cxdp, 2, d2_cxdp%re > 0) -&
                    sum(d2_cxdp, 2, d2_cxdp%re > 0)/real(count(d2_cxdp%re > 0, 2), xdp))) < xdptol&
                    , 'mean(d2_cxdp, 2, d2_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 1, d3_cxdp%re > 0) -&
                    sum(d3_cxdp, 1, d3_cxdp%re > 0)/real(count(d3_cxdp%re > 0, 1), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 1, d3_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 2, d3_cxdp%re > 0) -&
                    sum(d3_cxdp, 2, d3_cxdp%re > 0)/real(count(d3_cxdp%re > 0, 2), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 2, d3_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cxdp, 3, d3_cxdp%re > 0) -&
                    sum(d3_cxdp, 3, d3_cxdp%re > 0)/real(count(d3_cxdp%re > 0, 3), xdp))) < xdptol&
                    , 'mean(d3_cxdp, 3, d3_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 1, d4_cxdp%re > 0) -&
                    sum(d4_cxdp, 1, d4_cxdp%re > 0)/real(count(d4_cxdp%re > 0, 1), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 1, d4_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 2, d4_cxdp%re > 0) -&
                    sum(d4_cxdp, 2, d4_cxdp%re > 0)/real(count(d4_cxdp%re > 0, 2), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 2, d4_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 3, d4_cxdp%re > 0) -&
                    sum(d4_cxdp, 3, d4_cxdp%re > 0)/real(count(d4_cxdp%re > 0, 3), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 3, d4_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cxdp, 4, d4_cxdp%re > 0) -&
                    sum(d4_cxdp, 4, d4_cxdp%re > 0)/real(count(d4_cxdp%re > 0, 4), xdp))) < xdptol&
                    , 'mean(d4_cxdp, 4, d4_cxdp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine
    subroutine test_stats_mean_all_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_cqp), sum(d1_cqp)/real(size(d1_cqp), qp)&
                    , 'mean(d1_cqp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d2_cqp), sum(d2_cqp)/real(size(d2_cqp), qp)&
                    , 'mean(d2_cqp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d3_cqp), sum(d3_cqp)/real(size(d3_cqp), qp)&
                    , 'mean(d3_cqp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d4_cqp), sum(d4_cqp)/real(size(d4_cqp), qp)&
                    , 'mean(d4_cqp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_all_optmask_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(real(mean(d1_cqp, .false.)))&
                    , 'mean(d1_cqp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d2_cqp, .false.)))&
                    , 'mean(d2_cqp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d3_cqp, .false.)))&
                    , 'mean(d3_cqp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(real(mean(d4_cqp, .false.)))&
                    , 'mean(d4_cqp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_cqp, 1) - sum(d1_cqp, 1)/real(size(d1_cqp, 1), qp)) <qptol&
                    , 'mean(d1_cqp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_cqp, 1) -&
                    sum(d2_cqp, 1)/real(size(d2_cqp, 1), qp))) < qptol&
                    , 'mean(d2_cqp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_cqp, 2) -&
                    sum(d2_cqp, 2)/real(size(d2_cqp, 2), qp))) < qptol&
                    , 'mean(d2_cqp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 1) -&
                    sum(d3_cqp, 1)/real(size(d3_cqp, 1), qp))) < qptol&
                    , 'mean(d3_cqp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 2) -&
                    sum(d3_cqp, 2)/real(size(d3_cqp, 2), qp))) < qptol&
                    , 'mean(d3_cqp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 3) -&
                    sum(d3_cqp, 3)/real(size(d3_cqp, 3), qp))) < qptol&
                    , 'mean(d3_cqp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 1) -&
                    sum(d4_cqp, 1)/real(size(d4_cqp, 1), qp))) < qptol&
                    , 'mean(d4_cqp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 2) -&
                    sum(d4_cqp, 2)/real(size(d4_cqp, 2), qp))) < qptol&
                    , 'mean(d4_cqp, 2): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 3) -&
                    sum(d4_cqp, 3)/real(size(d4_cqp, 3), qp))) < qptol&
                    , 'mean(d4_cqp, 3): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 4) -&
                    sum(d4_cqp, 4)/real(size(d4_cqp, 4), qp))) < qptol&
                    , 'mean(d4_cqp, 4): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_optmask_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, ieee_is_nan(real(mean(d1_cqp, 1, .false.)))&
                    , 'mean(d1_cqp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(real(mean(d2_cqp, 1, .false.))))&
                    , 'mean(d2_cqp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d2_cqp, 2, .false.))))&
                    , 'mean(d2_cqp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cqp, 1, .false.))))&
                    , 'mean(d3_cqp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cqp, 2, .false.))))&
                    , 'mean(d3_cqp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d3_cqp, 3, .false.))))&
                    , 'mean(d3_cqp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cqp, 1, .false.))))&
                    , 'mean(d4_cqp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cqp, 2, .false.))))&
                    , 'mean(d4_cqp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cqp, 3, .false.))))&
                    , 'mean(d4_cqp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(real(mean(d4_cqp, 4, .false.))))&
                    , 'mean(d4_cqp, 4, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_all_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mean(d1_cqp, d1_cqp%re > 0)&
                    , sum(d1_cqp, d1_cqp%re > 0)/real(count(d1_cqp%re > 0), qp)&
                    , 'mean(d1_cqp, d1_cqp%re > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d2_cqp, d2_cqp%re > 0)&
                    , sum(d2_cqp, d2_cqp%re > 0)/real(count(d2_cqp%re > 0), qp)&
                    , 'mean(d2_cqp, d2_cqp%re > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d3_cqp, d3_cqp%re > 0)&
                    , sum(d3_cqp, d3_cqp%re > 0)/real(count(d3_cqp%re > 0), qp)&
                    , 'mean(d3_cqp, d3_cqp%re > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, mean(d4_cqp, d4_cqp%re > 0)&
                    , sum(d4_cqp, d4_cqp%re > 0)/real(count(d4_cqp%re > 0), qp)&
                    , 'mean(d4_cqp, d4_cqp%re > 0): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_mean_mask_cqp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(mean(d1_cqp, 1, d1_cqp%re > 0) -&
                    sum(d1_cqp, 1, d1_cqp%re > 0)/real(count(d1_cqp%re > 0, 1), qp)) < qptol&
                    , 'mean(d1_cqp, 1, d1_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(mean(d2_cqp, 1, d2_cqp%re > 0) -&
                    sum(d2_cqp, 1, d2_cqp%re > 0)/real(count(d2_cqp%re > 0, 1), qp))) < qptol&
                    , 'mean(d2_cqp, 1, d2_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d2_cqp, 2, d2_cqp%re > 0) -&
                    sum(d2_cqp, 2, d2_cqp%re > 0)/real(count(d2_cqp%re > 0, 2), qp))) < qptol&
                    , 'mean(d2_cqp, 2, d2_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 1, d3_cqp%re > 0) -&
                    sum(d3_cqp, 1, d3_cqp%re > 0)/real(count(d3_cqp%re > 0, 1), qp))) < qptol&
                    , 'mean(d3_cqp, 1, d3_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 2, d3_cqp%re > 0) -&
                    sum(d3_cqp, 2, d3_cqp%re > 0)/real(count(d3_cqp%re > 0, 2), qp))) < qptol&
                    , 'mean(d3_cqp, 2, d3_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d3_cqp, 3, d3_cqp%re > 0) -&
                    sum(d3_cqp, 3, d3_cqp%re > 0)/real(count(d3_cqp%re > 0, 3), qp))) < qptol&
                    , 'mean(d3_cqp, 3, d3_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 1, d4_cqp%re > 0) -&
                    sum(d4_cqp, 1, d4_cqp%re > 0)/real(count(d4_cqp%re > 0, 1), qp))) < qptol&
                    , 'mean(d4_cqp, 1, d4_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 2, d4_cqp%re > 0) -&
                    sum(d4_cqp, 2, d4_cqp%re > 0)/real(count(d4_cqp%re > 0, 2), qp))) < qptol&
                    , 'mean(d4_cqp, 2, d4_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 3, d4_cqp%re > 0) -&
                    sum(d4_cqp, 3, d4_cqp%re > 0)/real(count(d4_cqp%re > 0, 3), qp))) < qptol&
                    , 'mean(d4_cqp, 3, d4_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
        call check(error&
                    , sum(abs(mean(d4_cqp, 4, d4_cqp%re > 0) -&
                    sum(d4_cqp, 4, d4_cqp%re > 0)/real(count(d4_cqp%re > 0, 4), qp))) < qptol&
                    , 'mean(d4_cqp, 4, d4_cqp%re > 0): uncorrect answer'&
                    )
        if (allocated(error)) return
    end subroutine
