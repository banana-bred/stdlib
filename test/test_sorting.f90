...
contains
    ...
    subroutine collect_sorting(testsuite)
        ...
        testsuite = [ &
            ...
            new_unittest('real_sort_adjointes_xdp', test_real_sort_adjointes_xdp), &
            new_unittest('real_sort_adjointes_qp', test_real_sort_adjointes_qp), &
    ...
    subroutine test_real_sort_adjointes_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        logical                     :: ltest

        call test_real_sort_adjoint_xdp( blocks, "Blocks", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( decrease, "Decreasing", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( identical, "Identical", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( increase, "Increasing", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( rand1, "Random dense", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( rand2, "Random order", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( rand0, "Random sparse", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( rand3, "Random 3", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_xdp( rand10, "Random 10", ltest )
        call check(error, ltest)
        if (allocated(error)) return

    end subroutine test_real_sort_adjointes_xdp

    subroutine test_real_sort_adjoint_xdp( a, a_name, ltest )
        integer(int32), intent(inout) :: a(:)
        character(*), intent(in)      :: a_name
        logical, intent(out)          :: ltest
        ...
        integer(int64)                 :: t0, t1, tdiff
        real(dp)                       :: rate
        real(xdp)                         :: adjoint(size(a))
        real(xdp)                         :: iwork(size(a))
        integer(int64)                 :: i, j
        integer(int64)                 :: i_adj
        logical                        :: valid
        logical                        :: valid_adj
    ...
    end subroutine test_real_sort_adjoint_xdp
    subroutine test_real_sort_adjointes_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        logical                     :: ltest

        call test_real_sort_adjoint_qp( blocks, "Blocks", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( decrease, "Decreasing", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( identical, "Identical", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( increase, "Increasing", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( rand1, "Random dense", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( rand2, "Random order", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( rand0, "Random sparse", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( rand3, "Random 3", ltest )
        call check(error, ltest)
        if (allocated(error)) return

        call test_real_sort_adjoint_qp( rand10, "Random 10", ltest )
        call check(error, ltest)
        if (allocated(error)) return

    end subroutine test_real_sort_adjointes_qp

    subroutine test_real_sort_adjoint_qp( a, a_name, ltest )
        integer(int32), intent(inout) :: a(:)
        character(*), intent(in)      :: a_name
        logical, intent(out)          :: ltest
        ...
        integer(int64)                 :: t0, t1, tdiff
        real(dp)                       :: rate
        real(qp)                         :: adjoint(size(a))
        real(qp)                         :: iwork(size(a))
        integer(int64)                 :: i, j
        integer(int64)                 :: i_adj
        logical                        :: valid
        logical                        :: valid_adj
    ...
    end subroutine test_real_sort_adjoint_qp
