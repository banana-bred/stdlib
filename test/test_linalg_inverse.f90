...
module test_linalg_inverse
    ...
    subroutine test_inverse_matrix(tests)
        ...
        tests = [tests,new_unittest("x_eye_inverse",test_x_eye_inverse), &
                       new_unittest("x_singular_inverse",test_x_singular_inverse), &
                       new_unittest("x_random_spd_inverse",test_x_random_spd_inverse)]
        tests = [tests,new_unittest("q_eye_inverse",test_q_eye_inverse), &
                       new_unittest("q_singular_inverse",test_q_singular_inverse), &
                       new_unittest("q_random_spd_inverse",test_q_random_spd_inverse)]
        ...
        tests = [tests,new_unittest("y_eye_inverse",test_y_eye_inverse), &
                       new_unittest("y_singular_inverse",test_y_singular_inverse), &
                       new_unittest("y_random_spd_inverse",test_y_random_spd_inverse)]
        tests = [tests,new_unittest("w_eye_inverse",test_w_eye_inverse), &
                       new_unittest("w_singular_inverse",test_w_singular_inverse), &
                       new_unittest("w_random_spd_inverse",test_w_random_spd_inverse)]
    ...
    subroutine test_x_eye_inverse(error)
        ...
        call check(error,state%ok(),'inverse_x_eye (subroutine): '//state%print())
        if (allocated(error)) return
        
        call check(error,all(abs(a-inva)<epsilon(0.0_xdp)),'inverse_x_eye (subroutine): data converged')
        if (allocated(error)) return   
