...
contains
    ...
    subroutine test_uni_rvs_0
      ...
      subroutine test_uni_rvs_rxdp
          real(xdp) :: res(15), scale, loc
          integer :: i, seed, get, k

          real(xdp), parameter :: ans(15) =                                       &
                          [0.914826628538749186958511927514337003_xdp,      &
                          0.367330098664966409049981166390352882_xdp,       &
                          1.77591243057709280428468900936422870_xdp,        &
                          0.885921308987590139238932351872790605_xdp,       &
                          0.950735656542987861428173346212133765_xdp,       &
                          -0.659562573857055134407545438079978339_xdp,      &
                          -0.116661718506947176265953203255776316_xdp,      &
                          0.837391893893859151631886561517603695_xdp,       &
                          -0.703954396598600540269075054311542772_xdp,      &
                          0.382592729851141566399519433616660535_xdp,       &
                          -0.132472493978185168472805344208609313_xdp,      &
                          -0.878723366294216184924081858298450243_xdp,      &
                          -0.901660046141515819639877804547722917_xdp,      &
                          -0.164090614147737401395943379611708224_xdp,      &
                          -0.333886718190384290672056977200554684_xdp]

          print *, "Test rvs_uniform_rxdp"
          seed = 258147369; k = 5
          call random_seed(seed, get)
          ...
          call check(all(abs(res - ans) < xdptol),                          &
              msg="rvs_uniform_rxdp failed", warn=warn)
      end subroutine test_uni_rvs_rxdp
      ...
      subroutine test_uni_rvs_rqp
          ...
          call check(all(abs(res - ans) < qptol),                          &
              msg="rvs_uniform_rqp failed", warn=warn)
      ...
      subroutine test_uni_rvs_cxdp
          complex(xdp) :: res(15), scale, loc
          integer :: i, seed, get, k

          complex(xdp), parameter :: ans(15) =                                       &
                             [(0.457413314269374593479255963757168502_xdp,  &
                               0.183665049332483204524990583195176441_xdp), &
                              (0.887956215288546402142344504682114348_xdp,  &
                               0.442960654493795069619466175936395302_xdp), &
                              (0.475367828271493930714086673106066883_xdp,  &
                               0.170218713071472432796227280960010830_xdp), &
                              (0.441669140746526411867023398372111842_xdp,  &
                               0.918695946946929575815943280758801848_xdp), &
                              (0.148022801700699729865462472844228614_xdp,  &
                               0.691296364925570783199759716808330268_xdp), &
                       (-6.623624698909258423640267210430465639E-0002_xdp,  &
                               0.560638316852891907537959070850774879_xdp), &
                             (-0.450830023070757909819938902273861459_xdp,  &
                               0.917954692926131299302028310194145888_xdp), &
                             (-0.166943359095192145336028488600277342_xdp,  &
                                1.05997401970850635422038976685144007_xdp), &
                             (-0.429652190199228276035192664039641386_xdp,  &
                               0.523558274341032421628217008446881664_xdp), &
                              (0.427181091476823815433760955784237012_xdp,  &
                                1.34628934976074521312483511792379431_xdp), &
                             (-0.343281426018765739582860874179459643_xdp,  &
                                1.15357331316264255516301773241139017_xdp), &
                             (-0.127590074749816595467422075671493076_xdp,  &
                                1.06891199479835175001340985545539297_xdp), &
                              (0.262287586904722758163188700564205647_xdp,  &
                                1.29508919831907332032017166056903079_xdp), &
                             (-0.192677407376582732201342196276527829_xdp,  &
                                1.32794925614337933073016984053538181_xdp), &
                             (-0.264742129752461530234342035328154452_xdp,  &
                                1.01282963412172621886497836385387927_xdp)]

          print *, "Test rvs_uniform_cxdp"
          seed = 258147369; k = 5
          call random_seed(seed, get)
          loc = (-0.5_xdp,0.5_xdp); scale = (1.0_xdp, 1.0_xdp)
          do i = 1, 5
              res(i) = uni_rvs(scale)             ! 1 dummy
          end do
          do i = 6,10
          ...
          res(11:15) = uni_rvs(loc, scale, k)     ! 3 dummies
          call check(all(abs(res - ans) < xdptol),                          &
              msg="rvs_uniform_cxdp failed", warn=warn)
      end subroutine test_uni_rvs_cxdp

      subroutine test_uni_rvs_cqp
          complex(qp) :: res(15), scale, loc
          integer :: i, seed, get, k

          complex(qp), parameter :: ans(15) =                                       &
          ...
          res(11:15) = uni_rvs(loc, scale, k)     ! 3 dummies
          call check(all(abs(res - ans) < qptol),                          &
              msg="rvs_uniform_cqp failed", warn=warn)
    ...
    subroutine test_uni_pdf_rxdp
        real(xdp) :: x1, x2(3,4), loc, scale
        integer :: seed, get, i
        real(xdp) :: res(3, 5)
        real(xdp), parameter :: ans(15) = [(0.5_xdp, i=1,15)]

        print *, "Test pdf_uniform_rxdp"
        seed = 147258639
        call random_seed(seed, get)
        loc = 0.0_xdp; scale = 2.0_xdp
        x1 = uni_rvs(loc, scale)
        x2 = reshape(uni_rvs(loc, scale, 12), [3,4])
        res(:,1) = uni_pdf(x1, loc, scale)
        res(:, 2:5) = uni_pdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < xdptol),             &
            msg = "pdf_uniform_rxdp failed", warn=warn)
    end subroutine test_uni_pdf_rxdp

    subroutine test_uni_pdf_rqp
        real(qp) :: x1, x2(3,4), loc, scale
        integer :: seed, get, i
        real(qp) :: res(3, 5)
        real(qp), parameter :: ans(15) = [(0.5_qp, i=1,15)]

        print *, "Test pdf_uniform_rqp"
        seed = 147258639
        call random_seed(seed, get)
        ...
        call check(all(abs(res - reshape(ans,[3,5])) < qptol),             &
            msg = "pdf_uniform_rqp failed", warn=warn)
    ...
    subroutine test_uni_pdf_cxdp
        complex(xdp) :: x1, x2(3,4), loc, scale
        integer :: seed, get, i
        real(xdp) :: res(3, 5)
        real(xdp), parameter :: ans(15) = [(1.0_xdp, i=1,15)]

        print *, "Test pdf_uniform_cxdp"
        seed = 147258639
        call random_seed(seed, get)
        loc = (-0.5_xdp, 0.5_xdp); scale = (1.0_xdp, 1.0_xdp)
        x1 = uni_rvs(loc, scale)
        ...
        res(:, 2:5) = uni_pdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < xdptol),             &
            msg = "pdf_uniform_cxdp failed", warn=warn)
    end subroutine test_uni_pdf_cxdp

    subroutine test_uni_pdf_cqp
        complex(qp) :: x1, x2(3,4), loc, scale
        integer :: seed, get, i
        real(qp) :: res(3, 5)
        real(qp), parameter :: ans(15) = [(1.0_qp, i=1,15)]

        print *, "Test pdf_uniform_cqp"
        seed = 147258639
        call random_seed(seed, get)
        loc = (-0.5_qp, 0.5_qp); scale = (1.0_qp, 1.0_qp)
        x1 = uni_rvs(loc, scale)
        ...
        res(:, 2:5) = uni_pdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < qptol),             &
            msg = "pdf_uniform_cqp failed", warn=warn)
    ...
    subroutine test_uni_cdf_rxdp
        real(xdp) :: x1, x2(3,4), loc, scale
        integer :: seed, get
        real(xdp) :: res(3,5)
        real(xdp), parameter :: ans(15) =                                         &
                             [0.170192944297557408050991512027394492_xdp,   &
                              0.170192944297557408050991512027394492_xdp,   &
                              0.170192944297557408050991512027394492_xdp,   &
                              0.276106146274646191418611351764411665_xdp,   &
                              0.754930097473875072466853453079238534_xdp,   &
                              0.406620682573118008562573777453508228_xdp,   &
                              0.187742819191801080247472555129206739_xdp,   &
                              0.651605526090507591874256831943057477_xdp,   &
                              0.934733949732104885121941606485052034_xdp,   &
                              0.151271491851613287815681019310432021_xdp,   &
                              0.987674522797719611766353864368284121_xdp,   &
                              0.130533899463404684526679488953959662_xdp,   &
                              0.106271905921876880229959283497009892_xdp,   &
                         9.27578652240113182836367400341259781E-0002_xdp,   &
                              0.203399426853420439709196898547816090_xdp]

        print *, "Test cdf_uniform_rxdp"
        seed = 369147258
        call random_seed(seed, get)
        loc = 0.0_xdp; scale = 2.0_xdp
        x1 = uni_rvs(loc, scale)
        ...
        res(:, 2:5) = uni_cdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < xdptol),             &
            msg = "cdf_uniform_rxdp failed", warn=warn)
    end subroutine test_uni_cdf_rxdp
    ...
    subroutine test_uni_cdf_rqp
        ...
        res(:, 2:5) = uni_cdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < qptol),             &
            msg = "cdf_uniform_rqp failed", warn=warn)
    ...
    subroutine test_uni_cdf_cxdp
        complex(xdp) :: x1, x2(3,4), loc, scale
        integer :: seed, get
        real(xdp) :: res(3,5)
        real(xdp), parameter :: ans(15) =                                   &
                        [4.69913179731340971083526490627998168E-0002_xdp,   &
                         4.69913179731340971083526490627998168E-0002_xdp,   &
                         4.69913179731340971083526490627998168E-0002_xdp,   &
                              0.306970191529817593217448363707416739_xdp,   &
                              0.122334258469188588238756489506609443_xdp,   &
                              0.141398599060326408705075175176932616_xdp,   &
                              0.128925006861443729884744412460848140_xdp,   &
                         9.85755512660026594506599410104817938E-0003_xdp,   &
                         8.16527497645585445208592050401597260E-0002_xdp,   &
                              0.163921605454974749736935624944263178_xdp,   &
                         7.81712317416218284294000447064256003E-0002_xdp,   &
                              0.446415807686727375005224206895756087_xdp,   &
                         5.31753272901435018841591264266743165E-0004_xdp,   &
                              0.101455865191097416942685556683943046_xdp,   &
                              0.155276470981788516449112374966730510_xdp]

        print *, "Test cdf_uniform_cxdp"
        seed = 369147258
        call random_seed(seed, get)
        loc = (-0.5_xdp, -0.5_xdp); scale = (1.0_xdp, 1.0_xdp)
        x1 = uni_rvs(loc, scale)
        ...
        res(:, 2:5) = uni_cdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < xdptol),             &
            msg = "cdf_uniform_cxdp failed", warn=warn)
    end subroutine test_uni_cdf_cxdp
    ...
    subroutine test_uni_cdf_cqp
        ...
        real(qp) :: res(3,5)
        real(qp), parameter :: ans(15) =                                   &
                        [4.69913179731340971083526490627998168E-0002_qp,   &
                         4.69913179731340971083526490627998168E-0002_qp,   &
                         4.69913179731340971083526490627998168E-0002_qp,   &
                              0.306970191529817593217448363707416739_qp,   &
                              0.122334258469188588238756489506609443_qp,   &
                              0.141398599060326408705075175176932616_qp,   &
                              0.128925006861443729884744412460848140_qp,   &
                         9.85755512660026594506599410104817938E-0003_qp,   &
                         8.16527497645585445208592050401597260E-0002_qp,   &
                              0.163921605454974749736935624944263178_qp,   &
                         7.81712317416218284294000447064256003E-0002_qp,   &
                              0.446415807686727375005224206895756087_qp,   &
                         5.31753272901435018841591264266743165E-0004_qp,   &
                              0.101455865191097416942685556683943046_qp,   &
                              0.155276470981788516449112374966730510_qp]

        print *, "Test cdf_uniform_cqp"
        seed = 369147258
        call random_seed(seed, get)
        loc = (-0.5_qp, -0.5_qp); scale = (1.0_qp, 1.0_qp)
        x1 = uni_rvs(loc, scale)
        ...
        res(:, 2:5) = uni_cdf(x2, loc, scale)
        call check(all(abs(res - reshape(ans,[3,5])) < qptol),             &
            msg = "cdf_uniform_cqp failed", warn=warn)
