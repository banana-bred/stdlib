...
subroutine test_sum(error)
    ...
    block
        ...
        real(xdp) :: xsum(ncalc), err(ncalc)
        logical, allocatable :: mask(:), nmask(:)

        allocate(x(n))
        do i = 1, n 
            x(i) = 8*atan(1._xdp)*(real(i,kind=xdp)-0.5_xdp)/real(n,kind=xdp)**2
        end do
        allocate(mask(n),source=.false.); mask(1:n:2) = .true.
        allocate(nmask(n)); nmask = .not.mask
        ! scramble array
        ...
        xsum(2) = stdlib_sum_kahan(x) ! chunked Kahan summation
        xsum(3) = stdlib_sum(x)       ! chunked summation
        err(1:ncalc) = abs(1._xdp-xsum(1:ncalc)/total_sum)
    ...
    block
        ...
        real(qp), parameter :: total_sum = 4*atan(1._qp), tolerance = epsilon(1._qp)*100
        real(qp) :: xsum(ncalc), err(ncalc)
        logical, allocatable :: mask(:), nmask(:)

        allocate(x(n))
        do i = 1, n 
            x(i) = 8*atan(1._qp)*(real(i,kind=qp)-0.5_qp)/real(n,kind=qp)**2
        end do
        allocate(mask(n),source=.false.); mask(1:n:2) = .true.
        allocate(nmask(n)); nmask = .not.mask
        ! scramble array
        ...
        xsum(2) = stdlib_sum_kahan(x) ! chunked Kahan summation
        xsum(3) = stdlib_sum(x)       ! chunked summation
        err(1:ncalc) = abs(1._qp-xsum(1:ncalc)/total_sum)
    ...
    block
        ...
        real(xdp), parameter :: total_sum = 4*atan(1._xdp), tolerance = epsilon(1._xdp)*100
        real(xdp) :: err(ncalc)
        complex(xdp) :: xsum(ncalc)
        logical, allocatable :: mask(:), nmask(:)

        allocate(x(n))
        do i = 1, n
            x(i) = (8*atan(1._xdp)*(real(i,kind=xdp)-0.5_xdp)/n**2)*cmplx(1._xdp,1._xdp)
        end do
        ...
        xsum(2) = stdlib_sum_kahan(x) ! chunked Kahan summation
        xsum(3) = stdlib_sum(x)       ! chunked summation
        err(1:ncalc) = abs(1._xdp-(xsum(1:ncalc)%re)/total_sum)
    ...
    block
        ...
        real(qp), parameter :: total_sum = 4*atan(1._qp), tolerance = epsilon(1._qp)*100
        real(qp) :: err(ncalc)
        complex(qp) :: xsum(ncalc)
        logical, allocatable :: mask(:), nmask(:)

        allocate(x(n))
        do i = 1, n
            x(i) = (8*atan(1._qp)*(real(i,kind=qp)-0.5_qp)/n**2)*cmplx(1._qp,1._qp)
        end do
        ...
        xsum(2) = stdlib_sum_kahan(x) ! chunked Kahan summation
        xsum(3) = stdlib_sum(x)       ! chunked summation
        err(1:ncalc) = abs(1._qp-(xsum(1:ncalc)%re)/total_sum)
...
subroutine test_dot_product(error)
    ...
    block
        ...
        allocate(x(n))
        do i = 1, n 
            x(i) = 2*sqrt( 2*atan(1._xdp)*(real(i,kind=xdp)-0.5_xdp) )/n
        end do
        ! scramble array
        do i = 1, n
            call random_number(u) 
    ...
    block
        ...
        allocate(x(n))
        do i = 1, n 
            x(i) = 2*sqrt( 2*atan(1._qp)*(real(i,kind=qp)-0.5_qp) )/n
        end do
        ! scramble array
        do i = 1, n
            call random_number(u) 
    ...
    block
        ...
        allocate(x(n))
        do i = 1, n
            x(i) = ( 2*sqrt( 2*atan(1._xdp)*(real(i,kind=xdp)-0.5_xdp) ) / n )*cmplx(1._xdp,1._xdp)
        end do
        ! scramble array
        do i = 1, n
            call random_number(u) 
    ...
    block
        ...
        allocate(x(n))
        do i = 1, n
            x(i) = ( 2*sqrt( 2*atan(1._qp)*(real(i,kind=qp)-0.5_qp) ) / n )*cmplx(1._qp,1._qp)
        end do
        ! scramble array
        do i = 1, n
            call random_number(u) 
        ...
        xsum(2) = stdlib_dot_product_kahan(x,x) ! chunked Kahan dot_product
        xsum(3) = stdlib_dot_product(x,x)       ! chunked dot_product
        err(1:ncalc) = abs(1._qp-xsum(1:ncalc)%re/(2*total_sum))
    ...
    block ! test for https://github.com/fortran-lang/stdlib/issues/1016
        complex(qp) :: x(128), y(128)
        real(qp) :: z(128,2)
        real(qp), parameter :: tolerance = epsilon(1._qp)*100000
        real(qp) :: err(2)
        complex(qp) :: p(3)
