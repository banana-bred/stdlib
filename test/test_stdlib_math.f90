...
contains
    ...
    subroutine collect_stdlib_math(testsuite)
        ...
        testsuite = [ &
            ...
            , new_unittest("swap_xdp", test_swap_xdp) &
            , new_unittest("swap_qp", test_swap_qp) &
            , new_unittest("swap_csp", test_swap_csp) &
            , new_unittest("swap_cdp", test_swap_cdp) &
            , new_unittest("swap_cxdp", test_swap_cxdp) &
            , new_unittest("swap_cqp", test_swap_cqp) &
            ...
            , new_unittest("arg-cmplx-xdp", test_arg_xdp) &
            , new_unittest("argd-cmplx-xdp", test_argd_xdp) &
            , new_unittest("argpi-cmplx-xdp", test_argpi_xdp) &
            , new_unittest("arg-cmplx-qp", test_arg_qp) &
            , new_unittest("argd-cmplx-qp", test_argd_qp) &
            , new_unittest("argpi-cmplx-qp", test_argpi_qp) &
            ...
            , new_unittest("deg2rad-real-xdp", test_deg2rad_xdp) &
            , new_unittest("rad2deg-real-xdp", test_rad2deg_xdp) &
            , new_unittest("deg2rad-real-qp", test_deg2rad_qp) &
            , new_unittest("rad2deg-real-qp", test_rad2deg_qp) &
            ...
            , new_unittest("is_close-real-xdp", test_is_close_real_xdp) &
            , new_unittest("is_close-cmplx-xdp", test_is_close_cmplx_xdp) &
            , new_unittest("all_close-real-xdp", test_all_close_real_xdp) &
            , new_unittest("all_close-cmplx-xdp", test_all_close_cmplx_xdp) &
            , new_unittest("is_close-real-qp", test_is_close_real_qp) &
            , new_unittest("is_close-cmplx-qp", test_is_close_cmplx_qp) &
            , new_unittest("all_close-real-qp", test_all_close_real_qp) &
            , new_unittest("all_close-cmplx-qp", test_all_close_cmplx_qp) &
            
            !> Tests for `diff`
            , new_unittest("diff-real-sp", test_diff_real_sp) &
            , new_unittest("diff-real-dp", test_diff_real_dp) &
            , new_unittest("diff-real-xdp", test_diff_real_xdp) &
            , new_unittest("diff-real-qp", test_diff_real_qp) &
            ...
            , new_unittest("arange-real-xdp", test_arange_real_xdp) &
            , new_unittest("arange-real-qp", test_arange_real_qp) &
    ...
    subroutine test_swap_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: x(3), y(3)
        ...
        call check(error, all( x == [real(xdp) :: 4, 5, 6] ) )
        if (allocated(error)) return
        call check(error, all( y == [real(xdp) :: 1, 2, 3] ) )
        if (allocated(error)) return
        ...
        call check(error, all( x == [real(xdp) :: 4, 5, 6] ) )
        if (allocated(error)) return
    end subroutine test_swap_xdp
    subroutine test_swap_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: x(3), y(3)
        ...
        call check(error, all( x == [real(qp) :: 4, 5, 6] ) )
        if (allocated(error)) return
        call check(error, all( y == [real(qp) :: 1, 2, 3] ) )
        if (allocated(error)) return
    ...
    subroutine test_swap_cxdp(error)
        type(error_type), allocatable, intent(out) :: error
        complex(xdp) :: x(3), y(3)
        ...
        call check(error, all( x == cmplx( [4, 5, 6] , [1, 2, 3] ) ) )
        if (allocated(error)) return
    end subroutine test_swap_cxdp
    subroutine test_swap_cqp(error)
        type(error_type), allocatable, intent(out) :: error
        complex(qp) :: x(3), y(3)
    ...
    subroutine test_arg_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp), parameter :: tol = sqrt(epsilon(1.0_xdp))
        real(xdp), allocatable :: theta(:)
        
        call check(error, abs(arg(2*exp((0.0_xdp, 0.5_xdp))) - 0.5_xdp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(arg((0.0_xdp, 0.0_xdp)) - 0.0_xdp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_xdp, 179.0_xdp, 3.58_xdp)
        call check(error, all(abs(arg(exp(cmplx(0.0_xdp, theta/180*PI_xdp, xdp))) - theta/180*PI_xdp) < tol), &
            "test_array")
        
    end subroutine test_arg_xdp
    
    subroutine test_argd_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp), parameter :: tol = sqrt(epsilon(1.0_xdp))
        real(xdp), allocatable :: theta(:)
        
        call check(error, abs(argd((-1.0_xdp, 0.0_xdp)) - 180.0_xdp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(argd((0.0_xdp, 0.0_xdp)) - 0.0_xdp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_xdp, 179.0_xdp, 3.58_xdp)
        call check(error, all(abs(argd(exp(cmplx(0.0_xdp, theta/180*PI_xdp, xdp))) - theta) < tol), &
            "test_array")
        
    end subroutine test_argd_xdp
    
    subroutine test_argpi_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp), parameter :: tol = sqrt(epsilon(1.0_xdp))
        real(xdp), allocatable :: theta(:)
        
        call check(error, abs(argpi((-1.0_xdp, 0.0_xdp)) - 1.0_xdp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(argpi((0.0_xdp, 0.0_xdp)) - 0.0_xdp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_xdp, 179.0_xdp, 3.58_xdp)
        call check(error, all(abs(argpi(exp(cmplx(0.0_xdp, theta/180*PI_xdp, xdp))) - theta/180) < tol), &
            "test_array")
        
    end subroutine test_argpi_xdp
    subroutine test_arg_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp), parameter :: tol = sqrt(epsilon(1.0_qp))
        real(qp), allocatable :: theta(:)
        
        call check(error, abs(arg(2*exp((0.0_qp, 0.5_qp))) - 0.5_qp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(arg((0.0_qp, 0.0_qp)) - 0.0_qp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_qp, 179.0_qp, 3.58_qp)
        call check(error, all(abs(arg(exp(cmplx(0.0_qp, theta/180*PI_qp, qp))) - theta/180*PI_qp) < tol), &
            "test_array")
        
    end subroutine test_arg_qp
    
    subroutine test_argd_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp), parameter :: tol = sqrt(epsilon(1.0_qp))
        real(qp), allocatable :: theta(:)
        
        call check(error, abs(argd((-1.0_qp, 0.0_qp)) - 180.0_qp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(argd((0.0_qp, 0.0_qp)) - 0.0_qp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_qp, 179.0_qp, 3.58_qp)
        call check(error, all(abs(argd(exp(cmplx(0.0_qp, theta/180*PI_qp, qp))) - theta) < tol), &
            "test_array")
        
    end subroutine test_argd_qp
    
    subroutine test_argpi_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp), parameter :: tol = sqrt(epsilon(1.0_qp))
        real(qp), allocatable :: theta(:)
        
        call check(error, abs(argpi((-1.0_qp, 0.0_qp)) - 1.0_qp) < tol, &
            "test_nonzero_scalar")
        if (allocated(error)) return
        call check(error, abs(argpi((0.0_qp, 0.0_qp)) - 0.0_qp) < tol, &
            "test_zero_scalar")
        if (allocated(error)) return
        
        theta = arange(-179.0_qp, 179.0_qp, 3.58_qp)
        call check(error, all(abs(argpi(exp(cmplx(0.0_qp, theta/180*PI_qp, qp))) - theta/180) < tol), &
            "test_array")
        
    end subroutine test_argpi_qp
    ...
    subroutine test_deg2rad_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp), parameter :: tol = sqrt(epsilon(1.0_xdp))

        call check(error, PI_xdp, deg2rad(180.0_xdp), thr=tol)
        if (allocated(error)) return

    end subroutine test_deg2rad_xdp
    
    subroutine test_rad2deg_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp), parameter :: tol = sqrt(epsilon(1.0_xdp))

        call check(error, 180.0_xdp, rad2deg(PI_xdp), thr=tol)
        if (allocated(error)) return

    end subroutine test_rad2deg_xdp
    subroutine test_deg2rad_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp), parameter :: tol = sqrt(epsilon(1.0_qp))

        call check(error, PI_qp, deg2rad(180.0_qp), thr=tol)
        if (allocated(error)) return

    end subroutine test_deg2rad_qp
    
    subroutine test_rad2deg_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp), parameter :: tol = sqrt(epsilon(1.0_qp))

        call check(error, 180.0_qp, rad2deg(PI_qp), thr=tol)
        if (allocated(error)) return

    end subroutine test_rad2deg_qp
    ...
    subroutine test_is_close_real_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: x, NAN
        x = -3; NAN = sqrt(x)
        
        call check(error, is_close(2.5_xdp, 2.5_xdp), .true.)
        if (allocated(error)) return
        call check(error, is_close(0.0_xdp, -0.0_xdp), .true.)
        if (allocated(error)) return
        call check(error, is_close(2.5_xdp, 1.2_xdp), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, NAN), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, 0.0_xdp), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, NAN, equal_nan=.true.), .true.)
    
    end subroutine test_is_close_real_xdp
    
    subroutine test_is_close_cmplx_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: x, NAN
        x = -3; NAN = sqrt(x)
        
        call check(error, is_close((2.5_xdp, 1.5_xdp), (2.5_xdp, 1.5_xdp)), .true.)
        if (allocated(error)) return
        call check(error, is_close((2.5_xdp, 1.2_xdp), (2.5_xdp, 1.5_xdp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, xdp), cmplx(NAN, NAN, xdp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, xdp), cmplx(NAN, 0.0_xdp, xdp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, xdp), cmplx(NAN, NAN, xdp), equal_nan=.true.), .true.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, 1.2_xdp, xdp), cmplx(NAN, 1.2_xdp, xdp), equal_nan=.true.), .true.)
        
    end subroutine test_is_close_cmplx_xdp
    
    subroutine test_all_close_real_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: x(2, 2), eps, NAN
        x = 1; eps = -3; NAN = sqrt(eps)
        
        eps = sqrt(epsilon(1.0_xdp))
    ...
    end subroutine test_all_close_real_xdp
    
    subroutine test_all_close_cmplx_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: eps, NAN
        complex(xdp) :: x(2, 2)
        x = (1, 1); eps = -3; NAN = sqrt(eps)
        ...
        call check(error, all_close(x + x*eps + 1.0e-6, x), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, xdp), x), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, xdp), x, equal_nan=.true.), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, xdp), x + cmplx(NAN, NAN, xdp), equal_nan=.true.), .true.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, xdp), x + cmplx(NAN, NAN, xdp)), .false.)
        
    end subroutine test_all_close_cmplx_xdp
    subroutine test_is_close_real_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: x, NAN
        x = -3; NAN = sqrt(x)
        
        call check(error, is_close(2.5_qp, 2.5_qp), .true.)
        if (allocated(error)) return
        call check(error, is_close(0.0_qp, -0.0_qp), .true.)
        if (allocated(error)) return
        call check(error, is_close(2.5_qp, 1.2_qp), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, NAN), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, 0.0_qp), .false.)
        if (allocated(error)) return
        call check(error, is_close(NAN, NAN, equal_nan=.true.), .true.)
    
    end subroutine test_is_close_real_qp
    
    subroutine test_is_close_cmplx_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: x, NAN
        x = -3; NAN = sqrt(x)
        
        call check(error, is_close((2.5_qp, 1.5_qp), (2.5_qp, 1.5_qp)), .true.)
        if (allocated(error)) return
        call check(error, is_close((2.5_qp, 1.2_qp), (2.5_qp, 1.5_qp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, qp), cmplx(NAN, NAN, qp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, qp), cmplx(NAN, 0.0_qp, qp)), .false.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, NAN, qp), cmplx(NAN, NAN, qp), equal_nan=.true.), .true.)
        if (allocated(error)) return
        call check(error, is_close(cmplx(NAN, 1.2_qp, qp), cmplx(NAN, 1.2_qp, qp), equal_nan=.true.), .true.)
        
    end subroutine test_is_close_cmplx_qp
    
    subroutine test_all_close_real_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: x(2, 2), eps, NAN
        x = 1; eps = -3; NAN = sqrt(eps)
        
        eps = sqrt(epsilon(1.0_qp))
    ...
    end subroutine test_all_close_real_qp
    
    subroutine test_all_close_cmplx_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: eps, NAN
        complex(qp) :: x(2, 2)
        x = (1, 1); eps = -3; NAN = sqrt(eps)
        ...
        call check(error, all_close(x + x*eps + 1.0e-6, x), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, qp), x), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, qp), x, equal_nan=.true.), .false.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, qp), x + cmplx(NAN, NAN, qp), equal_nan=.true.), .true.)
        if (allocated(error)) return
        call check(error, all_close(x + cmplx(NAN, NAN, qp), x + cmplx(NAN, NAN, qp)), .false.)
        
    end subroutine test_all_close_cmplx_qp
    ...
    subroutine test_diff_real_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        real(xdp) :: x(6) = [real(xdp) :: 0, 5, 15, 30, 50, 75]
        real(xdp) :: A(1, 3) = reshape([real(xdp) :: 1, 3, 5], [1, 3])
        real(xdp) :: B(2) = [real(xdp) :: 1, 2]
        
        !> rank-1 diff
        call check(error, all_close(diff(x), [real(xdp) :: 5, 10, 15, 20, 25]), &
            "diff(<rank-1>) in test_diff_real_xdp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, n=0), x), &
            "diff(<rank-1>, n=0) in test_diff_real_xdp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, n=2), [real(xdp) :: 5, 5, 5, 5]), &
            "diff(<rank-1>, n=2) in test_diff_real_xdp failed")
        if (allocated(error)) return
        
        call check(error, all_close(diff(x, prepend=[real(xdp) :: 1]), [real(xdp) :: -1, 5, 10, 15, 20, 25]), &
            "diff(<rank-1>, prepend=[real(xdp) :: 1]) in test_diff_real_xdp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, append=[real(xdp) :: 1]), [real(xdp) :: 5, 10, 15, 20, 25, -74]), &
            "diff(<rank-1>, append=[real(xdp) :: 1]) in test_diff_real_xdp failed")
        if (allocated(error)) return
        
        !> rank-2 diff
        call check(error, all_close(diff(reshape(A, [3,1]), n=1, dim=1), reshape([real(xdp) :: 2, 2], [2, 1])), &
            "diff(<rank-2>, n=1, dim=1) in test_diff_real_xdp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(A, n=1, dim=2), reshape([real(xdp) :: 2, 2], [1, 2])), &
            "diff(<rank-2>, n=1, dim=2) in test_diff_real_xdp failed")
        if (allocated(error)) return
        
        call check(error, all_close(diff(A, n=1, dim=2, prepend=reshape([real(xdp) :: 1], [1, 1]), &
            append=reshape([real(xdp) :: 2], [1, 1])), reshape([real(xdp) :: 0, 2, 2, -3], [1, 4])), &
            "diff(<rank-2>, n=1, dim=2, prepend=reshape([real(xdp) :: 1], [1, 1]), &
            &append=reshape([real(xdp) :: 2], [1, 1])) in test_diff_real_xdp failed")
        if (allocated(error)) return
        
        !> size(B, dim) <= n
        call check(error, size(diff(B, 2)), 0, "size(diff(B, 2)) in test_diff_real_xdp failed")
        if (allocated(error)) return
        call check(error, size(diff(B, 3)), 0, "size(diff(B, 3)) in test_diff_real_xdp failed")
        
    end subroutine test_diff_real_xdp
    subroutine test_diff_real_qp(error)
        type(error_type), allocatable, intent(out) :: error
        real(qp) :: x(6) = [real(qp) :: 0, 5, 15, 30, 50, 75]
        real(qp) :: A(1, 3) = reshape([real(qp) :: 1, 3, 5], [1, 3])
        real(qp) :: B(2) = [real(qp) :: 1, 2]
        
        !> rank-1 diff
        call check(error, all_close(diff(x), [real(qp) :: 5, 10, 15, 20, 25]), &
            "diff(<rank-1>) in test_diff_real_qp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, n=0), x), &
            "diff(<rank-1>, n=0) in test_diff_real_qp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, n=2), [real(qp) :: 5, 5, 5, 5]), &
            "diff(<rank-1>, n=2) in test_diff_real_qp failed")
        if (allocated(error)) return
        
        call check(error, all_close(diff(x, prepend=[real(qp) :: 1]), [real(qp) :: -1, 5, 10, 15, 20, 25]), &
            "diff(<rank-1>, prepend=[real(qp) :: 1]) in test_diff_real_qp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(x, append=[real(qp) :: 1]), [real(qp) :: 5, 10, 15, 20, 25, -74]), &
            "diff(<rank-1>, append=[real(qp) :: 1]) in test_diff_real_qp failed")
        if (allocated(error)) return
        
        !> rank-2 diff
        call check(error, all_close(diff(reshape(A, [3,1]), n=1, dim=1), reshape([real(qp) :: 2, 2], [2, 1])), &
            "diff(<rank-2>, n=1, dim=1) in test_diff_real_qp failed")
        if (allocated(error)) return
        call check(error, all_close(diff(A, n=1, dim=2), reshape([real(qp) :: 2, 2], [1, 2])), &
            "diff(<rank-2>, n=1, dim=2) in test_diff_real_qp failed")
        if (allocated(error)) return
        
        call check(error, all_close(diff(A, n=1, dim=2, prepend=reshape([real(qp) :: 1], [1, 1]), &
            append=reshape([real(qp) :: 2], [1, 1])), reshape([real(qp) :: 0, 2, 2, -3], [1, 4])), &
            "diff(<rank-2>, n=1, dim=2, prepend=reshape([real(qp) :: 1], [1, 1]), &
            &append=reshape([real(qp) :: 2], [1, 1])) in test_diff_real_qp failed")
        if (allocated(error)) return
        
        !> size(B, dim) <= n
        call check(error, size(diff(B, 2)), 0, "size(diff(B, 2)) in test_diff_real_qp failed")
        if (allocated(error)) return
        call check(error, size(diff(B, 3)), 0, "size(diff(B, 3)) in test_diff_real_qp failed")
        
    end subroutine test_diff_real_qp
    ...
    subroutine test_arange_real_xdp(error)
        type(error_type), allocatable, intent(out) :: error
        
        ! Normal
        call check(error, all_close(arange(3.0_xdp), [1.0_xdp, 2.0_xdp, 3.0_xdp]), &
                   "all(arange(3.0_xdp), [1.0_xdp,2.0_xdp,3.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(-1.0_xdp), [1.0_xdp, 0.0_xdp, -1.0_xdp]), &
                   "all_close(arange(-1.0_xdp), [1.0_xdp,0.0_xdp,-1.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_xdp, 2.0_xdp), [0.0_xdp, 1.0_xdp, 2.0_xdp]), &
                   "all_close(arange(0.0_xdp,2.0_xdp), [0.0_xdp,1.0_xdp,2.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_xdp, -1.0_xdp), [1.0_xdp, 0.0_xdp, -1.0_xdp]), &
                   "all_close(arange(1.0_xdp,-1.0_xdp), [1.0_xdp,0.0_xdp,-1.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_xdp, 1.0_xdp), [1.0_xdp]), &
                   "all_close(arange(1.0_xdp,1.0_xdp), [1.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_xdp, 2.0_xdp, 2.0_xdp), [0.0_xdp, 2.0_xdp]), &
                   "all_close(arange(0.0_xdp,2.0_xdp,2.0_xdp), [0.0_xdp,2.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_xdp, -1.0_xdp, 2.0_xdp), [1.0_xdp, -1.0_xdp]), &
                   "all_close(arange(1.0_xdp,-1.0_xdp,2.0_xdp), [1.0_xdp,-1.0_xdp]) failed.")
        if (allocated(error)) return
        ...
        call check(error, all_close(arange(1.0_xdp, -1.0_xdp, -2.0_xdp), [1.0_xdp, -1.0_xdp]), &
                   "all_close(arange(1.0_xdp,-1.0_xdp,-2.0_xdp), [1.0_xdp,-1.0_xdp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_xdp, 2.0_xdp, 0.0_xdp), [0.0_xdp,1.0_xdp,2.0_xdp]), &
                   "all_close(arange(0.0_xdp, 2.0_xdp, 0.0_xdp), [0.0_xdp,1.0_xdp,2.0_xdp]) failed.")
    end subroutine test_arange_real_xdp
    subroutine test_arange_real_qp(error)
        type(error_type), allocatable, intent(out) :: error
        
        ! Normal
        call check(error, all_close(arange(3.0_qp), [1.0_qp, 2.0_qp, 3.0_qp]), &
                   "all(arange(3.0_qp), [1.0_qp,2.0_qp,3.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(-1.0_qp), [1.0_qp, 0.0_qp, -1.0_qp]), &
                   "all_close(arange(-1.0_qp), [1.0_qp,0.0_qp,-1.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_qp, 2.0_qp), [0.0_qp, 1.0_qp, 2.0_qp]), &
                   "all_close(arange(0.0_qp,2.0_qp), [0.0_qp,1.0_qp,2.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_qp, -1.0_qp), [1.0_qp, 0.0_qp, -1.0_qp]), &
                   "all_close(arange(1.0_qp,-1.0_qp), [1.0_qp,0.0_qp,-1.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_qp, 1.0_qp), [1.0_qp]), &
                   "all_close(arange(1.0_qp,1.0_qp), [1.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_qp, 2.0_qp, 2.0_qp), [0.0_qp, 2.0_qp]), &
                   "all_close(arange(0.0_qp,2.0_qp,2.0_qp), [0.0_qp,2.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(1.0_qp, -1.0_qp, 2.0_qp), [1.0_qp, -1.0_qp]), &
                   "all_close(arange(1.0_qp,-1.0_qp,2.0_qp), [1.0_qp,-1.0_qp]) failed.")
        if (allocated(error)) return
        ...
        call check(error, all_close(arange(1.0_qp, -1.0_qp, -2.0_qp), [1.0_qp, -1.0_qp]), &
                   "all_close(arange(1.0_qp,-1.0_qp,-2.0_qp), [1.0_qp,-1.0_qp]) failed.")
        if (allocated(error)) return
        call check(error, all_close(arange(0.0_qp, 2.0_qp, 0.0_qp), [0.0_qp,1.0_qp,2.0_qp]), &
                   "all_close(arange(0.0_qp, 2.0_qp, 0.0_qp), [0.0_qp,1.0_qp,2.0_qp]) failed.")
    end subroutine test_arange_real_qp
