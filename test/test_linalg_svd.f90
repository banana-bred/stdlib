...
module test_linalg_svd
    ...
    subroutine test_svd(tests)
        ...
        tests = [tests,new_unittest("test_svd_x",test_svd_x)]        
        tests = [tests,new_unittest("test_svd_q",test_svd_q)]        
        ...
        tests = [tests,new_unittest("test_complex_svd_y",test_complex_svd_y)]        
        tests = [tests,new_unittest("test_complex_svd_w",test_complex_svd_w)]        

        tests = [tests,new_unittest("test_svd_row_s",test_svd_row_s)]         
        tests = [tests,new_unittest("test_svd_row_d",test_svd_row_d)]         
        tests = [tests,new_unittest("test_svd_row_x",test_svd_row_x)]         
        tests = [tests,new_unittest("test_svd_row_q",test_svd_row_q)]         
        tests = [tests,new_unittest("test_svd_row_c",test_svd_row_c)]         
        tests = [tests,new_unittest("test_svd_row_z",test_svd_row_z)]         
        tests = [tests,new_unittest("test_svd_row_y",test_svd_row_y)]         
        tests = [tests,new_unittest("test_svd_row_w",test_svd_row_w)]         
    ...
    ! Issue #835: bounds checking triggers an error with 1-sized A matrix
    subroutine test_svd_row_x(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        integer(ilp), parameter :: m = 1, n = 1
        real(xdp), parameter :: tol = sqrt(epsilon(0.0_xdp))
        real(xdp) :: Arand(m, n), S(n)
        real(xdp) :: A(m, n), U(m, m), Vt(n, n)
    ...
    end subroutine test_svd_row_x

    ! Issue #835: bounds checking triggers an error with 1-sized A matrix
    subroutine test_svd_row_q(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        integer(ilp), parameter :: m = 1, n = 1
        real(qp), parameter :: tol = sqrt(epsilon(0.0_qp))
        real(qp) :: Arand(m, n), S(n)
        real(qp) :: A(m, n), U(m, m), Vt(n, n)
    ...
    ! Issue #835: bounds checking triggers an error with 1-sized A matrix
    subroutine test_svd_row_y(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(xdp), parameter :: tol = sqrt(epsilon(0.0_xdp))
        real(xdp) :: Arand(m, n), S(n)
        complex(xdp) :: A(m, n), U(m, m), Vt(n, n)
    ...
    end subroutine test_svd_row_y

    ! Issue #835: bounds checking triggers an error with 1-sized A matrix
    subroutine test_svd_row_w(error)
        type(error_type), allocatable, intent(out) :: error
