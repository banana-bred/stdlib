...
module test_stats_median
    ...
    real(xdp) , parameter :: d1_xdp(12) = [real(xdp) :: 10, 2, -3, -4, 6, -6, 7, -8, 9, 0, 1, 20]
    real(xdp) :: d2_xdp(3, 4) = reshape(d1_xdp, [3, 4])
    real(xdp) :: d3_xdp(2, 3, 2) = reshape(d1_xdp, [2, 3, 2])
    real(xdp) , parameter :: d1odd_xdp(13) = [real(xdp) :: d1_xdp, 20]
    real(xdp) :: d2odd_xdp(3, 5) = reshape(d1odd_xdp, [3, 5], [real(xdp) :: 0])
    real(xdp) :: d3odd_xdp(1, 3, 5) = reshape(d1odd_xdp, [1, 3, 5], [real(xdp) :: 0])
    real(qp) , parameter :: d1_qp(12) = [real(qp) :: 10, 2, -3, -4, 6, -6, 7, -8, 9, 0, 1, 20]
    real(qp) :: d2_qp(3, 4) = reshape(d1_qp, [3, 4])
    real(qp) :: d3_qp(2, 3, 2) = reshape(d1_qp, [2, 3, 2])
    real(qp) , parameter :: d1odd_qp(13) = [real(qp) :: d1_qp, 20]
    real(qp) :: d2odd_qp(3, 5) = reshape(d1odd_qp, [3, 5], [real(qp) :: 0])
    real(qp) :: d3odd_qp(1, 3, 5) = reshape(d1odd_qp, [1, 3, 5], [real(qp) :: 0])
...
contains
    ...
    subroutine collect_stats_median(testsuite)
        ...
        testsuite = [ &
            ...
            , new_unittest("test_stats_median_size_xdp", test_stats_median_size_xdp) &
            , new_unittest("test_stats_median_odd_size_xdp", test_stats_median_odd_size_xdp) &
            , new_unittest("test_stats_median_all_xdp", test_stats_median_all_xdp) &
            , new_unittest("test_stats_median_all_odd_xdp", test_stats_median_all_odd_xdp) &
            , new_unittest("test_stats_median_all_optmask_xdp", test_stats_median_all_optmask_xdp) &
            , new_unittest("test_stats_median_xdp", test_stats_median_xdp) &
            , new_unittest("test_stats_median_odd_xdp", test_stats_median_odd_xdp) &
            , new_unittest("test_stats_median_optmask_xdp", test_stats_median_optmask_xdp) &
            , new_unittest("test_stats_median_mask_all_xdp", test_stats_median_mask_all_xdp) &
            , new_unittest("test_stats_median_mask_xdp", test_stats_median_mask_xdp) &
            , new_unittest("test_stats_median_size_qp", test_stats_median_size_qp) &
            , new_unittest("test_stats_median_odd_size_qp", test_stats_median_odd_size_qp) &
            , new_unittest("test_stats_median_all_qp", test_stats_median_all_qp) &
            , new_unittest("test_stats_median_all_odd_qp", test_stats_median_all_odd_qp) &
            , new_unittest("test_stats_median_all_optmask_qp", test_stats_median_all_optmask_qp) &
            , new_unittest("test_stats_median_qp", test_stats_median_qp) &
            , new_unittest("test_stats_median_odd_qp", test_stats_median_odd_qp) &
            , new_unittest("test_stats_median_optmask_qp", test_stats_median_optmask_qp) &
            , new_unittest("test_stats_median_mask_all_qp", test_stats_median_mask_all_qp) &
            , new_unittest("test_stats_median_mask_qp", test_stats_median_mask_qp) &
            ]
    ...
    subroutine test_stats_median_size_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        real(xdp), allocatable :: d0(:)

        allocate(d0(0))
        !check just to be sure that the setup of d0 is correct
        call check(error, size(d0), 0, 'size(d0): should be of size 0')

        call check(error, mod(size(d1_xdp), 2), 0&
                    , 'mod(size(d1_xdp), 2): should be an even number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d2_xdp), 2), 0&
                    , 'mod(size(d2_xdp), 2): should be an even number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d3_xdp), 2), 0&
                    , 'mod(size(d3_xdp), 2): should be an even number'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_odd_size_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mod(size(d1odd_xdp), 2), 1&
                    , 'mod(size(d1_xdp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d2odd_xdp), 2), 1&
                    , 'mod(size(d2_xdp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d3odd_xdp), 2), 1&
                    , 'mod(size(d3_xdp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_all_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error, median(d1_xdp),  1.5_xdp&
                    , 'median(d1_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, median(d2_xdp),  1.5_xdp&
                    , 'median(d2_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
        call check(error, median(d3_xdp),  1.5_xdp&
                    , 'median(d3_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_all_odd_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, median(d1odd_xdp),  2._xdp&
                    , 'median(d1odd_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return

        call check(error, median(d2odd_xdp),  1._xdp&
                    , 'median(d2odd_xdp): uncorrect answer'&
                    , thr = xdptol)
        if (allocated(error)) return
    ...
    subroutine test_stats_median_all_optmask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        real(xdp), allocatable :: d0_xdp(:)

        allocate(d0_xdp(0))

        call check(error, ieee_is_nan(median(d0_xdp, .false.))&
                    , 'median(d0_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d1_xdp, .false.))&
                    , 'median(d1_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d2_xdp, .false.))&
                    , 'median(d2_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d3_xdp, .false.))&
                    , 'median(d3_xdp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , abs(median(d1_xdp, 1) - 1.5_xdp) < xdptol&
                    , 'median(d1_xdp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2_xdp, 1) - [2._xdp, -4._xdp, 7._xdp, 1._xdp])) < xdptol&
                    , 'median(d2_xdp, 1): uncorrect answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2_xdp, 2) - [3.5_xdp, 1.5_xdp, 3._xdp])) < xdptol&
                    ,'median(d2_xdp, 2): uncorrect answer')
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_odd_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(median(d1odd_xdp, 1) - 2._xdp) < xdptol&
                    , 'median(d1odd_xdp, 1): wrong answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2odd_xdp, 1) - [2._xdp, -4._xdp, 7._xdp, 1._xdp, 0._xdp])) < xdptol&
                    , 'median(d2odd_xdp, 1): wrong answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2odd_xdp, 2) - [7._xdp, 1._xdp, 0._xdp])) < xdptol&
                    , 'median(d2odd_xdp, 2): wrong answer')
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_optmask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error, ieee_is_nan(median(d1_xdp, 1, .false.))&
                    , 'median(d1_xdp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(median(d2_xdp, 1, .false.)))&
                    , 'median(d2_xdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d2_xdp, 2, .false.)))&
                    , 'median(d2_xdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_xdp, 1, .false.)))&
                    , 'median(d3_xdp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_xdp, 2, .false.)))&
                    , 'median(d3_xdp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_xdp, 3, .false.)))&
                    , 'median(d3_xdp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_mask_all_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , ieee_is_nan(median(d1_xdp, d1_xdp > huge(d1_xdp)))&
                    , 'median(d1_xdp, d1_xdp > huge(d1_xdp))' )
        if (allocated(error)) return
        call check(error&
                    , ieee_is_nan(median(d2_xdp, d2_xdp > huge(d2_xdp)))&
                    , 'median(d2_xdp, d2_xdp > huge(d2_xdp))' )
        if (allocated(error)) return
        call check(error&
                    , ieee_is_nan(median(d3_xdp, d3_xdp > huge(d3_xdp)))&
                    , 'median(d3_xdp, d3_xdp > huge(d3_xdp))' )
        if (allocated(error)) return

        call check(error&
                    , (median(d1_xdp, d1_xdp > 0) - 7._xdp) < xdptol&
                    , 'median(d1_xdp, d1_xdp > 0)' ) 
        if (allocated(error)) return
        call check(error&
                    , (median(d2_xdp, d2_xdp > 0) - 7._xdp) < xdptol&
                    , 'median(d2_xdp, d2_xdp > 0)' ) 
        if (allocated(error)) return
        call check(error&
                    , (median(d3_xdp, d3_xdp > 0) - 7._xdp) < xdptol&
                    , 'median(d3_xdp, d3_xdp > 0)' ) 
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_mask_xdp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , ieee_is_nan(median(d1_xdp, 1, d1_xdp > huge(d1_xdp)))&
                    , 'median(d1_xdp, 1, d1_xdp > huge(d1_xdp)): answer should be IEEE NaN' )
        if (allocated(error)) return

        call check(error&
                    , any(ieee_is_nan(median(d2_xdp, 1, d2_xdp > huge(d2_xdp))))&
                    , 'median(d2_xdp, 1, d2_xdp > huge(d2_xdp)): answer should be IEEE NaN' )
        if (allocated(error)) return
        call check(error&
                    , any(ieee_is_nan(median(d3_xdp, 1, d3_xdp > huge(d3_xdp))))&
                    , 'median(d3_xdp, 1, d3_xdp > huge(d3_xdp)): answer should be IEEE NaN' )
        if (allocated(error)) return

        call check(error&
                    , (median(d1_xdp, 1, d1_xdp > 0) - 7._xdp) < xdptol&
                    , 'median(d1_xdp, 1, d1_xdp >0): uncorrect answer') 
        if (allocated(error)) return

        call check(error&
                    , sum(abs( (median(d2_xdp, 1, d2_xdp > 0) - [ 6._xdp, 6._xdp, 8._xdp, 10.5_xdp] )  )) &
                    < xdptol&
                    , 'median(d2_xdp, 1, d2_xdp > 0): uncorrect answer') 
        if (allocated(error)) return

        call check(error&
                    , sum(abs((median(d2_xdp, 2, d2_xdp > 0) - [ 8.5_xdp, 2._xdp, 14.5_xdp] )))&
                    < xdptol&
                    , 'median(d2_xdp, 2, d2_xdp > 0)') 
        if (allocated(error)) return

        call check(error&
                    , any(ieee_is_nan(median(d3_xdp, 1, d3_xdp > 0)))&
                    , 'median(d3_xdp, 1, d3_xdp > 0): should contain at least 1 IEEE NaN')

    end subroutine

    subroutine test_stats_median_size_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        real(qp), allocatable :: d0(:)

        allocate(d0(0))
        !check just to be sure that the setup of d0 is correct
        call check(error, size(d0), 0, 'size(d0): should be of size 0')

        call check(error, mod(size(d1_qp), 2), 0&
                    , 'mod(size(d1_qp), 2): should be an even number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d2_qp), 2), 0&
                    , 'mod(size(d2_qp), 2): should be an even number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d3_qp), 2), 0&
                    , 'mod(size(d3_qp), 2): should be an even number'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_odd_size_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, mod(size(d1odd_qp), 2), 1&
                    , 'mod(size(d1_qp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d2odd_qp), 2), 1&
                    , 'mod(size(d2_qp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
        call check(error, mod(size(d3odd_qp), 2), 1&
                    , 'mod(size(d3_qp), 2): should be an odd number'&
                    )
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_all_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error, median(d1_qp),  1.5_qp&
                    , 'median(d1_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, median(d2_qp),  1.5_qp&
                    , 'median(d2_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
        call check(error, median(d3_qp),  1.5_qp&
                    , 'median(d3_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_all_odd_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error, median(d1odd_qp),  2._qp&
                    , 'median(d1odd_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return

        call check(error, median(d2odd_qp),  1._qp&
                    , 'median(d2odd_qp): uncorrect answer'&
                    , thr = qptol)
        if (allocated(error)) return
    ...
    subroutine test_stats_median_all_optmask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        real(qp), allocatable :: d0_qp(:)

        allocate(d0_qp(0))

        call check(error, ieee_is_nan(median(d0_qp, .false.))&
                    , 'median(d0_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d1_qp, .false.))&
                    , 'median(d1_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d2_qp, .false.))&
                    , 'median(d2_qp, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, ieee_is_nan(median(d3_qp, .false.))&
                    , 'median(d3_qp, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , abs(median(d1_qp, 1) - 1.5_qp) < qptol&
                    , 'median(d1_qp, 1): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2_qp, 1) - [2._qp, -4._qp, 7._qp, 1._qp])) < qptol&
                    , 'median(d2_qp, 1): uncorrect answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2_qp, 2) - [3.5_qp, 1.5_qp, 3._qp])) < qptol&
                    ,'median(d2_qp, 2): uncorrect answer')
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_odd_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error

        call check(error&
                    , abs(median(d1odd_qp, 1) - 2._qp) < qptol&
                    , 'median(d1odd_qp, 1): wrong answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2odd_qp, 1) - [2._qp, -4._qp, 7._qp, 1._qp, 0._qp])) < qptol&
                    , 'median(d2odd_qp, 1): wrong answer')
        if (allocated(error)) return

        call check(error&
                    , sum(abs(median(d2odd_qp, 2) - [7._qp, 1._qp, 0._qp])) < qptol&
                    , 'median(d2odd_qp, 2): wrong answer')
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_optmask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error, ieee_is_nan(median(d1_qp, 1, .false.))&
                    , 'median(d1_qp, 1, .false.): uncorrect answer'&
                    )
        if (allocated(error)) return

        call check(error, any(ieee_is_nan(median(d2_qp, 1, .false.)))&
                    , 'median(d2_qp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d2_qp, 2, .false.)))&
                    , 'median(d2_qp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_qp, 1, .false.)))&
                    , 'median(d3_qp, 1, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_qp, 2, .false.)))&
                    , 'median(d3_qp, 2, .false.): uncorrect answer')
        if (allocated(error)) return
        call check(error, any(ieee_is_nan(median(d3_qp, 3, .false.)))&
                    , 'median(d3_qp, 3, .false.): uncorrect answer')
        if (allocated(error)) return
    end subroutine

    subroutine test_stats_median_mask_all_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , ieee_is_nan(median(d1_qp, d1_qp > huge(d1_qp)))&
                    , 'median(d1_qp, d1_qp > huge(d1_qp))' )
        if (allocated(error)) return
        call check(error&
                    , ieee_is_nan(median(d2_qp, d2_qp > huge(d2_qp)))&
                    , 'median(d2_qp, d2_qp > huge(d2_qp))' )
        if (allocated(error)) return
        call check(error&
                    , ieee_is_nan(median(d3_qp, d3_qp > huge(d3_qp)))&
                    , 'median(d3_qp, d3_qp > huge(d3_qp))' )
        if (allocated(error)) return

        call check(error&
                    , (median(d1_qp, d1_qp > 0) - 7._qp) < qptol&
                    , 'median(d1_qp, d1_qp > 0)' ) 
        if (allocated(error)) return
        call check(error&
                    , (median(d2_qp, d2_qp > 0) - 7._qp) < qptol&
                    , 'median(d2_qp, d2_qp > 0)' ) 
        if (allocated(error)) return
        call check(error&
                    , (median(d3_qp, d3_qp > 0) - 7._qp) < qptol&
                    , 'median(d3_qp, d3_qp > 0)' ) 
        if (allocated(error)) return

    end subroutine

    subroutine test_stats_median_mask_qp(error)
        !> Error handling
        type(error_type), allocatable, intent(out) :: error
        ...
        call check(error&
                    , ieee_is_nan(median(d1_qp, 1, d1_qp > huge(d1_qp)))&
                    , 'median(d1_qp, 1, d1_qp > huge(d1_qp)): answer should be IEEE NaN' )
        if (allocated(error)) return

        call check(error&
                    , any(ieee_is_nan(median(d2_qp, 1, d2_qp > huge(d2_qp))))&
                    , 'median(d2_qp, 1, d2_qp > huge(d2_qp)): answer should be IEEE NaN' )
        if (allocated(error)) return
        call check(error&
                    , any(ieee_is_nan(median(d3_qp, 1, d3_qp > huge(d3_qp))))&
                    , 'median(d3_qp, 1, d3_qp > huge(d3_qp)): answer should be IEEE NaN' )
        if (allocated(error)) return

        call check(error&
                    , (median(d1_qp, 1, d1_qp > 0) - 7._qp) < qptol&
                    , 'median(d1_qp, 1, d1_qp >0): uncorrect answer') 
        if (allocated(error)) return

        call check(error&
                    , sum(abs( (median(d2_qp, 1, d2_qp > 0) - [ 6._qp, 6._qp, 8._qp, 10.5_qp] )  )) &
                    < qptol&
                    , 'median(d2_qp, 1, d2_qp > 0): uncorrect answer') 
        if (allocated(error)) return

        call check(error&
                    , sum(abs((median(d2_qp, 2, d2_qp > 0) - [ 8.5_qp, 2._qp, 14.5_qp] )))&
                    < qptol&
                    , 'median(d2_qp, 2, d2_qp > 0)') 
        if (allocated(error)) return

        call check(error&
                    , any(ieee_is_nan(median(d3_qp, 1, d3_qp > 0)))&
                    , 'median(d3_qp, 1, d3_qp > 0): should contain at least 1 IEEE NaN')
