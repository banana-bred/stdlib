...
contains
    ...
    impure elemental function gamma_cdp(z) result(res)
        ...
        real(xdp), parameter :: half = 0.5_xdp,             &
                             one = 1.0_xdp, pi = acos(- one), sqpi = sqrt(pi)
        complex(xdp) :: y, x, sum


        integer, parameter :: n = 24
        real(xdp), parameter :: r = 25.617904_xdp
        real(xdp), parameter :: d(0 : n)=                                         &
                         [1.0087261714899910504854136977047144166e-11_xdp,  &
                              1.6339627701280724777912729825256860624_xdp,  &
                          -1.4205787702221583745972794018472259342e+1_xdp,  &
                           5.6689501646428786119793943350900908698e+1_xdp,  &
                          -1.3766376824252176069406853670529834070e+2_xdp,  &
                           2.2739972766608392140035874845640820558e+2_xdp,  &
                          -2.7058382145757164380300118233258834430e+2_xdp,  &
                          2.39614374587263042692333711131832094166e+2_xdp,  &
                          -1.6090450559507517723393498276315290189e+2_xdp,  &
                          8.27378183187161305711485619113605553100e+1_xdp,  &
                          -3.2678977082742592701862249152153110206e+1_xdp,  &
                             9.89018079175824824537131521501652931756_xdp,  &
                             -2.2762136356329318377213053650799013041_xdp,  &
                          3.93265017303573867227590563182750070164e-1_xdp,  &
                          -5.0051054352146209116457193223422284239e-2_xdp,  &
                          4.57142601898244576789629257292603538238e-3_xdp,  &
                          -2.8922592124650765614787233510990416584e-4_xdp,  &
                          1.20833375377219592849746118012697473202e-5_xdp,  &
                          -3.1220812187551248389268359432609135033e-7_xdp,  &
                          4.55117045361638520378367871355819524460e-9_xdp,  &
                         -3.2757632817493581828033170342853173968e-11_xdp,  &
                         9.49784279240135747819870224486376897253e-14_xdp,  &
                         -7.9480594917454410117072562195702526836e-17_xdp,  &
                         1.04692819439870077791406760109955648941e-20_xdp,  &
                         -5.8990280044857540075384586350723191533e-26_xdp]
        ! parameters from above referenced source.
    ...
    !
        ...
        real(dp) :: d
        integer :: m, i
        complex(xdp) :: zr, zr2, sum, s
        real(dp), parameter :: z_limit = 10.0_dp, zero_k1 = 0.0_dp
        integer, parameter :: n = 20
        real(xdp), parameter :: zero = 0.0_xdp, one = 1.0_xdp,              &
                             pi = acos(-one), ln2pi = log(2 * pi)
        real(xdp), parameter :: a(n) = [                                          &
                           .8333333333333333333333333333333333333333E-1_xdp,&
                          -.2777777777777777777777777777777777777778E-2_xdp,&
                           .7936507936507936507936507936507936507937E-3_xdp,&
                          -.5952380952380952380952380952380952380952E-3_xdp,&
                           .8417508417508417508417508417508417508418E-3_xdp,&
                          -.1917526917526917526917526917526917526918E-2_xdp,&
                           .6410256410256410256410256410256410256410E-2_xdp,&
                          -.2955065359477124183006535947712418300654E-1_xdp,&
                           .1796443723688305731649384900158893966944E+0_xdp,&
                          -.1392432216905901116427432216905901116427E+1_xdp,&
                           .1340286404416839199447895100069013112491E+2_xdp,&
                          -.1568482846260020173063651324520889738281E+3_xdp,&
                           .2193103333333333333333333333333333333333E+4_xdp,&
                          -.3610877125372498935717326521924223073648E+5_xdp,&
                           .6914722688513130671083952507756734675533E+6_xdp,&
                          -.1523822153940741619228336495888678051866E+8_xdp,&
                           .3829007513914141414141414141414141414141E+9_xdp,&
                         -.1088226603578439108901514916552510537473E+11_xdp,&
                          .3473202837650022522522522522522522522523E+12_xdp,&
                         -.1236960214226927445425171034927132488108E+14_xdp]
        ! parameters from above reference
        ...
        sum = (((sum + a(7)) * zr2 + a(6)) * zr2 + a(5)) * zr2
        sum = (((sum + a(4)) * zr2 + a(3)) * zr2 + a(2)) * zr2
        sum = (sum + a(1)) * zr + ln2pi / 2 - z1 + (z1 - 0.5_xdp) * log(z1)

        if(m /= 0) then

            s = cmplx(zero, zero, kind = xdp)

            do i = 1, m

                s = s + log(cmplx(z1, kind = xdp) - i)
