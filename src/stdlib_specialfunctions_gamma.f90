module stdlib_specialfunctions_gamma
    ...
    real(xdp), parameter :: tol_xdp = epsilon(1.0_xdp)
...
contains
    ...
    impure elemental function gamma_cxdp(z) result(res)
        complex(xdp), intent(in) :: z
        complex(xdp) :: res
        integer :: i

        real(xdp), parameter :: zero_k1 = 0.0_xdp
        real(qp), parameter :: half = 0.5_qp,             &
                             one = 1.0_qp, pi = acos(- one), sqpi = sqrt(pi)
        complex(qp) :: y, x, sum
        ...
        if(abs(z % im) < tol_xdp) then

            res = cmplx(gamma(z % re), kind = xdp)
            return
        ...
        if(z % re < zero_k1) then
            ...
            x = cmplx(abs(z % re), - z % im, kind = xdp)
            y = x - one
    ...
    impure elemental function l_gamma_iint8xdp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int8), intent(in) :: z
        ...
        integer(int8) :: i
        integer(int8), parameter :: zero = 0_int8, one = 1_int8, two = 2_int8
        real(xdp), parameter :: zero_k2 = 0.0_xdp
        ...
        case (two :)
            ...
            do i = one, z - one
               ...
               res = res + log(real(i, xdp))

            end do

        end select
    end function l_gamma_iint8xdp
    ...
    impure elemental function l_gamma_iint16xdp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int16), intent(in) :: z
    ...
    end function l_gamma_iint16xdp
    ...
    impure elemental function l_gamma_iint32xdp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int32), intent(in) :: z
    ...
    end function l_gamma_iint32xdp
    ...
    impure elemental function l_gamma_iint64xdp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int64), intent(in) :: z
    ...
    end function l_gamma_iint64xdp
    ...
    impure elemental function l_gamma_cxdp(z) result (res)
    !
    ! log_gamma function for any complex number, excluding negative whole number
    ! "Computation of special functions", Shanjie Zhang & Jianmin Jin, 1996, p.48
    ! "Computing the principal branch of log-gamma", D.E.G. Hare,
    ...
    !
        complex(xdp), intent(in) :: z
        complex(xdp) :: res, z1, z2
        real(xdp) :: d
        integer :: m, i
        complex(qp) :: zr, zr2, sum, s
        real(xdp), parameter :: z_limit = 10.0_xdp, zero_k1 = 0.0_xdp
        integer, parameter :: n = 20
        real(qp), parameter :: zero = 0.0_qp, one = 1.0_qp,              &
                             pi = acos(-one), ln2pi = log(2 * pi)
        real(qp), parameter :: a(n) = [                                          &
        ...
        if(z % re < zero_k1) then
            ...
            z2 = cmplx(abs(z % re), - z % im, kind = xdp) + 1
        ...
        if(z % re < zero_k1) then
            ...
            sum = log(pi) - log(sin(pi * z)) - sum
            m = ceiling((2 * z % re - 3) / 4)
            sum % im = sum % im + 2 * pi * m * sign(1.0_xdp, z % im)

        end if

        res = cmplx(sum, kind = xdp)
    end function l_gamma_cxdp
    ...
    !
        ...
        real(xdp) :: res
        integer(int8), parameter :: zero = 0_int8, one = 1_int8, two = 2_int8
        real(xdp), parameter :: zero_xdp = 0.0_xdp, one_xdp = 1.0_xdp
    ...
    !
        ...
        real(xdp) :: res, p_lim, a, b, g, c, d, y
        real(xdp), parameter :: zero = 0.0_qp, one = 1.0_qp
        real(xdp), parameter :: dm = tiny(1.0_qp) * 10 ** 6
        real(dp), parameter :: zero_k1 = 0.0_dp
    ...
    impure elemental function gpx_rxdp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with real argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(x < -9.0_xdp) then
            ...
            p_lim = 5.0_xdp * (sqrt(abs(x)) - 1.0_xdp)

        elseif(x >= -9.0_xdp .and. x <= zero_k1) then
        ...
        if(x < zero_k1 .and. p < p_lim .and. abs(anint(p) - p) > tol_xdp)   &
            call error_stop("Error(gpx): Incomplete gamma function with "      &
            //"negative x must come with a whole number p not too small")
        ...
        if(p >= p_lim) then     !use modified Lentz method of continued fraction
            ...
            g = ieee_value(1._xdp, ieee_quiet_nan)

        end if

        res = g
    end function gpx_rxdp
    ...
    impure elemental function gpx_iint8xdp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        real(xdp) :: res, p_lim, a, b, g, c, d, y
        integer :: n
        real(xdp), parameter :: zero = 0.0_xdp, one = 1.0_xdp
        real(xdp), parameter :: dm = tiny(1.0_xdp) * 10 ** 6
        integer(int8), parameter :: zero_k1 = 0_int8, two = 2_int8
        ...
        else if(x >= -9.0_xdp .and. x <= zero) then
        ...
        if(real(p, xdp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int8) > (p - two) / two .or. y < b * tol_xdp) exit

            end do

            if(y >= b * tol_xdp .and. mod(p, two) /= zero_k1)               &
                b = b + d * c / a
    ...
    impure elemental function gpx_iint16xdp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, xdp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int16) > (p - two) / two .or. y < b * tol_xdp) exit
    ...
    impure elemental function gpx_iint32xdp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, xdp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int32) > (p - two) / two .or. y < b * tol_xdp) exit
    ...
    impure elemental function gpx_iint64xdp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, xdp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int64) > (p - two) / two .or. y < b * tol_xdp) exit
    ...
    impure elemental function ingamma_low_rxdp(p, x) result(res)
    !
    ! Approximation of lower incomplete gamma function with real p.
    !
        real(xdp), intent(in) :: p, x
    ...
    impure elemental function ingamma_low_iint8xdp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
        ...
        else if(x > real(p, xdp)) then

            s1 = l_gamma(p, one)
            y = one - exp(-x + p * log(x) - s1) * gpx(p, x)
            res = exp(s1 + log(y))

        else if(x <= real(p, xdp) .and. x > zero) then
    ...
    end function ingamma_low_iint8xdp
    ...
    impure elemental function ingamma_low_iint16xdp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    end function ingamma_low_iint16xdp
    ...
    impure elemental function ingamma_low_iint32xdp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    end function ingamma_low_iint32xdp
    ...
    impure elemental function ingamma_low_iint64xdp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    end function ingamma_low_iint64xdp
    ...
    impure elemental function l_ingamma_low_rxdp(p, x) result(res)
    ...
    end function l_ingamma_low_rxdp
    ...
    impure elemental function l_ingamma_low_iint8xdp(p, x)        &
        result(res)
    ...
    end function l_ingamma_low_iint8xdp
    ...
    impure elemental function l_ingamma_low_iint16xdp(p, x)        &
        result(res)
    ...
    end function l_ingamma_low_iint16xdp
    ...
    impure elemental function l_ingamma_low_iint32xdp(p, x)        &
        result(res)
    ...
    end function l_ingamma_low_iint32xdp
    ...
    impure elemental function l_ingamma_low_iint64xdp(p, x)        &
        result(res)
    ...
    end function l_ingamma_low_iint64xdp
    ...
    impure elemental function ingamma_up_rxdp(p, x) result(res)
    !
    ! Approximation of upper incomplete gamma function with real p.
    !
        real(xdp), intent(in) :: p, x
    ...
    impure elemental function ingamma_up_iint8xdp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    end function ingamma_up_iint8xdp
    ...
    impure elemental function ingamma_up_iint16xdp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    end function ingamma_up_iint16xdp
    ...
    impure elemental function ingamma_up_iint32xdp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    end function ingamma_up_iint32xdp
    ...
    impure elemental function ingamma_up_iint64xdp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    end function ingamma_up_iint64xdp
    ...
    impure elemental function l_ingamma_up_rxdp(p, x) result(res)
    ...
    end function l_ingamma_up_rxdp
    ...
    impure elemental function l_ingamma_up_iint8xdp(p, x)         &
        result(res)
    ...
    end function l_ingamma_up_iint8xdp
    ...
    impure elemental function l_ingamma_up_iint16xdp(p, x)         &
        result(res)
    ...
    end function l_ingamma_up_iint16xdp
    ...
    impure elemental function l_ingamma_up_iint32xdp(p, x)         &
        result(res)
    ...
    end function l_ingamma_up_iint32xdp
    ...
    impure elemental function l_ingamma_up_iint64xdp(p, x)         &
        result(res)
    ...
    end function l_ingamma_up_iint64xdp
    ...
    impure elemental function regamma_p_rxdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for real p
    !
        real(xdp), intent(in) :: p, x
    ...
    impure elemental function regamma_p_iint8xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int8), intent(in) :: p
    ...
    end function regamma_p_iint8xdp
    ...
    impure elemental function regamma_p_iint16xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int16), intent(in) :: p
    ...
    end function regamma_p_iint16xdp
    ...
    impure elemental function regamma_p_iint32xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int32), intent(in) :: p
    ...
    end function regamma_p_iint32xdp
    ...
    impure elemental function regamma_p_iint64xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int64), intent(in) :: p
    ...
    end function regamma_p_iint64xdp
    ...
    impure elemental function regamma_q_rxdp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function Q(p,x) for real p
    !
        real(xdp), intent(in) :: p, x
    ...
    impure elemental function regamma_q_iint8xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int8), intent(in) :: p
        ...
        elseif(x <= real(p, xdp)) then

            s1 = -x + p * log(abs(x)) - l_gamma(p, one)
            res = one - exp(s1 + log(gpx(p,x)))

        end if
     end function regamma_q_iint8xdp
    ...
    impure elemental function regamma_q_iint16xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int16), intent(in) :: p
     ...
     end function regamma_q_iint16xdp
    ...
    impure elemental function regamma_q_iint32xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int32), intent(in) :: p
     ...
     end function regamma_q_iint32xdp
    ...
    impure elemental function regamma_q_iint64xdp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int64), intent(in) :: p
     ...
     end function regamma_q_iint64xdp
