  ...
  contains
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_xswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_xswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_xswap( l, a( 1_ilp, j ), 1_ilp, a( 1_ilp, m ), 1_ilp )
           call stdlib_xswap( l, b( 1_ilp, j ), 1_ilp, b( 1_ilp, m ), 1_ilp )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           coef = one / real( 2_ilp*nr,KIND=xdp)
           coef2 = coef*coef
           coef5 = half*coef2
           nrp2 = nr + 2_ilp
           beta = zero
           ...
           ! start generalized conjugate gradient iteration
           250 continue
           gamma = stdlib_xdot( nr, work( ilo+4*n ), 1_ilp, work( ilo+4*n ), 1_ilp ) +stdlib_xdot( nr, &
                     work( ilo+5*n ), 1_ilp, work( ilo+5*n ), 1_ilp )
           ew = zero
           ewc = zero
           do i = ilo, ihi
           ...
           t = coef5*( ewc-three*ew )
           tc = coef5*( ew-three*ewc )
           call stdlib_xscal( nr, beta, work( ilo ), 1_ilp )
           call stdlib_xscal( nr, beta, work( ilo+n ), 1_ilp )
           call stdlib_xaxpy( nr, coef, work( ilo+4*n ), 1_ilp, work( ilo+n ), 1_ilp )
           call stdlib_xaxpy( nr, coef, work( ilo+5*n ), 1_ilp, work( ilo ), 1_ilp )
           do i = ilo, ihi
              work( i ) = work( i ) + tc
              work( i+n ) = work( i+n ) + t
           end do
           ...
           sum = stdlib_xdot( nr, work( ilo+n ), 1_ilp, work( ilo+2*n ), 1_ilp ) +stdlib_xdot( nr, work( &
                     ilo ), 1_ilp, work( ilo+3*n ), 1_ilp )
           alpha = gamma / sum
           ! determine correction to current iteration
           cmax = zero
           ...
           if( cmax<half )go to 350
           call stdlib_xaxpy( nr, -alpha, work( ilo+2*n ), 1_ilp, work( ilo+4*n ), 1_ilp )
           call stdlib_xaxpy( nr, -alpha, work( ilo+3*n ), 1_ilp, work( ilo+5*n ), 1_ilp )
           pgamma = gamma
           it = it + 1_ilp
           if( it<=nrp2 )go to 250
           ! end generalized conjugate gradient iteration
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp)
           do i = ilo, ihi
              irab = stdlib_ixamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_ixamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp)
              ir = int(lscale( i ) + sign( half, lscale( i ) ),KIND=ilp)
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_ixamax( ihi, a( 1_ilp, i ), 1_ilp )
              cab = abs( a( icab, i ) )
              icab = stdlib_ixamax( ihi, b( 1_ilp, i ), 1_ilp )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_xscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_xscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_xscal( ihi, rscale( j ), a( 1_ilp, j ), 1_ilp )
              call stdlib_xscal( ihi, rscale( j ), b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_yswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_yswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_yswap( l, a( 1_ilp, j ), 1_ilp, a( 1_ilp, m ), 1_ilp )
           call stdlib_yswap( l, b( 1_ilp, j ), 1_ilp, b( 1_ilp, m ), 1_ilp )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp)
           do i = ilo, ihi
              irab = stdlib_iyamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_iyamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp)
              ir = int(lscale( i ) + sign( half, lscale( i ) ),KIND=ilp)
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_iyamax( ihi, a( 1_ilp, i ), 1_ilp )
              cab = abs( a( icab, i ) )
              icab = stdlib_iyamax( ihi, b( 1_ilp, i ), 1_ilp )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_ydscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_ydscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_ydscal( ihi, rscale( j ), a( 1_ilp, j ), 1_ilp )
              call stdlib_ydscal( ihi, rscale( j ), b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( icompq==3_ilp )call stdlib_xlaset( 'FULL', n, n, zero, one, q, ldq )
           if( icompz==3_ilp )call stdlib_xlaset( 'FULL', n, n, zero, one, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 temp = a( jrow-1, jcol )
                 call stdlib_xlartg( temp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = zero
                 call stdlib_xrot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_xrot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_xrot( n, q( 1_ilp, jrow-1 ), 1_ilp, q( 1_ilp, jrow ), 1_ilp, c, s )
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 temp = b( jrow, jrow )
                 call stdlib_xlartg( temp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = zero
                 call stdlib_xrot( ihi, a( 1_ilp, jrow ), 1_ilp, a( 1_ilp, jrow-1 ), 1_ilp, c, s )
                 call stdlib_xrot( jrow-1, b( 1_ilp, jrow ), 1_ilp, b( 1_ilp, jrow-1 ), 1_ilp, c,s )
                 if( ilz )call stdlib_xrot( n, z( 1_ilp, jrow ), 1_ilp, z( 1_ilp, jrow-1 ), 1_ilp, c, s )
              end do
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( icompq==3_ilp )call stdlib_ylaset( 'FULL', n, n, czero, cone, q, ldq )
           if( icompz==3_ilp )call stdlib_ylaset( 'FULL', n, n, czero, cone, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 ctemp = a( jrow-1, jcol )
                 call stdlib_ylartg( ctemp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = czero
                 call stdlib_yrot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_yrot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_yrot( n, q( 1_ilp, jrow-1 ), 1_ilp, q( 1_ilp, jrow ), 1_ilp, c,conjg( s ) )
                           
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 ctemp = b( jrow, jrow )
                 call stdlib_ylartg( ctemp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = czero
                 call stdlib_yrot( ihi, a( 1_ilp, jrow ), 1_ilp, a( 1_ilp, jrow-1 ), 1_ilp, c, s )
                 call stdlib_yrot( jrow-1, b( 1_ilp, jrow ), 1_ilp, b( 1_ilp, jrow-1 ), 1_ilp, c,s )
                 if( ilz )call stdlib_yrot( n, z( 1_ilp, jrow ), 1_ilp, z( 1_ilp, jrow-1 ), 1_ilp, c, s )
              end do
           end do
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
       ...
       ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( initq )call stdlib_xlaset( 'ALL', n, n, zero, one, q, ldq )
           if( initz )call stdlib_xlaset( 'ALL', n, n, zero, one, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp )call stdlib_xlaset( 'LOWER', n-1, n-1, zero, zero, b(2_ilp, 1_ilp), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp
           if( nh<=1_ilp ) then
              work( 1_ilp ) = one
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              do jcol = ilo, ihi-2, nb
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_xlaset( 'ALL', nblst, nblst, zero, one, work, nblst )
                 pw = nblst * nblst + 1_ilp
                 do i = 1, n2nb
                    call stdlib_xlaset( 'ALL', 2_ilp*nnb, 2_ilp*nnb, zero, one,work( pw ), 2_ilp*nnb )
                    pw = pw + 4_ilp*nnb*nnb
                 end do
                 ! reduce columns jcol:jcol+nnb-1 of a to hessenberg form.
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_xlartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = c
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_xlartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = zero
                          call stdlib_xrot( jj-top, b( top+1, jj+1 ), 1_ilp,b( top+1, jj ), 1_ilp, c, s )
                    ...
                    ! update a by transformations from right.
                    ! explicit loop unrolling provides better performance
                    ! compared to stdlib_xlasr.
                     ! call stdlib_xlasr( 'right', 'variable', 'backward', ihi-top,
           ! $                     ihi-j, a( j+2, j ), b( j+2, j ),
           ! $                     a( top+1, j+1 ), lda )
                    jj = mod( ihi-j-1, 3_ilp )
                    do i = ihi-j-3, jj+1, -3
                    ...
                    if( jj>0_ilp ) then
                       do i = jj, 1, -1
                          call stdlib_xrot( ihi-top, a( top+1, j+i+1 ), 1_ilp,a( top+1, j+i ), 1_ilp, a( &
                                    j+1+i, j ),-b( j+1+i, j ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp
                       call stdlib_xgemv( 'TRANSPOSE', nblst, len, one, work,nblst, a( jrow, j+1 )&
                                 , 1_ilp, zero,work( pw ), 1_ilp )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_xtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp ), nblst,work( pw+len ), 1_ilp )
                       call stdlib_xgemv( 'TRANSPOSE', len, nblst-len, one,work( (len+1)*nblst - &
                       len + 1_ilp ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp, one,work( pw+len ), 1_ilp )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_xtrmv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp*nnb, work( pw ),1_ilp )
                          call stdlib_xtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp*len*nnb ),2_ilp*nnb, work( pw + len ), 1_ilp )
                          call stdlib_xgemv( 'TRANSPOSE', nnb, len, one,work( ppwo ), 2_ilp*nnb, a( &
                                    jrow, j+1 ), 1_ilp,one, work( pw ), 1_ilp )
                          call stdlib_xgemv( 'TRANSPOSE', len, nnb, one,work( ppwo + 2_ilp*len*nnb + &
                                    nnb ), 2_ilp*nnb,a( jrow+nnb, j+1 ), 1_ilp, one,work( pw+len ), 1_ilp )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 cola = n - jcol - nnb + 1_ilp
                 j = ihi - nblst + 1_ilp
                 call stdlib_xgemm( 'TRANSPOSE', 'NO TRANSPOSE', nblst,cola, nblst, one, work, &
                           nblst,a( j, jcol+nnb ), lda, zero, work( pw ),nblst )
                 call stdlib_xlacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_xorm22( 'LEFT', 'TRANSPOSE', 2_ilp*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_xgemm( 'TRANSPOSE', 'NO TRANSPOSE', 2_ilp*nnb,cola, 2_ilp*nnb, one, &
                                 work( ppwo ), 2_ilp*nnb,a( j, jcol+nnb ), lda, zero, work( pw ),2_ilp*nnb )
                       call stdlib_xlacpy( 'ALL', 2_ilp*nnb, cola, work( pw ), 2_ilp*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, q( &
                              topq, j ), ldq,work, nblst, zero, work( pw ), nh )
                    call stdlib_xlacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_xorm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, one,&
                                     q( topq, j ), ldq,work( ppwo ), 2_ilp*nnb, zero, work( pw ),nh )
                          call stdlib_xlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp ) then
                    ...
                    call stdlib_xlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, zero, zero,a( jcol + 2_ilp, &
                              jcol ), lda )
                    call stdlib_xlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, zero, zero,b( jcol + 2_ilp, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated orthogonal matrices to a and b.
                 if ( top>0_ilp ) then
                    j = ihi - nblst + 1_ilp
                    call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, a( &
                              1_ilp, j ), lda,work, nblst, zero, work( pw ), top )
                    call stdlib_xlacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_xorm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,a( 1_ilp, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                                    one, a( 1_ilp, j ), lda,work( ppwo ), 2_ilp*nnb, zero,work( pw ), top )
                          call stdlib_xlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,a( 1_ilp, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp
                    call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, b( &
                              1_ilp, j ), ldb,work, nblst, zero, work( pw ), top )
                    call stdlib_xlacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                                    one, b( 1_ilp, j ), ldb,work( ppwo ), 2_ilp*nnb, zero,work( pw ), top )
                          call stdlib_xlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,b( 1_ilp, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, z( &
                              topq, j ), ldz,work, nblst, zero, work( pw ), nh )
                    call stdlib_xlacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, one,&
                                     z( topq, j ), ldz,work( ppwo ), 2_ilp*nnb, zero, work( pw ),nh )
                          call stdlib_xlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_xgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp ) = real( lwkopt,KIND=xdp)
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( initq )call stdlib_ylaset( 'ALL', n, n, czero, cone, q, ldq )
           if( initz )call stdlib_ylaset( 'ALL', n, n, czero, cone, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp )call stdlib_ylaset( 'LOWER', n-1, n-1, czero, czero, b(2_ilp, 1_ilp), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp
           if( nh<=1_ilp ) then
              work( 1_ilp ) = cone
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              do jcol = ilo, ihi-2, nb
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_ylaset( 'ALL', nblst, nblst, czero, cone, work, nblst )
                 pw = nblst * nblst + 1_ilp
                 do i = 1, n2nb
                    call stdlib_ylaset( 'ALL', 2_ilp*nnb, 2_ilp*nnb, czero, cone,work( pw ), 2_ilp*nnb )
                 ...
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_ylartg( temp, a( i, j ), c, s, a( i-1, j ) )
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_ylartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = czero
                          call stdlib_yrot( jj-top, b( top+1, jj+1 ), 1_ilp,b( top+1, jj ), 1_ilp, c, s )
                    ...
                    if( jj>0_ilp ) then
                       do i = jj, 1, -1
                          ...
                          call stdlib_yrot( ihi-top, a( top+1, j+i+1 ), 1_ilp,a( top+1, j+i ), 1_ilp, c,-&
                                    conjg( b( j+1+i, j ) ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp
                       call stdlib_ygemv( 'CONJUGATE', nblst, len, cone, work,nblst, a( jrow, j+1 &
                                 ), 1_ilp, czero,work( pw ), 1_ilp )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_ytrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp ), nblst,work( pw+len ), 1_ilp )
                       call stdlib_ygemv( 'CONJUGATE', len, nblst-len, cone,work( (len+1)*nblst - &
                       len + 1_ilp ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp, cone,work( pw+len ), 1_ilp )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_ytrmv( 'UPPER', 'CONJUGATE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp*nnb, work( pw ),1_ilp )
                          call stdlib_ytrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp*len*nnb ),2_ilp*nnb, work( pw + len ), 1_ilp )
                          call stdlib_ygemv( 'CONJUGATE', nnb, len, cone,work( ppwo ), 2_ilp*nnb, a( &
                                    jrow, j+1 ), 1_ilp,cone, work( pw ), 1_ilp )
                          call stdlib_ygemv( 'CONJUGATE', len, nnb, cone,work( ppwo + 2_ilp*len*nnb + &
                                    nnb ), 2_ilp*nnb,a( jrow+nnb, j+1 ), 1_ilp, cone,work( pw+len ), 1_ilp )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 cola = n - jcol - nnb + 1_ilp
                 j = ihi - nblst + 1_ilp
                 call stdlib_ygemm( 'CONJUGATE', 'NO TRANSPOSE', nblst,cola, nblst, cone, work, &
                           nblst,a( j, jcol+nnb ), lda, czero, work( pw ),nblst )
                 call stdlib_ylacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_yunm22( 'LEFT', 'CONJUGATE', 2_ilp*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_ygemm( 'CONJUGATE', 'NO TRANSPOSE', 2_ilp*nnb,cola, 2_ilp*nnb, cone, &
                       work( ppwo ), 2_ilp*nnb,a( j, jcol+nnb ), lda, czero, work( pw ),2_ilp*nnb )
                                 
                       call stdlib_ylacpy( 'ALL', 2_ilp*nnb, cola, work( pw ), 2_ilp*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, q( &
                              topq, j ), ldq,work, nblst, czero, work( pw ), nh )
                    call stdlib_ylacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_yunm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, &
                          cone, q( topq, j ), ldq,work( ppwo ), 2_ilp*nnb, czero, work( pw ),nh )
                                    
                          call stdlib_ylacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp ) then
                    ...
                    call stdlib_ylaset( 'LOWER', ihi - jcol - 1_ilp, nnb, czero, czero,a( jcol + 2_ilp, &
                              jcol ), lda )
                    call stdlib_ylaset( 'LOWER', ihi - jcol - 1_ilp, nnb, czero, czero,b( jcol + 2_ilp, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated unitary matrices to a and b.
                 if ( top>0_ilp ) then
                    j = ihi - nblst + 1_ilp
                    call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, a( &
                              1_ilp, j ), lda,work, nblst, czero, work( pw ), top )
                    call stdlib_ylacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_yunm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,a( 1_ilp, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                          cone, a( 1_ilp, j ), lda,work( ppwo ), 2_ilp*nnb, czero,work( pw ), top )
                                    
                          call stdlib_ylacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,a( 1_ilp, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp
                    call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, b( &
                              1_ilp, j ), ldb,work, nblst, czero, work( pw ), top )
                    call stdlib_ylacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_ylacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,b( 1_ilp, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, z( &
                              topq, j ), ldz,work, nblst, czero, work( pw ), nh )
                    call stdlib_ylacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_ylacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_ygghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp ) = cmplx( lwkopt,KIND=xdp)
           return
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
          ...
          ! $                     safety = one )
           ...
           safmax = one / safmin
           ulp = stdlib_xlamch( 'E' )*stdlib_xlamch( 'B' )
           anorm = stdlib_xlanhs( 'F', in, h( ilo, ilo ), ldh, work )
           bnorm = stdlib_xlanhs( 'F', in, t( ilo, ilo ), ldt, work )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           loop_360: do jiter = 1, maxit
              ...
              loop_60: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          temp = h( jch, jch )
                          call stdlib_xlartg( temp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = zero
                          call stdlib_xrot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_xrot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_xrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, s )
                       ...
                       do jch = j, ilast - 1
                          temp = t( jch, jch+1 )
                          call stdlib_xlartg( temp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = zero
                          if( jch<ilastm-1 )call stdlib_xrot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_xrot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_xrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, s )
                                    
                          temp = h( jch+1, jch )
                          call stdlib_xlartg( temp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = zero
                          call stdlib_xrot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp,h( ifrstm, jch-1 ), &
                                    1_ilp, c, s )
                          call stdlib_xrot( jch-ifrstm, t( ifrstm, jch ), 1_ilp,t( ifrstm, jch-1 ), 1_ilp,&
                                     c, s )
                          if( ilz )call stdlib_xrot( n, z( 1_ilp, jch ), 1_ilp, z( 1_ilp, jch-1 ), 1_ilp,c, s )
              ...
              70 continue
              temp = h( ilast, ilast )
              call stdlib_xlartg( temp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = zero
              call stdlib_xrot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp,h( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              call stdlib_xrot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp,t( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              if( ilz )call stdlib_xrot( n, z( 1_ilp, ilast ), 1_ilp, z( 1_ilp, ilast-1 ), 1_ilp, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alphar, alphai,
                                    ! and beta
                                    80 continue
              if( t( ilast, ilast )<zero ) then
              ...
              if( ( iiter / 10_ilp )*10_ilp==iiter ) then
                 ...
                 ! shifts based on the generalized eigenvalues of the
                 ! bottom-right 2x2 block of a and b. the first eigenvalue
                 ! returned by stdlib_xlag2 is the wilkinson shift (aep p.512_xdp),
                 call stdlib_xlag2( h( ilast-1, ilast-1 ), ldh,t( ilast-1, ilast-1 ), ldt, &
                           safmin*safety, s1,s2, wr, wr2, wi )
                 if ( abs( (wr/s1)*t( ilast, ilast ) - h( ilast, ilast ) )> abs( (wr2/s2)*t( &
                           ilast, ilast )- h( ilast, ilast ) ) ) then
                    temp = wr
              ...
              temp = s1*h( istart, istart ) - wr*t( istart, istart )
              temp2 = s1*h( istart+1, istart )
              call stdlib_xlartg( temp, temp2, c, s, tempr )
              ! sweep
              loop_190: do j = istart, ilast - 1
                 if( j>istart ) then
                    temp = h( j, j-1 )
                    call stdlib_xlartg( temp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = zero
                 end if
                 do jc = j, ilastm
                    temp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 temp = t( j+1, j+1 )
                 call stdlib_xlartg( temp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = zero
                 do jr = ifrstm, min( j+2, ilast )
                    temp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -s*h( jr, j+1 ) + c*h( jr, j )
              ...
              if( ifirst+1==ilast ) then
                 ...
                 ! step 1: standardize, that is, rotate so that
                         ...
                         ! b = (         )  with b11 non-negative.
                             ! (  0  b22 )
                 call stdlib_xlasv2( t( ilast-1, ilast-1 ), t( ilast-1, ilast ),t( ilast, ilast ),&
                            b22, b11, sr, cr, sl, cl )
                 if( b11<zero ) then
                    cr = -cr
                    sr = -sr
                 ...
                 call stdlib_xrot( ilastm+1-ifirst, h( ilast-1, ilast-1 ), ldh,h( ilast, ilast-1 )&
                           , ldh, cl, sl )
                 call stdlib_xrot( ilast+1-ifrstm, h( ifrstm, ilast-1 ), 1_ilp,h( ifrstm, ilast ), 1_ilp, &
                           cr, sr )
                 if( ilast<ilastm )call stdlib_xrot( ilastm-ilast, t( ilast-1, ilast+1 ), ldt,t( &
                           ilast, ilast+1 ), ldt, cl, sl )
                 if( ifrstm<ilast-1 )call stdlib_xrot( ifirst-ifrstm, t( ifrstm, ilast-1 ), 1_ilp,t( &
                           ifrstm, ilast ), 1_ilp, cr, sr )
                 if( ilq )call stdlib_xrot( n, q( 1_ilp, ilast-1 ), 1_ilp, q( 1_ilp, ilast ), 1_ilp, cl,sl )
                           
                 if( ilz )call stdlib_xrot( n, z( 1_ilp, ilast-1 ), 1_ilp, z( 1_ilp, ilast ), 1_ilp, cr,sr )
                 ...
                 if( abs( c11r )+abs( c11i )+abs( c12 )>abs( c21 )+abs( c22r )+abs( c22i ) ) &
                           then
                    t1 = stdlib_xlapy3( c12, c11r, c11i )
                    cz = c12 / t1
                    szr = -c11r / t1
                    szi = -c11i / t1
                 else
                    cz = stdlib_xlapy2( c22r, c22i )
                    if( cz<=safmin ) then
                       cz = zero
                       szr = one
                       szi = zero
                 ...
                 b1r = cq*cz*b11 + tempr*b22
                 b1i = tempi*b22
                 b1a = stdlib_xlapy2( b1r, b1i )
                 b2r = cq*cz*b22 + tempr*b11
                 b2i = -tempi*b11
                 b2a = stdlib_xlapy2( b2r, b2i )
                 ! normalize so beta > 0, and im( alpha1 ) > 0
                 beta( ilast-1 ) = b1a
                 beta( ilast ) = b2a
                 alphar( ilast-1 ) = ( wr*b1a )*s1inv
                 ...
                 v( 3_ilp ) = ad32l*ad21l
                 istart = ifirst
                 call stdlib_xlarfg( 3_ilp, v( 1_ilp ), v( 2_ilp ), 1_ilp, tau )
                 v( 1_ilp ) = one
                 ! sweep
                 loop_290: do j = istart, ilast - 2
                    ! all but last elements: use 3x3 householder transforms.
                    ...
                    if( j>istart ) then
                       ...
                       v( 2_ilp ) = h( j+1, j-1 )
                       v( 3_ilp ) = h( j+2, j-1 )
                       call stdlib_xlarfg( 3_ilp, h( j, j-1 ), v( 2_ilp ), 1_ilp, tau )
                       v( 1_ilp ) = one
                       h( j+1, j-1 ) = zero
                       h( j+2, j-1 ) = zero
                    end if
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
           ...
           if( n<=0_ilp ) then
              work( 1_ilp ) = cmplx( 1_ilp,KIND=xdp)
              return
           end if
           ! initialize q and z
           if( icompq==3_ilp )call stdlib_ylaset( 'FULL', n, n, czero, cone, q, ldq )
           ...
           safmin = stdlib_xlamch( 'S' )
           ulp = stdlib_xlamch( 'E' )*stdlib_xlamch( 'B' )
           anorm = stdlib_ylanhs( 'F', in, h( ilo, ilo ), ldh, rwork )
           bnorm = stdlib_ylanhs( 'F', in, t( ilo, ilo ), ldt, rwork )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           do j = ihi + 1, n
              ...
              if( absb>safmin ) then
                 ...
                 t( j, j ) = absb
                 if( ilschr ) then
                    call stdlib_yscal( j-1, signbc, t( 1_ilp, j ), 1_ilp )
                    call stdlib_yscal( j, signbc, h( 1_ilp, j ), 1_ilp )
                 else
                    call stdlib_yscal( 1_ilp, signbc, h( j, j ), 1_ilp )
                 end if
                 if( ilz )call stdlib_yscal( n, signbc, z( 1_ilp, j ), 1_ilp )
              else
                 t( j, j ) = czero
              end if
              alpha( j ) = h( j, j )
           ...
           loop_170: do jiter = 1, maxit
              ...
              loop_40: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          ctemp = h( jch, jch )
                          call stdlib_ylartg( ctemp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = czero
                          call stdlib_yrot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_yrot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_yrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, conjg(&
                                     s ) )
                          if( ilazr2 )h( jch, jch-1 ) = h( jch, jch-1 )*c
                          ilazr2 = .false.
                          if( abs1( t( jch+1, jch+1 ) )>=btol ) then
                       ...
                       do jch = j, ilast - 1
                          ctemp = t( jch, jch+1 )
                          call stdlib_ylartg( ctemp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = czero
                          if( jch<ilastm-1 )call stdlib_yrot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_yrot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_yrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, conjg(&
                                     s ) )
                          ctemp = h( jch+1, jch )
                          call stdlib_ylartg( ctemp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = czero
                          call stdlib_yrot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp,h( ifrstm, jch-1 ), &
                                    1_ilp, c, s )
                          call stdlib_yrot( jch-ifrstm, t( ifrstm, jch ), 1_ilp,t( ifrstm, jch-1 ), 1_ilp,&
                                     c, s )
                          if( ilz )call stdlib_yrot( n, z( 1_ilp, jch ), 1_ilp, z( 1_ilp, jch-1 ), 1_ilp,c, s )
              ...
              50 continue
              ctemp = h( ilast, ilast )
              call stdlib_ylartg( ctemp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = czero
              call stdlib_yrot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp,h( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              call stdlib_yrot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp,t( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              if( ilz )call stdlib_yrot( n, z( 1_ilp, ilast ), 1_ilp, z( 1_ilp, ilast-1 ), 1_ilp, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alpha and beta
              60 continue
              absb = abs( t( ilast, ilast ) )
              if( absb>safmin ) then
                 ...
                 t( ilast, ilast ) = absb
                 if( ilschr ) then
                    call stdlib_yscal( ilast-ifrstm, signbc, t( ifrstm, ilast ), 1_ilp )
                    call stdlib_yscal( ilast+1-ifrstm, signbc, h( ifrstm, ilast ),1_ilp )
                 else
                    call stdlib_yscal( 1_ilp, signbc, h( ilast, ilast ), 1_ilp )
                 end if
                 if( ilz )call stdlib_yscal( n, signbc, z( 1_ilp, ilast ), 1_ilp )
              else
                 t( ilast, ilast ) = czero
              end if
              alpha( ilast ) = h( ilast, ilast )
              ...
              if( ( iiter / 10_ilp )*10_ilp/=iiter ) then
                 ...
                 if( ctemp/=zero ) then
                    ...
                    shift = shift - ctemp*stdlib_yladiv( ctemp, ( x+y ) )
                 end if
              else
                 ! exceptional shift.  chosen for no particularly good reason.
                 if( ( iiter / 20_ilp )*20_ilp==iiter .and.bscale*abs1(t( ilast, ilast ))>safmin ) &
              ...
              ! initial q
              ctemp2 = ascale*h( istart+1, istart )
              call stdlib_ylartg( ctemp, ctemp2, c, s, ctemp3 )
              ! sweep
              loop_150: do j = istart, ilast - 1
                 if( j>istart ) then
                    ctemp = h( j, j-1 )
                    call stdlib_ylartg( ctemp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = czero
                 end if
                 do jc = j, ilastm
                    ctemp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 ctemp = t( j+1, j+1 )
                 call stdlib_ylartg( ctemp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = czero
                 do jr = ifrstm, min( j+2, ilast )
                    ctemp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -conjg( s )*h( jr, j+1 ) + c*h( jr, j )
     ...
     !! DGGBAL.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_xscal( m, rscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_xscal( m, lscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp)
                    if( k==i )cycle loop_40
                    call stdlib_xswap( m, v( i, 1_ilp ), ldv, v( k, 1_ilp ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
     ...
     !! ZGGBAL.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_ydscal( m, rscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_ydscal( m, lscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp)
                    if( k==i )cycle loop_40
                    call stdlib_yswap( m, v( i, 1_ilp ), ldv, v( k, 1_ilp ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
