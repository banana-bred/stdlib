...
pure module function stdlib_sum_1d_xdp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    real(xdp), intent(in) :: a(:)
    real(xdp) :: s
    real(xdp) :: abatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_1d_xdp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    real(xdp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    real(xdp) :: s
    ...
    abatch(1:r)       = merge( zero_xdp , a(1:r) , mask(1:r) )
    abatch(r+1:chunk) = zero_xdp
    do i = r+1, n-r, chunk
        abatch(1:chunk) = abatch(1:chunk) + merge( zero_xdp , a(i:i+chunk-1), mask(i:i+chunk-1) )
    end do
...
pure module function stdlib_sum_1d_cxdp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(xdp), intent(in) :: a(:)
    complex(xdp) :: s
    complex(xdp) :: abatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_1d_cxdp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(xdp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    complex(xdp) :: s
    ...
    abatch(1:r)       = merge( zero_cxdp , a(1:r) , mask(1:r) )
    abatch(r+1:chunk) = zero_cxdp
    do i = r+1, n-r, chunk
        abatch(1:chunk) = abatch(1:chunk) + merge( zero_cxdp , a(i:i+chunk-1), mask(i:i+chunk-1) )
    end do
...
pure module function stdlib_sum_kahan_1d_xdp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    real(xdp), intent(in) :: a(:)
    real(xdp) :: s
    real(xdp) :: sbatch(chunk)
    real(xdp) :: cbatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_kahan_1d_xdp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    real(xdp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    real(xdp) :: s
    ...
    sbatch(1:r) = merge( zero_xdp , a(1:r) , mask(1:r) )
    sbatch(r+1:chunk)  = zero_xdp
    cbatch = zero_xdp
    do i = r+1, n-r, chunk
      call kahan_kernel( a(i:i+chunk-1) , sbatch(1:chunk) , cbatch(1:chunk), mask(i:i+chunk-1) )
...
pure module function stdlib_sum_kahan_1d_cxdp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(xdp), intent(in) :: a(:)
    complex(xdp) :: s
    complex(xdp) :: sbatch(chunk)
    complex(xdp) :: cbatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_kahan_1d_cxdp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(xdp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    complex(xdp) :: s
    ...
    sbatch(1:r) = merge( zero_cxdp , a(1:r) , mask(1:r) )
    sbatch(r+1:chunk)  = zero_cxdp
    cbatch = zero_cxdp
    do i = r+1, n-r, chunk
      call kahan_kernel( a(i:i+chunk-1) , sbatch(1:chunk) , cbatch(1:chunk), mask(i:i+chunk-1) )
...
pure module function stdlib_sum_2d_xdp( x , mask ) result( s )
...
contains
    pure real(xdp) function sum_recast(b,n)
      integer(ilp), intent(in) :: n
      real(xdp), intent(in) :: b(n)
      sum_recast = stdlib_sum(b)
    end function
    pure real(xdp) function sum_recast_mask(b,m,n)
      integer(ilp), intent(in) :: n
      real(xdp), intent(in) :: b(n)
      logical, intent(in) :: m(n)
      sum_recast_mask = stdlib_sum(b,m)
...
pure module function stdlib_sum_2d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
    integer :: j 
...
pure module function stdlib_sum_3d_xdp( x , mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    real(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_3d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
    integer :: j 
...
pure module function stdlib_sum_4d_xdp( x , mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    real(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_4d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3), size(x,&
        & 4), mask=3<dim))
    integer :: j 
...
pure module function stdlib_sum_2d_cxdp( x , mask ) result( s )
...
contains
    pure complex(xdp) function sum_recast(b,n)
      integer(ilp), intent(in) :: n
      complex(xdp), intent(in) :: b(n)
      sum_recast = stdlib_sum(b)
    end function
    pure complex(xdp) function sum_recast_mask(b,m,n)
      integer(ilp), intent(in) :: n
      complex(xdp), intent(in) :: b(n)
      logical, intent(in) :: m(n)
      sum_recast_mask = stdlib_sum(b,m)
...
pure module function stdlib_sum_2d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
    integer :: j 
...
pure module function stdlib_sum_3d_cxdp( x , mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    complex(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_3d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
    integer :: j 
...
pure module function stdlib_sum_4d_cxdp( x , mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_4d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
        & size(x, 4), mask=3<dim))
    integer :: j 
...
pure module function stdlib_sum_kahan_2d_xdp( x , mask ) result( s )
    real(xdp), intent(in) :: x(:,:)
    logical, intent(in), optional :: mask(:,:)
    real(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_2d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
...
pure module function stdlib_sum_kahan_3d_xdp( x , mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    real(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_3d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
...
pure module function stdlib_sum_kahan_4d_xdp( x , mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    real(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_4d_dim_xdp( x , dim, mask ) result( s )
    real(xdp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    real(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3), size(x,&
...
pure module function stdlib_sum_kahan_2d_cxdp( x , mask ) result( s )
    complex(xdp), intent(in) :: x(:,:)
    logical, intent(in), optional :: mask(:,:)
    complex(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_2d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
...
pure module function stdlib_sum_kahan_3d_cxdp( x , mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    complex(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_3d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
...
pure module function stdlib_sum_kahan_4d_cxdp( x , mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(xdp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_4d_dim_cxdp( x , dim, mask ) result( s )
    complex(xdp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(xdp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
