submodule(stdlib_lapack_eig_svd_lsq) stdlib_lapack_eigv_tridiag
  ...
  contains
     ...
     pure module subroutine stdlib_I64_qlaebz( ijob, nitmax, n, mmax, minp, nbmin, abstol,reltol, pivmin, d, &
     !! DLAEBZ: contains the iteration loops which compute and use the
     !! function N(w), which is the count of eigenvalues of a symmetric
     !! tridiagonal matrix T less than or equal to its argument  w.  It
     !! performs a choice of two types of loops:
     ...
     !! values.
        ...
        ! =====================================================================
           ...
           mout = kl
           return
     end subroutine stdlib_I64_qlaebz
     ...
     pure integer(ilp64) module function stdlib_I64_qlaneg( n, d, lld, sigma, pivmin, r )
     !! DLANEG: computes the Sturm count, the number of negative pivots
     !! encountered while factoring tridiagonal T - sigma I = L D L^T.
     !! This implementation works directly on the factors without forming
     !! the tridiagonal matrix T.  The Sturm count is also the number of
     ...
     !! (Tech report version in LAWN 172 with the same title.)
        ...
        ! =====================================================================
           ...
           ! correct limit.
              if( sawnan ) then
                 ...
                 do j = bj, min(bj+blklen-1, r-1)
                    ...
                    if( dplus<zero ) neg1 = neg1 + 1_ilp64
                    tmp = t / dplus
                    if (stdlib_I64_qisnan(tmp)) tmp = one
                    t = tmp * lld(j) - sigma
                 end do
              end if
              negcnt = negcnt + neg1
           ...
           gamma = (t + sigma) + p
           if( gamma<zero ) negcnt = negcnt+1
           stdlib_I64_qlaneg = negcnt
     end function stdlib_I64_qlaneg
     ...
     pure module subroutine stdlib_I64_qlaed0( icompq, qsiz, n, d, e, q, ldq, qstore, ldqs,work, iwork, info &
     !! DLAED0: computes all eigenvalues and corresponding eigenvectors of a
     !! symmetric tridiagonal matrix using the divide and conquer method.
               )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           loop_70: do i = 0, spm1
              ...
              if( icompq==2_ilp64 ) then
                 call stdlib_I64_qsteqr( 'I', matsiz, d( submat ), e( submat ),q( submat, submat ), &
                           ldq, work, info )
                 if( info/=0 )go to 130
              else
                 call stdlib_I64_qsteqr( 'I', matsiz, d( submat ), e( submat ),work( iq-1+iwork( &
                           iqptr+curr ) ), matsiz, work,info )
                 if( info/=0 )go to 130
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_qgemm( 'N', 'N', qsiz, matsiz, matsiz, one,q( 1_ilp64, submat ), ldq, &
                    work( iq-1+iwork( iqptr+curr ) ), matsiz, zero, qstore( 1_ilp64, submat ),ldqs )
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.
           ! stdlib_I64_qlaed1 is used only for the full eigensystem of a tridiagonal
           ! matrix.
           ! stdlib_I64_qlaed7 handles the cases in which eigenvalues only or eigenvalues
           ! and eigenvectors of a full symmetric matrix (which was reduced to
           ! tridiagonal form) are desired.
                 if( icompq==2_ilp64 ) then
                    call stdlib_I64_qlaed1( matsiz, d( submat ), q( submat, submat ),ldq, iwork( &
                    indxq+submat ),e( submat+msd2-1 ), msd2, work,iwork( subpbs+1 ), info )
                              
                 else
                    call stdlib_I64_qlaed7( icompq, matsiz, qsiz, tlvls, curlvl, curprb,d( submat ), &
                    qstore( 1_ilp64, submat ), ldqs,iwork( indxq+submat ), e( submat+msd2-1 ),msd2, &
                    work( iq ), iwork( iqptr ),iwork( iprmpt ), iwork( iperm ),iwork( igivpt ), &
                    iwork( igivcl ),work( igivnm ), work( iwrem ),iwork( subpbs+1 ), info )
           ...
           if( icompq==1_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_qcopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
              end do
              call stdlib_I64_qcopy( n, work, 1_ilp64, d, 1_ilp64 )
           else if( icompq==2_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_qcopy( n, q( 1_ilp64, j ), 1_ilp64, work( n*i+1 ), 1_ilp64 )
              end do
              call stdlib_I64_qcopy( n, work, 1_ilp64, d, 1_ilp64 )
              call stdlib_I64_qlacpy( 'A', n, n, work( n+1 ), n, q, ldq )
           else
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
           ...
           140 continue
           return
     end subroutine stdlib_I64_qlaed0
     ...
     pure module subroutine stdlib_I64_wlaed0( qsiz, n, d, e, q, ldq, qstore, ldqs, rwork,iwork, info )
     !! Using the divide and conquer method, ZLAED0: computes all eigenvalues
     !! of a symmetric tridiagonal matrix which is one diagonal block of
     !! those from reducing a dense or band Hermitian matrix and
     !! corresponding eigenvectors of the dense or band matrix.
        ...
        ! warning:      n could be as big as qsiz!
          ...
          ! $        then
           ...
           do i = 0, spm1
              ...
              ll = iq - 1_ilp64 + iwork( iqptr+curr )
              call stdlib_I64_qsteqr( 'I', matsiz, d( submat ), e( submat ),rwork( ll ), matsiz, &
                        rwork, info )
              call stdlib_I64_wlacrm( qsiz, matsiz, q( 1_ilp64, submat ), ldq, rwork( ll ),matsiz, qstore( &
                        1_ilp64, submat ), ldqs,rwork( iwrem ) )
              iwork( iqptr+curr+1 ) = iwork( iqptr+curr ) + matsiz**2_ilp64
              curr = curr + 1_ilp64
              if( info>0_ilp64 ) then
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.  stdlib_I64_wlaed7 handles the case
           ! when the eigenvectors of a full or band hermitian matrix (which
           ! was reduced to tridiagonal form) are desired.
           ! i am free to use q as a valuable working space until loop 150.
                 call stdlib_I64_wlaed7( matsiz, msd2, qsiz, tlvls, curlvl, curprb,d( submat ), &
                 qstore( 1_ilp64, submat ), ldqs,e( submat+msd2-1 ), iwork( indxq+submat ),rwork( iq ), &
                 iwork( iqptr ), iwork( iprmpt ),iwork( iperm ), iwork( igivpt ),iwork( igivcl ), &
                           rwork( igivnm ),q( 1_ilp64, submat ), rwork( iwrem ),iwork( subpbs+1 ), info )
                 if( info>0_ilp64 ) then
           ...
           do i = 1, n
              j = iwork( indxq+i )
              rwork( i ) = d( j )
              call stdlib_I64_wcopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
           end do
           call stdlib_I64_qcopy( n, rwork, 1_ilp64, d, 1_ilp64 )
           return
     end subroutine stdlib_I64_wlaed0
     ...
     pure module subroutine stdlib_I64_qlaed1( n, d, q, ldq, indxq, rho, cutpnt, work, iwork,info )
     !! DLAED1: computes the updated eigensystem of a diagonal
     !! matrix after modification by a rank-one symmetric matrix.  This
     !! routine is used only for the eigenproblem which requires all
     !! eigenvalues and eigenvectors of a tridiagonal matrix.  DLAED7 handles
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           ! the following values are integer pointers which indicate
           ! the portion of the workspace
           ! used by a particular array in stdlib_I64_qlaed2 and stdlib_I64_qlaed3.
           iz = 1_ilp64
           idlmda = iz + n
           iw = idlmda + n
           iq2 = iw + n
           ...
           ! form the z-vector which consists of the last row of q_1 and the
           ! first row of q_2.
           call stdlib_I64_qcopy( cutpnt, q( cutpnt, 1_ilp64 ), ldq, work( iz ), 1_ilp64 )
           zpp1 = cutpnt + 1_ilp64
           call stdlib_I64_qcopy( n-cutpnt, q( zpp1, zpp1 ), ldq, work( iz+cutpnt ), 1_ilp64 )
           ! deflate eigenvalues.
           call stdlib_I64_qlaed2( k, n, cutpnt, d, q, ldq, indxq, rho, work( iz ),work( idlmda ), &
           work( iw ), work( iq2 ),iwork( indx ), iwork( indxc ), iwork( indxp ),iwork( coltyp ), &
                     info )
           if( info/=0 )go to 20
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              is = ( iwork( coltyp )+iwork( coltyp+1 ) )*cutpnt +( iwork( coltyp+1 )+iwork( &
                        coltyp+2 ) )*( n-cutpnt ) + iq2
              call stdlib_I64_qlaed3( k, n, cutpnt, d, q, ldq, rho, work( idlmda ),work( iq2 ), iwork(&
                         indxc ), iwork( coltyp ),work( iw ), work( is ), info )
              if( info/=0 )go to 20
           ! prepare the indxq sorting permutation.
              n1 = k
              n2 = n - k
              call stdlib_I64_qlamrg( n1, n2, d, 1_ilp64, -1_ilp64, indxq )
           else
              do i = 1, n
                 indxq( i ) = i
              end do
           ...
           20 continue
           return
     end subroutine stdlib_I64_qlaed1
     ...
     pure module subroutine stdlib_I64_qlaed2( k, n, n1, d, q, ldq, indxq, rho, z, dlamda, w,q2, indx, indxc,&
     !! DLAED2: merges the two sets of eigenvalues together into a single
     !! sorted set.  Then it tries to deflate the size of the problem.
     !! There are two ways in which deflation can occur:  when two or more
     !! eigenvalues are close together or if there is a tiny entry in the
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           n1p1 = n1 + 1_ilp64
           if( rho<zero ) then
              call stdlib_I64_qscal( n2, mone, z( n1p1 ), 1_ilp64 )
           end if
           ! normalize z so that norm(z) = 1.  since z is the concatenation of
           ! two normalized vectors, norm2(z) = sqrt(2).
           t = one / sqrt( two )
           call stdlib_I64_qscal( n, t, z, 1_ilp64 )
           ! rho = abs( norm(z)**2 * rho )
           rho = abs( two*rho )
           ! sort the eigenvalues into increasing order
           do i = n1p1, n
           ...
           call stdlib_I64_qlamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indxc )
           do i = 1, n
              indx( i ) = indxq( indxc( i ) )
           end do
           ! calculate the allowable deflation tolerance
           imax = stdlib_I64_iqamax( n, z, 1_ilp64 )
           jmax = stdlib_I64_iqamax( n, d, 1_ilp64 )
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 i = indx( j )
                 call stdlib_I64_qcopy( n, q( 1_ilp64, i ), 1_ilp64, q2( iq2 ), 1_ilp64 )
                 dlamda( j ) = d( i )
                 iq2 = iq2 + n
              end do
              call stdlib_I64_qlacpy( 'A', n, n, q2, n, q, ldq )
              call stdlib_I64_qcopy( n, dlamda, 1_ilp64, d, 1_ilp64 )
              go to 190
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( nj ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 if( coltyp( nj )/=coltyp( pj ) )coltyp( nj ) = 2_ilp64
                 coltyp( pj ) = 4_ilp64
                 call stdlib_I64_qrot( n, q( 1_ilp64, pj ), 1_ilp64, q( 1_ilp64, nj ), 1_ilp64, c, s )
                 t = d( pj )*c**2_ilp64 + d( nj )*s**2_ilp64
                 d( nj ) = d( pj )*s**2_ilp64 + d( nj )*c**2_ilp64
                 d( pj ) = t
                 k2 = k2 - 1_ilp64
           ...
           do j = 1, ctot( 1 )
              js = indx( i )
              call stdlib_I64_qcopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
           end do
           do j = 1, ctot( 2 )
              js = indx( i )
              call stdlib_I64_qcopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              call stdlib_I64_qcopy( n2, q( n1+1, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
              iq2 = iq2 + n2
           ...
           do j = 1, ctot( 4 )
              js = indx( i )
              call stdlib_I64_qcopy( n, q( 1_ilp64, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              iq2 = iq2 + n
              z( i ) = d( js )
              i = i + 1_ilp64
           end do
           ...
           ! into the last n - k slots of d and q respectively.
           if( k<n ) then
              call stdlib_I64_qlacpy( 'A', n, ctot( 4_ilp64 ), q2( iq1 ), n,q( 1_ilp64, k+1 ), ldq )
              call stdlib_I64_qcopy( n-k, z( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
           end if
           ! copy ctot into coltyp for referencing in stdlib_I64_qlaed3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           190 continue
           return
     end subroutine stdlib_I64_qlaed2
     ...
     pure module subroutine stdlib_I64_qlaed3( k, n, n1, d, q, ldq, rho, dlamda, q2, indx,ctot, w, s, info )
     !! DLAED3: finds the roots of the secular equation, as defined by the
     !! values in D, W, and RHO, between 1 and K.  It makes the
     !! appropriate calls to DLAED4 and then updates the eigenvectors by
     !! multiplying the matrix of eigenvectors of the pair of eigensystems
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! this code.
           do i = 1, k
              dlamda( i ) = stdlib_I64_qlamc3( dlamda( i ), dlamda( i ) ) - dlamda( i )
           end do
           do j = 1, k
              call stdlib_I64_qlaed4( k, j, dlamda, w, q( 1_ilp64, j ), rho, d( j ), info )
              ! if the zero finder fails, the computation is terminated.
              if( info/=0 )go to 120
           end do
           if( k==1 )go to 110
           ...
           ! compute updated w.
           call stdlib_I64_qcopy( k, w, 1_ilp64, s, 1_ilp64 )
           ! initialize w(i) = q(i,i)
           call stdlib_I64_qcopy( k, q, ldq+1, w, 1_ilp64 )
           do j = 1, k
              do i = 1, j - 1
                 w( i ) = w( i )*( q( i, j ) / ( dlamda( i )-dlamda( j ) ) )
              end do
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_qnrm2( k, s, 1_ilp64 )
              do i = 1, k
                 ii = indx( i )
                 q( i, j ) = s( ii ) / temp
              end do
           ...
           n12 = ctot( 1_ilp64 ) + ctot( 2_ilp64 )
           n23 = ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_qlacpy( 'A', n23, k, q( ctot( 1_ilp64 )+1_ilp64, 1_ilp64 ), ldq, s, n23 )
           iq2 = n1*n12 + 1_ilp64
           if( n23/=0_ilp64 ) then
              call stdlib_I64_qgemm( 'N', 'N', n2, k, n23, one, q2( iq2 ), n2, s, n23,zero, q( n1+1, &
                        1_ilp64 ), ldq )
           else
              call stdlib_I64_qlaset( 'A', n2, k, zero, zero, q( n1+1, 1_ilp64 ), ldq )
           end if
           call stdlib_I64_qlacpy( 'A', n12, k, q, ldq, s, n12 )
           if( n12/=0_ilp64 ) then
              call stdlib_I64_qgemm( 'N', 'N', n1, k, n12, one, q2, n1, s, n12, zero, q,ldq )
           else
              call stdlib_I64_qlaset( 'A', n1, k, zero, zero, q( 1_ilp64, 1_ilp64 ), ldq )
           end if
           120 continue
           return
     end subroutine stdlib_I64_qlaed3
     ...
     pure module subroutine stdlib_I64_qlaed4( n, i, d, z, delta, rho, dlam, info )
     !! This subroutine computes the I-th updated eigenvalue of a symmetric
     !! rank-one modification to a diagonal matrix whose elements are
     !! given in the array d, and that
     !! D(i) < D(j)  for  i < j
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==2_ilp64 ) then
              call stdlib_I64_qlaed5( i, d, z, delta, rho, dlam )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_I64_qlamch( 'EPSILON' )
           ...
           if( i==n ) then
              ...
              if( .not.swtch3 ) then
                 ...
                 zz( 2_ilp64 ) = z( ii )*z( ii )
                 call stdlib_I64_qlaed6( niter, orgati, c, delta( iim1 ), zz, w, eta,info )
                 if( info/=0 )go to 250
              end if
              ! note, eta should be positive if w is negative, and
              ! eta should be negative otherwise. however,
           ...
           250 continue
           return
     end subroutine stdlib_I64_qlaed4
     ...
     pure module subroutine stdlib_I64_qlaed5( i, d, z, delta, rho, dlam )
     !! This subroutine computes the I-th eigenvalue of a symmetric rank-one
     !! modification of a 2-by-2 diagonal matrix
     !! diag( D )  +  RHO * Z * transpose(Z) .
     !! The diagonal elements in the array D are assumed to satisfy
     ...
     !! Z is one.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(qp), intent(out) :: delta(2_ilp64)
        ! =====================================================================
           ...
           return
     end subroutine stdlib_I64_qlaed5
     ...
     pure module subroutine stdlib_I64_qlaed6( kniter, orgati, rho, d, z, finit, tau, info )
     !! DLAED6: computes the positive or negative root (closest to the origin)
     !! of
     !! z(1)        z(2)        z(3)
     !! f(x) =   rho + --------- + ---------- + ---------
     ...
     !! in some extremely rare situations.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(qp), intent(out) :: tau
           ! Array Arguments 
           real(qp), intent(in) :: d(3_ilp64), z(3_ilp64)
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: maxit = 40_ilp64
           ...
           ! Local Arrays 
           real(qp) :: dscale(3_ilp64), zscale(3_ilp64)
           ! Local Scalars 
           logical(lk) :: scale
           integer(ilp64) :: i, iter, niter
           real(qp) :: a, b, base, c, ddf, df, eps, erretm, eta, f, fc, sclfac, sclinv, small1, &
           ...
           ! others but recomputed at each call
           eps = stdlib_I64_qlamch( 'EPSILON' )
           base = stdlib_I64_qlamch( 'BASE' )
           small1 = base**( int( log( stdlib_I64_qlamch( 'SAFMIN' ) ) / log( base ) /three,KIND=ilp64) )
           ...
           if( scale )tau = tau*sclinv
           return
     end subroutine stdlib_I64_qlaed6
     ...
     pure module subroutine stdlib_I64_qlaed7( icompq, n, qsiz, tlvls, curlvl, curpbm, d, q,ldq, indxq, rho, &
     !! DLAED7: computes the updated eigensystem of a diagonal
     !! matrix after modification by a rank-one symmetric matrix. This
     !! routine is used only for the eigenproblem which requires all
     !! eigenvalues and optionally eigenvectors of a dense symmetric matrix
     ...
     !! the overall problem.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp64), intent(out) :: indxq(*), iwork(*)
           real(qp), intent(inout) :: d(*), givnum(2_ilp64,*), q(ldq,*), qstore(*)
           real(qp), intent(out) :: work(*)
        ! =====================================================================
           ...
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_qlaed8 and stdlib_I64_qlaed9.
           if( icompq==1_ilp64 ) then
              ldq2 = qsiz
           else
              ldq2 = n
           ...
           curr = ptr + curpbm
           call stdlib_I64_qlaeda( n, tlvls, curlvl, curpbm, prmptr, perm, givptr,givcol, givnum, &
                     qstore, qptr, work( iz ),work( iz+n ), info )
           ! when solving the final problem, we no longer need the stored data,
           ! so we will overwrite the data from this level onto the previously
           ! used storage space.
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_qlaed8( icompq, k, n, qsiz, d, q, ldq, indxq, rho, cutpnt,work( iz ), work(&
            idlmda ), work( iq2 ), ldq2,work( iw ), perm( prmptr( curr ) ), givptr( curr+1 ),&
            givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), iwork( indxp ),iwork( indx ),&
                       info )
           prmptr( curr+1 ) = prmptr( curr ) + n
           ...
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_qlaed9( k, 1_ilp64, k, n, d, work( is ), k, rho, work( idlmda ),work( iw ), &
                        qstore( qptr( curr ) ), k, info )
              if( info/=0 )go to 30
              if( icompq==1_ilp64 ) then
                 call stdlib_I64_qgemm( 'N', 'N', qsiz, k, k, one, work( iq2 ), ldq2,qstore( qptr( &
                           curr ) ), k, zero, q, ldq )
              end if
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
           ! prepare the indxq sorting permutation.
           ...
           30 continue
           return
     end subroutine stdlib_I64_qlaed7
     ...
     pure module subroutine stdlib_I64_wlaed7( n, cutpnt, qsiz, tlvls, curlvl, curpbm, d, q,ldq, rho, indxq, &
     !! ZLAED7: computes the updated eigensystem of a diagonal
     !! matrix after modification by a rank-one symmetric matrix. This
     !! routine is used only for the eigenproblem which requires all
     !! eigenvalues and optionally eigenvectors of a dense or banded
     ...
     !! the overall problem.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp64), intent(out) :: indxq(*), iwork(*)
           real(qp), intent(inout) :: d(*), givnum(2_ilp64,*), qstore(*)
           real(qp), intent(out) :: rwork(*)
           complex(qp), intent(inout) :: q(ldq,*)
           complex(qp), intent(out) :: work(*)
        ! =====================================================================
           ...
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_qlaed2 and stdlib_I64_dlaed3.
           iz = 1_ilp64
           idlmda = iz + n
           iw = idlmda + n
           iq = iw + n
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_wlaed8( k, n, qsiz, q, ldq, d, rho, cutpnt, rwork( iz ),rwork( idlmda ), &
           work, qsiz, rwork( iw ),iwork( indxp ), iwork( indx ), indxq,perm( prmptr( curr ) ), &
           givptr( curr+1 ),givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), info )
           ...
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_qlaed9( k, 1_ilp64, k, n, d, rwork( iq ), k, rho,rwork( idlmda ), rwork( iw ),&
                        qstore( qptr( curr ) ), k, info )
              call stdlib_I64_wlacrm( qsiz, k, work, qsiz, qstore( qptr( curr ) ), k, q,ldq, rwork( &
                        iq ) )
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
              if( info/=0_ilp64 ) then
                 return
           ...
           return
     end subroutine stdlib_I64_wlaed7
     ...
     pure module subroutine stdlib_I64_qlaed8( icompq, k, n, qsiz, d, q, ldq, indxq, rho,cutpnt, z, dlamda, &
     !! DLAED8: merges the two sets of eigenvalues together into a single
     !! sorted set.  Then it tries to deflate the size of the problem.
     !! There are two ways in which deflation can occur:  when two or more
     !! eigenvalues are close together or if there is a tiny element in the
     ...
     !! equation problem is reduced by one.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp64), intent(inout) :: indxq(*)
           real(qp), intent(inout) :: d(*), q(ldq,*), z(*)
           real(qp), intent(out) :: dlamda(*), givnum(2_ilp64,*), q2(ldq2,*), w(*)
        ! =====================================================================
           ! Parameters 
           real(qp), parameter :: mone = -1.0_qp
           ...
           i = 1_ilp64
           j = cutpnt + 1_ilp64
           call stdlib_I64_qlamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indx )
           do i = 1, n
              d( i ) = dlamda( indx( i ) )
              z( i ) = w( indx( i ) )
           end do
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              if( icompq==0_ilp64 ) then
                 ...
                 do j = 1, n
                    perm( j ) = indxq( indx( j ) )
                    call stdlib_I64_qcopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
                 end do
                 call stdlib_I64_qlacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ),ldq )
              end if
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 2_ilp64, givptr ) = s
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_qrot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j &
                              ) ) ), 1_ilp64, c, s )
                 end if
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
           ...
           if( k<n ) then
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_qcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              else
                 call stdlib_I64_qcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
                 call stdlib_I64_qlacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2,q( 1_ilp64, k+1 ), ldq )
              end if
           end if
           return
     end subroutine stdlib_I64_qlaed8
     ...
     pure module subroutine stdlib_I64_wlaed8( k, n, qsiz, q, ldq, d, rho, cutpnt, z, dlamda,q2, ldq2, w, &
     !! ZLAED8: merges the two sets of eigenvalues together into a single
     !! sorted set.  Then it tries to deflate the size of the problem.
     !! There are two ways in which deflation can occur:  when two or more
     !! eigenvalues are close together or if there is a tiny element in the
     ...
     !! equation problem is reduced by one.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp64), intent(inout) :: indxq(*)
           real(qp), intent(inout) :: d(*), z(*)
           real(qp), intent(out) :: dlamda(*), givnum(2_ilp64,*), w(*)
           complex(qp), intent(inout) :: q(ldq,*)
           complex(qp), intent(out) :: q2(ldq2,*)
        ! =====================================================================
           ! Parameters 
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 perm( j ) = indxq( indx( j ) )
                 call stdlib_I64_wcopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
              end do
              call stdlib_I64_wlacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ), ldq )
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 1_ilp64, givptr ) = c
                 givnum( 2_ilp64, givptr ) = s
                 call stdlib_I64_wdrot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j ) &
                           ) ), 1_ilp64, c, s )
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
                 d( jlam ) = t
           ...
           if( k<n ) then
              call stdlib_I64_qcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              call stdlib_I64_wlacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2, q( 1_ilp64, k+1 ),ldq )
           end if
           return
     end subroutine stdlib_I64_wlaed8
     ...
     pure module subroutine stdlib_I64_qlaed9( k, kstart, kstop, n, d, q, ldq, rho, dlamda, w,s, lds, info )
     !! DLAED9: finds the roots of the secular equation, as defined by the
     !! values in D, Z, and RHO, between KSTART and KSTOP.  It makes the
     !! appropriate calls to DLAED4 and then stores the new matrix of
     !! eigenvectors for use in calculating the next level of Z vectors.
        ...
        ! =====================================================================
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_qnrm2( k, q( 1_ilp64, j ), 1_ilp64 )
              do i = 1, k
                 s( i, j ) = q( i, j ) / temp
              end do
           end do
           120 continue
           return
     end subroutine stdlib_I64_qlaed9
