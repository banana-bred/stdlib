  ...
  contains
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_xgetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xgetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_ygetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ygetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_xgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_xlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_xlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_xgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_xlantr( 'M', 'U', 'N', info, info, af, ldaf,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_xlange( 'M', n, info, a, lda, work ) / rpvgrw
                 end if
                 work( 1_ilp ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_xlange( norm, n, n, a, lda, work )
           rpvgrw = stdlib_xlantr( 'M', 'U', 'N', n, n, af, ldaf, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_xlange( 'M', n, n, a, lda, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_xgecon( norm, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_xgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_ygeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_ylaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_ylacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_ygetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_ylantr( 'M', 'U', 'N', info, info, af, ldaf,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_ylange( 'M', n, info, a, lda, rwork ) /rpvgrw
                 end if
                 rwork( 1_ilp ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_ylange( norm, n, n, a, lda, rwork )
           rpvgrw = stdlib_ylantr( 'M', 'U', 'N', n, n, af, ldaf, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_ylange( 'M', n, n, a, lda, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_ygecon( norm, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ygetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_ygerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_xgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xgbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_ygbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ygbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_xgbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_xlaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp )
                 j2 = min( j+kl, n )
                 call stdlib_xcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp,afb( kl+ku+1-j+j1, j ), 1_ilp )
                           
              end do
              call stdlib_xgbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_xlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp, &
                           kl+ku+2-info ), 1_ilp ), ldafb,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_xlangb( norm, n, kl, ku, ab, ldab, work )
           rpvgrw = stdlib_xlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_xlangb( 'M', n, kl, ku, ab, ldab, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_xgbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, iwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xgbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_xgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_ygbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_ylaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp )
                 j2 = min( j+kl, n )
                 call stdlib_ycopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp,afb( kl+ku+1-j+j1, j ), 1_ilp )
                           
              end do
              call stdlib_ygbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_ylantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp, &
                           kl+ku+2-info ), 1_ilp ), ldafb,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_ylangb( norm, n, kl, ku, ab, ldab, rwork )
           rpvgrw = stdlib_ylantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_ylangb( 'M', n, kl, ku, ab, ldab, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_ygbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, rwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ygbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_ygbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ...
              if( n>1_ilp ) then
                 call stdlib_xcopy( n-1, dl, 1_ilp, dlf, 1_ilp )
                 call stdlib_xcopy( n-1, du, 1_ilp, duf, 1_ilp )
              end if
              call stdlib_xgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_xlangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xgtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,iwork, info )
                     
           ! compute the solution vectors x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xgttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_xgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the lu factorization of a.
              call stdlib_ycopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp ) then
                 call stdlib_ycopy( n-1, dl, 1_ilp, dlf, 1_ilp )
                 call stdlib_ycopy( n-1, du, 1_ilp, duf, 1_ilp )
              end if
              call stdlib_ygttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_ylangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_ygtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ygttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_ygtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_I64_xgetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xgetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_I64_qgetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qgetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_I64_ygetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ygetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of a.
           call stdlib_I64_wgetrf( n, n, a, lda, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wgetrs( 'NO TRANSPOSE', n, nrhs, a, lda, ipiv, b, ldb,info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_xgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_xlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_I64_xlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_I64_xgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_I64_xlantr( 'M', 'U', 'N', info, info, af, ldaf,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_I64_xlange( 'M', n, info, a, lda, work ) / rpvgrw
                 end if
                 work( 1_ilp64 ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_xlange( norm, n, n, a, lda, work )
           rpvgrw = stdlib_I64_xlantr( 'M', 'U', 'N', n, n, af, ldaf, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_xlange( 'M', n, n, a, lda, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xgecon( norm, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_xgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_qgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_qlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_I64_qlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_I64_qgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_I64_qlantr( 'M', 'U', 'N', info, info, af, ldaf,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_I64_qlange( 'M', n, info, a, lda, work ) / rpvgrw
                 end if
                 work( 1_ilp64 ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_qlange( norm, n, n, a, lda, work )
           rpvgrw = stdlib_I64_qlantr( 'M', 'U', 'N', n, n, af, ldaf, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_qlange( 'M', n, n, a, lda, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qgecon( norm, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_qgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_ygeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_ylaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_I64_ylacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_I64_ygetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_I64_ylantr( 'M', 'U', 'N', info, info, af, ldaf,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_I64_ylange( 'M', n, info, a, lda, rwork ) /rpvgrw
                 end if
                 rwork( 1_ilp64 ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_ylange( norm, n, n, a, lda, rwork )
           rpvgrw = stdlib_I64_ylantr( 'M', 'U', 'N', n, n, af, ldaf, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_ylange( 'M', n, n, a, lda, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ygecon( norm, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ygetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_ygerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_wgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_wlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_I64_wlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_I64_wgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_I64_wlantr( 'M', 'U', 'N', info, info, af, ldaf,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_I64_wlange( 'M', n, info, a, lda, rwork ) /rpvgrw
                 end if
                 rwork( 1_ilp64 ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_wlange( norm, n, n, a, lda, rwork )
           rpvgrw = stdlib_I64_wlantr( 'M', 'U', 'N', n, n, af, ldaf, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_wlange( 'M', n, n, a, lda, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wgecon( norm, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_wgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_I64_xgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xgbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_I64_qgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qgbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_I64_ygbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ygbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_I64_wgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wgbtrs( 'NO TRANSPOSE', n, kl, ku, nrhs, ab, ldab, ipiv,b, ldb, info )
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_xgbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_xlaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp64 )
                 j2 = min( j+kl, n )
                 call stdlib_I64_xcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp64,afb( kl+ku+1-j+j1, j ), 1_ilp64 )
                           
              end do
              call stdlib_I64_xgbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_I64_xlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp64, &
                           kl+ku+2-info ), 1_ilp64 ), ldafb,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_I64_xlangb( norm, n, kl, ku, ab, ldab, work )
           rpvgrw = stdlib_I64_xlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_xlangb( 'M', n, kl, ku, ab, ldab, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xgbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, iwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xgbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_xgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_qgbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_qlaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp64 )
                 j2 = min( j+kl, n )
                 call stdlib_I64_qcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp64,afb( kl+ku+1-j+j1, j ), 1_ilp64 )
                           
              end do
              call stdlib_I64_qgbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_I64_qlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp64, &
                           kl+ku+2-info ), 1_ilp64 ), ldafb,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_I64_qlangb( norm, n, kl, ku, ab, ldab, work )
           rpvgrw = stdlib_I64_qlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_qlangb( 'M', n, kl, ku, ab, ldab, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qgbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, iwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qgbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_qgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_ygbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_ylaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp64 )
                 j2 = min( j+kl, n )
                 call stdlib_I64_ycopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp64,afb( kl+ku+1-j+j1, j ), 1_ilp64 )
                           
              end do
              call stdlib_I64_ygbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_I64_ylantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp64, &
                           kl+ku+2-info ), 1_ilp64 ), ldafb,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_I64_ylangb( norm, n, kl, ku, ab, ldab, rwork )
           rpvgrw = stdlib_I64_ylantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_ylangb( 'M', n, kl, ku, ab, ldab, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ygbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, rwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ygbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_ygbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_wgbequ( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, infequ )
                        
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_wlaqgb( n, n, kl, ku, ab, ldab, r, c, rowcnd, colcnd,amax, equed )
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp64 )
                 j2 = min( j+kl, n )
                 call stdlib_I64_wcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp64,afb( kl+ku+1-j+j1, j ), 1_ilp64 )
                           
              end do
              call stdlib_I64_wgbtrf( n, n, kl, ku, afb, ldafb, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 ...
                 rpvgrw = stdlib_I64_wlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp64, &
                           kl+ku+2-info ), 1_ilp64 ), ldafb,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_I64_wlangb( norm, n, kl, ku, ab, ldab, rwork )
           rpvgrw = stdlib_I64_wlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_I64_wlangb( 'M', n, kl, ku, ab, ldab, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wgbcon( norm, n, kl, ku, afb, ldafb, ipiv, anorm, rcond,work, rwork, info )
                     
           ! compute the solution matrix x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wgbtrs( trans, n, kl, ku, nrhs, afb, ldafb, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_wgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ...
              if( n>1_ilp64 ) then
                 call stdlib_I64_xcopy( n-1, dl, 1_ilp64, dlf, 1_ilp64 )
                 call stdlib_I64_xcopy( n-1, du, 1_ilp64, duf, 1_ilp64 )
              end if
              call stdlib_I64_xgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_xlangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xgtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,iwork, info )
                     
           ! compute the solution vectors x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xgttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_xgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ...
              if( n>1_ilp64 ) then
                 call stdlib_I64_qcopy( n-1, dl, 1_ilp64, dlf, 1_ilp64 )
                 call stdlib_I64_qcopy( n-1, du, 1_ilp64, duf, 1_ilp64 )
              end if
              call stdlib_I64_qgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_qlangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qgtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,iwork, info )
                     
           ! compute the solution vectors x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qgttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_qgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the lu factorization of a.
              call stdlib_I64_ycopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 ) then
                 call stdlib_I64_ycopy( n-1, dl, 1_ilp64, dlf, 1_ilp64 )
                 call stdlib_I64_ycopy( n-1, du, 1_ilp64, duf, 1_ilp64 )
              end if
              call stdlib_I64_ygttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_ylangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ygtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ygttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_ygtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the lu factorization of a.
              call stdlib_I64_wcopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 ) then
                 call stdlib_I64_wcopy( n-1, dl, 1_ilp64, dlf, 1_ilp64 )
                 call stdlib_I64_wcopy( n-1, du, 1_ilp64, duf, 1_ilp64 )
              end if
              call stdlib_I64_wgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_I64_wlangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wgtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wgttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_wgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
