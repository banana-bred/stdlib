...
!! Compute inverse of a square matrix    
     ...
     module subroutine stdlib_linalg_invert_inplace_x(a,pivot,err)
         ...
         if (info==0) then
            ...
            ! Get optimal worksize (returned in work(1)) (inflate by a 5% safety margin)
            nb = stdlib_ilaenv(1,'xgetri',' ',n,-1,-1,-1)
            lwork = max(1,min(huge(0_ilp),ceiling(1.05_xdp*real(n,xdp)*nb,kind=ilp)))
     ...
     module subroutine stdlib_linalg_invert_split_x(a,inva,pivot,err)
         ...
         if (any(sa/=sinva)) then
             ...
             !> Compute matrix inverse
             call stdlib_linalg_invert_inplace_x(inva,pivot=pivot,err=err0)      
     ...
     module function stdlib_linalg_inverse_x(a,err) result(inva)
         ...
         !> Compute matrix inverse
         call stdlib_linalg_invert_inplace_x(inva,err=err)
     ...
     module function stdlib_linalg_inverse_x_operator(a) result(inva)
         ...
         ! Provide an error handler to return NaNs on issues
         inva = stdlib_linalg_inverse_x(a,err=err)
         ...
         if (err%error()) then 
            ...
            inva = ieee_value(1.0_xdp,ieee_quiet_nan)
         endif
     ...
     module subroutine stdlib_linalg_invert_inplace_y(a,pivot,err)
         ...
         if (info==0) then
            ...
            ! Get optimal worksize (returned in work(1)) (inflate by a 5% safety margin)
            nb = stdlib_ilaenv(1,'ygetri',' ',n,-1,-1,-1)
            lwork = max(1,min(huge(0_ilp),ceiling(1.05_xdp*real(n,xdp)*nb,kind=ilp)))
     ...
     module subroutine stdlib_linalg_invert_split_y(a,inva,pivot,err)
         ...
         if (any(sa/=sinva)) then
             ...
             !> Compute matrix inverse
             call stdlib_linalg_invert_inplace_y(inva,pivot=pivot,err=err0)      
     ...
     module function stdlib_linalg_inverse_y(a,err) result(inva)
         ...
         !> Compute matrix inverse
         call stdlib_linalg_invert_inplace_y(inva,err=err)
     ...
     module function stdlib_linalg_inverse_y_operator(a) result(inva)
         ...
         ! Provide an error handler to return NaNs on issues
         inva = stdlib_linalg_inverse_y(a,err=err)
         ...
         if (err%error()) then 
            ...
            inva = cmplx(ieee_value(1.0_xdp,ieee_quiet_nan), &
                         ieee_value(1.0_xdp,ieee_quiet_nan), kind=xdp)
         endif
