  ...
  contains
     ...
     !! positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_xpotrf( uplo, n, b, ldb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_xsyev( jobz, uplo, n, a, lda, w, work, lwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_xtrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, one,b, ldb, a, lda )
                 ...
                 call stdlib_xtrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, one,b, ldb, a, lda )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_xsyevd( jobz, uplo, n, a, lda, w, work, lwork, iwork, liwork,info )
           lopt = max( real( lopt,KIND=xdp), real( work( 1_ilp ),KIND=xdp) )
           liopt = max( real( liopt,KIND=xdp), real( iwork( 1_ilp ),KIND=xdp) )
           if( wantz .and. info==0_ilp ) then
              ! backtransform eigenvectors to the original problem.
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ! for a*x=(lambda)*b*x and a*b*x=(lambda)*x;
                 ...
                 call stdlib_xtrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, n, one,b, ldb, a, lda )
                 ...
                 call stdlib_xtrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, n, one,b, ldb, a, lda )
     ...
     !! range of values or a range of indices for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_xsyevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol,m, w, z, ldz, &
                     work, lwork, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_xtrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, m, one, b,ldb, z, ldz )
                 ...
                 call stdlib_xtrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, m, one, b,ldb, z, ldz )
     ...
     !! and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_xpptrf( uplo, n, bp, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xspgst( itype, uplo, n, ap, bp, info )
           call stdlib_xspev( jobz, uplo, n, ap, w, z, ldz, work, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 do j = 1, neig
                    call stdlib_xtpsv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**t*y
                 ...
                 do j = 1, neig
                    call stdlib_xtpmv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              end if
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xspgst( itype, uplo, n, ap, bp, info )
           call stdlib_xspevd( jobz, uplo, n, ap, w, z, ldz, work, lwork, iwork,liwork, info )
                     
           lwmin = max( real( lwmin,KIND=xdp), real( work( 1_ilp ),KIND=xdp) )
           liwmin = max( real( liwmin,KIND=xdp), real( iwork( 1_ilp ),KIND=xdp) )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
     ...
     !! for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_xspgst( itype, uplo, n, ap, bp, info )
           call stdlib_xspevx( jobz, range, uplo, n, ap, vl, vu, il, iu, abstol, m,w, z, ldz, &
                     work, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and banded, and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a split cholesky factorization of b.
           call stdlib_xpbstf( uplo, n, kb, bb, ldbb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ...
           inde = 1_ilp
           indwrk = inde + n
           call stdlib_xsbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, z, ldz,work( indwrk ), &
                     iinfo )
           ! reduce to tridiagonal form.
           if( wantz ) then
              vect = 'U'
           ...
           call stdlib_xsbtrd( vect, uplo, n, ka, ab, ldab, w, work( inde ), z, ldz,work( indwrk )&
                     , iinfo )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           indwk2 = indwrk + n*n
           llwrk2 = lwork - indwk2 + 1_ilp
           call stdlib_xsbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, z, ldz,work, iinfo )
     ...
     !! a range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem.
           call stdlib_xsbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, q, ldq,work, iinfo )
           ...
           call stdlib_xsbtrd( vect, uplo, n, ka, ab, ldab, work( indd ),work( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
           ...
           ! otherwise, call stdlib_xstebz and, if eigenvectors are desired,
           ! call stdlib_xstein.
           if( wantz ) then
              order = 'B'
           else
           ...
           indisp = indibl + n
           indiwo = indisp + n
           call stdlib_xstebz( range, order, n, vl, vu, il, iu, abstol,work( indd ), work( inde ),&
            m, nsplit, w,iwork( indibl ), iwork( indisp ), work( indwrk ),iwork( indiwo ), info )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - nb + 1, kk + 1, -nb
                 ...
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_xlatrd( uplo, i+nb-1, nb, a, lda, e, tau, work,ldwork )
                 ! update the unreduced submatrix a(1:i-1,1:i-1), using an
                 ! update of the form:  a := a - v*w**t - w*v**t
                 call stdlib_xsyr2k( uplo, 'NO TRANSPOSE', i-1, nb, -one, a( 1_ilp, i ),lda, work, &
                           ldwork, one, a, lda )
                 ! copy superdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_xsytd2( uplo, kk, a, lda, d, e, tau, iinfo )
           else
              ! reduce the lower triangle of a
              do i = 1, n - nx, nb
                 ! reduce columns i:i+nb-1 to tridiagonal form and form the
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_xlatrd( uplo, n-i+1, nb, a( i, i ), lda, e( i ),tau( i ), work, &
                           ldwork )
                 ! update the unreduced submatrix a(i+ib:n,i+ib:n), using
                 ! an update of the form:  a := a - v*w**t - w*v**t
                 call stdlib_xsyr2k( uplo, 'NO TRANSPOSE', n-i-nb+1, nb, -one,a( i+nb, i ), lda, &
                           work( nb+1 ), ldwork, one,a( i+nb, i+nb ), lda )
                 ! copy subdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_xsytd2( uplo, n-i+1, a( i, i ), lda, d( i ), e( i ),tau( i ), iinfo )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(1:i-1,i+1)
                 call stdlib_xlarfg( i, a( i, i+1 ), a( 1_ilp, i+1 ), 1_ilp, taui )
                 e( i ) = a( i, i+1 )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    a( i, i+1 ) = one
                    ! compute  x := tau * a * v  storing x in tau(1:i)
                    call stdlib_xsymv( uplo, i, taui, a, lda, a( 1_ilp, i+1 ), 1_ilp, zero,tau, 1_ilp )
                              
                    ! compute  w := x - 1/2 * tau * (x**t * v) * v
                    alpha = -half*taui*stdlib_xdot( i, tau, 1_ilp, a( 1_ilp, i+1 ), 1_ilp )
                    call stdlib_xaxpy( i, alpha, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_xsyr2( uplo, i, -one, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp, a,lda )
                    a( i, i+1 ) = e( i )
                 end if
                 d( i+1 ) = a( i+1, i+1 )
                 tau( i ) = taui
              ...
              do i = 1, n - 1
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(i+2:n,i)
                 call stdlib_xlarfg( n-i, a( i+1, i ), a( min( i+2, n ), i ), 1_ilp,taui )
                 e( i ) = a( i+1, i )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    a( i+1, i ) = one
                    ! compute  x := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_xsymv( uplo, n-i, taui, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, zero, &
                              tau( i ), 1_ilp )
                    ! compute  w := x - 1/2 * tau * (x**t * v) * v
                    alpha = -half*taui*stdlib_xdot( n-i, tau( i ), 1_ilp, a( i+1, i ),1_ilp )
                    call stdlib_xaxpy( n-i, alpha, a( i+1, i ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_xsyr2( uplo, n-i, -one, a( i+1, i ), 1_ilp, tau( i ), 1_ilp,a( i+1, i+1 ),&
                               lda )
                    a( i+1, i ) = e( i )
                 end if
                 d( i ) = a( i, i )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_xsytrd with uplo = 'u'
              ! shift the vectors which define the elementary reflectors one
              ! column to the left, and set the last row and column of q to
              ! those of the unit matrix
              do j = 1, n - 1
              ...
              a( n, n ) = one
              ! generate q(1:n-1,1:n-1)
              call stdlib_xorgql( n-1, n-1, n-1, a, lda, tau, work, lwork, iinfo )
           else
              ! q was determined by a call to stdlib_xsytrd with uplo = 'l'.
              ! shift the vectors which define the elementary reflectors one
              ! column to the right, and set the first row and column of q to
              ! those of the unit matrix
              do j = n, 2, -1
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_xorgqr( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(nq-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_xsytrd with uplo = 'u'
              call stdlib_xormql( side, trans, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda, tau, c,ldc, work, &
                        lwork, iinfo )
           else
              ! q was determined by a call to stdlib_xsytrd with uplo = 'l'
              if( left ) then
              ...
              call stdlib_xormqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), ldc,&
                         work, lwork, iinfo )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
               ...
               if( ttype==1_ilp ) then
                   ...
                   ctmp = ( a( ofdpos, st ) )
                   call stdlib_xlarfg( lm, ctmp, v( vpos+1 ), 1_ilp,tau( taupos ) )
                   a( ofdpos, st ) = ctmp
                   lm = ed - st + 1_ilp
                   call stdlib_xlarfy( uplo, lm, v( vpos ), 1_ilp,( tau( taupos ) ),a( dpos, st ), &
                             lda-1, work)
               endif
               if( ttype==3_ilp ) then
                   lm = ed - st + 1_ilp
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_xlarfx( 'LEFT', ln, lm, v( vpos ),( tau( taupos ) ),a( dpos-nb,&
                                  j1 ), lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_xlarfg( lm, ctmp, v( vpos+1 ), 1_ilp, tau( taupos ) )
                       a( dpos-nb, j1 ) = ctmp
                       call stdlib_xlarfx( 'RIGHT', ln-1, lm, v( vpos ),tau( taupos ),a( dpos-nb+&
                                 1_ilp, j1 ), lda-1, work)
                   endif
               endif
           ! lower case
               ...
               if( ttype==1_ilp ) then
                   ...
                   call stdlib_xlarfg( lm, a( ofdpos, st-1 ), v( vpos+1 ), 1_ilp,tau( taupos ) )
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_xlarfx( 'RIGHT', lm, ln, v( vpos ),tau( taupos ), a( dpos+nb, &
                                 st ),lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_xlarfg( lm, a( dpos+nb, st ), v( vpos+1 ), 1_ilp,tau( taupos ) )
                                 
                       call stdlib_xlarfx( 'LEFT', lm, ln-1, v( vpos ),( tau( taupos ) ),a( dpos+&
                                 nb-1, st+1 ), lda-1, work)
                   endif
               endif
           endif
     ...
     !! positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_ypotrf( uplo, n, b, ldb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_yheev( jobz, uplo, n, a, lda, w, work, lwork, rwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_ytrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, cone,b, ldb, a, lda &
                           )
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**h *y
                 ...
                 call stdlib_ytrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, cone,b, ldb, a, lda &
                           )
              end if
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_yheevd( jobz, uplo, n, a, lda, w, work, lwork, rwork, lrwork,iwork, liwork,&
                      info )
           lopt = max( real( lopt,KIND=xdp), real( work( 1_ilp ),KIND=xdp) )
           lropt = max( real( lropt,KIND=xdp), real( rwork( 1_ilp ),KIND=xdp) )
           liopt = max( real( liopt,KIND=xdp), real( iwork( 1_ilp ),KIND=xdp) )
           if( wantz .and. info==0_ilp ) then
              ! backtransform eigenvectors to the original problem.
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_ytrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, n, cone,b, ldb, a, lda )
                 ...
                 call stdlib_ytrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, n, cone,b, ldb, a, lda )
     ...
     !! range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_yheevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol,m, w, z, ldz, &
                     work, lwork, rwork, iwork, ifail,info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_ytrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, m, cone, b,ldb, z, ldz )
                 ...
                 call stdlib_ytrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, m, cone, b,ldb, z, ldz )
     ...
     !! and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_ypptrf( uplo, n, bp, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhpgst( itype, uplo, n, ap, bp, info )
           call stdlib_yhpev( jobz, uplo, n, ap, w, z, ldz, work, rwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 do j = 1, neig
                    call stdlib_ytpsv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**h *y
                 ...
                 do j = 1, neig
                    call stdlib_ytpmv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              end if
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhpgst( itype, uplo, n, ap, bp, info )
           call stdlib_yhpevd( jobz, uplo, n, ap, w, z, ldz, work, lwork, rwork,lrwork, iwork, &
                     liwork, info )
           lwmin = max( real( lwmin,KIND=xdp), real( work( 1_ilp ),KIND=xdp) )
           lrwmin = max( real( lrwmin,KIND=xdp), real( rwork( 1_ilp ),KIND=xdp) )
           liwmin = max( real( liwmin,KIND=xdp), real( iwork( 1_ilp ),KIND=xdp) )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
     ...
     !! desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_yhpgst( itype, uplo, n, ap, bp, info )
           call stdlib_yhpevx( jobz, range, uplo, n, ap, vl, vu, il, iu, abstol, m,w, z, ldz, &
                     work, rwork, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and banded, and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a split cholesky factorization of b.
           call stdlib_ypbstf( uplo, n, kb, bb, ldbb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ...
           inde = 1_ilp
           indwrk = inde + n
           call stdlib_yhbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, z, ldz,work, rwork( &
                     indwrk ), iinfo )
           ! reduce to tridiagonal form.
           if( wantz ) then
              vect = 'U'
           ...
           call stdlib_yhbtrd( vect, uplo, n, ka, ab, ldab, w, rwork( inde ), z,ldz, work, iinfo )
                     
           ! for eigenvalues only, call stdlib_xsterf.  for eigenvectors, call stdlib_ysteqr.
           if( .not.wantz ) then
              call stdlib_xsterf( n, w, rwork( inde ), info )
           else
              call stdlib_ysteqr( jobz, n, w, rwork( inde ), z, ldz,rwork( indwrk ), info )
     ...
     !! a range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem.
           call stdlib_yhbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, q, ldq,work, rwork, &
                     iinfo )
           ! solve the standard eigenvalue problem.
           ! reduce hermitian band matrix to tridiagonal form.
           indd = 1_ilp
           ...
           call stdlib_yhbtrd( vect, uplo, n, ka, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_xstebz( range, order, n, vl, vu, il, iu, abstol,rwork( indd ), rwork( inde &
           ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ), &
                     info )
           if( wantz ) then
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - nb + 1, kk + 1, -nb
                 ...
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_ylatrd( uplo, i+nb-1, nb, a, lda, e, tau, work,ldwork )
                 ! update the unreduced submatrix a(1:i-1,1:i-1), using an
                 ! update of the form:  a := a - v*w**h - w*v**h
                 call stdlib_yher2k( uplo, 'NO TRANSPOSE', i-1, nb, -cone,a( 1_ilp, i ), lda, work, &
                           ldwork, one, a, lda )
                 ! copy superdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_yhetd2( uplo, kk, a, lda, d, e, tau, iinfo )
           else
              ! reduce the lower triangle of a
              do i = 1, n - nx, nb
                 ! reduce columns i:i+nb-1 to tridiagonal form and form the
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_ylatrd( uplo, n-i+1, nb, a( i, i ), lda, e( i ),tau( i ), work, &
                           ldwork )
                 ! update the unreduced submatrix a(i+nb:n,i+nb:n), using
                 ! an update of the form:  a := a - v*w**h - w*v**h
                 call stdlib_yher2k( uplo, 'NO TRANSPOSE', n-i-nb+1, nb, -cone,a( i+nb, i ), lda, &
                           work( nb+1 ), ldwork, one,a( i+nb, i+nb ), lda )
                 ! copy subdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_yhetd2( uplo, n-i+1, a( i, i ), lda, d( i ), e( i ),tau( i ), iinfo )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ...
                 ! to annihilate a(1:i-1,i+1)
                 alpha = a( i, i+1 )
                 call stdlib_ylarfg( i, alpha, a( 1_ilp, i+1 ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=xdp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    a( i, i+1 ) = cone
                    ! compute  x := tau * a * v  storing x in tau(1:i)
                    call stdlib_yhemv( uplo, i, taui, a, lda, a( 1_ilp, i+1 ), 1_ilp, czero,tau, 1_ilp )
                              
                    ! compute  w := x - 1/2 * tau * (x**h * v) * v
                    alpha = -chalf*taui*stdlib_ydotc( i, tau, 1_ilp, a( 1_ilp, i+1 ), 1_ilp )
                    call stdlib_yaxpy( i, alpha, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_yher2( uplo, i, -cone, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp, a,lda )
                 else
              ...
              d( 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=xdp)
           else
              ! reduce the lower triangle of a
              a( 1_ilp, 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=xdp)
              do i = 1, n - 1
                 ! generate elementary reflector h(i) = i - tau * v * v**h
                 ! to annihilate a(i+2:n,i)
                 alpha = a( i+1, i )
                 call stdlib_ylarfg( n-i, alpha, a( min( i+2, n ), i ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=xdp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    a( i+1, i ) = cone
                    ! compute  x := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_yhemv( uplo, n-i, taui, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, czero, &
                              tau( i ), 1_ilp )
                    ! compute  w := x - 1/2 * tau * (x**h * v) * v
                    alpha = -chalf*taui*stdlib_ydotc( n-i, tau( i ), 1_ilp, a( i+1, i ),1_ilp )
                    call stdlib_yaxpy( n-i, alpha, a( i+1, i ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_yher2( uplo, n-i, -cone, a( i+1, i ), 1_ilp, tau( i ), 1_ilp,a( i+1, i+1 )&
                              , lda )
                 else
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_yhetrd with uplo = 'u'
              ! shift the vectors which define the elementary reflectors cone
              ! column to the left, and set the last row and column of q to
              ! those of the unit matrix
              do j = 1, n - 1
              ...
              a( n, n ) = cone
              ! generate q(1:n-1,1:n-1)
              call stdlib_yungql( n-1, n-1, n-1, a, lda, tau, work, lwork, iinfo )
           else
              ! q was determined by a call to stdlib_yhetrd with uplo = 'l'.
              ! shift the vectors which define the elementary reflectors cone
              ! column to the right, and set the first row and column of q to
              ! those of the unit matrix
              do j = n, 2, -1
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_yungqr( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(nq-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_yhetrd with uplo = 'u'
              call stdlib_yunmql( side, trans, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda, tau, c,ldc, work, &
                        lwork, iinfo )
           else
              ! q was determined by a call to stdlib_yhetrd with uplo = 'l'
              if( left ) then
              ...
              call stdlib_yunmqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), ldc,&
                         work, lwork, iinfo )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( n<=kd+1 ) then
               if( upper ) then
                   do i = 1, n
                       llk = min( kd+1, i )
                       call stdlib_ycopy( llk, a( i-llk+1, i ), 1_ilp,ab( kd+1-llk+1, i ), 1_ilp )
                   end do
               else
                   do i = 1, n
                       llk = min( kd+1, n-i+1 )
                       call stdlib_ycopy( llk, a( i, i ), 1_ilp, ab( 1_ilp, i ), 1_ilp )
                   end do
               endif
               work( 1_ilp ) = 1_ilp
               return
           ...
           ! set the workspace of the triangular matrix t to czero once such a
           ! way every time t is generated the upper/lower portion will be always czero
           call stdlib_ylaset( "A", ldt, kd, czero, czero, work( tpos ), ldt )
           if( upper ) then
               do i = 1, n - kd, kd
                  pn = n-i-kd+1
                  pk = min( n-i-kd+1, kd )
                  ! compute the lq factorization of the current block
                  call stdlib_ygelqf( kd, pn, a( i, i+kd ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_ycopy( llk, a( j, j ), lda, ab( kd+1, j ), ldab-1 )
                  end do
                  call stdlib_ylaset( 'LOWER', pk, pk, czero, cone,a( i, i+kd ), lda )
                  ! form the matrix t
                  call stdlib_ylarft( 'FORWARD', 'ROWWISE', pn, pk,a( i, i+kd ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_ygemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pn, pk,cone,  work( tpos ), &
                            ldt,a( i, i+kd ), lda,czero, work( s2pos ), lds2 )
                  call stdlib_yhemm( 'RIGHT', uplo, pk, pn,cone,  a( i+kd, i+kd ), lda,work( &
                            s2pos ), lds2,czero, work( wpos ), ldw )
                  call stdlib_ygemm( 'NO TRANSPOSE', 'CONJUGATE', pk, pk, pn,cone,  work( wpos ), &
                            ldw,work( s2pos ), lds2,czero, work( s1pos ), lds1 )
                  call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pk, pn, pk,-chalf, work( &
                            s1pos ), lds1,a( i, i+kd ), lda,cone,   work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v'*w - w'*v
                  call stdlib_yher2k( uplo, 'CONJUGATE', pn, pk,-cone, a( i, i+kd ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
               end do
              ! copy the upper band to ab which is the band storage matrix
              do j = n-kd+1, n
               ...
               loop_40: do i = 1, n - kd, kd
                  ...
                  pk = min( n-i-kd+1, kd )
                  ! compute the qr factorization of the current block
                  call stdlib_ygeqrf( pn, kd, a( i+kd, i ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_ycopy( llk, a( j, j ), 1_ilp, ab( 1_ilp, j ), 1_ilp )
                  end do
                  call stdlib_ylaset( 'UPPER', pk, pk, czero, cone,a( i+kd, i ), lda )
                  ! form the matrix t
                  call stdlib_ylarft( 'FORWARD', 'COLUMNWISE', pn, pk,a( i+kd, i ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,cone, a( i+kd, i )&
                            , lda,work( tpos ), ldt,czero, work( s2pos ), lds2 )
                  call stdlib_yhemm( 'LEFT', uplo, pn, pk,cone, a( i+kd, i+kd ), lda,work( s2pos )&
                            , lds2,czero, work( wpos ), ldw )
                  call stdlib_ygemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pk, pn,cone, work( s2pos ), &
                            lds2,work( wpos ), ldw,czero, work( s1pos ), lds1 )
                  call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,-chalf, a( i+kd, &
                            i ), lda,work( s1pos ), lds1,cone, work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v*w' - w*v'
                  call stdlib_yher2k( uplo, 'NO TRANSPOSE', pn, pk,-cone, a( i+kd, i ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
                  ! ==================================================================
                  ! restore a for comparison and checking to be removed
                   ! do 45 j = i, i+pk-1
                      ! llk = min( kd, n-j ) + 1
                      ! call stdlib_ycopy( llk, ab( 1, j ), 1, a( j, j ), 1 )
                      45 continue
                  ! ==================================================================
               end do loop_40
              ! copy the lower band to ab which is the band storage matrix
     ...
     !! Q**H * A * Q = T.
        ...
        ! =====================================================================
           ...
           if( kd==1_ilp ) then
               ...
               if( upper ) then
                   do i = 1, n - 1
                       ...
                       if( i<n-1 )ab( abofdpos, i+2 ) = ab( abofdpos, i+2 )*tmp
                        ! if( wantz ) then
                           ! call stdlib_yscal( n, conjg( tmp ), q( 1, i+1 ), 1 )
                        ! end if
                   end do
               else
                   do i = 1, n - 1
                      ...
                      if( i<n-1 )ab( abofdpos, i+1 ) = ab( abofdpos, i+1 )*tmp
                       ! if( wantq ) then
                          ! call stdlib_yscal( n, tmp, q( 1, i+1 ), 1 )
                       ! end if
                   end do
               endif
               hous( 1_ilp ) = 1_ilp
           ...
           call stdlib_ylacpy( "A", kd+1, n, ab, ldab, work( apos ), lda )
           call stdlib_ylaset( "A", kd,   n, czero, czero, work( awpos ), lda )
           ! openmp parallelisation start here
           !$OMP PARALLEL PRIVATE( TID, THGRID, BLKLASTIND ) &
           !$OMP&         PRIVATE( THED, I, M, K, ST, ED, STT, SWEEPID ) &
           !$OMP&         PRIVATE( MYID, TTYPE, COLPT, STIND, EDIND ) &
                       ...
                       loop_130: do sweepid = st, ed
                           loop_140: do k = 1, grsiz
                               ...
                               !$ if( ttype/=1 ) then
                                   ...
                                   !$OMP&     DEPEND(out:WORK(MYID))
                                   !$ tid      = omp_get_thread_num()
                                   !$ call stdlib_yhb2st_kernels( uplo, wantq, ttype,stind, edind, &
                                   !$ sweepid, n, kd, ib,work ( inda ), lda,hous( indv ), hous( &
                                   !$ indtau ), ldv,work( indw + tid*kd ) )
                                   !$OMP END TASK
                               !$ else
        ...
        ! =====================================================================
           ...
           if( upper ) then
               ...
               if( ttype==1_ilp ) then
                   ...
                   ctmp = conjg( a( ofdpos, st ) )
                   call stdlib_ylarfg( lm, ctmp, v( vpos+1 ), 1_ilp,tau( taupos ) )
                   a( ofdpos, st ) = ctmp
                   lm = ed - st + 1_ilp
                   call stdlib_ylarfy( uplo, lm, v( vpos ), 1_ilp,conjg( tau( taupos ) ),a( dpos, st )&
                             , lda-1, work)
               endif
               if( ttype==3_ilp ) then
                   lm = ed - st + 1_ilp
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_ylarfx( 'LEFT', ln, lm, v( vpos ),conjg( tau( taupos ) ),a( &
                                 dpos-nb, j1 ), lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_ylarfg( lm, ctmp, v( vpos+1 ), 1_ilp, tau( taupos ) )
                       a( dpos-nb, j1 ) = ctmp
                       call stdlib_ylarfx( 'RIGHT', ln-1, lm, v( vpos ),tau( taupos ),a( dpos-nb+&
                                 1_ilp, j1 ), lda-1, work)
                   endif
               endif
           ! lower case
               ...
               if( ttype==1_ilp ) then
                   ...
                   call stdlib_ylarfg( lm, a( ofdpos, st-1 ), v( vpos+1 ), 1_ilp,tau( taupos ) )
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_ylarfx( 'RIGHT', lm, ln, v( vpos ),tau( taupos ), a( dpos+nb, &
                                 st ),lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_ylarfg( lm, a( dpos+nb, st ), v( vpos+1 ), 1_ilp,tau( taupos ) )
                                 
                       call stdlib_ylarfx( 'LEFT', lm, ln-1, v( vpos ),conjg( tau( taupos ) ),a( &
                                 dpos+nb-1, st+1 ), lda-1, work)
                   endif
               endif
           endif
     ...
     !! Q**T * A * Q = T.
        ...
        ! =====================================================================
           ...
           stepercol = ceiling( real(shift,KIND=xdp)/real(grsiz,KIND=xdp) )
           thgrnb    = ceiling( real(n-1,KIND=xdp)/real(thgrsiz,KIND=xdp) )
           call stdlib_xlacpy( "A", kd+1, n, ab, ldab, work( apos ), lda )
           call stdlib_xlaset( "A", kd,   n, zero, zero, work( awpos ), lda )
           ! openmp parallelisation start here
           !$OMP PARALLEL PRIVATE( TID, THGRID, BLKLASTIND ) &
           !$OMP&         PRIVATE( THED, I, M, K, ST, ED, STT, SWEEPID ) &
           !$OMP&         PRIVATE( MYID, TTYPE, COLPT, STIND, EDIND ) &
                       ...
                       loop_130: do sweepid = st, ed
                           loop_140: do k = 1, grsiz
                               ...
                               !$ if( ttype/=1 ) then
                                   ...
                                   !$OMP&     DEPEND(out:WORK(MYID))
                                   !$ tid      = omp_get_thread_num()
                                   !$ call stdlib_xsb2st_kernels( uplo, wantq, ttype,stind, edind, &
                                   !$ sweepid, n, kd, ib,work ( inda ), lda,hous( indv ), hous( &
                                   !$ indtau ), ldv,work( indw + tid*kd ) )
                                   !$OMP END TASK
                               !$ else
        ...
        ! =====================================================================
           ...
           if( n<=kd+1 ) then
               if( upper ) then
                   do i = 1, n
                       llk = min( kd+1, i )
                       call stdlib_xcopy( llk, a( i-llk+1, i ), 1_ilp,ab( kd+1-llk+1, i ), 1_ilp )
                   end do
               else
                   do i = 1, n
                       llk = min( kd+1, n-i+1 )
                       call stdlib_xcopy( llk, a( i, i ), 1_ilp, ab( 1_ilp, i ), 1_ilp )
                   end do
               endif
               work( 1_ilp ) = 1_ilp
               return
           ...
           ! set the workspace of the triangular matrix t to zero once such a
           ! way every time t is generated the upper/lower portion will be always zero
           call stdlib_xlaset( "A", ldt, kd, zero, zero, work( tpos ), ldt )
           if( upper ) then
               do i = 1, n - kd, kd
                  pn = n-i-kd+1
                  pk = min( n-i-kd+1, kd )
                  ! compute the lq factorization of the current block
                  call stdlib_xgelqf( kd, pn, a( i, i+kd ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_xcopy( llk, a( j, j ), lda, ab( kd+1, j ), ldab-1 )
                  end do
                  call stdlib_xlaset( 'LOWER', pk, pk, zero, one,a( i, i+kd ), lda )
                  ! form the matrix t
                  call stdlib_xlarft( 'FORWARD', 'ROWWISE', pn, pk,a( i, i+kd ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_xgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pn, pk,one,  work( tpos ), &
                            ldt,a( i, i+kd ), lda,zero, work( s2pos ), lds2 )
                  call stdlib_xsymm( 'RIGHT', uplo, pk, pn,one,  a( i+kd, i+kd ), lda,work( s2pos &
                            ), lds2,zero, work( wpos ), ldw )
                  call stdlib_xgemm( 'NO TRANSPOSE', 'CONJUGATE', pk, pk, pn,one,  work( wpos ), &
                            ldw,work( s2pos ), lds2,zero, work( s1pos ), lds1 )
                  call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pk, pn, pk,-half, work( &
                            s1pos ), lds1,a( i, i+kd ), lda,one,   work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v'*w - w'*v
                  call stdlib_xsyr2k( uplo, 'CONJUGATE', pn, pk,-one, a( i, i+kd ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
               end do
              ! copy the upper band to ab which is the band storage matrix
              do j = n-kd+1, n
               ...
               loop_40: do i = 1, n - kd, kd
                  ...
                  pk = min( n-i-kd+1, kd )
                  ! compute the qr factorization of the current block
                  call stdlib_xgeqrf( pn, kd, a( i+kd, i ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_xcopy( llk, a( j, j ), 1_ilp, ab( 1_ilp, j ), 1_ilp )
                  end do
                  call stdlib_xlaset( 'UPPER', pk, pk, zero, one,a( i+kd, i ), lda )
                  ! form the matrix t
                  call stdlib_xlarft( 'FORWARD', 'COLUMNWISE', pn, pk,a( i+kd, i ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,one, a( i+kd, i ),&
                             lda,work( tpos ), ldt,zero, work( s2pos ), lds2 )
                  call stdlib_xsymm( 'LEFT', uplo, pn, pk,one, a( i+kd, i+kd ), lda,work( s2pos ),&
                             lds2,zero, work( wpos ), ldw )
                  call stdlib_xgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pk, pn,one, work( s2pos ), &
                            lds2,work( wpos ), ldw,zero, work( s1pos ), lds1 )
                  call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,-half, a( i+kd, i &
                            ), lda,work( s1pos ), lds1,one, work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v*w' - w*v'
                  call stdlib_xsyr2k( uplo, 'NO TRANSPOSE', pn, pk,-one, a( i+kd, i ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
                  ! ==================================================================
                  ! restore a for comparison and checking to be removed
                   ! do 45 j = i, i+pk-1
                      ! llk = min( kd, n-j ) + 1
                      ! call stdlib_xcopy( llk, ab( 1, j ), 1, a( j, j ), 1 )
                      45 continue
                  ! ==================================================================
               end do loop_40
              ! copy the lower band to ab which is the band storage matrix
