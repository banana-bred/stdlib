module stdlib_codata_type
    ...
    contains 
        ...
        procedure :: to_real_xdp
        procedure :: to_real_qp
        generic :: to_real => to_real_sp, to_real_dp, to_real_xdp, to_real_qp
    end type
...
elemental pure real(xdp) function to_real_xdp(self, mold, uncertainty) result(r)
    !! version: experimental
    !!
    !! Get the constant value or uncertainty for the kind xdp
    !! ([Specification](../page/specs/stdlib_constants.html))
    
    class(codata_constant_type), intent(in) :: self !! Codata constant
    real(xdp), intent(in) :: mold !! dummy argument to disambiguate at compile time the generic interface
    logical, intent(in), optional :: uncertainty !! Set to true if the uncertainty is required. Default to .false..
        !! 
    logical :: u
