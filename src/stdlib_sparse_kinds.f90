...
module stdlib_sparse_kinds
    ...
    type, public, extends(COO_type) :: COO_xdp_type
        real(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_coo_xdp
        procedure, non_overridable :: add_value => add_value_coo_xdp
        procedure, non_overridable :: add_block => add_block_coo_xdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(COO_type) :: COO_cxdp_type
        complex(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_coo_cxdp
        procedure, non_overridable :: add_value => add_value_coo_cxdp
        procedure, non_overridable :: add_block => add_block_coo_cxdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSR_type) :: CSR_xdp_type
        real(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csr_xdp
        procedure, non_overridable :: add_value => add_value_csr_xdp
        procedure, non_overridable :: add_block => add_block_csr_xdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSR_type) :: CSR_cxdp_type
        complex(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csr_cxdp
        procedure, non_overridable :: add_value => add_value_csr_cxdp
        procedure, non_overridable :: add_block => add_block_csr_cxdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSC_type) :: CSC_xdp_type
        real(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csc_xdp
        procedure, non_overridable :: add_value => add_value_csc_xdp
        procedure, non_overridable :: add_block => add_block_csc_xdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSC_type) :: CSC_cxdp_type
        complex(xdp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csc_cxdp
        procedure, non_overridable :: add_value => add_value_csc_cxdp
        procedure, non_overridable :: add_block => add_block_csc_cxdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(ELL_type) :: ELL_xdp_type
        real(xdp), allocatable :: data(:,:) 
    contains
        procedure, non_overridable :: at => at_value_ell_xdp
        procedure, non_overridable :: add_value => add_value_ell_xdp
        procedure, non_overridable :: add_block => add_block_ell_xdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(ELL_type) :: ELL_cxdp_type
        complex(xdp), allocatable :: data(:,:) 
    contains
        procedure, non_overridable :: at => at_value_ell_cxdp
        procedure, non_overridable :: add_value => add_value_ell_cxdp
        procedure, non_overridable :: add_block => add_block_ell_cxdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(SELLC_type) :: SELLC_xdp_type
        real(xdp), allocatable :: data(:,:)
    contains
        procedure, non_overridable :: at => at_value_sellc_xdp
        procedure, non_overridable :: add_value => add_value_sellc_xdp
        procedure, non_overridable :: add_block => add_block_sellc_xdp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(SELLC_type) :: SELLC_cxdp_type
        complex(xdp), allocatable :: data(:,:)
    contains
        procedure, non_overridable :: at => at_value_sellc_cxdp
        procedure, non_overridable :: add_value => add_value_sellc_cxdp
        procedure, non_overridable :: add_block => add_block_sellc_cxdp
        generic :: add => add_value, add_block
    end type
...
contains
    ...
    subroutine malloc_coo(self,num_rows,num_cols,nnz)
        ...
        select type(self)
            ...
            type is(COO_xdp_type)
                block
                real(xdp), allocatable :: temp_data_xdp(:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_xdp(nnz) , source = zero_xdp )
                else
                    allocate(temp_data_xdp(nnz) , source = self%data )
                end if
                call move_alloc(from=temp_data_xdp,to=self%data)
                end block
            type is(COO_qp_type)
                block
            ...
            type is(COO_cxdp_type)
                block
                complex(xdp), allocatable :: temp_data_cxdp(:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_cxdp(nnz) , source = zero_cxdp )
                else
                    allocate(temp_data_cxdp(nnz) , source = self%data )
                end if
                call move_alloc(from=temp_data_cxdp,to=self%data)
                end block
    ...
    subroutine malloc_ell(self,num_rows,num_cols,num_nz_rows)
        ...
        select type(self)
            ...
            type is(ELL_xdp_type)
                block
                real(xdp), allocatable :: temp_data_xdp(:,:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_xdp(num_rows,num_nz_rows) , source = zero_xdp )
                else
                    allocate(temp_data_xdp(num_rows,num_nz_rows) , source = self%data )
                end if
                call move_alloc(from=temp_data_xdp,to=self%data)
                end block
            type is(ELL_qp_type)
            ...
            type is(ELL_cxdp_type)
                block
                complex(xdp), allocatable :: temp_data_cxdp(:,:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_cxdp(num_rows,num_nz_rows) , source = zero_cxdp )
                else
                    allocate(temp_data_cxdp(num_rows,num_nz_rows) , source = self%data )
                end if
                call move_alloc(from=temp_data_cxdp,to=self%data)
                end block
    ...
    pure real(xdp) function at_value_coo_xdp(self,ik,jk) result(val)
        class(COO_xdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
        if( (ik<1 .or. ik>self%nrows) .or. (jk<1 .or. jk>self%ncols) ) then
            val = ieee_value( 0._xdp , ieee_quiet_nan)
            return
        end if
        ik_ = ik; jk_ = jk
        transpose = (self%storage == sparse_lower .and. ik > jk) .or. (self%storage == sparse_upper .and. ik < jk)
    ...
    subroutine add_value_coo_xdp(self,ik,jk,val)
        class(COO_xdp_type), intent(inout) :: self
    ...
    subroutine add_block_coo_xdp(self,ik,jk,val)
        class(COO_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(xdp) function at_value_coo_cxdp(self,ik,jk) result(val)
        class(COO_cxdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_coo_cxdp(self,ik,jk,val)
        class(COO_cxdp_type), intent(inout) :: self
    ...
    subroutine add_block_coo_cxdp(self,ik,jk,val)
        class(COO_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(xdp) function at_value_csr_xdp(self,ik,jk) result(val)
        class(CSR_xdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csr_xdp(self,ik,jk,val)
        class(CSR_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csr_xdp(self,ik,jk,val)
        class(CSR_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(xdp) function at_value_csr_cxdp(self,ik,jk) result(val)
        class(CSR_cxdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csr_cxdp(self,ik,jk,val)
        class(CSR_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csr_cxdp(self,ik,jk,val)
        class(CSR_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(xdp) function at_value_csc_xdp(self,ik,jk) result(val)
        class(CSC_xdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csc_xdp(self,ik,jk,val)
        class(CSC_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csc_xdp(self,ik,jk,val)
        class(CSC_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(xdp) function at_value_csc_cxdp(self,ik,jk) result(val)
        class(CSC_cxdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csc_cxdp(self,ik,jk,val)
        class(CSC_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csc_cxdp(self,ik,jk,val)
        class(CSC_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(xdp) function at_value_ell_xdp(self,ik,jk) result(val)
        class(ELL_xdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_ell_xdp(self,ik,jk,val)
        class(ELL_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_ell_xdp(self,ik,jk,val)
        class(ELL_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(xdp) function at_value_ell_cxdp(self,ik,jk) result(val)
        class(ELL_cxdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_ell_cxdp(self,ik,jk,val)
        class(ELL_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_ell_cxdp(self,ik,jk,val)
        class(ELL_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(xdp) function at_value_sellc_xdp(self,ik,jk) result(val)
        class(SELLC_xdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_, idx
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_sellc_xdp(self,ik,jk,val)
        class(SELLC_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k, idx
        ! naive implementation
    ...
    subroutine add_block_sellc_xdp(self,ik,jk,val)
        class(SELLC_xdp_type), intent(inout) :: self
        real(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j, idx
    ...
    pure complex(xdp) function at_value_sellc_cxdp(self,ik,jk) result(val)
        class(SELLC_cxdp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_, idx
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_sellc_cxdp(self,ik,jk,val)
        class(SELLC_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k, idx
        ! naive implementation
    ...
    subroutine add_block_sellc_cxdp(self,ik,jk,val)
        class(SELLC_cxdp_type), intent(inout) :: self
        complex(xdp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j, idx
