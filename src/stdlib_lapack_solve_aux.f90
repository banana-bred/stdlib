  ...
  contains
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           isave( 2_ilp ) = stdlib_ixamax( n, x, 1_ilp )
           isave( 3_ilp ) = 2_ilp
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_xcopy( n, x, 1_ilp, v, 1_ilp )
           estold = est
           est = stdlib_xasum( n, v, 1_ilp )
           do i = 1, n
              if( x(i)>=zero ) then
                 xs = one
              else
           ...
           ! x has been overwritten by a*x.
           140 continue
           temp = two*( stdlib_xasum( n, x, 1_ilp ) / real( 3_ilp*n,KIND=xdp) )
           if( temp>est ) then
              call stdlib_xcopy( n, x, 1_ilp, v, 1_ilp )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           isave( 2_ilp ) = stdlib_iymax1( n, x, 1_ilp )
           isave( 3_ilp ) = 2_ilp
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_ycopy( n, x, 1_ilp, v, 1_ilp )
           estold = est
           est = stdlib_xzsum1( n, v, 1_ilp )
           ! test for cycling.
           if( est<=estold )go to 100
           do i = 1, n
              absxi = abs( x( i ) )
           ...
           ! x has been overwritten by a*x.
           120 continue
           temp = two*( stdlib_xzsum1( n, x, 1_ilp ) / real( 3_ilp*n,KIND=xdp) )
           if( temp>est ) then
              call stdlib_ycopy( n, x, 1_ilp, v, 1_ilp )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           est = stdlib_I64_xasum( n, x, 1_ilp64 )
           do i = 1, n
              if( x(i)>=zero ) then
                 x(i) = one
              else
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           isave( 2_ilp64 ) = stdlib_I64_ixamax( n, x, 1_ilp64 )
           isave( 3_ilp64 ) = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_I64_xcopy( n, x, 1_ilp64, v, 1_ilp64 )
           estold = est
           est = stdlib_I64_xasum( n, v, 1_ilp64 )
           do i = 1, n
              if( x(i)>=zero ) then
                 xs = one
              else
           ...
           ! x has been overwritten by a*x.
           140 continue
           temp = two*( stdlib_I64_xasum( n, x, 1_ilp64 ) / real( 3_ilp64*n,KIND=xdp) )
           if( temp>est ) then
              call stdlib_I64_xcopy( n, x, 1_ilp64, v, 1_ilp64 )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           est = stdlib_I64_qasum( n, x, 1_ilp64 )
           do i = 1, n
              if( x(i)>=zero ) then
                 x(i) = one
              else
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           isave( 2_ilp64 ) = stdlib_I64_iqamax( n, x, 1_ilp64 )
           isave( 3_ilp64 ) = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_I64_qcopy( n, x, 1_ilp64, v, 1_ilp64 )
           estold = est
           est = stdlib_I64_qasum( n, v, 1_ilp64 )
           do i = 1, n
              if( x(i)>=zero ) then
                 xs = one
              else
           ...
           ! x has been overwritten by a*x.
           140 continue
           temp = two*( stdlib_I64_qasum( n, x, 1_ilp64 ) / real( 3_ilp64*n,KIND=qp) )
           if( temp>est ) then
              call stdlib_I64_qcopy( n, x, 1_ilp64, v, 1_ilp64 )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           est = stdlib_I64_xzsum1( n, x, 1_ilp64 )
           do i = 1, n
              absxi = abs( x( i ) )
              if( absxi>safmin ) then
                 x( i ) = cmplx( real( x( i ),KIND=xdp) / absxi,aimag( x( i ) ) / absxi,KIND=xdp)
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           isave( 2_ilp64 ) = stdlib_I64_iymax1( n, x, 1_ilp64 )
           isave( 3_ilp64 ) = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_I64_ycopy( n, x, 1_ilp64, v, 1_ilp64 )
           estold = est
           est = stdlib_I64_xzsum1( n, v, 1_ilp64 )
           ! test for cycling.
           if( est<=estold )go to 100
           do i = 1, n
              absxi = abs( x( i ) )
           ...
           ! x has been overwritten by a*x.
           120 continue
           temp = two*( stdlib_I64_xzsum1( n, x, 1_ilp64 ) / real( 3_ilp64*n,KIND=xdp) )
           if( temp>est ) then
              call stdlib_I64_ycopy( n, x, 1_ilp64, v, 1_ilp64 )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           est = stdlib_I64_qzsum1( n, x, 1_ilp64 )
           do i = 1, n
              absxi = abs( x( i ) )
              if( absxi>safmin ) then
                 x( i ) = cmplx( real( x( i ),KIND=qp) / absxi,aimag( x( i ) ) / absxi,KIND=qp)
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           isave( 2_ilp64 ) = stdlib_I64_iwmax1( n, x, 1_ilp64 )
           isave( 3_ilp64 ) = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_I64_wcopy( n, x, 1_ilp64, v, 1_ilp64 )
           estold = est
           est = stdlib_I64_qzsum1( n, v, 1_ilp64 )
           ! test for cycling.
           if( est<=estold )go to 100
           do i = 1, n
              absxi = abs( x( i ) )
           ...
           ! x has been overwritten by a*x.
           120 continue
           temp = two*( stdlib_I64_qzsum1( n, x, 1_ilp64 ) / real( 3_ilp64*n,KIND=qp) )
           if( temp>est ) then
              call stdlib_I64_wcopy( n, x, 1_ilp64, v, 1_ilp64 )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           j = stdlib_I64_ixamax( n, x, 1_ilp64 )
           iter = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           j = stdlib_I64_iqamax( n, x, 1_ilp64 )
           iter = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           j = stdlib_I64_iymax1( n, x, 1_ilp64 )
           iter = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           j = stdlib_I64_iwmax1( n, x, 1_ilp64 )
           iter = 2_ilp64
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
