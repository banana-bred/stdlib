...
!! Singular-Value Decomposition    
     ...
     !!                
         ...
         !> Compute singular values
         call stdlib_linalg_svd_x(amat,s,overwrite_a=.false.,err=err)
     ...
     !!        
         ...
         if (info==0) then
            ...
            ! Check if the returned working storage space is smaller than the largest value
            ! allowed by lwork
            lwork = merge(nint(real(work_dummy(1),kind=xdp), kind=ilp) &
                          , huge(lwork) &
                          , real(work_dummy(1),kind=xdp) < real(huge(lwork),kind=xdp) )
            allocate(work(lwork))
     ...
     !!                
         ...
         !> Compute singular values
         call stdlib_linalg_svd_y(amat,s,overwrite_a=.false.,err=err)
