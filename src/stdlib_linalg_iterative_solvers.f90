...
module stdlib_linalg_iterative_solvers
    ...
    type, public :: stdlib_linop_xdp_type
        procedure(vector_sub_xdp), nopass, pointer    :: matvec => null()
        procedure(reduction_sub_xdp), nopass, pointer :: inner_product => default_dot_xdp
    end type
    ...
    type, public :: stdlib_solver_workspace_xdp_type
        real(xdp), allocatable :: tmp(:,:)
        procedure(logger_sub_xdp), pointer, nopass :: callback => null()
    end type 
    ...
    abstract interface
        ...
        subroutine vector_sub_xdp(x,y,alpha,beta,op)
            import :: xdp
        ...
        pure real(xdp) function reduction_sub_xdp(x,y) result(r)
            import :: xdp
            real(xdp), intent(in) :: x(:)
            real(xdp), intent(in) :: y(:)
        end function
        subroutine logger_sub_xdp(x,norm_sq,iter)
            import :: xdp
            real(xdp), intent(in) :: x(:)
            real(xdp), intent(in) :: norm_sq
            integer, intent(in) :: iter
        end subroutine
    end interface
    ...
    interface stdlib_solve_cg_kernel
        ...
        module subroutine stdlib_solve_cg_kernel_xdp(A,b,x,rtol,atol,maxiter,workspace)
            class(stdlib_linop_xdp_type), intent(in) :: A !! linear operator
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
            real(xdp), intent(in) :: rtol !! relative tolerance for convergence
            real(xdp), intent(in) :: atol !! absolut tolerance for convergence
            integer, intent(in) :: maxiter !! maximum number of iterations
            type(stdlib_solver_workspace_xdp_type), intent(inout) :: workspace !! workspace for the solver
        end subroutine
    end interface
    ...
    interface stdlib_solve_cg
        ...
        module subroutine stdlib_solve_cg_dense_xdp(A,b,x,di,rtol,atol,maxiter,restart,workspace)
            !! linear operator matrix
            real(xdp), intent(in) :: A(:,:) 
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
            real(xdp), intent(in), optional :: rtol !! relative tolerance for convergence
            real(xdp), intent(in), optional :: atol !! absolute tolerance for convergence
            logical(1), intent(in), optional, target  :: di(:) !! dirichlet conditions mask
            integer, intent(in), optional :: maxiter !! maximum number of iterations
            logical, intent(in), optional :: restart !! restart flag
            type(stdlib_solver_workspace_xdp_type), optional, intent(inout), target :: workspace !! workspace for the solver
        end subroutine
        ...
        module subroutine stdlib_solve_cg_CSR_xdp(A,b,x,di,rtol,atol,maxiter,restart,workspace)
            !! linear operator matrix
            type(CSR_xdp_type), intent(in) :: A
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
            real(xdp), intent(in), optional :: rtol !! relative tolerance for convergence
            real(xdp), intent(in), optional :: atol !! absolute tolerance for convergence
    ...
    interface stdlib_solve_pcg_kernel
        ...
        module subroutine stdlib_solve_pcg_kernel_xdp(A,M,b,x,rtol,atol,maxiter,workspace)
            class(stdlib_linop_xdp_type), intent(in) :: A !! linear operator
            class(stdlib_linop_xdp_type), intent(in) :: M !! preconditioner linear operator
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
            real(xdp), intent(in) :: rtol !! relative tolerance for convergence
            real(xdp), intent(in) :: atol !! absolute tolerance for convergence
            integer, intent(in) :: maxiter !! maximum number of iterations
            type(stdlib_solver_workspace_xdp_type), intent(inout) :: workspace !! workspace for the solver
        end subroutine
    end interface
    ...
    interface stdlib_solve_pcg
        ...
        module subroutine stdlib_solve_pcg_dense_xdp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            real(xdp), intent(in) :: A(:,:)
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
            ...
            logical, intent(in), optional :: restart !! restart flag
            integer, intent(in), optional  :: precond !! preconditioner method enumerator
            class(stdlib_linop_xdp_type), optional , intent(in), target :: M !! preconditioner linear operator
            type(stdlib_solver_workspace_xdp_type), optional, intent(inout), target :: workspace !! workspace for the solver
        end subroutine
        ...
        module subroutine stdlib_solve_pcg_CSR_xdp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            type(CSR_xdp_type), intent(in) :: A
            real(xdp), intent(in) :: b(:) !! right-hand side vector
            real(xdp), intent(inout) :: x(:) !! solution vector and initial guess
...
contains
    ...
    pure real(xdp) function default_dot_xdp(x,y) result(r)
        use stdlib_intrinsics, only: stdlib_dot_product
        real(xdp), intent(in) :: x(:)
        real(xdp), intent(in) :: y(:)
        r = stdlib_dot_product(x,y)
