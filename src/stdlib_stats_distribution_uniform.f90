module stdlib_stats_distribution_uniform
    ...
    !! rvs_uniform-uniform-distribution-random-variates))
        ...
        module procedure rvs_unif_1_rxdp     ! 1 dummy variable
        module procedure rvs_unif_1_csp     ! 1 dummy variable
        module procedure rvs_unif_1_cdp     ! 1 dummy variable
        module procedure rvs_unif_1_cxdp     ! 1 dummy variable
        ...
        module procedure rvs_unif_rxdp       ! 2 dummy variables
        module procedure rvs_unif_csp       ! 2 dummy variables
        module procedure rvs_unif_cdp       ! 2 dummy variables
        module procedure rvs_unif_cxdp       ! 2 dummy variables
        ...
        module procedure rvs_unif_array_rxdp ! 3 dummy variables
        module procedure rvs_unif_array_csp ! 3 dummy variables
        module procedure rvs_unif_array_cdp ! 3 dummy variables
        module procedure rvs_unif_array_cxdp ! 3 dummy variables
    ...
    !! pdf_uniform-uniform-probability-density-function))
        ...
        module procedure pdf_unif_rxdp
        module procedure pdf_unif_csp
        module procedure pdf_unif_cdp
        module procedure pdf_unif_cxdp
    ...
    !!
        ...
        module procedure cdf_unif_rxdp
        module procedure cdf_unif_csp
        module procedure cdf_unif_cdp
        module procedure cdf_unif_cxdp
    ...
    !!
        ...
        module procedure shuffle_rxdp
        module procedure shuffle_csp
        module procedure shuffle_cdp
        module procedure shuffle_cxdp
...
contains
    ...
    impure elemental function rvs_unif_0_rxdp( ) result(res)
    !
    ! Uniformly distributed float in [0,1]
    ! Based on the paper by Frederic Goualard, "Generating Random Floating-
    ! Point Numbers By Dividing Integers: a Case Study", Proceedings of
    ...
    !
        ...
        tmp = shiftr(dist_rand(INT_ONE), 11)        ! Get random from [0,2^53-1]
        res = real(tmp * MESENNE_NUMBER, kind = xdp) ! convert to [0,1]
    end function rvs_unif_0_rxdp
    ...
    impure elemental function rvs_unif_1_rxdp(scale) result(res)
    !
    ! Uniformly distributed float in [0, scale]
    !
        ...
        if(scale == 0._xdp) call error_stop("Error(rvs_unif_1): "           &
            //"Uniform distribution scale parameter must be non-zero")
        res = scale * rvs_unif_0_rxdp( )
    end function rvs_unif_1_rxdp
    ...
    impure elemental function rvs_unif_rxdp(loc, scale) result(res)
    !
    ! Uniformly distributed float in [loc, loc + scale]
    !
        ...
        if(scale == 0._xdp) call error_stop("Error(rvs_unif): "             &
           //"Uniform distribution scale parameter must be non-zero")
        res = loc + scale * rvs_unif_0_rxdp( )
    end function rvs_unif_rxdp
    ...
    impure elemental function rvs_unif_1_cxdp(scale) result(res)
    !
    ! Uniformly distributed complex in [(0,0i), (scale, i(scale))]
    ! The real part and imaginary part are independent of each other, so that
    ! the joint distribution is on an unit square [(0,0i), (scale,i(scale))]
    !
        complex(xdp), intent(in) :: scale
        complex(xdp) :: res
        real(xdp) :: r1, tr, ti

        if(scale == (0.0_xdp, 0.0_xdp)) call error_stop("Error(rvs_uni_" &
           //"1): Uniform distribution scale parameter must be non-zero")
        r1 = rvs_unif_0_rxdp( )
        if(scale % re == 0.0_xdp) then
            ti = scale % im * r1
            tr = 0.0_xdp
        else if(scale % im == 0.0_xdp) then
            tr = scale % re * r1
            ti = 0.0_xdp
        else
            tr = scale % re * r1
    ...
    impure elemental function rvs_unif_cxdp(loc, scale) result(res)
    !
    ! Uniformly distributed complex in [(loc,iloc), (loc + scale, i(loc +
    ! scale))].
    ! The real part and imaginary part are independent of each other, so that
    ...
    function rvs_unif_array_rxdp(loc, scale, array_size) result(res)
        ...
        if(scale == 0._xdp) call error_stop("Error(rvs_unif_array):"        &
           //" Uniform distribution scale parameter must be non-zero")
        do i = 1, array_size
            tmp = shiftr(dist_rand(INT_ONE), 11)
            t = real(tmp * MESENNE_NUMBER, kind = xdp)
            res(i) = loc + scale * t
        end do
    end function rvs_unif_array_rxdp
    ...
    function rvs_unif_array_cxdp(loc, scale, array_size) result(res)
        ...
        if(scale == (0.0_xdp, 0.0_xdp)) call error_stop("Error(rvs_unif" &
           //"_array): Uniform distribution scale parameter must be non-zero")
        do i = 1, array_size
            tmp = shiftr(dist_rand(INT_ONE), 11)
            r1 = real(tmp * MESENNE_NUMBER, kind = xdp)
            if(scale % re == 0.0_xdp) then
                tr = loc % re
                ti = loc % im + scale % im * r1
            else if(scale % im == 0.0_xdp) then
    ...
    end function rvs_unif_array_cxdp
    ...
    elemental function pdf_unif_rxdp(x, loc, scale) result(res)
    ...
    elemental function pdf_unif_cxdp(x, loc, scale) result(res)
    ...
    elemental function cdf_unif_rxdp(x, loc, scale) result(res)
    ...
    elemental function cdf_unif_cxdp(x, loc, scale) result(res)
    ...
    function shuffle_rxdp( list ) result(res)

        real(xdp), intent(in) :: list(:)
        real(xdp) :: res(size(list))
        real(xdp) :: tmp
        integer :: n, i, j
    ...
    function shuffle_cxdp( list ) result(res)

        complex(xdp), intent(in) :: list(:)
        complex(xdp) :: res(size(list))
        complex(xdp) :: tmp
        integer :: n, i, j
