...
contains
      ...
      module function var_2_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim))
      ...
      module function var_3_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
      ...
      module function var_4_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
            & size(x, 4), mask=3<dim))
      ...
      module function var_2_cxdp_cxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        complex(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim))
      ...
      module function var_3_cxdp_cxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        complex(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
      ...
      module function var_4_cxdp_cxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n
        complex(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x,&
            & 3), size(x, 4), mask=3<dim))
      ...
      module function var_mask_2_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n(merge(size(x, 1), size(x, 2), mask=1<dim))
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim))
      ...
      module function var_mask_3_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
      ...
      module function var_mask_4_rxdp_rxdp(x, dim, mask, corrected) result(res)
        ...
        integer :: i
        real(xdp) :: n(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
            & size(x, 4), mask=3<dim))
        real(xdp) :: mean(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
            & size(x, 4), mask=3<dim))
