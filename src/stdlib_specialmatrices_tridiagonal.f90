submodule (stdlib_specialmatrices) tridiagonal_matrices
    ...
    pure module function initialize_tridiagonal_pure_xdp(dl, dv, du) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        real(xdp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
        type(tridiagonal_xdp_type) :: A
        !! Corresponding tridiagonal matrix.
    ...
    pure module function initialize_constant_tridiagonal_pure_xdp(dl, dv, du, n) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        real(xdp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module function initialize_tridiagonal_impure_xdp(dl, dv, du, err) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        real(xdp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
    ...
    module function initialize_constant_tridiagonal_impure_xdp(dl, dv, du, n, err) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        real(xdp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    pure module function initialize_tridiagonal_pure_cxdp(dl, dv, du) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        complex(xdp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
        type(tridiagonal_cxdp_type) :: A
        !! Corresponding tridiagonal matrix.
    ...
    pure module function initialize_constant_tridiagonal_pure_cxdp(dl, dv, du, n) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        complex(xdp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module function initialize_tridiagonal_impure_cxdp(dl, dv, du, err) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        complex(xdp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
    ...
    module function initialize_constant_tridiagonal_impure_cxdp(dl, dv, du, n, err) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        complex(xdp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module subroutine spmv_tridiag_1d_xdp(A, x, y, alpha, beta, op)
        type(tridiagonal_xdp_type), intent(in) :: A
        real(xdp), intent(in), contiguous, target :: x(:)
        real(xdp), intent(inout), contiguous, target :: y(:)
        real(xdp), intent(in), optional :: alpha
        real(xdp), intent(in), optional :: beta
        character(1), intent(in), optional :: op

        ! Internal variables.
        real(xdp) :: alpha_, beta_
        integer(ilp) :: n, nrhs, ldx, ldy
        character(1) :: op_
        real(xdp), pointer :: xmat(:, :), ymat(:, :)

        ! Deal with optional arguments.
        alpha_ = 1.0_xdp ; if (present(alpha)) alpha_ = alpha
        beta_  = 0.0_xdp ; if (present(beta))  beta_  = beta
        op_    = "N"        ; if (present(op))    op_    = op

        ! Prepare Lapack arguments.
        n = A%n ; ldx = n ; ldy = n ; y = 0.0_xdp
        nrhs =  1 
    ...
    module subroutine spmv_tridiag_2d_xdp(A, x, y, alpha, beta, op)
        type(tridiagonal_xdp_type), intent(in) :: A
        real(xdp), intent(in), contiguous, target :: x(:,:)
        real(xdp), intent(inout), contiguous, target :: y(:,:)
        real(xdp), intent(in), optional :: alpha
    ...
    module subroutine spmv_tridiag_1d_cxdp(A, x, y, alpha, beta, op)
        type(tridiagonal_cxdp_type), intent(in) :: A
        complex(xdp), intent(in), contiguous, target :: x(:)
        complex(xdp), intent(inout), contiguous, target :: y(:)
        real(xdp), intent(in), optional :: alpha
        real(xdp), intent(in), optional :: beta
        character(1), intent(in), optional :: op
    ...
    module subroutine spmv_tridiag_2d_cxdp(A, x, y, alpha, beta, op)
        type(tridiagonal_cxdp_type), intent(in) :: A
        complex(xdp), intent(in), contiguous, target :: x(:,:)
        complex(xdp), intent(inout), contiguous, target :: y(:,:)
        real(xdp), intent(in), optional :: alpha
    ...
    pure module function tridiagonal_to_dense_xdp(A) result(B)
        !! Convert a `tridiagonal` matrix to its dense representation.
        type(tridiagonal_xdp_type), intent(in) :: A
        !! Input tridiagonal matrix.
        real(xdp), allocatable :: B(:, :)
        !! Corresponding dense matrix.
        ...
        associate (n => A%n)
        allocate(B(n, n), source=zero_xdp)
        B(1, 1) = A%dv(1) ; B(1, 2) = A%du(1)
        do concurrent (i=2:n-1)
            B(i, i-1) = A%dl(i-1)
            B(i, i) = A%dv(i)
    ...
    pure module function tridiagonal_to_dense_cxdp(A) result(B)
        !! Convert a `tridiagonal` matrix to its dense representation.
        type(tridiagonal_cxdp_type), intent(in) :: A
        !! Input tridiagonal matrix.
        complex(xdp), allocatable :: B(:, :)
        !! Corresponding dense matrix.
        ...
        associate (n => A%n)
        allocate(B(n, n), source=zero_cxdp)
        B(1, 1) = A%dv(1) ; B(1, 2) = A%du(1)
        do concurrent (i=2:n-1)
            B(i, i-1) = A%dl(i-1)
            B(i, i) = A%dv(i)
    ...
    pure module function transpose_tridiagonal_xdp(A) result(B)
        type(tridiagonal_xdp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_xdp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    end function
    ...
    pure module function transpose_tridiagonal_cxdp(A) result(B)
        type(tridiagonal_cxdp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_cxdp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    end function
    ...
    pure module function hermitian_tridiagonal_xdp(A) result(B)
        type(tridiagonal_xdp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_xdp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    ...
    pure module function hermitian_tridiagonal_cxdp(A) result(B)
        type(tridiagonal_cxdp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_cxdp_type) :: B
        B = tridiagonal(conjg(A%du), conjg(A%dv), conjg(A%dl))
    ...
    pure module function scalar_multiplication_tridiagonal_xdp(alpha, A) result(B)
    ...
    pure module function scalar_multiplication_bis_tridiagonal_xdp(A, alpha) result(B)
        type(tridiagonal_xdp_type), intent(in) :: A
        real(xdp), intent(in) :: alpha
        type(tridiagonal_xdp_type) :: B
        B = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function scalar_multiplication_tridiagonal_cxdp(alpha, A) result(B)
    ...
    pure module function scalar_multiplication_bis_tridiagonal_cxdp(A, alpha) result(B)
        type(tridiagonal_cxdp_type), intent(in) :: A
        complex(xdp), intent(in) :: alpha
        type(tridiagonal_cxdp_type) :: B
        B = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function matrix_add_tridiagonal_xdp(A, B) result(C)
        type(tridiagonal_xdp_type), intent(in) :: A
        type(tridiagonal_xdp_type), intent(in) :: B
        type(tridiagonal_xdp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
        C%dl = C%dl + B%dl; C%dv = C%dv + B%dv; C%du = C%du + B%du
    end function

    pure module function matrix_sub_tridiagonal_xdp(A, B) result(C)
        type(tridiagonal_xdp_type), intent(in) :: A
        type(tridiagonal_xdp_type), intent(in) :: B
        type(tridiagonal_xdp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function matrix_add_tridiagonal_cxdp(A, B) result(C)
        type(tridiagonal_cxdp_type), intent(in) :: A
        type(tridiagonal_cxdp_type), intent(in) :: B
        type(tridiagonal_cxdp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
        C%dl = C%dl + B%dl; C%dv = C%dv + B%dv; C%du = C%du + B%du
    end function

    pure module function matrix_sub_tridiagonal_cxdp(A, B) result(C)
        type(tridiagonal_cxdp_type), intent(in) :: A
        type(tridiagonal_cxdp_type), intent(in) :: B
        type(tridiagonal_cxdp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
