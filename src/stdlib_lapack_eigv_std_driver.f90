submodule(stdlib_lapack_eig_svd_lsq) stdlib_lapack_eigv_std_driver
  ...
  contains
     ...
     module subroutine stdlib_I64_qsyev( jobz, uplo, n, a, lda, w, work, lwork, info )
     !! DSYEV: computes all eigenvalues and, optionally, eigenvectors of a
     !! real symmetric matrix A.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! get machine constants.
           safmin = stdlib_I64_qlamch( 'SAFE MINIMUM' )
           eps = stdlib_I64_qlamch( 'PRECISION' )
           smlnum = safmin / eps
           bignum = one / smlnum
           rmin = sqrt( smlnum )
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_qlansy( 'M', uplo, n, a, lda, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_qlascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_qsytrd to reduce symmetric matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_qsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_qorgtr to generate the orthogonal matrix, then call stdlib_I64_qsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_qorgtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              call stdlib_I64_qsteqr( jobz, n, w, work( inde ), a, lda, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
              ...
              call stdlib_I64_qscal( imax, one / sigma, w, 1_ilp64 )
           end if
           ! set work(1) to optimal workspace size.
           work( 1_ilp64 ) = lwkopt
           return
     end subroutine stdlib_I64_qsyev
     ...
     module subroutine stdlib_I64_qsyevd( jobz, uplo, n, a, lda, w, work, lwork, iwork,liwork, info )
     !! DSYEVD: computes all eigenvalues and, optionally, eigenvectors of a
     !! real symmetric matrix A. If eigenvectors are desired, it uses a
     !! divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! workspace than DSYEVX.
        ...
        ! =====================================================================
           ...
           call stdlib_I64_qsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_qstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_qormtr to multiply it by the
           ! householder transformations stored in a.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_qstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_I64_qormtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_qlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_qscal( n, one / sigma, w, 1_ilp64 )
           work( 1_ilp64 ) = lopt
           iwork( 1_ilp64 ) = liopt
           return
     end subroutine stdlib_I64_qsyevd
     ...
     module subroutine stdlib_I64_qsyevr( jobz, range, uplo, n, a, lda, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! DSYEVR: computes selected eigenvalues and, optionally, eigenvectors
     !! of a real symmetric matrix A.  Eigenvalues and eigenvectors can be
     !! selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
     ...
     !! manner.
       ...
       ! =====================================================================
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_qscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_qscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_qsterf or stdlib_I64_qstemr fail.
           ! work(indtau:indtau+n-1) stores the scalar factors of the
           ! elementary reflectors used in stdlib_I64_qsytrd.
           indtau = 1_ilp64
           ! work(indd:indd+n-1) stores the tridiagonal's diagonal entries.
           indd = indtau + n
           ! work(inde:inde+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_qsytrd.
           inde = indd + n
           ! work(inddd:inddd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_qstemr (the stdlib_I64_qsterf path copies the diagonal to w).
           inddd = inde + n
           ! work(indee:indee+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_qsterf and stdlib_I64_qstemr.
           indee = inddd + n
           ! indwk is the starting offset of the left-over workspace, and
           ! llwork is the remaining workspace size.
           indwk = indee + n
           llwork = lwork - indwk + 1_ilp64
           ! iwork(indibl:indibl+m-1) corresponds to iblock in stdlib_I64_qstebz and
           ! stores the block indices of each of the m<=n eigenvalues.
           indibl = 1_ilp64
           ! iwork(indisp:indisp+nsplit-1) corresponds to isplit in stdlib_I64_qstebz and
           ! stores the starting and finishing indices of each block.
           indisp = indibl + n
           ! iwork(indifl:indifl+n-1) stores the indices of eigenvectors
           ! that corresponding to eigenvectors that fail to converge in
           ! stdlib_I64_qstein.  this information is discarded; if any fail, the driver
           ! returns info > 0.
           indifl = indisp + n
           ! indiwo is the offset of the remaining integer workspace.
           indiwo = indifl + n
           ! call stdlib_I64_qsytrd to reduce symmetric matrix to tridiagonal form.
           call stdlib_I64_qsytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_qsterf or stdlib_I64_qstemr and stdlib_I64_qormtr.
           if( ( alleig .or. ( indeig .and. il==1_ilp64 .and. iu==n ) ) .and.ieeeok==1_ilp64 ) then
              if( .not.wantz ) then
                 call stdlib_I64_qcopy( n, work( indd ), 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_qcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_qsterf( n, w, work( indee ), info )
              else
                 call stdlib_I64_qcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_qcopy( n, work( indd ), 1_ilp64, work( inddd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_qstemr( jobz, 'A', n, work( inddd ), work( indee ),vl, vu, il, iu, m,&
                            w, z, ldz, n, isuppz,tryrac, work( indwk ), lwork, iwork, liwork,info )
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_qstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = inde
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_qormtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
                 ! everything worked.  skip stdlib_I64_qstebz/stdlib_I64_qstein.  iwork(:) are
                 ! undefined.
                 m = n
                 go to 30
              end if
           ...
           ! otherwise, call stdlib_I64_qstebz and, if eigenvectors are desired, stdlib_I64_qstein.
           ! also call stdlib_I64_qstebz and stdlib_I64_qstein if stdlib_I64_qstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_qstebz( range, order, n, vll, vuu, il, iu, abstll,work( indd ), work( inde &
           ), m, nsplit, w,iwork( indibl ), iwork( indisp ), work( indwk ),iwork( indiwo ), info )
                     
           if( wantz ) then
              call stdlib_I64_qstein( n, work( indd ), work( inde ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,work( indwk ), iwork( indiwo ), iwork( indifl ),info )
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_qstein.
              indwkn = inde
              llwrkn = lwork - indwkn + 1_ilp64
              call stdlib_I64_qormtr( 'L', uplo, 'N', n, m, a, lda, work( indtau ), z,ldz, work( &
                        indwkn ), llwrkn, iinfo )
           ...
           ! if matrix was scaled, then rescale eigenvalues appropriately.
        ! jump here if stdlib_I64_qstemr/stdlib_I64_qstein succeeded.
        30 continue
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
                 imax = m
           ...
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.  note: we do not sort the ifail portion of iwork.
           ! it may not be initialized (if stdlib_I64_qstemr/stdlib_I64_qstein succeeded), and we do
           ! not return this detailed information to the user.
           if( wantz ) then
              do j = 1, m - 1
                 i = 0_ilp64
                 ...
                 if( i/=0_ilp64 ) then
                    w( i ) = w( j )
                    w( j ) = tmp1
                    call stdlib_I64_qswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, j ), 1_ilp64 )
                 end if
              end do
           end if
           ! set work(1) to optimal workspace size.
           ...
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_qsyevr
     ...
     module subroutine stdlib_I64_qsyevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! DSYEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a real symmetric matrix A.  Eigenvalues and eigenvectors can be
     !! selected by specifying either a range of values or a range of indices
     !! for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           call stdlib_I64_qsytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_qsterf or stdlib_I64_qorgtr and stdlib_I64_dsteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_qlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_qorgtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_qcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_qsteqr( jobz, n, w, work( indee ), z, ldz,work( indwrk ), info )
           ...
           ! otherwise, call stdlib_I64_qstebz and, if eigenvectors are desired, stdlib_I64_dstein.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           ...
           work( 1_ilp64 ) = lwkopt
           return
     end subroutine stdlib_I64_qsyevx
     ...
     module subroutine stdlib_I64_qspev( jobz, uplo, n, ap, w, z, ldz, work, info )
     !! DSPEV: computes all the eigenvalues and, optionally, eigenvectors of a
     !! real symmetric matrix A in packed storage.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_qlansp( 'M', uplo, n, ap, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_qscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_qsptrd to reduce symmetric packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           call stdlib_I64_qsptrd( uplo, n, ap, w, work( inde ), work( indtau ), iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_qopgtr to generate the orthogonal matrix, then call stdlib_I64_qsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_qopgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              call stdlib_I64_qsteqr( jobz, n, w, work( inde ), z, ldz, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
           ...
           return
     end subroutine stdlib_I64_qspev
     ...
     module subroutine stdlib_I64_qspevd( jobz, uplo, n, ap, w, z, ldz, work, lwork,iwork, liwork, info )
     !! DSPEVD: computes all the eigenvalues and, optionally, eigenvectors
     !! of a real symmetric matrix A in packed storage. If eigenvectors are
     !! desired, it uses a divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_qstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_qopmtr to multiply it by the
           ! householder transformations represented in ap.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              llwork = lwork - indwrk + 1_ilp64
              call stdlib_I64_qstedc( 'I', n, w, work( inde ), z, ldz, work( indwrk ),llwork, iwork, &
                        liwork, info )
              call stdlib_I64_qopmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_qscal( n, one / sigma, w, 1_ilp64 )
           ...
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_qspevd
     ...
     module subroutine stdlib_I64_qspevx( jobz, range, uplo, n, ap, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! DSPEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a real symmetric matrix A in packed storage.  Eigenvalues/vectors
     !! can be selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           indd = inde + n
           indwrk = indd + n
           call stdlib_I64_qsptrd( uplo, n, ap, work( indd ), work( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_qsterf or stdlib_I64_qopgtr and stdlib_I64_dsteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
                 test = .true.
           ...
           if( wantz ) then
              ...
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_qstein.
              call stdlib_I64_qopmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
           ...
           return
     end subroutine stdlib_I64_qspevx
     ...
     module subroutine stdlib_I64_qsbev( jobz, uplo, n, kd, ab, ldab, w, z, ldz, work,info )
     !! DSBEV: computes all the eigenvalues and, optionally, eigenvectors of
     !! a real symmetric band matrix A.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_qlansb( 'M', uplo, n, kd, ab, ldab, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_qlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_qlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_qsbtrd to reduce symmetric band matrix to tridiagonal form.
           inde = 1_ilp64
           indwrk = inde + n
           call stdlib_I64_qsbtrd( jobz, uplo, n, kd, ab, ldab, w, work( inde ), z, ldz,work( indwrk )&
                     , iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, call stdlib_I64_dsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_qsteqr( jobz, n, w, work( inde ), z, ldz, work( indwrk ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
           ...
           return
     end subroutine stdlib_I64_qsbev
     ...
     module subroutine stdlib_I64_qsbevd( jobz, uplo, n, kd, ab, ldab, w, z, ldz, work,lwork, iwork, liwork, &
     !! DSBEVD: computes all the eigenvalues and, optionally, eigenvectors of
     !! a real symmetric band matrix A. If eigenvectors are desired, it uses
     !! a divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           call stdlib_I64_qsbtrd( jobz, uplo, n, kd, ab, ldab, w, work( inde ), z, ldz,work( indwrk )&
                     , iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, call stdlib_I64_dstedc.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_qstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_I64_qgemm( 'N', 'N', n, n, n, one, z, ldz, work( indwrk ), n,zero, work( &
                        indwk2 ), n )
              call stdlib_I64_qlacpy( 'A', n, n, work( indwk2 ), n, z, ldz )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_qscal( n, one / sigma, w, 1_ilp64 )
           work( 1_ilp64 ) = lwmin
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_qsbevd
     ...
     module subroutine stdlib_I64_qsbevx( jobz, range, uplo, n, kd, ab, ldab, q, ldq, vl,vu, il, iu, abstol, &
     !! DSBEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a real symmetric band matrix A.  Eigenvalues and eigenvectors can
     !! be selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           inde = indd + n
           indwrk = inde + n
           call stdlib_I64_qsbtrd( jobz, uplo, n, kd, ab, ldab, work( indd ),work( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_qsterf or stdlib_I64_dsteqr.  if this fails for some
           ! eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
                 test = .true.
           ...
           if ((alleig .or. test) .and. (abstol<=zero)) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_qlacpy( 'A', n, n, q, ldq, z, ldz )
                 call stdlib_I64_qcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_qsteqr( jobz, n, w, work( indee ), z, ldz,work( indwrk ), info )
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_I64_qstein.
              do j = 1, m
                 call stdlib_I64_qcopy( n, z( 1_ilp64, j ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
                 call stdlib_I64_qgemv( 'N', n, n, one, q, ldq, work, 1_ilp64, zero,z( 1_ilp64, j ), 1_ilp64 )
              end do
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           30 continue
           ...
           return
     end subroutine stdlib_I64_qsbevx
     ...
     module subroutine stdlib_I64_wheev( jobz, uplo, n, a, lda, w, work, lwork, rwork,info )
     !! ZHEEV: computes all eigenvalues and, optionally, eigenvectors of a
     !! complex Hermitian matrix A.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=qp)
              work( 1_ilp64 ) = 1_ilp64
              if( wantz )a( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_wlanhe( 'M', uplo, n, a, lda, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_wlascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_whetrd to reduce hermitian matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_whetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_wungtr to generate the unitary matrix, then call stdlib_I64_wsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_wungtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              indwrk = inde + n
              call stdlib_I64_wsteqr( jobz, n, w, rwork( inde ), a, lda,rwork( indwrk ), info )
           ...
           work( 1_ilp64 ) = lwkopt
           return
     end subroutine stdlib_I64_wheev
     ...
     module subroutine stdlib_I64_wheevd( jobz, uplo, n, a, lda, w, work, lwork, rwork,lrwork, iwork, liwork,&
     !! ZHEEVD: computes all eigenvalues and, optionally, eigenvectors of a
     !! complex Hermitian matrix A.  If eigenvectors are desired, it uses a
     !! divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           call stdlib_I64_whetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_wstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_wunmtr to multiply it to the
           ! householder transformations represented as householder vectors in
           ! a.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_wstedc( 'I', n, w, rwork( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, rwork( indrwk ), llrwk,iwork, liwork, info )
              call stdlib_I64_wunmtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_wlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
           ...
           iwork( 1_ilp64 ) = liopt
           return
     end subroutine stdlib_I64_wheevd
     ...
     module subroutine stdlib_I64_wheevr( jobz, range, uplo, n, a, lda, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! ZHEEVR: computes selected eigenvalues and, optionally, eigenvectors
     !! of a complex Hermitian matrix A.  Eigenvalues and eigenvectors can
     !! be selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp64 ) then
              ...
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( a( 1_ilp64, 1_ilp64 ),KIND=qp) .and. vu>=real( a( 1_ilp64, 1_ilp64 ),KIND=qp) )then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=qp)
                 end if
              end if
           ...
           anrm = stdlib_I64_wlansy( 'M', uplo, n, a, lda, rwork )
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           else if( anrm>rmax ) then
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_wdscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_wdscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_qsterf or stdlib_I64_wstemr fail.
           ! work(indtau:indtau+n-1) stores the complex scalar factors of the
           ! elementary reflectors used in stdlib_I64_whetrd.
           indtau = 1_ilp64
           ! indwk is the starting offset of the remaining complex workspace,
           ! and llwork is the remaining complex workspace size.
           indwk = indtau + n
           ...
           indrd = 1_ilp64
           ! rwork(indre:indre+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_whetrd.
           indre = indrd + n
           ! rwork(indrdd:indrdd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_wstemr (the stdlib_I64_qsterf path copies the diagonal to w).
           indrdd = indre + n
           ! rwork(indree:indree+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_qsterf and stdlib_I64_wstemr.
           indree = indrdd + n
           ! indrwk is the starting offset of the left-over real workspace, and
           ! llrwork is the remaining workspace size.
           indrwk = indree + n
           ...
           indiwo = indifl + n
           ! call stdlib_I64_whetrd to reduce hermitian matrix to tridiagonal form.
           call stdlib_I64_whetrd( uplo, n, a, lda, rwork( indrd ), rwork( indre ),work( indtau ), &
                     work( indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_qsterf or stdlib_I64_wstemr and stdlib_I64_wunmtr.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig.or.test ) .and. ( ieeeok==1_ilp64 ) ) then
              if( .not.wantz ) then
                 call stdlib_I64_qcopy( n, rwork( indrd ), 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_qcopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_qsterf( n, w, rwork( indree ), info )
              else
                 call stdlib_I64_qcopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_qcopy( n, rwork( indrd ), 1_ilp64, rwork( indrdd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_wstemr( jobz, 'A', n, rwork( indrdd ),rwork( indree ), vl, vu, il, &
                 iu, m, w,z, ldz, n, isuppz, tryrac,rwork( indrwk ), llrwork,iwork, liwork, info )
                           
                 ! apply unitary matrix used in reduction to tridiagonal
                 ! form to eigenvectors returned by stdlib_I64_wstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = indwk
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_wunmtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
           ...
           ! otherwise, call stdlib_I64_qstebz and, if eigenvectors are desired, stdlib_I64_wstein.
           ! also call stdlib_I64_qstebz and stdlib_I64_wstein if stdlib_I64_wstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_qstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indrd ), rwork( &
           indre ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwo )&
                     , info )
           if( wantz ) then
              call stdlib_I64_wstein( n, rwork( indrd ), rwork( indre ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwo ), iwork( indifl ),info )
              ! apply unitary matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_wstein.
              indwkn = indwk
              llwrkn = lwork - indwkn + 1_ilp64
              call stdlib_I64_wunmtr( 'L', uplo, 'N', n, m, a, lda, work( indtau ), z,ldz, work( &
                        indwkn ), llwrkn, iinfo )
           ...
           if( wantz ) then
              do j = 1, m - 1
                 ...
                 if( i/=0_ilp64 ) then
                    ...
                    w( j ) = tmp1
                    iwork( indibl+j-1 ) = itmp1
                    call stdlib_I64_wswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, j ), 1_ilp64 )
                 end if
              end do
           end if
           ! set work(1) to optimal workspace size.
           ...
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_wheevr
     ...
     module subroutine stdlib_I64_wheevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! ZHEEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a complex Hermitian matrix A.  Eigenvalues and eigenvectors can
     !! be selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_whetrd( uplo, n, a, lda, rwork( indd ), rwork( inde ),work( indtau ), work(&
                      indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_qsterf or stdlib_I64_wungtr and stdlib_I64_wsteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              call stdlib_I64_qcopy( n, rwork( indd ), 1_ilp64, w, 1_ilp64 )
              indee = indrwk + 2_ilp64*n
              if( .not.wantz ) then
                 call stdlib_I64_qcopy( n-1, rwork( inde ), 1_ilp64, rwork( indee ), 1_ilp64 )
                 call stdlib_I64_qsterf( n, w, rwork( indee ), info )
              else
                 call stdlib_I64_wlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_wungtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_qcopy( n-1, rwork( inde ), 1_ilp64, rwork( indee ), 1_ilp64 )
                 call stdlib_I64_wsteqr( jobz, n, w, rwork( indee ), z, ldz,rwork( indrwk ), info )
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_I64_qstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indd ), rwork( &
           inde ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ),&
                      info )
           if( wantz ) then
              call stdlib_I64_wstein( n, rwork( indd ), rwork( inde ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwk ), ifail, info )
              ! apply unitary matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_wstein.
              call stdlib_I64_wunmtr( 'L', uplo, 'N', n, m, a, lda, work( indtau ), z,ldz, work( &
           ...
           work( 1_ilp64 ) = lwkopt
           return
     end subroutine stdlib_I64_wheevx
     ...
     module subroutine stdlib_I64_whpev( jobz, uplo, n, ap, w, z, ldz, work, rwork,info )
     !! ZHPEV: computes all the eigenvalues and, optionally, eigenvectors of a
     !! complex Hermitian matrix in packed storage.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=qp)
              rwork( 1_ilp64 ) = 1_ilp64
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_wlanhp( 'M', uplo, n, ap, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_wdscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_whptrd to reduce hermitian packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           call stdlib_I64_whptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_wupgtr to generate the orthogonal matrix, then call stdlib_I64_wsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_wupgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              indrwk = inde + n
              call stdlib_I64_wsteqr( jobz, n, w, rwork( inde ), z, ldz,rwork( indrwk ), info )
           ...
           return
     end subroutine stdlib_I64_whpev
     ...
     module subroutine stdlib_I64_whpevd( jobz, uplo, n, ap, w, z, ldz, work, lwork,rwork, lrwork, iwork, &
     !! ZHPEVD: computes all the eigenvalues and, optionally, eigenvectors of
     !! a complex Hermitian matrix A in packed storage.  If eigenvectors are
     !! desired, it uses a divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           call stdlib_I64_whptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, first call
           ! stdlib_I64_wupgtr to generate the orthogonal matrix, then call stdlib_I64_wstedc.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_wstedc( 'I', n, w, rwork( inde ), z, ldz, work( indwrk ),llwrk, rwork( &
                        indrwk ), llrwk, iwork, liwork,info )
              call stdlib_I64_wupmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
           ...
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_whpevd
     ...
     module subroutine stdlib_I64_whpevx( jobz, range, uplo, n, ap, vl, vu, il, iu,abstol, m, w, z, ldz, &
     !! ZHPEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a complex Hermitian matrix A in packed storage.
     !! Eigenvalues/vectors can be selected by specifying either a range of
     !! values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( ap( 1_ilp64 ),KIND=qp) .and. vu>=real( ap( 1_ilp64 ),KIND=qp) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=qp)
                 end if
              end if
           ...
           indtau = 1_ilp64
           indwrk = indtau + n
           call stdlib_I64_whptrd( uplo, n, ap, rwork( indd ), rwork( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_qsterf or stdlib_I64_wupgtr and stdlib_I64_wsteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_I64_wstein.
              indwrk = indtau + n
              call stdlib_I64_wupmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
           ...
           return
     end subroutine stdlib_I64_whpevx
     ...
     module subroutine stdlib_I64_whbev( jobz, uplo, n, kd, ab, ldab, w, z, ldz, work,rwork, info )
     !! ZHBEV: computes all the eigenvalues and, optionally, eigenvectors of
     !! a complex Hermitian band matrix A.
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if( n==1_ilp64 ) then
              if( lower ) then
                 w( 1_ilp64 ) = real( ab( 1_ilp64, 1_ilp64 ),KIND=qp)
              else
                 w( 1_ilp64 ) = real( ab( kd+1, 1_ilp64 ),KIND=qp)
              end if
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_wlanhb( 'M', uplo, n, kd, ab, ldab, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_wlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_wlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_whbtrd to reduce hermitian band matrix to tridiagonal form.
           inde = 1_ilp64
           call stdlib_I64_whbtrd( jobz, uplo, n, kd, ab, ldab, w, rwork( inde ), z,ldz, work, iinfo )
                     
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, call stdlib_I64_wsteqr.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              indrwk = inde + n
           ...
           return
     end subroutine stdlib_I64_whbev
     ...
     module subroutine stdlib_I64_whbevd( jobz, uplo, n, kd, ab, ldab, w, z, ldz, work,lwork, rwork, lrwork, &
     !! ZHBEVD: computes all the eigenvalues and, optionally, eigenvectors of
     !! a complex Hermitian band matrix A.  If eigenvectors are desired, it
     !! uses a divide and conquer algorithm.
     !! The divide and conquer algorithm makes very mild assumptions about
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! for eigenvalues only, call stdlib_I64_qsterf.  for eigenvectors, call stdlib_I64_wstedc.
           if( .not.wantz ) then
              call stdlib_I64_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_wstedc( 'I', n, w, rwork( inde ), work, n, work( indwk2 ),llwk2, rwork( &
                        indwrk ), llrwk, iwork, liwork,info )
              call stdlib_I64_wgemm( 'N', 'N', n, n, n, cone, z, ldz, work, n, czero,work( indwk2 ), &
                        n )
              call stdlib_I64_wlacpy( 'A', n, n, work( indwk2 ), n, z, ldz )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
           ...
           iwork( 1_ilp64 ) = liwmin
           return
     end subroutine stdlib_I64_whbevd
     ...
     module subroutine stdlib_I64_whbevx( jobz, range, uplo, n, kd, ab, ldab, q, ldq, vl,vu, il, iu, abstol, &
     !! ZHBEVX: computes selected eigenvalues and, optionally, eigenvectors
     !! of a complex Hermitian band matrix A.  Eigenvalues and eigenvectors
     !! can be selected by specifying either a range of values or a range of
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           indrwk = inde + n
           indwrk = 1_ilp64
           call stdlib_I64_whbtrd( jobz, uplo, n, kd, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_qsterf or stdlib_I64_wsteqr.  if this fails for some
           ! eigenvalue, then try stdlib_I64_qstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
           ...
           if ((alleig .or. test) .and. (abstol<=zero)) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_wlacpy( 'A', n, n, q, ldq, z, ldz )
                 call stdlib_I64_qcopy( n-1, rwork( inde ), 1_ilp64, rwork( indee ), 1_ilp64 )
                 call stdlib_I64_wsteqr( jobz, n, w, rwork( indee ), z, ldz,rwork( indrwk ), info )
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_I64_wstein.
              do j = 1, m
                 call stdlib_I64_wcopy( n, z( 1_ilp64, j ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
                 call stdlib_I64_wgemv( 'N', n, n, cone, q, ldq, work, 1_ilp64, czero,z( 1_ilp64, j ), 1_ilp64 )
              end do
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           30 continue
           ...
           return
     end subroutine stdlib_I64_whbevx
