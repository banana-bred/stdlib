  ...
  contains
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           character :: normin
           integer(ilp) :: ix, kase, kase1
           real(xdp) :: ainvnm, scale, sl, smlnum, su
           ! Local Arrays 
           integer(ilp) :: isave(3_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           smlnum = stdlib_xlamch( 'SAFE MINIMUM' )
           ! estimate the norm of inv(a).
           ainvnm = zero
           normin = 'N'
           if( onenrm ) then
           ...
           kase = 0_ilp
           10 continue
           call stdlib_xlacn2( n, work( n+1 ), work, iwork, ainvnm, kase, isave )
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(l).
                 call stdlib_xlatrs( 'LOWER', 'NO TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           work( 2_ilp*n+1 ), info )
                 ! multiply by inv(u).
                 call stdlib_xlatrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,a, lda, work, &
                           su, work( 3_ilp*n+1 ), info )
              else
                 ! multiply by inv(u**t).
                 call stdlib_xlatrs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', normin, n, a,lda, work, su,&
                            work( 3_ilp*n+1 ), info )
                 ! multiply by inv(l**t).
                 call stdlib_xlatrs( 'LOWER', 'TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           work( 2_ilp*n+1 ), info )
              end if
              ! divide x by 1/(sl*su) if doing so will not cause overflow.
              scale = sl*su
              normin = 'Y'
              if( scale/=one ) then
                 ix = stdlib_ixamax( n, work, 1_ilp )
                 if( scale<abs( work( ix ) )*smlnum .or. scale==zero )go to 20
                 call stdlib_xrscl( n, scale, work, 1_ilp )
              end if
              go to 10
           end if
           ! compute the estimate of the reciprocal condition number.
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(l).
                 call stdlib_ylatrs( 'LOWER', 'NO TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           rwork, info )
                 ! multiply by inv(u).
                 call stdlib_ylatrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,a, lda, work, &
                           su, rwork( n+1 ), info )
              else
                 ! multiply by inv(u**h).
                 call stdlib_ylatrs( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',normin, n, a, lda,&
                            work, su, rwork( n+1 ),info )
                 ! multiply by inv(l**h).
                 call stdlib_ylatrs( 'LOWER', 'CONJUGATE TRANSPOSE', 'UNIT', normin,n, a, lda, &
                           work, sl, rwork, info )
              end if
              ! divide x by 1/(sl*su) if doing so will not cause overflow.
              scale = sl*su
     ...
     !! This is the right-looking Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=min( m, n ) ) then
              ! use unblocked code.
              call stdlib_xgetrf2( m, n, a, lda, ipiv, info )
           else
              ! use blocked code.
              do j = 1, min( m, n ), nb
                 jb = min( min( m, n )-j+1, nb )
                 ! factor diagonal and subdiagonal blocks and test for exact
                 ! singularity.
                 call stdlib_xgetrf2( m-j+1, jb, a( j, j ), lda, ipiv( j ), iinfo )
                 ! adjust info and the pivot indices.
                 if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + j - 1_ilp
                 do i = j, min( m, j+jb-1 )
                    ipiv( i ) = j - 1_ilp + ipiv( i )
                 ...
                 ! apply interchanges to columns 1:j-1.
                 call stdlib_xlaswp( j-1, a, lda, j, j+jb-1, ipiv, 1_ilp )
                 if( j+jb<=n ) then
                    ! apply interchanges to columns j+jb:n.
                    call stdlib_xlaswp( n-j-jb+1, a( 1_ilp, j+jb ), lda, j, j+jb-1,ipiv, 1_ilp )
                    ! compute block row of u.
                    call stdlib_xtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', jb,n-j-jb+1, one, &
                              a( j, j ), lda, a( j, j+jb ),lda )
                    if( j+jb<=m ) then
                       ! update trailing submatrix.
                       call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-j-jb+1,n-j-jb+1, jb, -&
                       one, a( j+jb, j ), lda,a( j, j+jb ), lda, one, a( j+jb, j+jb ),lda )
     ...
     !! This is the right-looking Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=min( m, n ) ) then
              ! use unblocked code.
              call stdlib_ygetrf2( m, n, a, lda, ipiv, info )
           else
              ! use blocked code.
              do j = 1, min( m, n ), nb
                 jb = min( min( m, n )-j+1, nb )
                 ! factor diagonal and subdiagonal blocks and test for exact
                 ! singularity.
                 call stdlib_ygetrf2( m-j+1, jb, a( j, j ), lda, ipiv( j ), iinfo )
                 ! adjust info and the pivot indices.
                 if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + j - 1_ilp
                 do i = j, min( m, j+jb-1 )
                    ipiv( i ) = j - 1_ilp + ipiv( i )
                 ...
                 ! apply interchanges to columns 1:j-1.
                 call stdlib_ylaswp( j-1, a, lda, j, j+jb-1, ipiv, 1_ilp )
                 if( j+jb<=n ) then
                    ! apply interchanges to columns j+jb:n.
                    call stdlib_ylaswp( n-j-jb+1, a( 1_ilp, j+jb ), lda, j, j+jb-1,ipiv, 1_ilp )
                    ! compute block row of u.
                    call stdlib_ytrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', jb,n-j-jb+1, cone,&
                               a( j, j ), lda, a( j, j+jb ),lda )
                    if( j+jb<=m ) then
                       ! update trailing submatrix.
                       call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-j-jb+1,n-j-jb+1, jb, -&
                       cone, a( j+jb, j ), lda,a( j, j+jb ), lda, cone, a( j+jb, j+jb ),lda )
     ...
     !! then calls itself to factor A22 and do the swaps on A21.
        ...
        ! =====================================================================
           ...
           if ( m==1_ilp ) then
              ...
              ! find pivot and test for singularity
              i = stdlib_ixamax( m, a( 1_ilp, 1_ilp ), 1_ilp )
              ipiv( 1_ilp ) = i
              if( a( i, 1_ilp )/=zero ) then
                 ! apply the interchange
                 if( i/=1_ilp ) then
                 ...
                 ! compute elements 2:m of the column
                 if( abs(a( 1_ilp, 1_ilp )) >= sfmin ) then
                    call stdlib_xscal( m-1, one / a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), 1_ilp )
                 else
                    do i = 1, m-1
                       a( 1_ilp+i, 1_ilp ) = a( 1_ilp+i, 1_ilp ) / a( 1_ilp, 1_ilp )
                    end do
              ...
              ! factor [ --- ]
                     ! [ a21 ]
              call stdlib_xgetrf2( m, n1, a, lda, ipiv, iinfo )
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo
                                    ! [ a12 ]
              ! apply interchanges to [ --- ]
                                    ! [ a22 ]
              call stdlib_xlaswp( n2, a( 1_ilp, n1+1 ), lda, 1_ilp, n1, ipiv, 1_ilp )
              ! solve a12
              call stdlib_xtrsm( 'L', 'L', 'N', 'U', n1, n2, one, a, lda,a( 1_ilp, n1+1 ), lda )
                        
              ! update a22
              call stdlib_xgemm( 'N', 'N', m-n1, n2, n1, -one, a( n1+1, 1_ilp ), lda,a( 1_ilp, n1+1 ), &
                        lda, one, a( n1+1, n1+1 ), lda )
              ! factor a22
              call stdlib_xgetrf2( m-n1, n2, a( n1+1, n1+1 ), lda, ipiv( n1+1 ),iinfo )
              ! adjust info and the pivot indices
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo + n1
              do i = n1+1, min( m, n )
                 ipiv( i ) = ipiv( i ) + n1
              ...
              ! apply interchanges to a21
              call stdlib_xlaswp( n1, a( 1_ilp, 1_ilp ), lda, n1+1, min( m, n), ipiv, 1_ilp )
           end if
           return
     ...
     !! then calls itself to factor A22 and do the swaps on A21.
        ...
        ! =====================================================================
           ...
           if ( m==1_ilp ) then
              ...
              sfmin = stdlib_xlamch('S')
              ! find pivot and test for singularity
              i = stdlib_iyamax( m, a( 1_ilp, 1_ilp ), 1_ilp )
              ipiv( 1_ilp ) = i
              if( a( i, 1_ilp )/=czero ) then
                 ! apply the interchange
                 if( i/=1_ilp ) then
                 ...
                 ! compute elements 2:m of the column
                 if( abs(a( 1_ilp, 1_ilp )) >= sfmin ) then
                    call stdlib_yscal( m-1, cone / a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), 1_ilp )
                 else
                    do i = 1, m-1
                       a( 1_ilp+i, 1_ilp ) = a( 1_ilp+i, 1_ilp ) / a( 1_ilp, 1_ilp )
                    end do
              ...
              ! factor [ --- ]
                     ! [ a21 ]
              call stdlib_ygetrf2( m, n1, a, lda, ipiv, iinfo )
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo
                                    ! [ a12 ]
              ! apply interchanges to [ --- ]
                                    ! [ a22 ]
              call stdlib_ylaswp( n2, a( 1_ilp, n1+1 ), lda, 1_ilp, n1, ipiv, 1_ilp )
              ! solve a12
              call stdlib_ytrsm( 'L', 'L', 'N', 'U', n1, n2, cone, a, lda,a( 1_ilp, n1+1 ), lda )
                        
              ! update a22
              call stdlib_ygemm( 'N', 'N', m-n1, n2, n1, -cone, a( n1+1, 1_ilp ), lda,a( 1_ilp, n1+1 ), &
                        lda, cone, a( n1+1, n1+1 ), lda )
              ! factor a22
              call stdlib_ygetrf2( m-n1, n2, a( n1+1, n1+1 ), lda, ipiv( n1+1 ),iinfo )
              ! adjust info and the pivot indices
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo + n1
              do i = n1+1, min( m, n )
                 ipiv( i ) = ipiv( i ) + n1
              ...
              ! apply interchanges to a21
              call stdlib_ylaswp( n1, a( 1_ilp, 1_ilp ), lda, n1+1, min( m, n), ipiv, 1_ilp )
           end if
           return
     ...
     !! This is the right-looking Level 2 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           do j = 1, min( m, n )
              ! find pivot and test for singularity.
              jp = j - 1_ilp + stdlib_ixamax( m-j+1, a( j, j ), 1_ilp )
              ipiv( j ) = jp
              if( a( jp, j )/=zero ) then
                 ! apply the interchange to columns 1:n.
                 if( jp/=j )call stdlib_xswap( n, a( j, 1_ilp ), lda, a( jp, 1_ilp ), lda )
                 ! compute elements j+1:m of j-th column.
                 if( j<m ) then
                    if( abs(a( j, j )) >= sfmin ) then
                       call stdlib_xscal( m-j, one / a( j, j ), a( j+1, j ), 1_ilp )
                    else
                      do i = 1, m-j
                         a( j+i, j ) = a( j+i, j ) / a( j, j )
                      end do
              ...
              if( j<min( m, n ) ) then
                 ! update trailing submatrix.
                 call stdlib_xger( m-j, n-j, -one, a( j+1, j ), 1_ilp, a( j, j+1 ), lda,a( j+1, j+1 ),&
                            lda )
              end if
           end do
           return
     ...
     !! This is the right-looking Level 2 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           do j = 1, min( m, n )
              ! find pivot and test for singularity.
              jp = j - 1_ilp + stdlib_iyamax( m-j+1, a( j, j ), 1_ilp )
              ipiv( j ) = jp
              if( a( jp, j )/=czero ) then
                 ! apply the interchange to columns 1:n.
                 if( jp/=j )call stdlib_yswap( n, a( j, 1_ilp ), lda, a( jp, 1_ilp ), lda )
                 ! compute elements j+1:m of j-th column.
                 if( j<m ) then
                    if( abs(a( j, j )) >= sfmin ) then
                       call stdlib_yscal( m-j, cone / a( j, j ), a( j+1, j ), 1_ilp )
                    else
                       do i = 1, m-j
                          a( j+i, j ) = a( j+i, j ) / a( j, j )
                       end do
              ...
              if( j<min( m, n ) ) then
                 ! update trailing submatrix.
                 call stdlib_ygeru( m-j, n-j, -cone, a( j+1, j ), 1_ilp, a( j, j+1 ),lda, a( j+1, j+1 &
                           ), lda )
              end if
           end do
           return
     ...
     !! by DGETRF.
        ...
        ! =====================================================================
           ...
           if( notran ) then
              ! solve a * x = b.
              ! apply row interchanges to the right hand sides.
              call stdlib_xlaswp( nrhs, b, ldb, 1_ilp, n, ipiv, 1_ilp )
              ! solve l*x = b, overwriting b with x.
              call stdlib_xtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, nrhs,one, a, lda, b, &
                        ldb )
              ! solve u*x = b, overwriting b with x.
              call stdlib_xtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n,nrhs, one, a, lda,&
                         b, ldb )
           else
              ! solve a**t * x = b.
              ! solve u**t *x = b, overwriting b with x.
              call stdlib_xtrsm( 'LEFT', 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, nrhs,one, a, lda, b,&
                         ldb )
              ! solve l**t *x = b, overwriting b with x.
              call stdlib_xtrsm( 'LEFT', 'LOWER', 'TRANSPOSE', 'UNIT', n, nrhs, one,a, lda, b, &
                        ldb )
              ! apply row interchanges to the solution vectors.
              call stdlib_xlaswp( nrhs, b, ldb, 1_ilp, n, ipiv, -1_ilp )
           end if
     ...
     !! by ZGETRF.
        ...
        ! =====================================================================
           ...
           if( notran ) then
              ! solve a * x = b.
              ! apply row interchanges to the right hand sides.
              call stdlib_ylaswp( nrhs, b, ldb, 1_ilp, n, ipiv, 1_ilp )
              ! solve l*x = b, overwriting b with x.
              call stdlib_ytrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, nrhs,cone, a, lda, b,&
                         ldb )
              ! solve u*x = b, overwriting b with x.
              call stdlib_ytrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n,nrhs, cone, a, &
                        lda, b, ldb )
           else
              ! solve a**t * x = b  or a**h * x = b.
              ! solve u**t *x = b or u**h *x = b, overwriting b with x.
              call stdlib_ytrsm( 'LEFT', 'UPPER', trans, 'NON-UNIT', n, nrhs, cone,a, lda, b, ldb &
                        )
              ! solve l**t *x = b, or l**h *x = b overwriting b with x.
              call stdlib_ytrsm( 'LEFT', 'LOWER', trans, 'UNIT', n, nrhs, cone, a,lda, b, ldb )
     ...
     !! inv(A)*L = inv(U) for inv(A).
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( n==0 )return
           ! form inv(u).  if info > 0 from stdlib_xtrtri, then u is singular,
           ! and the inverse is not computed.
           call stdlib_xtrtri( 'UPPER', 'NON-UNIT', n, a, lda, info )
           if( info>0 )return
           nbmin = 2_ilp
           ldwork = n
           if( nb>1_ilp .and. nb<n ) then
           ...
           if( nb<nbmin .or. nb>=n ) then
              ...
              do j = n, 1, -1
                 ...
                 ! compute current column of inv(a).
                 if( j<n )call stdlib_xgemv( 'NO TRANSPOSE', n, n-j, -one, a( 1_ilp, j+1 ),lda, work( &
                           j+1 ), 1_ilp, one, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! use blocked code.
              ...
              do j = nn, 1, -nb
                 ...
                 ! compute current block column of inv(a).
                 if( j+jb<=n )call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n, jb,n-j-jb+1, -&
                           one, a( 1_ilp, j+jb ), lda,work( j+jb ), ldwork, one, a( 1_ilp, j ), lda )
                 call stdlib_xtrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, jb,one, work( j )&
                           , ldwork, a( 1_ilp, j ), lda )
              end do
           end if
           ! apply column interchanges.
           do j = n - 1, 1, -1
              jp = ipiv( j )
              if( jp/=j )call stdlib_xswap( n, a( 1_ilp, j ), 1_ilp, a( 1_ilp, jp ), 1_ilp )
           end do
           work( 1_ilp ) = iws
           return
     ...
     !! inv(A)*L = inv(U) for inv(A).
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( n==0 )return
           ! form inv(u).  if info > 0 from stdlib_ytrtri, then u is singular,
           ! and the inverse is not computed.
           call stdlib_ytrtri( 'UPPER', 'NON-UNIT', n, a, lda, info )
           if( info>0 )return
           nbmin = 2_ilp
           ldwork = n
           if( nb>1_ilp .and. nb<n ) then
           ...
           if( nb<nbmin .or. nb>=n ) then
              ...
              do j = n, 1, -1
                 ...
                 ! compute current column of inv(a).
                 if( j<n )call stdlib_ygemv( 'NO TRANSPOSE', n, n-j, -cone, a( 1_ilp, j+1 ),lda, work(&
                            j+1 ), 1_ilp, cone, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! use blocked code.
              ...
              do j = nn, 1, -nb
                 ...
                 ! compute current block column of inv(a).
                 if( j+jb<=n )call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n, jb,n-j-jb+1, -&
                           cone, a( 1_ilp, j+jb ), lda,work( j+jb ), ldwork, cone, a( 1_ilp, j ), lda )
                 call stdlib_ytrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, jb,cone, work( j &
                           ), ldwork, a( 1_ilp, j ), lda )
              end do
           end if
           ! apply column interchanges.
           do j = n - 1, 1, -1
              jp = ipiv( j )
              if( jp/=j )call stdlib_yswap( n, a( 1_ilp, j ), 1_ilp, a( 1_ilp, jp ), 1_ilp )
           end do
           work( 1_ilp ) = iws
           return
     ...
     !! the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! compute residual r = b - op(a) * x,
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_xcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_xgemv( trans, n, n, -one, a, lda, x( 1_ilp, j ), 1_ilp, one,work( n+1 ), 1_ilp )
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_xgetrs( trans, n, 1_ilp, af, ldaf, ipiv, work( n+1 ), n,info )
                 call stdlib_xaxpy( n, one, work( n+1 ), 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              ! is incremented by safe1 if the i-th component of
              ! abs(op(a))*abs(x) + abs(b) is less than safe2.
              ! use stdlib_xlacn2 to estimate the infinity-norm of the matrix
                 ! inv(op(a)) * diag(w),
              ! where w = abs(r) + nz*eps*( abs(op(a))*abs(x)+abs(b) )))
              do i = 1, n
                 if( work( i )>safe2 ) then
              ...
              kase = 0_ilp
              100 continue
              call stdlib_xlacn2( n, work( 2_ilp*n+1 ), work( n+1 ), iwork, ferr( j ),kase, isave )
                        
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_xgetrs( transt, n, 1_ilp, af, ldaf, ipiv, work( n+1 ),n, info )
     ...
     !! the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_ygemv( trans, n, n, -cone, a, lda, x( 1_ilp, j ), 1_ilp, cone, work,1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_ygetrs( trans, n, 1_ilp, af, ldaf, ipiv, work, n, info )
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_ygetrs( transt, n, 1_ilp, af, ldaf, ipiv, work, n,info )
                    do i = 1, n
                       work( i ) = rwork( i )*work( i )
                    end do
                 else
                    ...
                    call stdlib_ygetrs( transn, n, 1_ilp, af, ldaf, ipiv, work, n,info )
                 end if
                 go to 100
              end if
              ! normalize error.
     ...
     !! works well in practice.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, j
           real(xdp) :: bignum, rcmax, rcmin, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp
     ...
     !! between sqrt(radix) and 1/sqrt(radix).
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, j
           real(xdp) :: bignum, rcmax, rcmin, smlnum, radix, logrdx
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp
     ...
     !! column scaling factors in the vectors R and C.
        ...
        ! =====================================================================
           ...
           ! initialize large and small.
           small = stdlib_xlamch( 'SAFE MINIMUM' ) / stdlib_xlamch( 'PRECISION' )
           large = one / small
           if( rowcnd>=thresh .and. amax>=small .and. amax<=large )then
              ! no row scaling
              if( colcnd>=thresh ) then
     ...
     !! This is the Level 2 BLAS algorithm.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, ip, ipv, j, jp, jpv
           real(xdp) :: bignum, eps, smin, smlnum, xmax
           ! Intrinsic Functions 
           ! Executable Statements 
           info = 0_ilp
           ! quick return if possible
           ...
           loop_40: do i = 1, n - 1
              ...
              if( i==1_ilp )smin = max( eps*xmax, smlnum )
              ! swap rows
              if( ipv/=i )call stdlib_xswap( n, a( ipv, 1_ilp ), lda, a( i, 1_ilp ), lda )
              ipiv( i ) = ipv
              ! swap columns
              if( jpv/=i )call stdlib_xswap( n, a( 1_ilp, jpv ), 1_ilp, a( 1_ilp, i ), 1_ilp )
              jpiv( i ) = jpv
              ! check for singularity
              if( abs( a( i, i ) )<smin ) then
                 info = i
              ...
              call stdlib_xger( n-i, n-i, -one, a( i+1, i ), 1_ilp, a( i, i+1 ), lda,a( i+1, i+1 ), &
                        lda )
           end do loop_40
           if( abs( a( n, n ) )<smin ) then
              info = n
     ...
     !! This is a level 1 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              ...
              if( abs( a( 1_ilp, 1_ilp ) )<smlnum ) then
                 info = 1_ilp
                 a( 1_ilp, 1_ilp ) = cmplx( smlnum, zero,KIND=xdp)
              end if
              return
           end if
           ! factorize a using complete pivoting.
           ...
           loop_40: do i = 1, n - 1
              ...
              if( i==1_ilp )smin = max( eps*xmax, smlnum )
              ! swap rows
              if( ipv/=i )call stdlib_yswap( n, a( ipv, 1_ilp ), lda, a( i, 1_ilp ), lda )
              ipiv( i ) = ipv
              ! swap columns
              if( jpv/=i )call stdlib_yswap( n, a( 1_ilp, jpv ), 1_ilp, a( 1_ilp, i ), 1_ilp )
              jpiv( i ) = jpv
              ! check for singularity
              if( abs( a( i, i ) )<smin ) then
                 info = i
                 a( i, i ) = cmplx( smin, zero,KIND=xdp)
              end if
              do j = i + 1, n
                 a( j, i ) = a( j, i ) / a( i, i )
              end do
              call stdlib_ygeru( n-i, n-i, -cmplx( one,KIND=xdp), a( i+1, i ), 1_ilp,a( i, i+1 ), lda, &
                        a( i+1, i+1 ), lda )
           end do loop_40
           if( abs( a( n, n ) )<smin ) then
              info = n
              a( n, n ) = cmplx( smin, zero,KIND=xdp)
           end if
           ! set last pivots to n
           ipiv( n ) = n
           jpiv( n ) = n
     ...
     !! complete pivoting computed by DGETC2.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, j
           real(xdp) :: bignum, eps, smlnum, temp
           ! Intrinsic Functions 
           ! Executable Statements 
            ! set constant to control overflow
           eps = stdlib_xlamch( 'P' )
           ...
           call stdlib_xlabad( smlnum, bignum )
           ! apply permutations ipiv to rhs
           call stdlib_xlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, ipiv, 1_ilp )
           ! solve for l part
           do i = 1, n - 1
              do j = i + 1, n
                 rhs( j ) = rhs( j ) - a( j, i )*rhs( i )
           ...
           scale = one
           ! check for scaling
           i = stdlib_ixamax( n, rhs, 1_ilp )
           if( two*smlnum*abs( rhs( i ) )>abs( a( n, n ) ) ) then
              temp = ( one / two ) / abs( rhs( i ) )
              call stdlib_xscal( n, temp, rhs( 1_ilp ), 1_ilp )
              scale = scale*temp
           end if
           do i = n, 1, -1
              temp = one / a( i, i )
           ...
           ! apply permutations jpiv to the solution (rhs)
           call stdlib_xlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, jpiv, -1_ilp )
           return
     ...
     !! complete pivoting computed by ZGETC2.
        ...
        ! =====================================================================
           ...
           call stdlib_xlabad( smlnum, bignum )
           ! apply permutations ipiv to rhs
           call stdlib_ylaswp( 1_ilp, rhs, lda, 1_ilp, n-1, ipiv, 1_ilp )
           ! solve for l part
           do i = 1, n - 1
              do j = i + 1, n
                 rhs( j ) = rhs( j ) - a( j, i )*rhs( i )
           ...
           scale = one
           ! check for scaling
           i = stdlib_iyamax( n, rhs, 1_ilp )
           if( two*smlnum*abs( rhs( i ) )>abs( a( n, n ) ) ) then
              temp = cmplx( one / two, zero,KIND=xdp) / abs( rhs( i ) )
              call stdlib_yscal( n, temp, rhs( 1_ilp ), 1_ilp )
              scale = scale*real( temp,KIND=xdp)
           end if
           do i = n, 1, -1
              temp = cmplx( one, zero,KIND=xdp) / a( i, i )
              rhs( i ) = rhs( i )*temp
              do j = i + 1, n
                 rhs( i ) = rhs( i ) - rhs( j )*( a( i, j )*temp )
              end do
           ...
           ! apply permutations jpiv to the solution (rhs)
           call stdlib_ylaswp( 1_ilp, rhs, lda, 1_ilp, n-1, jpiv, -1_ilp )
           return
     ...
     !! unit diagonal elements and U is upper triangular.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, info, j, k
           real(xdp) :: bm, bp, pmone, sminu, splus, temp
           ! Local Arrays 
           integer(ilp) :: iwork(maxdim)
           real(xdp) :: work(4_ilp*maxdim), xm(maxdim), xp(maxdim)
           ! Intrinsic Functions 
           ! Executable Statements 
           if( ijob/=2_ilp ) then
              ! apply permutations ipiv to rhs
              call stdlib_xlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, ipiv, 1_ilp )
              ! solve for l-part choosing rhs either to +1 or -1.
              pmone = -one
              loop_10: do j = 1, n - 1
                 bp = rhs( j ) + one
                 ...
                 ! look-ahead for l-part rhs(1:n-1) = + or -1, splus and
                 ! smin computed more efficiently than in bsolve [1].
                 splus = splus + stdlib_xdot( n-j, z( j+1, j ), 1_ilp, z( j+1, j ), 1_ilp )
                 sminu = stdlib_xdot( n-j, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
                 splus = splus*rhs( j )
                 if( splus>sminu ) then
                    rhs( j ) = bp
                 else if( sminu>splus ) then
                 ...
                 ! compute the remaining r.h.s.
                 temp = -rhs( j )
                 call stdlib_xaxpy( n-j, temp, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
              end do loop_10
              ! solve for u-part, look-ahead for rhs(n) = +-1. this is not done
              ! in bsolve and will hopefully give us a better estimate because
              ! any ill-conditioning of the original matrix is transferred to u
              ! and not to l. u(n, n) is an approximation to sigma_min(lu).
              call stdlib_xcopy( n-1, rhs, 1_ilp, xp, 1_ilp )
              xp( n ) = rhs( n ) + one
              rhs( n ) = rhs( n ) - one
              splus = zero
              sminu = zero
              ...
              if( splus>sminu )call stdlib_xcopy( n, xp, 1_ilp, rhs, 1_ilp )
              ! apply the permutations jpiv to the computed solution (rhs)
              call stdlib_xlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, jpiv, -1_ilp )
              ! compute the sum of squares
              call stdlib_xlassq( n, rhs, 1_ilp, rdscal, rdsum )
           else
              ! ijob = 2, compute approximate nullvector xm of z
              call stdlib_xgecon( 'I', n, z, ldz, one, temp, work, iwork, info )
              call stdlib_xcopy( n, work( n+1 ), 1_ilp, xm, 1_ilp )
              ! compute rhs
              call stdlib_xlaswp( 1_ilp, xm, ldz, 1_ilp, n-1, ipiv, -1_ilp )
              temp = one / sqrt( stdlib_xdot( n, xm, 1_ilp, xm, 1_ilp ) )
              call stdlib_xscal( n, temp, xm, 1_ilp )
              call stdlib_xcopy( n, xm, 1_ilp, xp, 1_ilp )
              call stdlib_xaxpy( n, one, rhs, 1_ilp, xp, 1_ilp )
              call stdlib_xaxpy( n, -one, xm, 1_ilp, rhs, 1_ilp )
              call stdlib_xgesc2( n, z, ldz, rhs, ipiv, jpiv, temp )
              call stdlib_xgesc2( n, z, ldz, xp, ipiv, jpiv, temp )
              if( stdlib_xasum( n, xp, 1_ilp )>stdlib_xasum( n, rhs, 1_ilp ) )call stdlib_xcopy( n, xp, 1_ilp,&
                         rhs, 1_ilp )
              ! compute the sum of squares
              call stdlib_xlassq( n, rhs, 1_ilp, rdscal, rdsum )
           end if
     ...
     !! triangular with unit diagonal elements and U is upper triangular.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp) :: i, info, j, k
           real(xdp) :: rtemp, scale, sminu, splus
           complex(xdp) :: bm, bp, pmone, temp
           ! Local Arrays 
           real(xdp) :: rwork(maxdim)
           complex(xdp) :: work(4_ilp*maxdim), xm(maxdim), xp(maxdim)
           ! Intrinsic Functions 
           ! Executable Statements 
           if( ijob/=2_ilp ) then
              ! apply permutations ipiv to rhs
              call stdlib_ylaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, ipiv, 1_ilp )
              ! solve for l-part choosing rhs either to +1 or -1.
              pmone = -cone
              loop_10: do j = 1, n - 1
                 bp = rhs( j ) + cone
                 ...
                 ! lockahead for l- part rhs(1:n-1) = +-1
                 ! splus and smin computed more efficiently than in bsolve[1].
                 splus = splus + real( stdlib_ydotc( n-j, z( j+1, j ), 1_ilp, z( j+1,j ), 1_ilp ),KIND=xdp)
                           
                 sminu = real( stdlib_ydotc( n-j, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp ),KIND=xdp)
                 splus = splus*real( rhs( j ),KIND=xdp)
                 if( splus>sminu ) then
                    rhs( j ) = bp
                 else if( sminu>splus ) then
                    rhs( j ) = bm
                 ...
                 ! compute the remaining r.h.s.
                 temp = -rhs( j )
                 call stdlib_yaxpy( n-j, temp, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
              end do loop_10
              ! solve for u- part, lockahead for rhs(n) = +-1. this is not done
              ! in bsolve and will hopefully give us a better estimate because
              ! any ill-conditioning of the original matrix is transferred to u
              ! and not to l. u(n, n) is an approximation to sigma_min(lu).
              call stdlib_ycopy( n-1, rhs, 1_ilp, work, 1_ilp )
              work( n ) = rhs( n ) + cone
              rhs( n ) = rhs( n ) - cone
              splus = zero
              sminu = zero
              ...
              if( splus>sminu )call stdlib_ycopy( n, work, 1_ilp, rhs, 1_ilp )
              ! apply the permutations jpiv to the computed solution (rhs)
              call stdlib_ylaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, jpiv, -1_ilp )
              ! compute the sum of squares
              call stdlib_ylassq( n, rhs, 1_ilp, rdscal, rdsum )
              return
           end if
           ! entry ijob = 2
           ! compute approximate nullvector xm of z
           call stdlib_ygecon( 'I', n, z, ldz, one, rtemp, work, rwork, info )
           call stdlib_ycopy( n, work( n+1 ), 1_ilp, xm, 1_ilp )
           ! compute rhs
           call stdlib_ylaswp( 1_ilp, xm, ldz, 1_ilp, n-1, ipiv, -1_ilp )
           temp = cone / sqrt( stdlib_ydotc( n, xm, 1_ilp, xm, 1_ilp ) )
           call stdlib_yscal( n, temp, xm, 1_ilp )
           call stdlib_ycopy( n, xm, 1_ilp, xp, 1_ilp )
           call stdlib_yaxpy( n, cone, rhs, 1_ilp, xp, 1_ilp )
           call stdlib_yaxpy( n, -cone, xm, 1_ilp, rhs, 1_ilp )
           call stdlib_ygesc2( n, z, ldz, rhs, ipiv, jpiv, scale )
           call stdlib_ygesc2( n, z, ldz, xp, ipiv, jpiv, scale )
           if( stdlib_xzasum( n, xp, 1_ilp )>stdlib_xzasum( n, rhs, 1_ilp ) )call stdlib_ycopy( n, xp, 1_ilp, &
                     rhs, 1_ilp )
           ! compute the sum of squares
           call stdlib_ylassq( n, rhs, 1_ilp, rdscal, rdsum )
           return
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if (notrans) then
                    call stdlib_xgetrs( 'NO TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_xgetrs( 'TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 end if
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_xla_gercond = ( one / ainvnm )
           return
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           character :: normin
           integer(ilp) :: ix, j, jp, kase, kase1, kd, lm
           real(xdp) :: ainvnm, scale, smlnum, t
           ! Local Arrays 
           integer(ilp) :: isave(3_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
              ...
              if( kase==kase1 ) then
                 ...
                 if( lnoti ) then
                    do j = 1, n - 1
                       ...
                       call stdlib_xaxpy( lm, -t, ab( kd+1, j ), 1_ilp, work( j+1 ), 1_ilp )
                    end do
                 end if
                 ! multiply by inv(u).
                 call stdlib_xlatbs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, &
                           ldab, work, scale, work( 2_ilp*n+1 ),info )
              else
                 ! multiply by inv(u**t).
                 call stdlib_xlatbs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, ldab, &
                           work, scale, work( 2_ilp*n+1 ),info )
                 ! multiply by inv(l**t).
                 if( lnoti ) then
                    do j = n - 1, 1, -1
                       lm = min( kl, n-j )
                       work( j ) = work( j ) - stdlib_xdot( lm, ab( kd+1, j ), 1_ilp,work( j+1 ), 1_ilp )
              ...
              if( kase==kase1 ) then
                 ...
                 if( lnoti ) then
                    do j = 1, n - 1
                       ...
                       call stdlib_yaxpy( lm, -t, ab( kd+1, j ), 1_ilp, work( j+1 ), 1_ilp )
                    end do
                 end if
                 ! multiply by inv(u).
                 call stdlib_ylatbs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, &
                           ldab, work, scale, rwork, info )
              else
                 ! multiply by inv(u**h).
                 call stdlib_ylatbs( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',normin, n, kl+ku, &
                           ab, ldab, work, scale, rwork,info )
                 ! multiply by inv(l**h).
                 if( lnoti ) then
                    do j = n - 1, 1, -1
                       lm = min( kl, n-j )
                       work( j ) = work( j ) - stdlib_ydotc( lm, ab( kd+1, j ), 1_ilp,work( j+1 ), 1_ilp )
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           real(xdp) :: temp
           ! Local Arrays 
           real(xdp) :: work13(ldwork,nbmax), work31(ldwork,nbmax)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! kv is the number of superdiagonals in the factor u, allowing for
           ! fill-in
           ...
           if( nb<=1_ilp .or. nb>kl ) then
              ! use unblocked code
              call stdlib_xgbtf2( m, n, kl, ku, ab, ldab, ipiv, info )
           else
              ! use blocked code
              ! zero the superdiagonal elements of the work array work13
              do j = 1, nb
              ...
              loop_180: do j = 1, min( m, n ), nb
                 ...
                 loop_80: do jj = j, j + jb - 1
                    ...
                    ! subdiagonal elements in the current column.
                    km = min( kl, m-jj )
                    jp = stdlib_ixamax( km+1, ab( kv+1, jj ), 1_ilp )
                    ipiv( jj ) = jp + jj - j
                    if( ab( kv+jp, jj )/=zero ) then
                       ju = max( ju, min( jj+ku+jp-1, n ) )
                       if( jp/=1_ilp ) then
                          ! apply interchange to columns j to j+jb-1
                          if( jp+jj-1<j+kl ) then
                             call stdlib_xswap( jb, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j )&
                                       , ldab-1 )
                          else
                             ! the interchange affects columns j to jj-1 of a31
                             ! which are stored in the work array work31
                             call stdlib_xswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-&
                                       kl, 1_ilp ), ldwork )
                             call stdlib_xswap( j+jb-jj, ab( kv+1, jj ), ldab-1,ab( kv+jp, jj ), &
                                       ldab-1 )
                          end if
                       end if
                       ! compute multipliers
                       call stdlib_xscal( km, one / ab( kv+1, jj ), ab( kv+2, jj ),1_ilp )
                       ! update trailing submatrix within the band and within
                       ! the current block. jm is the index of the last column
                       ! which needs to be updated.
                       jm = min( ju, j+jb-1 )
                       if( jm>jj )call stdlib_xger( km, jm-jj, -one, ab( kv+2, jj ), 1_ilp,ab( kv, jj+&
                                 1_ilp ), ldab-1,ab( kv+1, jj+1 ), ldab-1 )
                    else
                       ! if pivot is zero, set info to the index of the pivot
                       ! unless a zero pivot has already been found.
                    ...
                    ! copy current column of a31 into the work array work31
                    nw = min( jj-j+1, i3 )
                    if( nw>0_ilp )call stdlib_xcopy( nw, ab( kv+kl+1-jj+j, jj ), 1_ilp,work31( 1_ilp, jj-j+1 )&
                              , 1_ilp )
                 end do loop_80
                 if( j+jb<=n ) then
                    ! apply the row interchanges to the other blocks.
                    j2 = min( ju-j+1, kv ) - jb
                    j3 = max( 0_ilp, ju-j-kv+1 )
                    ! use stdlib_xlaswp to apply the row interchanges to a12, a22, and
                    ! a32.
                    call stdlib_xlaswp( j2, ab( kv+1-jb, j+jb ), ldab-1, 1_ilp, jb,ipiv( j ), 1_ilp )
                    ...
                    if( j2>0_ilp ) then
                       ! update a12
                       call stdlib_xtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j2, one, ab(&
                                  kv+1, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1 )
                       if( i2>0_ilp ) then
                          ! update a22
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j2,jb, -one, ab( &
                          kv+1+jb, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1, one,ab( kv+1, j+jb ), &
                                    ldab-1 )
                       end if
                       if( i3>0_ilp ) then
                          ! update a32
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j2,jb, -one, &
                          work31, ldwork,ab( kv+1-jb, j+jb ), ldab-1, one,ab( kv+kl+1-jb, j+jb ), &
                                    ldab-1 )
                       end if
                    end if
                    if( j3>0_ilp ) then
                       ...
                       ! update a13 in the work array
                       call stdlib_xtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j3, one, ab(&
                                  kv+1, j ), ldab-1,work13, ldwork )
                       if( i2>0_ilp ) then
                          ! update a23
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j3,jb, -one, ab( &
                          kv+1+jb, j ), ldab-1,work13, ldwork, one, ab( 1_ilp+jb, j+kv ),ldab-1 )
                                    
                       end if
                       if( i3>0_ilp ) then
                          ! update a33
                          call stdlib_xgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j3,jb, -one, &
                                    work31, ldwork, work13,ldwork, one, ab( 1_ilp+kl, j+kv ), ldab-1 )
                       end if
                       ! copy the lower triangle of a13 back into place
                       do jj = 1, j3
                 ...
                 do jj = j + jb - 1, j, -1
                    ...
                    if( jp/=1_ilp ) then
                       ...
                       if( jp+jj-1<j+kl ) then
                          ! the interchange does not affect a31
                          call stdlib_xswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j ),&
                                     ldab-1 )
                       else
                          ! the interchange does affect a31
                          call stdlib_xswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-kl, &
                                    1_ilp ), ldwork )
                       end if
                    end if
                    ! copy the current column of a31 back into place
                    nw = min( i3, jj-j+1 )
                    if( nw>0_ilp )call stdlib_xcopy( nw, work31( 1_ilp, jj-j+1 ), 1_ilp,ab( kv+kl+1-jj+j, jj )&
                              , 1_ilp )
                 end do
              end do loop_180
           end if
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           complex(xdp) :: temp
           ! Local Arrays 
           complex(xdp) :: work13(ldwork,nbmax), work31(ldwork,nbmax)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! kv is the number of superdiagonals in the factor u, allowing for
           ! fill-in
           ...
           if( nb<=1_ilp .or. nb>kl ) then
              ! use unblocked code
              call stdlib_ygbtf2( m, n, kl, ku, ab, ldab, ipiv, info )
           else
              ! use blocked code
              ! czero the superdiagonal elements of the work array work13
              do j = 1, nb
              ...
              loop_180: do j = 1, min( m, n ), nb
                 ...
                 loop_80: do jj = j, j + jb - 1
                    ...
                    ! subdiagonal elements in the current column.
                    km = min( kl, m-jj )
                    jp = stdlib_iyamax( km+1, ab( kv+1, jj ), 1_ilp )
                    ipiv( jj ) = jp + jj - j
                    if( ab( kv+jp, jj )/=czero ) then
                       ju = max( ju, min( jj+ku+jp-1, n ) )
                       if( jp/=1_ilp ) then
                          ! apply interchange to columns j to j+jb-1
                          if( jp+jj-1<j+kl ) then
                             call stdlib_yswap( jb, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j )&
                                       , ldab-1 )
                          else
                             ! the interchange affects columns j to jj-1 of a31
                             ! which are stored in the work array work31
                             call stdlib_yswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-&
                                       kl, 1_ilp ), ldwork )
                             call stdlib_yswap( j+jb-jj, ab( kv+1, jj ), ldab-1,ab( kv+jp, jj ), &
                                       ldab-1 )
                          end if
                       end if
                       ! compute multipliers
                       call stdlib_yscal( km, cone / ab( kv+1, jj ), ab( kv+2, jj ),1_ilp )
                       ! update trailing submatrix within the band and within
                       ! the current block. jm is the index of the last column
                       ! which needs to be updated.
                       jm = min( ju, j+jb-1 )
                       if( jm>jj )call stdlib_ygeru( km, jm-jj, -cone, ab( kv+2, jj ), 1_ilp,ab( kv, &
                                 jj+1 ), ldab-1,ab( kv+1, jj+1 ), ldab-1 )
                    else
                       ! if pivot is czero, set info to the index of the pivot
                       ! unless a czero pivot has already been found.
                    ...
                    ! copy current column of a31 into the work array work31
                    nw = min( jj-j+1, i3 )
                    if( nw>0_ilp )call stdlib_ycopy( nw, ab( kv+kl+1-jj+j, jj ), 1_ilp,work31( 1_ilp, jj-j+1 )&
                              , 1_ilp )
                 end do loop_80
                 if( j+jb<=n ) then
                    ! apply the row interchanges to the other blocks.
                    j2 = min( ju-j+1, kv ) - jb
                    j3 = max( 0_ilp, ju-j-kv+1 )
                    ! use stdlib_ylaswp to apply the row interchanges to a12, a22, and
                    ! a32.
                    call stdlib_ylaswp( j2, ab( kv+1-jb, j+jb ), ldab-1, 1_ilp, jb,ipiv( j ), 1_ilp )
                    ...
                    if( j2>0_ilp ) then
                       ! update a12
                       call stdlib_ytrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j2, cone, &
                                 ab( kv+1, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1 )
                       if( i2>0_ilp ) then
                          ! update a22
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j2,jb, -cone, ab(&
                           kv+1+jb, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1, cone,ab( kv+1, j+jb )&
                                     , ldab-1 )
                       end if
                       if( i3>0_ilp ) then
                          ! update a32
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j2,jb, -cone, &
                          work31, ldwork,ab( kv+1-jb, j+jb ), ldab-1, cone,ab( kv+kl+1-jb, j+jb ),&
                                     ldab-1 )
                       end if
                    end if
                    if( j3>0_ilp ) then
                       ...
                       ! update a13 in the work array
                       call stdlib_ytrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j3, cone, &
                                 ab( kv+1, j ), ldab-1,work13, ldwork )
                       if( i2>0_ilp ) then
                          ! update a23
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j3,jb, -cone, ab(&
                           kv+1+jb, j ), ldab-1,work13, ldwork, cone, ab( 1_ilp+jb, j+kv ),ldab-1 )
                                     
                       end if
                       if( i3>0_ilp ) then
                          ! update a33
                          call stdlib_ygemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j3,jb, -cone, &
                                    work31, ldwork, work13,ldwork, cone, ab( 1_ilp+kl, j+kv ), ldab-1 )
                       end if
                       ! copy the lower triangle of a13 back into place
                       do jj = 1, j3
                 ...
                 do jj = j + jb - 1, j, -1
                    ...
                    if( jp/=1_ilp ) then
                       ...
                       if( jp+jj-1<j+kl ) then
                          ! the interchange does not affect a31
                          call stdlib_yswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j ),&
                                     ldab-1 )
                       else
                          ! the interchange does affect a31
                          call stdlib_yswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-kl, &
                                    1_ilp ), ldwork )
                       end if
                    end if
                    ! copy the current column of a31 back into place
                    nw = min( i3, jj-j+1 )
                    if( nw>0_ilp )call stdlib_ycopy( nw, work31( 1_ilp, jj-j+1 ), 1_ilp,ab( kv+kl+1-jj+j, jj )&
                              , 1_ilp )
                 end do
              end do loop_180
           end if
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           loop_40: do j = 1, min( m, n )
              ...
              ! subdiagonal elements in the current column.
              km = min( kl, m-j )
              jp = stdlib_ixamax( km+1, ab( kv+1, j ), 1_ilp )
              ipiv( j ) = jp + j - 1_ilp
              if( ab( kv+jp, j )/=zero ) then
                 ju = max( ju, min( j+ku+jp-1, n ) )
                 ! apply interchange to columns j to ju.
                 if( jp/=1_ilp )call stdlib_xswap( ju-j+1, ab( kv+jp, j ), ldab-1,ab( kv+1, j ), ldab-&
                           1_ilp )
                 if( km>0_ilp ) then
                    ! compute multipliers.
                    call stdlib_xscal( km, one / ab( kv+1, j ), ab( kv+2, j ), 1_ilp )
                    ! update trailing submatrix within the band.
                    if( ju>j )call stdlib_xger( km, ju-j, -one, ab( kv+2, j ), 1_ilp,ab( kv, j+1 ), &
                              ldab-1, ab( kv+1, j+1 ),ldab-1 )
                 end if
              else
                 ! if pivot is zero, set info to the index of the pivot
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           loop_40: do j = 1, min( m, n )
              ...
              ! subdiagonal elements in the current column.
              km = min( kl, m-j )
              jp = stdlib_iyamax( km+1, ab( kv+1, j ), 1_ilp )
              ipiv( j ) = jp + j - 1_ilp
              if( ab( kv+jp, j )/=czero ) then
                 ju = max( ju, min( j+ku+jp-1, n ) )
                 ! apply interchange to columns j to ju.
                 if( jp/=1_ilp )call stdlib_yswap( ju-j+1, ab( kv+jp, j ), ldab-1,ab( kv+1, j ), ldab-&
                           1_ilp )
                 if( km>0_ilp ) then
                    ! compute multipliers.
                    call stdlib_yscal( km, cone / ab( kv+1, j ), ab( kv+2, j ), 1_ilp )
                    ! update trailing submatrix within the band.
                    if( ju>j )call stdlib_ygeru( km, ju-j, -cone, ab( kv+2, j ), 1_ilp,ab( kv, j+1 ), &
                              ldab-1, ab( kv+1, j+1 ),ldab-1 )
                 end if
              else
                 ! if pivot is czero, set info to the index of the pivot
           ...
           if( notran ) then
              ...
              if( lnoti ) then
                 do j = 1, n - 1
                    lm = min( kl, n-j )
                    l = ipiv( j )
                    if( l/=j )call stdlib_xswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                    call stdlib_xger( lm, nrhs, -one, ab( kd+1, j ), 1_ilp, b( j, 1_ilp ),ldb, b( j+1, 1_ilp )&
                              , ldb )
                 end do
              end if
              do i = 1, nrhs
                 ! solve u*x = b, overwriting b with x.
                 call stdlib_xtbsv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, kl+ku,ab, ldab, b( 1_ilp, &
                           i ), 1_ilp )
              end do
           else
              ! solve a**t*x = b.
              do i = 1, nrhs
                 ! solve u**t*x = b, overwriting b with x.
                 call stdlib_xtbsv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, kl+ku, ab,ldab, b( 1_ilp, i )&
                           , 1_ilp )
              end do
              ! solve l**t*x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_xgemv( 'TRANSPOSE', lm, nrhs, -one, b( j+1, 1_ilp ),ldb, ab( kd+1, j )&
                              , 1_ilp, one, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_xswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                 end do
           ...
           if( notran ) then
              ...
              if( lnoti ) then
                 do j = 1, n - 1
                    lm = min( kl, n-j )
                    l = ipiv( j )
                    if( l/=j )call stdlib_yswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                    call stdlib_ygeru( lm, nrhs, -cone, ab( kd+1, j ), 1_ilp, b( j, 1_ilp ),ldb, b( j+1, &
                              1_ilp ), ldb )
                 end do
              end if
              do i = 1, nrhs
                 ! solve u*x = b, overwriting b with x.
                 call stdlib_ytbsv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, kl+ku,ab, ldab, b( 1_ilp, &
                           i ), 1_ilp )
              end do
           else if( stdlib_lsame( trans, 'T' ) ) then
              ! solve a**t * x = b.
              do i = 1, nrhs
                 ! solve u**t * x = b, overwriting b with x.
                 call stdlib_ytbsv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, kl+ku, ab,ldab, b( 1_ilp, i )&
                           , 1_ilp )
              end do
              ! solve l**t * x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_ygemv( 'TRANSPOSE', lm, nrhs, -cone, b( j+1, 1_ilp ),ldb, ab( kd+1, j &
                              ), 1_ilp, cone, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_yswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                 end do
              ...
              do i = 1, nrhs
                 ! solve u**h * x = b, overwriting b with x.
                 call stdlib_ytbsv( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT', n,kl+ku, ab, ldab,&
                            b( 1_ilp, i ), 1_ilp )
              end do
              ! solve l**h * x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_ylacgv( nrhs, b( j, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', lm, nrhs, -cone,b( j+1, 1_ilp ), ldb, &
                              ab( kd+1, j ), 1_ilp, cone,b( j, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_yswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_xcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_xgbmv( trans, n, n, kl, ku, -one, ab, ldab, x( 1_ilp, j ), 1_ilp,one, work( n+1 &
                        ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_xgbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, info )
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_xgbtrs( transt, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, &
                              info )
                    do i = 1, n
                       work( n+i ) = work( n+i )*work( i )
                    end do
                    ...
                    call stdlib_xgbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, &
                              info )
                 end if
                 go to 100
              end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_ycopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_ygbmv( trans, n, n, kl, ku, -cone, ab, ldab, x( 1_ilp, j ), 1_ilp,cone, work, 1_ilp &
                        )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_ygbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv, work, n,info )
                 call stdlib_yaxpy( n, cone, work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_ygbtrs( transt, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info )
                    ...
                    call stdlib_ygbtrs( transn, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info )
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( notrans ) then
                    call stdlib_xgbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_xgbtrs( 'TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info &
                              )
                 end if
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_xla_gbrcond = ( one / ainvnm )
           return
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(u)*inv(l).
                 call stdlib_xgttrs( 'NO TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv,work, n, info )
                           
              else
                 ! multiply by inv(l**t)*inv(u**t).
                 call stdlib_xgttrs( 'TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv, work,n, info )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           ! check that d(1:n) is non-zero.
           do i = 1, n
              if( d( i )==cmplx( zero,KIND=xdp) )return
           end do
           ainvnm = zero
           if( onenrm ) then
              kase1 = 1_ilp
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(u)*inv(l).
                 call stdlib_ygttrs( 'NO TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv,work, n, info )
                           
              else
                 ! multiply by inv(l**h)*inv(u**h).
                 call stdlib_ygttrs( 'CONJUGATE TRANSPOSE', n, 1_ilp, dl, d, du, du2,ipiv, work, n, &
                           info )
              end if
              go to 20
           end if
     ...
     !! by DGTTRF.
        ...
        ! =====================================================================
           ...
           if( nb>=nrhs ) then
              call stdlib_xgtts2( itrans, n, nrhs, dl, d, du, du2, ipiv, b, ldb )
           else
              do j = 1, nrhs, nb
                 jb = min( nrhs-j+1, nb )
                 call stdlib_xgtts2( itrans, n, jb, dl, d, du, du2, ipiv, b( 1_ilp, j ),ldb )
              end do
           end if
     ...
     !! by ZGTTRF.
        ...
        ! =====================================================================
           ...
           if( nb>=nrhs ) then
              call stdlib_ygtts2( itrans, n, nrhs, dl, d, du, du2, ipiv, b, ldb )
           else
              do j = 1, nrhs, nb
                 jb = min( nrhs-j+1, nb )
                 call stdlib_ygtts2( itrans, n, jb, dl, d, du, du2, ipiv, b( 1_ilp, j ),ldb )
              end do
           end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           character :: transn, transt
           integer(ilp) :: count, i, j, kase, nz
           real(xdp) :: eps, lstres, s, safe1, safe2, safmin
           ! Local Arrays 
           integer(ilp) :: isave(3_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           loop_110: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_xcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_xlagtm( trans, n, 1_ilp, -one, dl, d, du, x( 1_ilp, j ), ldx, one,work( n+1 ), &
                        n )
              ! compute abs(op(a))*abs(x) + abs(b) for use in the backward
              ! error bound.
              if( notran ) then
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_xgttrs( trans, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, info )
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_xgttrs( transt, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, &
                              info )
                    do i = 1, n
                       work( n+i ) = work( i )*work( n+i )
                    end do
                    ...
                    call stdlib_xgttrs( transn, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, &
                              info )
                 end if
                 go to 70
              end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_110: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_ycopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_ylagtm( trans, n, 1_ilp, -one, dl, d, du, x( 1_ilp, j ), ldx, one,work, n )
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_ygttrs( trans, n, 1_ilp, dlf, df, duf, du2, ipiv, work, n,info )
                 call stdlib_yaxpy( n, cmplx( one,KIND=xdp), work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_ygttrs( transt, n, 1_ilp, dlf, df, duf, du2, ipiv, work,n, info )
                    ...
                    call stdlib_ygttrs( transn, n, 1_ilp, dlf, df, duf, du2, ipiv, work,n, info )
