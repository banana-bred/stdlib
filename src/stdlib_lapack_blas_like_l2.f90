submodule(stdlib_lapack_base) stdlib_lapack_blas_like_l2
  ...
  contains
     ...
     !! or banded.
        ...
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           use stdlib_blas_constants_xdp, only: zero, half, one
           ! Scalar Arguments 
           character, intent(in) :: type
           integer(ilp), intent(out) :: info
           integer(ilp), intent(in) :: kl, ku, lda, m, n
        ...
        ! =====================================================================
           ...
           if( itype==-1_ilp ) then
              info = -1_ilp
           else if( cfrom==zero .or. stdlib_xisnan(cfrom) ) then
              info = -4_ilp
           else if( stdlib_xisnan(cto) ) then
              info = -5_ilp
           else if( m<0_ilp ) then
              info = -6_ilp
           else if( n<0_ilp .or. ( itype==4_ilp .and. n/=m ) .or.( itype==5_ilp .and. n/=m ) ) then
     ...
     !! or banded.
        ...
        ! =====================================================================
           ...
           if( itype==-1_ilp64 ) then
              info = -1_ilp64
           else if( cfrom==zero .or. stdlib_I64_xisnan(cfrom) ) then
              info = -4_ilp64
           else if( stdlib_I64_xisnan(cto) ) then
              info = -5_ilp64
           else if( m<0_ilp64 ) then
              info = -6_ilp64
           else if( n<0_ilp64 .or. ( itype==4_ilp64 .and. n/=m ) .or.( itype==5_ilp64 .and. n/=m ) ) then
     ...
     !! or banded.
        ...
        ! =====================================================================
           ...
           if( itype==-1_ilp64 ) then
              info = -1_ilp64
           else if( cfrom==zero .or. stdlib_I64_qisnan(cfrom) ) then
              info = -4_ilp64
           else if( stdlib_I64_qisnan(cto) ) then
              info = -5_ilp64
           else if( m<0_ilp64 ) then
              info = -6_ilp64
           else if( n<0_ilp64 .or. ( itype==4_ilp64 .and. n/=m ) .or.( itype==5_ilp64 .and. n/=m ) ) then
