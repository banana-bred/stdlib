  ...
  contains
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_xsytrf( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_xsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_xsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_xsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_ysytrf( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=xdp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           if( info/=0_ilp ) then
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_ysytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_ysytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_ysytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_xlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_xsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_xlansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_xsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_ylacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_ysytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_ysycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ysytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_ysyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_xsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_xsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_xsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_ysytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=xdp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_ysytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_ysytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_xsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_xsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_xsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_ysytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=xdp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_ysytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_ysytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_yhetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_yhetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_yhetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_ylacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_yhetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_yhecon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_yhetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_yherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_yhetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=xdp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**h)*(p**t) or
           ! a = p*u*d*(u**h)*(p**t).
           call stdlib_yhetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_yhetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling ZHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_yhetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_yhetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_xsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_ysptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ysptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_xcopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_xsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_xlansp( 'I', uplo, n, ap, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_xsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_ycopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_ysptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_yspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ysptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_ysprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_yhptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_yhptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_ycopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_yhptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylanhp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_yhpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_yhptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_yhprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_xsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_sytrf = int( work(1_ilp),KIND=ilp)
              call stdlib_xsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_sytrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_xsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_ysytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_sytrf = int( work(1_ilp),KIND=ilp)
              call stdlib_ysytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_sytrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_ysytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ysytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_yhetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_hetrf = int( work(1_ilp),KIND=ilp)
              call stdlib_yhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_hetrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_yhetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_yhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_xsytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_xsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_xsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_xsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_qsytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_qsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_qsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_qsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ysytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=xdp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ysytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_ysytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_ysytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_wsytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=qp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_wsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_wsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_wsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_xlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_xsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_xlansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_xsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_qlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_qsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_qlansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_qsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_ylacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_ysytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ysycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ysytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_ysyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_wlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_wsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_wsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_xsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_xsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_xsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_qsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_qsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_qsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ysytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=xdp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_ysytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_ysytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_wsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=qp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_wsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_wsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_xsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_xsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_xsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_qsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_qsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_qsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ysytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=xdp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ysytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_ysytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_wsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=qp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_wsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_wsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_yhetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_yhetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_yhetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_whetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_whetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_whetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_ylacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_yhetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_yhecon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_yhetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_yherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_wlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_whetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_whecon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_whetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_wherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_yhetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=xdp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**h)*(p**t) or
           ! a = p*u*d*(u**h)*(p**t).
           call stdlib_I64_yhetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_yhetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_whetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=qp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**h)*(p**t) or
           ! a = p*u*d*(u**h)*(p**t).
           call stdlib_I64_whetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_whetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling ZHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_yhetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_I64_yhetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling ZHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_whetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_I64_whetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_xsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_qsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ysptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ysptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_wsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_xcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_xsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_xlansp( 'I', uplo, n, ap, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_xsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_qcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_qsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_qlansp( 'I', uplo, n, ap, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_qsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_ycopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_ysptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_yspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ysptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_ysprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_wcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_wsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_wsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_yhptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_yhptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_whptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_whptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_ycopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_yhptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylanhp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_yhpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_yhptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_yhprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_wcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_whptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlanhp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_whpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_whptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_whprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_xsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_xsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_xsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_qsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_qsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_qsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_ysytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_ysytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_ysytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ysytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_wsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_wsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_wsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_yhetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_hetrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_yhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_hetrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_I64_yhetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_yhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_whetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_hetrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_whetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_hetrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp64 ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_I64_whetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_whetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
