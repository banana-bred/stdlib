...
module stdlib_sparse_conversion
    ...
    interface diag
        ...
        module procedure :: dense2diagonal_xdp
        module procedure :: coo2diagonal_xdp
        module procedure :: csr2diagonal_xdp
        module procedure :: csc2diagonal_xdp
        module procedure :: ell2diagonal_xdp
        ...
        module procedure :: dense2diagonal_cxdp
        module procedure :: coo2diagonal_cxdp
        module procedure :: csr2diagonal_cxdp
        module procedure :: csc2diagonal_cxdp
        module procedure :: ell2diagonal_cxdp
    ...
    interface from_ijv
        ...
        module procedure :: coo_from_ijv_xdp
        module procedure :: csr_from_ijv_xdp
        module procedure :: ell_from_ijv_xdp
        module procedure :: sellc_from_ijv_xdp
        ...
        module procedure :: coo_from_ijv_cxdp
        module procedure :: csr_from_ijv_cxdp
        module procedure :: ell_from_ijv_cxdp
        module procedure :: sellc_from_ijv_cxdp
...
contains
    ...
    subroutine dense2coo_xdp(dense,COO)
        real(xdp), intent(in) :: dense(:,:)
        type(COO_xdp_type), intent(out) :: COO
        integer(ilp) :: num_rows, num_cols, nnz
        integer(ilp) :: i, j, idx

        num_rows = size(dense,dim=1)
        num_cols = size(dense,dim=2)
        nnz      = count( abs(dense) > tiny(1._xdp) )
        ...
        do i = 1, num_rows
            do j = 1, num_cols
                if(abs(dense(i,j)) < tiny(1._xdp)) cycle
                COO%index(1,idx) = i
                COO%index(2,idx) = j
                COO%data(idx) = dense(i,j)
                idx = idx + 1
    ...
    subroutine dense2coo_cxdp(dense,COO)
        complex(xdp), intent(in) :: dense(:,:)
        type(COO_cxdp_type), intent(out) :: COO
        integer(ilp) :: num_rows, num_cols, nnz
        integer(ilp) :: i, j, idx
    ...
    subroutine coo2dense_xdp(COO,dense)
        type(COO_xdp_type), intent(in) :: COO
        real(xdp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: idx

        if(.not.allocated(dense)) allocate(dense(COO%nrows,COO%nrows),source=zero_xdp)
        do concurrent(idx = 1:COO%nnz)
            dense( COO%index(1,idx) , COO%index(2,idx) ) = COO%data(idx)
        end do
    end subroutine
    ...
    subroutine coo2dense_cxdp(COO,dense)
        type(COO_cxdp_type), intent(in) :: COO
        complex(xdp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: idx

        if(.not.allocated(dense)) allocate(dense(COO%nrows,COO%nrows),source=zero_cxdp)
        do concurrent(idx = 1:COO%nnz)
            dense( COO%index(1,idx) , COO%index(2,idx) ) = COO%data(idx)
        end do
    end subroutine
    ...
    subroutine coo2csr_xdp(COO,CSR)
        type(COO_xdp_type), intent(in)  :: COO
        type(CSR_xdp_type), intent(out) :: CSR
        integer(ilp) :: i
    ...
    subroutine coo2csr_cxdp(COO,CSR)
        type(COO_cxdp_type), intent(in)  :: COO
        type(CSR_cxdp_type), intent(out) :: CSR
        integer(ilp) :: i
    ...
    subroutine coo2csc_xdp(COO,CSC)
        type(COO_xdp_type), intent(in)  :: COO
        type(CSC_xdp_type), intent(out) :: CSC
        ...
        allocate(data, source = COO%data )
        nnz = COO%nnz
        call sort_coo_unique_xdp( temp, data, nnz, COO%nrows, COO%ncols )
    ...
    subroutine coo2csc_cxdp(COO,CSC)
        type(COO_cxdp_type), intent(in)  :: COO
        type(CSC_cxdp_type), intent(out) :: CSC
        ...
        allocate(data, source = COO%data )
        nnz = COO%nnz
        call sort_coo_unique_cxdp( temp, data, nnz, COO%nrows, COO%ncols )
    ...
    subroutine csr2dense_xdp(CSR,dense)
        type(CSR_xdp_type), intent(in) :: CSR
        real(xdp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: i, j

        if(.not.allocated(dense)) allocate(dense(CSR%nrows,CSR%nrows),source=zero_xdp)
        if( CSR%storage == sparse_full) then
            do i = 1, CSR%nrows
                do j = CSR%rowptr(i), CSR%rowptr(i+1)-1
                    dense(i,CSR%col(j)) = CSR%data(j)
    ...
    subroutine csr2dense_cxdp(CSR,dense)
        type(CSR_cxdp_type), intent(in) :: CSR
        complex(xdp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: i, j

        if(.not.allocated(dense)) allocate(dense(CSR%nrows,CSR%nrows),source=zero_cxdp)
        if( CSR%storage == sparse_full) then
            do i = 1, CSR%nrows
                do j = CSR%rowptr(i), CSR%rowptr(i+1)-1
                    dense(i,CSR%col(j)) = CSR%data(j)
    ...
    subroutine csr2coo_xdp(CSR,COO)
        type(CSR_xdp_type), intent(in)  :: CSR
        type(COO_xdp_type), intent(out) :: COO
        integer(ilp) :: i, j
    ...
    subroutine csr2coo_cxdp(CSR,COO)
        type(CSR_cxdp_type), intent(in)  :: CSR
        type(COO_cxdp_type), intent(out) :: COO
        integer(ilp) :: i, j
    ...
    subroutine csc2coo_xdp(CSC,COO)
        type(CSC_xdp_type), intent(in)  :: CSC
        type(COO_xdp_type), intent(out) :: COO
        integer(ilp) :: i, j
        ...
        call sort_coo_unique_xdp( COO%index, COO%data, COO%nnz, COO%nrows, COO%ncols )
    end subroutine
    ...
    subroutine csc2coo_cxdp(CSC,COO)
        type(CSC_cxdp_type), intent(in)  :: CSC
        type(COO_cxdp_type), intent(out) :: COO
        integer(ilp) :: i, j
        ...
        call sort_coo_unique_cxdp( COO%index, COO%data, COO%nnz, COO%nrows, COO%ncols )
    end subroutine
    ...
    subroutine csr2ell_xdp(CSR,ELL,num_nz_rows)
        type(CSR_xdp_type), intent(in)  :: CSR
        type(ELL_xdp_type), intent(out) :: ELL
        integer, intent(in), optional :: num_nz_rows !! number of non zeros per row
    ...
    subroutine csr2ell_cxdp(CSR,ELL,num_nz_rows)
        type(CSR_cxdp_type), intent(in)  :: CSR
        type(ELL_cxdp_type), intent(out) :: ELL
        integer, intent(in), optional :: num_nz_rows !! number of non zeros per row
    ...
    subroutine csr2sellc_xdp(CSR,SELLC,chunk)
        !! csr2sellc: This function enables transfering data from a CSR matrix to a SELL-C matrix
        !! This algorithm was gracefully provided by Ivan Privec and adapted by Jose Alves
        type(CSR_xdp_type), intent(in)    :: CSR
        type(SELLC_xdp_type), intent(out) :: SELLC
        integer, intent(in), optional :: chunk
        real(xdp), parameter :: zero = zero_xdp
        integer(ilp) :: i, j, num_chunks
    ...
    subroutine csr2sellc_cxdp(CSR,SELLC,chunk)
        !! csr2sellc: This function enables transfering data from a CSR matrix to a SELL-C matrix
        !! This algorithm was gracefully provided by Ivan Privec and adapted by Jose Alves
        type(CSR_cxdp_type), intent(in)    :: CSR
        type(SELLC_cxdp_type), intent(out) :: SELLC
        integer, intent(in), optional :: chunk
        complex(xdp), parameter :: zero = zero_cxdp
        integer(ilp) :: i, j, num_chunks
    ...
    recursive subroutine quicksort_i_xdp(a, b, first, last)
        integer(ilp), intent(inout) :: a(*) !! reference table to sort
        real(xdp), intent(inout)  :: b(*) !! secondary real data to sort w.r.t. a
        integer(ilp), intent(in)     :: first, last
        integer(ilp)  :: i, j, x, t
        real(xdp) :: d
        ...
        if (first < i-1) call quicksort_i_xdp(a, b, first, i-1)
        if (j+1 < last)  call quicksort_i_xdp(a, b, j+1, last)
    end subroutine 
    ...
    recursive subroutine quicksort_i_cxdp(a, b, first, last)
        integer(ilp), intent(inout) :: a(*) !! reference table to sort
        complex(xdp), intent(inout)  :: b(*) !! secondary real data to sort w.r.t. a
        integer(ilp), intent(in)     :: first, last
        integer(ilp)  :: i, j, x, t
        complex(xdp) :: d
        ...
        if (first < i-1) call quicksort_i_cxdp(a, b, first, i-1)
        if (j+1 < last)  call quicksort_i_cxdp(a, b, j+1, last)
    end subroutine 
    ...
    subroutine sort_coo_unique_xdp( a, data, n, num_rows, num_cols )
        !! Sort a 2d array in increasing order first by index 1 and then by index 2
        real(xdp), intent(inout) :: data(*)
        integer(ilp), intent(inout) :: a(2,*)
        integer(ilp), intent(inout) :: n
        integer(ilp), intent(in) :: num_rows
        integer(ilp), intent(in) :: num_cols
        ...
        do n_i = 1, num_rows
            ...
            adr1 = count_i_aux(n_i)
            dd = adr1-adr0+1
            if(dd>0) call quicksort_i_xdp(cols_(adr0),temp(adr0),1,dd)
        end do
        !---------------------------------------------------------
        ! Remove duplicates
        do ed = 1,n
    ...
    subroutine sort_coo_unique_cxdp( a, data, n, num_rows, num_cols )
        !! Sort a 2d array in increasing order first by index 1 and then by index 2
        complex(xdp), intent(inout) :: data(*)
        integer(ilp), intent(inout) :: a(2,*)
        integer(ilp), intent(inout) :: n
        integer(ilp), intent(in) :: num_rows
        integer(ilp), intent(in) :: num_cols
        ...
        do n_i = 1, num_rows
            ...
            adr1 = count_i_aux(n_i)
            dd = adr1-adr0+1
            if(dd>0) call quicksort_i_cxdp(cols_(adr0),temp(adr0),1,dd)
        end do
        !---------------------------------------------------------
        ! Remove duplicates
        do ed = 1,n
    ...
    subroutine coo_from_ijv_xdp(COO,row,col,data,nrows,ncols)
        type(COO_xdp_type), intent(inout) :: COO
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
        integer(ilp), intent(in), optional :: ncols
    ...
    subroutine coo_from_ijv_cxdp(COO,row,col,data,nrows,ncols)
        type(COO_cxdp_type), intent(inout) :: COO
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
        integer(ilp), intent(in), optional :: ncols
    ...
    subroutine csr_from_ijv_xdp(CSR,row,col,data,nrows,ncols)
        type(CSR_xdp_type), intent(inout) :: CSR
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine csr_from_ijv_cxdp(CSR,row,col,data,nrows,ncols)
        type(CSR_cxdp_type), intent(inout) :: CSR
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine ell_from_ijv_xdp(ELL,row,col,data,nrows,ncols,num_nz_rows)
        type(ELL_xdp_type), intent(inout) :: ELL
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine ell_from_ijv_cxdp(ELL,row,col,data,nrows,ncols,num_nz_rows)
        type(ELL_cxdp_type), intent(inout) :: ELL
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine sellc_from_ijv_xdp(SELLC,row,col,data,nrows,ncols,chunk)
        type(SELLC_xdp_type), intent(inout) :: SELLC
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine sellc_from_ijv_cxdp(SELLC,row,col,data,nrows,ncols,chunk)
        type(SELLC_cxdp_type), intent(inout) :: SELLC
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(xdp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine dense2diagonal_xdp(dense,diagonal)
        real(xdp), intent(in) :: dense(:,:)
        real(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: num_rows
        integer :: i
    ...
    subroutine dense2diagonal_cxdp(dense,diagonal)
        complex(xdp), intent(in) :: dense(:,:)
        complex(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: num_rows
        integer :: i
    ...
    subroutine coo2diagonal_xdp(COO,diagonal)
        type(COO_xdp_type), intent(in) :: COO
        real(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: idx
    ...
    subroutine coo2diagonal_cxdp(COO,diagonal)
        type(COO_cxdp_type), intent(in) :: COO
        complex(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: idx
    ...
    subroutine csr2diagonal_xdp(CSR,diagonal)
        type(CSR_xdp_type), intent(in) :: CSR
        real(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csr2diagonal_cxdp(CSR,diagonal)
        type(CSR_cxdp_type), intent(in) :: CSR
        complex(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csc2diagonal_xdp(CSC,diagonal)
        type(CSC_xdp_type), intent(in) :: CSC
        real(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csc2diagonal_cxdp(CSC,diagonal)
        type(CSC_cxdp_type), intent(in) :: CSC
        complex(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine ell2diagonal_xdp(ELL,diagonal)
        type(ELL_xdp_type), intent(in) :: ELL
        real(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, k
    ...
    subroutine ell2diagonal_cxdp(ELL,diagonal)
        type(ELL_cxdp_type), intent(in) :: ELL
        complex(xdp), intent(inout), allocatable :: diagonal(:)
        integer :: i, k
