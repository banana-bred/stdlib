  ...
  contains
     ...
     !! for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_yhpmv( uplo, n, -cone, ap, x( 1_ilp, j ), 1_ilp, cone, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_yhptrs( uplo, n, 1_ilp, afp, ipiv, work, n, info )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_yhetrs_rook( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_ylahef_rook;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_ylahef_rook( uplo, k, nb, kb, a, lda,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_yhetf2_rook( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_ylahef_rook( uplo, n-k+1, nb, kb, a( k, k ), lda,ipiv( k ), work, &
                           ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_yhetf2_rook( uplo, n-k+1, a( k, k ), lda, ipiv( k ),iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
     ...
     !! A11 (if UPLO = 'U') or A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           integer(ilp) :: imax, itemp, ii, j, jb, jj, jmax, jp1, jp2, k, kk, kkw, kp, kstep, kw, &
                     p
           real(xdp) :: absakk, alpha, colmax, dtemp, r1, rowmax, t, sfmin
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              p = k
              ! copy column k of a to column kw of w and update it
              if( k>1_ilp )call stdlib_ycopy( k-1, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( k>1_ilp )call stdlib_ycopy( k-1, w( 1_ilp, kw ), 1_ilp, a( 1_ilp, k ), 1_ilp )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ! begin pivot search loop body
                       ! copy column imax to column kw-1 of w and update it
                       if( imax>1_ilp )call stdlib_ycopy( imax-1, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ),1_ilp )
                       ...
                       if( k<n ) then
                          call stdlib_ygemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp, k+1 ), lda, w( &
                                    imax, kw+1 ), ldw,cone, w( 1_ilp, kw-1 ), 1_ilp )
                       ...
                       if( imax>1_ilp ) then
                          itemp = stdlib_iyamax( imax-1, w( 1_ilp, kw-1 ), 1_ilp )
                          dtemp = cabs1( w( itemp, kw-1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ...
                    call stdlib_ycopy( k-1-p, a( p+1, k ), 1_ilp, a( p, p+1 ),lda )
                    call stdlib_ylacgv( k-1-p, a( p, p+1 ), lda )
                    if( p>1_ilp )call stdlib_ycopy( p-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, p ), 1_ilp )
                    ! interchange rows k and p in the last k+1 to n columns of a
                    ! (columns k and k-1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_yswap( n-k, a( k, k+1 ), lda, a( p, k+1 ),lda )
                    call stdlib_yswap( n-kk+1, w( k, kkw ), ldw, w( p, kkw ),ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kkw of w.
                 if( kp/=kk ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the rectangular superdiagonal block
                 if( j>=2_ilp )call stdlib_ygemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( &
                           1_ilp, k+1 ), lda, w( j, kw+1 ), ldw,cone, a( 1_ilp, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in of rows in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp
                 if( jp2/=jj .and. j<=n )call stdlib_yswap( n-j+1, a( jp2, j ), lda, a( jj, j ), &
                           lda )
                 jj = jj + 1_ilp
                 if( kstep==2_ilp .and. jp1/=jj .and. j<=n )call stdlib_yswap( n-j+1, a( jp1, j ), &
                           lda, a( jj, j ), lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( k>1_ilp ) then
                 call stdlib_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ),lda, w( k, 1_ilp ), &
                           ldw, cone, w( k, k ), 1_ilp )
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( k<n )call stdlib_ycopy( n-k, w( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    72 continue
                       ...
                       if( k>1_ilp ) then
                          call stdlib_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone,a( k, 1_ilp ), lda, w( &
                                    imax, 1_ilp ), ldw,cone, w( k, k+1 ), 1_ilp )
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_iyamax( n-imax, w( imax+1, k+1 ), 1_ilp)
                          dtemp = cabs1( w( itemp, k+1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ...
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=xdp)
                    call stdlib_ycopy( p-k-1, a( k+1, k ), 1_ilp, a( p, k+1 ), lda )
                    call stdlib_ylacgv( p-k-1, a( p, k+1 ), lda )
                    if( p<n )call stdlib_ycopy( n-p, a( p+1, k ), 1_ilp, a( p+1, p ), 1_ilp )
                    ! interchange rows k and p in first k-1 columns of a
                    ! (columns k and k+1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in first kk columns of w.
                    if( k>1_ilp )call stdlib_yswap( k-1, a( k, 1_ilp ), lda, a( p, 1_ilp ), lda )
                    call stdlib_yswap( kk, w( k, 1_ilp ), ldw, w( p, 1_ilp ), ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kk of w.
                 if( kp/=kk ) then
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp
                 if( jp2/=jj .and. j>=1_ilp )call stdlib_yswap( j, a( jp2, 1_ilp ), lda, a( jj, 1_ilp ), lda )
                           
                 jj = jj -1_ilp
                 if( kstep==2_ilp .and. jp1/=jj .and. j>=1_ilp )call stdlib_yswap( j, a( jp1, 1_ilp ), lda, a(&
                            jj, 1_ilp ), lda )
              if( j>1 )go to 120
              ! set kb to the number of columns factorized
              kb = k - 1_ilp
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! ======================================================================
           ...
           logical(lk) :: done, upper
           integer(ilp) :: i, ii, imax, itemp, j, jmax, k, kk, kp, kstep, p
           real(xdp) :: absakk, alpha, colmax, d, d11, d22, r1, dtemp, rowmax, tt, sfmin
              ...
              if( ( max( absakk, colmax )==zero ) ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ...
                       if( imax>1_ilp ) then
                          itemp = stdlib_iyamax( imax-1, a( 1_ilp, imax ), 1_ilp )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p>1_ilp )call stdlib_yswap( p-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, p ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = p + 1, k - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                 ...
                 if( kp/=kk ) then
                    ! (1) swap columnar parts
                    if( kp>1_ilp )call stdlib_yswap( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k>1_ilp ) then
                       ...
                       if( abs( real( a( k, k ),KIND=xdp) )>=sfmin ) then
                          ...
                          call stdlib_yher( uplo, k-1, -d11, a( 1_ilp, k ), 1_ilp, a, lda )
                          ! store u(k) in column k
                          call stdlib_ydscal( k-1, d11, a( 1_ilp, k ), 1_ilp )
                       else
                          ! store l(k) in column k
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    42 continue
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_iyamax( n-imax, a( imax+1, imax ),1_ilp )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p<n )call stdlib_yswap( n-p, a( p+1, k ), 1_ilp, a( p+1, p ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = k + 1, p - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                    ...
                    if( k<n ) then
                       ...
                       if( abs( real( a( k, k ),KIND=xdp) )>=sfmin ) then
                          ...
                          ! a := a - l(k)*d(k)*l(k)**t
                             ! = a - w(k)*(1/d(k))*w(k)**t
                          d11 = one / real( a( k, k ),KIND=xdp)
                          call stdlib_yher( uplo, n-k, -d11, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
                                    
                          ! store l(k) in column k
                          call stdlib_ydscal( n-k, d11, a( k+1, k ), 1_ilp )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=xdp)
                          do ii = k + 1, n
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( kstep==1_ilp ) then
                 ...
                 kp = ipiv( k )
                 if( kp/=k ) then
                    if( kp>1_ilp )call stdlib_yswap( kp-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    do j = kp + 1, k - 1
                       temp = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( kp, j ) )
                       a( kp, j ) = temp
     ...
     !! For more information see Further Details section.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_ylahef_rk;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_ylahef_rk( uplo, k, nb, kb, a, lda, e,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_yhetf2_rk( uplo, k, a, lda, e, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<n ) then
                 do i = k, ( k - kb + 1 ), -1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_yswap( n-k, a( i, k+1 ), lda,a( ip, k+1 ), lda )
                    end if
                 end do
              end if
              ! decrease k and return to the start of the main loop
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_ylahef_rk( uplo, n-k+1, nb, kb, a( k, k ), lda, e( k ),ipiv( k ), &
                           work, ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_yhetf2_rk( uplo, n-k+1, a( k, k ), lda, e( k ),ipiv( k ), iinfo )
              ...
              if( k>1_ilp ) then
                 do i = k, ( k + kb - 1 ), 1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_yswap( k-1, a( i, 1_ilp ), lda,a( ip, 1_ilp ), lda )
                    end if
                 end do
              end if
              ! increase k and return to the start of the main loop
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! .....................................................
              ! copy first row a(1, 1:n) into h(1:n) (stored in work(1:n))
              call stdlib_ycopy( n, a( 1_ilp, 1_ilp ), lda, work( 1_ilp ), 1_ilp )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_ylahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp
              10 continue
              if( j>=n )go to 20
              ...
              k1 = max(1_ilp, j)-j
              ! panel factorization
              call stdlib_ylahef_aa( uplo, 2_ilp-k1, n-j, jb,a( max(1_ilp, j), j+1 ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp) ) then
                    call stdlib_yswap( j1-k1-2, a( 1_ilp, j2 ), 1_ilp,a( 1_ilp, ipiv(j2) ), 1_ilp )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp .or. jb>1_ilp ) then
                    ...
                    alpha = conjg( a( j, j+1 ) )
                    a( j, j+1 ) = cone
                    call stdlib_ycopy( n-j, a( j-1, j+1 ), lda,work( (j+1-j1+1)+jb*n ), 1_ilp )
                              
                    call stdlib_yscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    if( j1>1_ilp ) then
                    ...
                    do j2 = j+1, n, nb
                       nj = min( nb, n-j2+1 )
                       ! update (j2, j2) diagonal block with stdlib_ygemv
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_ygemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',1_ilp, mj, jb+1,-cone,&
                           a( j1-k2, j3 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j3, j3 ), lda )
                                     
                          j3 = j3 + 1_ilp
                       end do
                       ! update off-diagonal block of j2-th block row with stdlib_ygemm
                       call stdlib_ygemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',nj, n-j3+1, jb+1,-&
                       cone, a( j1-k2, j2 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j2, j3 ), lda &
                                 )
                    end do
                    ! recover t( j, j+1 )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_ycopy( n-j, a( j+1, j+1 ), lda, work( 1_ilp ), 1_ilp )
              end if
              go to 10
           else
              ! .....................................................
              ...
              ! copy first column a(1:n, 1) into h(1:n, 1)
               ! (stored in work(1:n))
              call stdlib_ycopy( n, a( 1_ilp, 1_ilp ), 1_ilp, work( 1_ilp ), 1_ilp )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_ylahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp
              ...
              k1 = max(1_ilp, j)-j
              ! panel factorization
              call stdlib_ylahef_aa( uplo, 2_ilp-k1, n-j, jb,a( j+1, max(1_ilp, j) ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp) ) then
                    call stdlib_yswap( j1-k1-2, a( j2, 1_ilp ), lda,a( ipiv(j2), 1_ilp ), lda )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp .or. jb>1_ilp ) then
                    ...
                    alpha = conjg( a( j+1, j ) )
                    a( j+1, j ) = cone
                    call stdlib_ycopy( n-j, a( j+1, j-1 ), 1_ilp,work( (j+1-j1+1)+jb*n ), 1_ilp )
                    call stdlib_yscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    ...
                    do j2 = j+1, n, nb
                       ...
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_ygemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',mj, 1_ilp, jb+1,-&
                          cone, work( (j3-j1+1)+k1*n ), n,a( j3, j1-k2 ), lda,cone, a( j3, j3 ), &
                                    lda )
                          j3 = j3 + 1_ilp
                       end do
                       ! update off-diagonal block of j2-th block column with stdlib_ygemm
                       call stdlib_ygemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',n-j3+1, nj, jb+1,-&
                       cone, work( (j3-j1+1)+k1*n ), n,a( j2, j1-k2 ), lda,cone, a( j3, j2 ), lda &
                                 )
                    end do
                    ! recover t( j+1, j )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_ycopy( n-j, a( j+1, j+1 ), 1_ilp, work( 1_ilp ), 1_ilp )
              end if
              go to 11
           end if
           20 continue
     ...
     !! the diagonal and subdiagonal of A are overwritten by those of T.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              if ( j>min(m, nb) )go to 20
              ! k is the column to be factorized
               ! when being called from stdlib_yhetrf_aa,
               ! > for the first block column, j1 is 1, hence j1+j-1 is j,
               ! > for the rest of the columns, j1 is 2, and j1+j-1 is j+1,
              k = j1+j-1
              if( j==m ) then
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_ylacgv( j-k1, a( 1_ilp, j ), 1_ilp )
                 call stdlib_ygemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( 1_ilp, j ), 1_ilp,&
                           cone, h( j, j ), 1_ilp )
                 call stdlib_ylacgv( j-k1, a( 1_ilp, j ), 1_ilp )
              end if
              ! copy h(i:n, i) into work
              call stdlib_ycopy( mj, h( j, j ), 1_ilp, work( 1_ilp ), 1_ilp )
              if( j>k1 ) then
                 ! compute work := work - l(j-1, j:n) * t(j-1,j),
                  ! where a(j-1, j) stores t(j-1, j) and a(j-2, j:n) stores u(j-1, j:n)
                 alpha = -conjg( a( k-1, j ) )
                 call stdlib_yaxpy( mj, alpha, a( k-2, j ), lda, work( 1_ilp ), 1_ilp )
              end if
              ! set a(j, j) = t(j, j)
              a( k, j ) = real( work( 1_ilp ),KIND=xdp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l(j, (j+1):n)
                  ! where a(j, j) stores t(j, j) and a(j-1, (j+1):n) stores u(j, (j+1):n)
                 if( k>1_ilp ) then
                    alpha = -a( k, j )
                    call stdlib_yaxpy( m-j, alpha, a( k-1, j+1 ), lda,work( 2_ilp ), 1_ilp )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_iyamax( m-j, work( 2_ilp ), 1_ilp ) + 1_ilp
                 piv = work( i2 )
                 ! apply hermitian pivot
                 if( (i2/=2_ilp) .and. (piv/=0_ilp) ) then
                    ! swap work(i1) and work(i2)
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_yswap( i2-i1-1, a( j1+i1-1, i1+1 ), lda,a( j1+i1, i2 ), 1_ilp )
                              
                    call stdlib_ylacgv( i2-i1, a( j1+i1-1, i1+1 ), lda )
                    call stdlib_ylacgv( i2-i1-1, a( j1+i1, i2 ), 1_ilp )
                    ! swap a(i1, i2+1:n) with a(i2, i2+1:n)
                    if( i2<m )call stdlib_yswap( m-i2, a( j1+i1-1, i2+1 ), lda,a( j1+i2-1, i2+1 ),&
                               lda )
                    ! swap a(i1, i1) with a(i2,i2)
                    piv = a( i1+j1-1, i1 )
                    a( j1+i1-1, i1 ) = a( j1+i2-1, i2 )
                    a( j1+i2-1, i2 ) = piv
                    ! swap h(i1, 1:j1) with h(i2, 1:j1)
                    call stdlib_yswap( i1-1, h( i1, 1_ilp ), ldh, h( i2, 1_ilp ), ldh )
                    ipiv( i1 ) = i2
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_yswap( i1-k1+1, a( 1_ilp, i1 ), 1_ilp,a( 1_ilp, i2 ), 1_ilp )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j:n, j),
                    call stdlib_ycopy( m-j, a( k+1, j+1 ), lda,h( j+1, j+1 ), 1_ilp )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( k, j+1 )/=czero ) then
                       alpha = cone / a( k, j+1 )
                       call stdlib_ycopy( m-j-1, work( 3_ilp ), 1_ilp, a( k, j+2 ), lda )
                       call stdlib_yscal( m-j-1, alpha, a( k, j+2 ), lda )
                    else
                       call stdlib_ylaset( 'FULL', 1_ilp, m-j-1, czero, czero,a( k, j+2 ), lda)
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_ylacgv( j-k1, a( j, 1_ilp ), lda )
                 call stdlib_ygemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( j, 1_ilp ), &
                           lda,cone, h( j, j ), 1_ilp )
                 call stdlib_ylacgv( j-k1, a( j, 1_ilp ), lda )
              end if
              ! copy h(j:n, j) into work
              ...
              if( j>k1 ) then
                 ! compute work := work - l(j:n, j-1) * t(j-1,j),
                  ! where a(j-1, j) = t(j-1, j) and a(j, j-2) = l(j, j-1)
                 alpha = -conjg( a( j, k-1 ) )
                 call stdlib_yaxpy( mj, alpha, a( j, k-2 ), 1_ilp, work( 1_ilp ), 1_ilp )
              end if
              ! set a(j, j) = t(j, j)
              a( j, k ) = real( work( 1_ilp ),KIND=xdp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l((j+1):n, j)
                  ! where a(j, j) = t(j, j) and a((j+1):n, j-1) = l((j+1):n, j)
                 if( k>1_ilp ) then
                    alpha = -a( j, k )
                    call stdlib_yaxpy( m-j, alpha, a( j+1, k-1 ), 1_ilp,work( 2_ilp ), 1_ilp )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_iyamax( m-j, work( 2_ilp ), 1_ilp ) + 1_ilp
                 piv = work( i2 )
                 ...
                 if( (i2/=2_ilp) .and. (piv/=0_ilp) ) then
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_yswap( i2-i1-1, a( i1+1, j1+i1-1 ), 1_ilp,a( i2, j1+i1 ), lda )
                              
                    call stdlib_ylacgv( i2-i1, a( i1+1, j1+i1-1 ), 1_ilp )
                    call stdlib_ylacgv( i2-i1-1, a( i2, j1+i1 ), lda )
                    ! swap a(i2+1:n, i1) with a(i2+1:n, i2)
                    if( i2<m )call stdlib_yswap( m-i2, a( i2+1, j1+i1-1 ), 1_ilp,a( i2+1, j1+i2-1 ), &
                              1_ilp )
                    ! swap a(i1, i1) with a(i2, i2)
                    piv = a( i1, j1+i1-1 )
                    a( i1, j1+i1-1 ) = a( i2, j1+i2-1 )
                    ...
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_yswap( i1-k1+1, a( i1, 1_ilp ), lda,a( i2, 1_ilp ), lda )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j+1:n, j),
                    call stdlib_ycopy( m-j, a( j+1, k+1 ), 1_ilp,h( j+1, j+1 ), 1_ilp )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( j+1, k )/=czero ) then
                       alpha = cone / a( j+1, k )
                       call stdlib_ycopy( m-j-1, work( 3_ilp ), 1_ilp, a( j+2, k ), 1_ilp )
                       call stdlib_yscal( m-j-1, alpha, a( j+2, k ), 1_ilp )
                    else
                       call stdlib_ylaset( 'FULL', m-j-1, 1_ilp, czero, czero,a( j+2, k ), lda )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( n>1_ilp ) then
                 ...
                 ! compute u**h \ b -> b    [ (u**h \p**t * b) ]
                 call stdlib_ytrsm( 'L', 'U', 'C', 'U', n-1, nrhs, cone, a( 1_ilp, 2_ilp ),lda, b( 2_ilp, 1_ilp ),&
                            ldb )
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (u**h \p**t * b) ]
              call stdlib_ylacpy( 'F', 1_ilp, n, a(1_ilp, 1_ilp), lda+1, work(n), 1_ilp )
              if( n>1_ilp ) then
                  call stdlib_ylacpy( 'F', 1_ilp, n-1, a( 1_ilp, 2_ilp ), lda+1, work( 2_ilp*n ), 1_ilp)
                  call stdlib_ylacpy( 'F', 1_ilp, n-1, a( 1_ilp, 2_ilp ), lda+1, work( 1_ilp ), 1_ilp )
                  call stdlib_ylacgv( n-1, work( 1_ilp ), 1_ilp )
              end if
              call stdlib_ygtsv( n, nrhs, work(1_ilp), work(n), work(2_ilp*n), b, ldb,info )
              ! 3) backward substitution with u
              if( n>1_ilp ) then
                 ! compute u \ b -> b   [ u \ (t \ (u**h \p**t * b) ) ]
                 call stdlib_ytrsm( 'L', 'U', 'N', 'U', n-1, nrhs, cone, a( 1_ilp, 2_ilp ),lda, b(2_ilp, 1_ilp), &
                           ldb)
                 ! pivot, p * b  [ p * (u**h \ (t \ (u \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
              ...
              if( n>1_ilp ) then
                 ...
                 ! compute l \ b -> b    [ (l \p**t * b) ]
                 call stdlib_ytrsm( 'L', 'L', 'N', 'U', n-1, nrhs, cone, a( 2_ilp, 1_ilp ),lda, b(2_ilp, 1_ilp), &
                           ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (l \p**t * b) ]
              call stdlib_ylacpy( 'F', 1_ilp, n, a(1_ilp, 1_ilp), lda+1, work(n), 1_ilp)
              if( n>1_ilp ) then
                  call stdlib_ylacpy( 'F', 1_ilp, n-1, a( 2_ilp, 1_ilp ), lda+1, work( 1_ilp ), 1_ilp)
                  call stdlib_ylacpy( 'F', 1_ilp, n-1, a( 2_ilp, 1_ilp ), lda+1, work( 2_ilp*n ), 1_ilp)
                  call stdlib_ylacgv( n-1, work( 2_ilp*n ), 1_ilp )
              end if
              call stdlib_ygtsv(n, nrhs, work(1_ilp), work(n), work(2_ilp*n), b, ldb,info)
              ! 3) backward substitution with l**h
              if( n>1_ilp ) then
                 ! compute l**h \ b -> b   [ l**h \ (t \ (l \p**t * b) ) ]
                 call stdlib_ytrsm( 'L', 'L', 'C', 'U', n-1, nrhs, cone, a( 2_ilp, 1_ilp ),lda, b( 2_ilp, 1_ilp ),&
                            ldb)
                 ! pivot, p * b  [ p * (l**h \ (t \ (l \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
