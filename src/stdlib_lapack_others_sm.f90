submodule(stdlib_lapack_others) stdlib_lapack_others_sm
  ...
  contains
     ...
     real(xdp) module function stdlib_xla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! DLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from stdlib_xsytrs.
           ...
           stdlib_xla_syrpvgrw = rpvgrw
     end function stdlib_xla_syrpvgrw
     ...
     real(xdp) module function stdlib_yla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! ZLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_yla_syrpvgrw = rpvgrw
     end function stdlib_yla_syrpvgrw
     ...
     pure real(xdp) module function stdlib_xla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! DLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_xla_gerpvgrw = rpvgrw
     end function stdlib_xla_gerpvgrw
     ...
     pure real(xdp) module function stdlib_yla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! ZLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_yla_gerpvgrw = rpvgrw
     end function stdlib_yla_gerpvgrw
     ...
     real(xdp) module function stdlib_yla_gbrcond_c( trans, n, kl, ku, ab,ldab, afb, ldafb, ipiv,c, &
     !! ZLA_GBRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               capply, info, work,rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( notrans ) then
                    call stdlib_ygbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_ygbtrs( 'CONJUGATE TRANSPOSE', n, kl, ku, 1_ilp, afb,ldafb, ipiv, &
                              work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_yla_gbrcond_c = one / ainvnm
           return
     end function stdlib_yla_gbrcond_c
     ...
     real(xdp) module function stdlib_yla_gercond_c( trans, n, a, lda, af,ldaf, ipiv, c, capply,info, &
     !! ZLA_GERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               work, rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if (notrans) then
                    call stdlib_ygetrs( 'NO TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_ygetrs( 'CONJUGATE TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info &
                              )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_yla_gercond_c = one / ainvnm
           return
     end function stdlib_yla_gercond_c
     ...
     real(xdp) module function stdlib_yla_hercond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_HERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_yhetrs( 'U', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_yhetrs( 'L', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_yla_hercond_c = one / ainvnm
           return
     end function stdlib_yla_hercond_c
     ...
     module subroutine stdlib_xla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! DLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           ! set safe1 essentially to be the underflow threshold times the
           ! number of additions in each row.
           safe1 = stdlib_xlamch( 'SAFE MINIMUM' )
           safe1 = (n+1)*safe1
           ! form  y := alpha*abs(a)*abs(x) + beta*abs(y).
           ...
           return
     end subroutine stdlib_xla_syamv
     ...
     module subroutine stdlib_yla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! ZLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           return
     end subroutine stdlib_yla_syamv
     ...
     real(xdp) module function stdlib_xla_syrcond( uplo, n, a, lda, af, ldaf,ipiv, cmode, c, info, work,&
     !! DLA_SYRCOND: estimates the Skeel condition number of  op(A) * op2(C)
     !! where op2 is determined by CMODE as follows
     !! CMODE =  1    op2(C) = C
     !! CMODE =  0    op2(C) = I
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_xsytrs( 'U', n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 else
                    call stdlib_xsytrs( 'L', n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_xla_syrcond = ( one / ainvnm )
           return
     end function stdlib_xla_syrcond
     ...
     real(xdp) module function stdlib_yla_syrcond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_SYRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_ysytrs( 'U', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_ysytrs( 'L', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_yla_syrcond_c = one / ainvnm
           return
     end function stdlib_yla_syrcond_c
     ...
     real(xdp) module function stdlib_yla_porcond_c( uplo, n, a, lda, af,ldaf, c, capply, info,work, &
     !! ZLA_PORCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector
               rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_ypotrs( 'U', n, 1_ilp, af, ldaf,work, n, info )
                 else
                    call stdlib_ypotrs( 'L', n, 1_ilp, af, ldaf,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_yla_porcond_c = one / ainvnm
           return
     end function stdlib_yla_porcond_c
     ...
     real(xdp) module function stdlib_I64_xla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! DLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from stdlib_I64_xsytrs.
           ! calls to stdlib_I64_dswap would be severe overkill.
           if ( upper ) then
              k = n
              do while ( k < ncols .and. k>0 )
                 if ( ipiv( k )>0_ilp64 ) then
           ...
           stdlib_I64_xla_syrpvgrw = rpvgrw
     end function stdlib_I64_xla_syrpvgrw

     real(qp) module function stdlib_I64_qla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! DLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from stdlib_I64_qsytrs.
           ! calls to stdlib_I64_dswap would be severe overkill.
           if ( upper ) then
              k = n
              do while ( k < ncols .and. k>0 )
           ...
           stdlib_I64_qla_syrpvgrw = rpvgrw
     end function stdlib_I64_qla_syrpvgrw
     ...
     real(xdp) module function stdlib_I64_yla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! ZLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from stdlib_I64_ysytrs.
           ! calls to stdlib_I64_dswap would be severe overkill.
           if ( upper ) then
              k = n
              do while ( k < ncols .and. k>0 )
           ...
           stdlib_I64_yla_syrpvgrw = rpvgrw
     end function stdlib_I64_yla_syrpvgrw

     real(qp) module function stdlib_I64_wla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! ZLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from stdlib_I64_wsytrs.
           ! calls to stdlib_I64_dswap would be severe overkill.
           if ( upper ) then
              k = n
              do while ( k < ncols .and. k>0 )
           ...
           stdlib_I64_wla_syrpvgrw = rpvgrw
     end function stdlib_I64_wla_syrpvgrw
     ...
     pure real(xdp) module function stdlib_I64_xla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! DLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_I64_xla_gerpvgrw = rpvgrw
     end function stdlib_I64_xla_gerpvgrw

     pure real(qp) module function stdlib_I64_qla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! DLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_I64_qla_gerpvgrw = rpvgrw
     end function stdlib_I64_qla_gerpvgrw
     ...
     pure real(xdp) module function stdlib_I64_yla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! ZLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_I64_yla_gerpvgrw = rpvgrw
     end function stdlib_I64_yla_gerpvgrw

     pure real(qp) module function stdlib_I64_wla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! ZLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     !! unreliable.
        ...
        ! =====================================================================
           ...
           stdlib_I64_wla_gerpvgrw = rpvgrw
     end function stdlib_I64_wla_gerpvgrw
     ...
     real(xdp) module function stdlib_I64_yla_gbrcond_c( trans, n, kl, ku, ab,ldab, afb, ldafb, ipiv,c, &
     !! ZLA_GBRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               capply, info, work,rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( notrans ) then
                    call stdlib_I64_ygbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp64, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_I64_ygbtrs( 'CONJUGATE TRANSPOSE', n, kl, ku, 1_ilp64, afb,ldafb, ipiv, &
                              work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_yla_gbrcond_c = one / ainvnm
           return
     end function stdlib_I64_yla_gbrcond_c

     real(qp) module function stdlib_I64_wla_gbrcond_c( trans, n, kl, ku, ab,ldab, afb, ldafb, ipiv,c, &
     !! ZLA_GBRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               capply, info, work,rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( notrans ) then
                    call stdlib_I64_wgbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp64, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_I64_wgbtrs( 'CONJUGATE TRANSPOSE', n, kl, ku, 1_ilp64, afb,ldafb, ipiv, &
                              work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_wla_gbrcond_c = one / ainvnm
           return
     end function stdlib_I64_wla_gbrcond_c
     ...
     real(xdp) module function stdlib_I64_yla_gercond_c( trans, n, a, lda, af,ldaf, ipiv, c, capply,info, &
     !! ZLA_GERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               work, rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if (notrans) then
                    call stdlib_I64_ygetrs( 'NO TRANSPOSE', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_I64_ygetrs( 'CONJUGATE TRANSPOSE', n, 1_ilp64, af, ldaf, ipiv,work, n, info &
                              )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_yla_gercond_c = one / ainvnm
           return
     end function stdlib_I64_yla_gercond_c

     real(qp) module function stdlib_I64_wla_gercond_c( trans, n, a, lda, af,ldaf, ipiv, c, capply,info, &
     !! ZLA_GERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               work, rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if (notrans) then
                    call stdlib_I64_wgetrs( 'NO TRANSPOSE', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_I64_wgetrs( 'CONJUGATE TRANSPOSE', n, 1_ilp64, af, ldaf, ipiv,work, n, info &
                              )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_wla_gercond_c = one / ainvnm
           return
     end function stdlib_I64_wla_gercond_c
     ...
     real(xdp) module function stdlib_I64_yla_hercond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_HERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_yhetrs( 'U', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_I64_yhetrs( 'L', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_yla_hercond_c = one / ainvnm
           return
     end function stdlib_I64_yla_hercond_c

     real(qp) module function stdlib_I64_wla_hercond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_HERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_whetrs( 'U', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_I64_whetrs( 'L', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_wla_hercond_c = one / ainvnm
           return
     end function stdlib_I64_wla_hercond_c
     ...
     module subroutine stdlib_I64_xla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! DLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           ! set safe1 essentially to be the underflow threshold times the
           ! number of additions in each row.
           safe1 = stdlib_I64_xlamch( 'SAFE MINIMUM' )
           safe1 = (n+1)*safe1
           ! form  y := alpha*abs(a)*abs(x) + beta*abs(y).
           ! the o(n^2) symb_wero tests could be replaced by o(n) queries to
           ! the inexact flag.  still doesn't help change the iteration order
           ...
           return
     end subroutine stdlib_I64_xla_syamv

     module subroutine stdlib_I64_qla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! DLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           ! set safe1 essentially to be the underflow threshold times the
           ! number of additions in each row.
           safe1 = stdlib_I64_qlamch( 'SAFE MINIMUM' )
           safe1 = (n+1)*safe1
           ! form  y := alpha*abs(a)*abs(x) + beta*abs(y).
           ! the o(n^2) symb_wero tests could be replaced by o(n) queries to
           ! the inexact flag.  still doesn't help change the iteration order
           ...
           return
     end subroutine stdlib_I64_qla_syamv
     ...
     module subroutine stdlib_I64_yla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! ZLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           return
     end subroutine stdlib_I64_yla_syamv

     module subroutine stdlib_I64_wla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! ZLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           return
     end subroutine stdlib_I64_wla_syamv
     ...
     real(xdp) module function stdlib_I64_xla_syrcond( uplo, n, a, lda, af, ldaf,ipiv, cmode, c, info, work,&
     !! DLA_SYRCOND: estimates the Skeel condition number of  op(A) * op2(C)
     !! where op2 is determined by CMODE as follows
     !! CMODE =  1    op2(C) = C
     !! CMODE =  0    op2(C) = I
     ...
     !! infinity-norm condition number.
        ...
        ! =====================================================================
           ...
           kase = 0_ilp64
           10 continue
           call stdlib_I64_xlacn2( n, work( n+1 ), work, iwork, ainvnm, kase, isave )
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ! multiply by r.
                 do i = 1, n
                 ...
                 if ( up ) then
                    call stdlib_I64_xsytrs( 'U', n, 1_ilp64, af, ldaf, ipiv, work, n, info )
                 else
                    call stdlib_I64_xsytrs( 'L', n, 1_ilp64, af, ldaf, ipiv, work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp64 ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_xla_syrcond = ( one / ainvnm )
           return
     end function stdlib_I64_xla_syrcond

     real(qp) module function stdlib_I64_qla_syrcond( uplo, n, a, lda, af, ldaf,ipiv, cmode, c, info, work,&
     !! DLA_SYRCOND: estimates the Skeel condition number of  op(A) * op2(C)
     !! where op2 is determined by CMODE as follows
     !! CMODE =  1    op2(C) = C
     !! CMODE =  0    op2(C) = I
     ...
     !! infinity-norm condition number.
        ...
        ! =====================================================================
           ...
           kase = 0_ilp64
           10 continue
           call stdlib_I64_qlacn2( n, work( n+1 ), work, iwork, ainvnm, kase, isave )
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ! multiply by r.
                 do i = 1, n
                 ...
                 if ( up ) then
                    call stdlib_I64_qsytrs( 'U', n, 1_ilp64, af, ldaf, ipiv, work, n, info )
                 else
                    call stdlib_I64_qsytrs( 'L', n, 1_ilp64, af, ldaf, ipiv, work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp64 ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_qla_syrcond = ( one / ainvnm )
           return
     end function stdlib_I64_qla_syrcond
     ...
     real(xdp) module function stdlib_I64_yla_syrcond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_SYRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_ysytrs( 'U', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_I64_ysytrs( 'L', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_yla_syrcond_c = one / ainvnm
           return
     end function stdlib_I64_yla_syrcond_c

     real(qp) module function stdlib_I64_wla_syrcond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_SYRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_wsytrs( 'U', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_I64_wsytrs( 'L', n, 1_ilp64, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_wla_syrcond_c = one / ainvnm
           return
     end function stdlib_I64_wla_syrcond_c
     ...
     real(xdp) module function stdlib_I64_yla_porcond_c( uplo, n, a, lda, af,ldaf, c, capply, info,work, &
     !! ZLA_PORCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector
               rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_ypotrs( 'U', n, 1_ilp64, af, ldaf,work, n, info )
                 else
                    call stdlib_I64_ypotrs( 'L', n, 1_ilp64, af, ldaf,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_yla_porcond_c = one / ainvnm
           return
     end function stdlib_I64_yla_porcond_c

     real(qp) module function stdlib_I64_wla_porcond_c( uplo, n, a, lda, af,ldaf, c, capply, info,work, &
     !! ZLA_PORCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector
               rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              if( kase==2_ilp64 ) then
                 ...
                 if ( up ) then
                    call stdlib_I64_wpotrs( 'U', n, 1_ilp64, af, ldaf,work, n, info )
                 else
                    call stdlib_I64_wpotrs( 'L', n, 1_ilp64, af, ldaf,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
           ...
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm /= zero )stdlib_I64_wla_porcond_c = one / ainvnm
           return
     end function stdlib_I64_wla_porcond_c
