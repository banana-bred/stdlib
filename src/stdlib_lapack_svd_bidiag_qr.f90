  ...
  contains
     ...
     !! the dqds Algorithm (Positive Case)", LAPACK Working Note.
        ...
        ! =====================================================================
           ...
           if( n<0_ilp ) then
              ...
              call stdlib_xlas2( d( 1_ilp ), e( 1_ilp ), d( 2_ilp ), sigmn, sigmx )
              d( 1_ilp ) = sigmx
              d( 2_ilp ) = sigmn
              return
           end if
           ...
           ! early return if sigmx is zero (matrix is already diagonal).
           if( sigmx==zero ) then
              call stdlib_xlasrt( 'D', n, d, iinfo )
              return
           end if
           do i = 1, n
              sigmx = max( sigmx, d( i ) )
           ...
           scale = sqrt( eps / safmin )
           call stdlib_xcopy( n, d, 1_ilp, work( 1_ilp ), 2_ilp )
           call stdlib_xcopy( n-1, e, 1_ilp, work( 2_ilp ), 2_ilp )
           call stdlib_xlascl( 'G', 0_ilp, 0_ilp, sigmx, scale, 2_ilp*n-1, 1_ilp, work, 2_ilp*n-1,iinfo )
           ! compute the q's and e's.
           do i = 1, 2*n - 1
              work( i ) = work( i )**2_ilp
           end do
           work( 2_ilp*n ) = zero
           call stdlib_xlasq2( n, work, info )
           if( info==0_ilp ) then
              do i = 1, n
                 d( i ) = sqrt( work( i ) )
              end do
              call stdlib_xlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, d, n, iinfo )
           else if( info==2_ilp ) then
           ! maximum number of iterations exceeded.  move data from work
           ! into d and e so the calling subroutine can try to finish
              do i = 1, n
              ...
              call stdlib_xlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, d, n, iinfo )
              call stdlib_xlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, e, n, iinfo )
           end if
           return
     ...
     !! is passed to DLASQ3.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input arguments.
           ! (in case stdlib_xlasq2 is not called by stdlib_xlasq1)
           info = 0_ilp
           eps = stdlib_xlamch( 'PRECISION' )
           safmin = stdlib_xlamch( 'SAFE MINIMUM' )
           tol = eps*hundrd
           ...
           if( e==zero ) then
              ...
              call stdlib_xlasrt( 'D', n, z, iinfo )
              z( 2_ilp*n-1 ) = d
              return
           end if
           trace = d + e
           ...
           ! initialise variables to pass to stdlib_xlasq3.
           ttype = 0_ilp
           dmin1 = zero
           dmin2 = zero
           dn    = zero
           ...
           loop_160: do iwhila = 1, n + 1
              ...
              ! pp = 0 for ping, pp = 1 for pong.
              ! pp = 2 indicates that flipping was applied to the z array and
                     ! and that the tests for deflation upon entry in stdlib_xlasq3
                     ! should not be performed.
              nbig = 100_ilp*( n0-i0+1 )
              loop_140: do iwhilb = 1, nbig
                 if( i0>n0 )go to 150
                 ! while submatrix unfinished take a good dqds step.
                 call stdlib_xlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
                           ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
                 pp = 1_ilp - pp
                 ! when emin is very small check for splits.
                 if( pp==0_ilp .and. n0-i0>=3_ilp ) then
           ...
           z( 2_ilp*n+1 ) = trace
           z( 2_ilp*n+2 ) = e
           z( 2_ilp*n+3 ) = real( iter,KIND=xdp)
           z( 2_ilp*n+4 ) = real( ndiv,KIND=xdp) / real( n**2_ilp,KIND=xdp)
           z( 2_ilp*n+5 ) = hundrd*nfail / real( iter,KIND=xdp)
           return
     ...
     !! is positive.
        ...
        ! =====================================================================
           ...
           ! choose a shift.
           call stdlib_xlasq4( i0, n0, z, pp, n0in, dmin, dmin1, dmin2, dn, dn1,dn2, tau, ttype, &
                     g )
           ! call dqds until dmin > 0.
           70 continue
           call stdlib_xlasq5( i0, n0, z, pp, tau, sigma, dmin, dmin1, dmin2, dn,dn1, dn2, ieee, &
                     eps )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp
           ! check status.
           if( dmin>=zero .and. dmin1>=zero ) then
              ...
              go to 70
           else if( stdlib_xisnan( dmin ) ) then
              ! nan.
              if( tau==zero ) then
                 go to 80
              else
           ...
           ! risk of underflow.
           80 continue
           call stdlib_xlasq6( i0, n0, z, pp, dmin, dmin1, dmin2, dn, dn1, dn2 )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp
           tau = zero
           90 continue
     ...
     pure module subroutine stdlib_I64_xlasq1( n, d, e, work, info )
     !! DLASQ1: computes the singular values of a real N-by-N bidiagonal
     !! matrix with diagonal D and off-diagonal E. The singular values
     !! are computed to high relative accuracy, in the absence of
     !! denormalization, underflow and overflow. The algorithm was first
     ...
     !! the dqds Algorithm (Positive Case)", LAPACK Working Note.
        ...
        ! =====================================================================
           ...
           if( n<0_ilp64 ) then
              ...
              call stdlib_I64_xlas2( d( 1_ilp64 ), e( 1_ilp64 ), d( 2_ilp64 ), sigmn, sigmx )
              d( 1_ilp64 ) = sigmx
              d( 2_ilp64 ) = sigmn
              return
           end if
           ...
           ! early return if sigmx is zero (matrix is already diagonal).
           if( sigmx==zero ) then
              call stdlib_I64_xlasrt( 'D', n, d, iinfo )
              return
           end if
           do i = 1, n
              sigmx = max( sigmx, d( i ) )
           ...
           scale = sqrt( eps / safmin )
           call stdlib_I64_xcopy( n, d, 1_ilp64, work( 1_ilp64 ), 2_ilp64 )
           call stdlib_I64_xcopy( n-1, e, 1_ilp64, work( 2_ilp64 ), 2_ilp64 )
           call stdlib_I64_xlascl( 'G', 0_ilp64, 0_ilp64, sigmx, scale, 2_ilp64*n-1, 1_ilp64, work, 2_ilp64*n-1,iinfo )
           ! compute the q's and e's.
           do i = 1, 2*n - 1
              work( i ) = work( i )**2_ilp64
           end do
           work( 2_ilp64*n ) = zero
           call stdlib_I64_xlasq2( n, work, info )
           if( info==0_ilp64 ) then
              do i = 1, n
                 d( i ) = sqrt( work( i ) )
              end do
              call stdlib_I64_xlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, d, n, iinfo )
           else if( info==2_ilp64 ) then
           ! maximum number of iterations exceeded.  move data from work
           ! into d and e so the calling subroutine can try to finish
              do i = 1, n
              ...
              call stdlib_I64_xlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, d, n, iinfo )
              call stdlib_I64_xlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, e, n, iinfo )
           end if
           return
     end subroutine stdlib_I64_xlasq1

     pure module subroutine stdlib_I64_qlasq1( n, d, e, work, info )
     !! DLASQ1: computes the singular values of a real N-by-N bidiagonal
     !! matrix with diagonal D and off-diagonal E. The singular values
     !! are computed to high relative accuracy, in the absence of
     !! denormalization, underflow and overflow. The algorithm was first
     ...
     !! the dqds Algorithm (Positive Case)", LAPACK Working Note.
        ...
        ! =====================================================================
           ...
           if( n<0_ilp64 ) then
              ...
              call stdlib_I64_qlas2( d( 1_ilp64 ), e( 1_ilp64 ), d( 2_ilp64 ), sigmn, sigmx )
              d( 1_ilp64 ) = sigmx
              d( 2_ilp64 ) = sigmn
              return
           end if
           ...
           ! early return if sigmx is zero (matrix is already diagonal).
           if( sigmx==zero ) then
              call stdlib_I64_qlasrt( 'D', n, d, iinfo )
              return
           end if
           do i = 1, n
              sigmx = max( sigmx, d( i ) )
           ...
           scale = sqrt( eps / safmin )
           call stdlib_I64_qcopy( n, d, 1_ilp64, work( 1_ilp64 ), 2_ilp64 )
           call stdlib_I64_qcopy( n-1, e, 1_ilp64, work( 2_ilp64 ), 2_ilp64 )
           call stdlib_I64_qlascl( 'G', 0_ilp64, 0_ilp64, sigmx, scale, 2_ilp64*n-1, 1_ilp64, work, 2_ilp64*n-1,iinfo )
           ! compute the q's and e's.
           do i = 1, 2*n - 1
              work( i ) = work( i )**2_ilp64
           end do
           work( 2_ilp64*n ) = zero
           call stdlib_I64_qlasq2( n, work, info )
           if( info==0_ilp64 ) then
              do i = 1, n
                 d( i ) = sqrt( work( i ) )
              end do
              call stdlib_I64_qlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, d, n, iinfo )
           else if( info==2_ilp64 ) then
           ! maximum number of iterations exceeded.  move data from work
           ! into d and e so the calling subroutine can try to finish
              do i = 1, n
              ...
              call stdlib_I64_qlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, d, n, iinfo )
              call stdlib_I64_qlascl( 'G', 0_ilp64, 0_ilp64, scale, sigmx, n, 1_ilp64, e, n, iinfo )
           end if
           return
     end subroutine stdlib_I64_qlasq1
     ...
     pure module subroutine stdlib_I64_xlasq2( n, z, info )
     !! DLASQ2: computes all the eigenvalues of the symmetric positive
     !! definite tridiagonal matrix associated with the qd array Z to high
     !! relative accuracy are computed to high relative accuracy, in the
     !! absence of denormalization, underflow and overflow.
     ...
     !! is passed to DLASQ3.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input arguments.
           ! (in case stdlib_I64_xlasq2 is not called by stdlib_I64_xlasq1)
           info = 0_ilp64
           eps = stdlib_I64_xlamch( 'PRECISION' )
           safmin = stdlib_I64_xlamch( 'SAFE MINIMUM' )
           tol = eps*hundrd
           ...
           if( e==zero ) then
              ...
              call stdlib_I64_xlasrt( 'D', n, z, iinfo )
              z( 2_ilp64*n-1 ) = d
              return
           end if
           trace = d + e
           ...
           ! initialise variables to pass to stdlib_I64_xlasq3.
           ttype = 0_ilp64
           dmin1 = zero
           dmin2 = zero
           dn    = zero
           ...
           loop_160: do iwhila = 1, n + 1
              ...
              ! pp = 0 for ping, pp = 1 for pong.
              ! pp = 2 indicates that flipping was applied to the z array and
                     ! and that the tests for deflation upon entry in stdlib_I64_xlasq3
                     ! should not be performed.
              nbig = 100_ilp64*( n0-i0+1 )
              loop_140: do iwhilb = 1, nbig
                 if( i0>n0 )go to 150
                 ! while submatrix unfinished take a good dqds step.
                 call stdlib_I64_xlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
                           ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
                 pp = 1_ilp64 - pp
                 ! when emin is very small check for splits.
                 if( pp==0_ilp64 .and. n0-i0>=3_ilp64 ) then
           ...
           z( 2_ilp64*n+1 ) = trace
           z( 2_ilp64*n+2 ) = e
           z( 2_ilp64*n+3 ) = real( iter,KIND=xdp)
           z( 2_ilp64*n+4 ) = real( ndiv,KIND=xdp) / real( n**2_ilp64,KIND=xdp)
           z( 2_ilp64*n+5 ) = hundrd*nfail / real( iter,KIND=xdp)
           return
     end subroutine stdlib_I64_xlasq2

     pure module subroutine stdlib_I64_qlasq2( n, z, info )
     !! DLASQ2: computes all the eigenvalues of the symmetric positive
     !! definite tridiagonal matrix associated with the qd array Z to high
     !! relative accuracy are computed to high relative accuracy, in the
     !! absence of denormalization, underflow and overflow.
     ...
     !! is passed to DLASQ3.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input arguments.
           ! (in case stdlib_I64_qlasq2 is not called by stdlib_I64_qlasq1)
           info = 0_ilp64
           eps = stdlib_I64_qlamch( 'PRECISION' )
           safmin = stdlib_I64_qlamch( 'SAFE MINIMUM' )
           tol = eps*hundrd
           ...
           if( e==zero ) then
              ...
              call stdlib_I64_qlasrt( 'D', n, z, iinfo )
              z( 2_ilp64*n-1 ) = d
              return
           end if
           trace = d + e
           ...
           ! initialise variables to pass to stdlib_I64_qlasq3.
           ttype = 0_ilp64
           dmin1 = zero
           dmin2 = zero
           dn    = zero
           ...
           loop_160: do iwhila = 1, n + 1
              ...
              ! pp = 0 for ping, pp = 1 for pong.
              ! pp = 2 indicates that flipping was applied to the z array and
                     ! and that the tests for deflation upon entry in stdlib_I64_qlasq3
                     ! should not be performed.
              nbig = 100_ilp64*( n0-i0+1 )
              loop_140: do iwhilb = 1, nbig
                 if( i0>n0 )go to 150
                 ! while submatrix unfinished take a good dqds step.
                 call stdlib_I64_qlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
                           ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
                 pp = 1_ilp64 - pp
                 ! when emin is very small check for splits.
                 if( pp==0_ilp64 .and. n0-i0>=3_ilp64 ) then
           ...
           z( 2_ilp64*n+1 ) = trace
           z( 2_ilp64*n+2 ) = e
           z( 2_ilp64*n+3 ) = real( iter,KIND=qp)
           z( 2_ilp64*n+4 ) = real( ndiv,KIND=qp) / real( n**2_ilp64,KIND=qp)
           z( 2_ilp64*n+5 ) = hundrd*nfail / real( iter,KIND=qp)
           return
     end subroutine stdlib_I64_qlasq2
     ...
     pure module subroutine stdlib_I64_xlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
     !! DLASQ3: checks for deflation, computes a shift (TAU) and calls dqds.
     !! In case of failure it changes shifts, and tries again until output
     !! is positive.
               ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
        ...
        ! =====================================================================
           ...
           ! choose a shift.
           call stdlib_I64_xlasq4( i0, n0, z, pp, n0in, dmin, dmin1, dmin2, dn, dn1,dn2, tau, ttype, &
                     g )
           ! call dqds until dmin > 0.
           70 continue
           call stdlib_I64_xlasq5( i0, n0, z, pp, tau, sigma, dmin, dmin1, dmin2, dn,dn1, dn2, ieee, &
                     eps )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp64
           ! check status.
           if( dmin>=zero .and. dmin1>=zero ) then
              ...
              go to 70
           else if( stdlib_I64_xisnan( dmin ) ) then
              ! nan.
              if( tau==zero ) then
                 go to 80
              else
           ...
           ! risk of underflow.
           80 continue
           call stdlib_I64_xlasq6( i0, n0, z, pp, dmin, dmin1, dmin2, dn, dn1, dn2 )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp64
           tau = zero
           90 continue
           ...
           sigma = t
           return
     end subroutine stdlib_I64_xlasq3

     pure module subroutine stdlib_I64_qlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
     !! DLASQ3: checks for deflation, computes a shift (TAU) and calls dqds.
     !! In case of failure it changes shifts, and tries again until output
     !! is positive.
               ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
        ...
        ! =====================================================================
           ...
           ! choose a shift.
           call stdlib_I64_qlasq4( i0, n0, z, pp, n0in, dmin, dmin1, dmin2, dn, dn1,dn2, tau, ttype, &
                     g )
           ! call dqds until dmin > 0.
           70 continue
           call stdlib_I64_qlasq5( i0, n0, z, pp, tau, sigma, dmin, dmin1, dmin2, dn,dn1, dn2, ieee, &
                     eps )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp64
           ! check status.
           if( dmin>=zero .and. dmin1>=zero ) then
              ...
              go to 70
           else if( stdlib_I64_qisnan( dmin ) ) then
              ! nan.
              if( tau==zero ) then
                 go to 80
              else
           ...
           ! risk of underflow.
           80 continue
           call stdlib_I64_qlasq6( i0, n0, z, pp, dmin, dmin1, dmin2, dn, dn1, dn2 )
           ndiv = ndiv + ( n0-i0+2 )
           iter = iter + 1_ilp64
           tau = zero
           90 continue
           ...
           sigma = t
           return
     end subroutine stdlib_I64_qlasq3
     ...
     pure module subroutine stdlib_I64_xlasq4( i0, n0, z, pp, n0in, dmin, dmin1, dmin2, dn,dn1, dn2, tau, &
     !! DLASQ4: computes an approximation TAU to the smallest eigenvalue
     !! using values of d from the previous transform.
               ttype, g )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           tau = s
           return
     end subroutine stdlib_I64_xlasq4

     pure module subroutine stdlib_I64_qlasq4( i0, n0, z, pp, n0in, dmin, dmin1, dmin2, dn,dn1, dn2, tau, &
     !! DLASQ4: computes an approximation TAU to the smallest eigenvalue
     !! using values of d from the previous transform.
               ttype, g )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           tau = s
           return
     end subroutine stdlib_I64_qlasq4
     ...
     pure module subroutine stdlib_I64_xlasq5( i0, n0, z, pp, tau, sigma, dmin, dmin1, dmin2,dn, dnm1, dnm2, &
     !! DLASQ5: computes one dqds transform in ping-pong form, one
     !! version for IEEE machines another for non IEEE machines.
               ieee, eps )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           z( 4_ilp64*n0-pp ) = emin
           return
     end subroutine stdlib_I64_xlasq5

     pure module subroutine stdlib_I64_qlasq5( i0, n0, z, pp, tau, sigma, dmin, dmin1, dmin2,dn, dnm1, dnm2, &
     !! DLASQ5: computes one dqds transform in ping-pong form, one
     !! version for IEEE machines another for non IEEE machines.
               ieee, eps )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           z( 4_ilp64*n0-pp ) = emin
           return
     end subroutine stdlib_I64_qlasq5
     ...
     pure module subroutine stdlib_I64_xlasq6( i0, n0, z, pp, dmin, dmin1, dmin2, dn,dnm1, dnm2 )
     !! DLASQ6: computes one dqd (shift equal to zero) transform in
     !! ping-pong form, with protection against underflow and overflow.
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           z( 4_ilp64*n0-pp ) = emin
           return
     end subroutine stdlib_I64_xlasq6

     pure module subroutine stdlib_I64_qlasq6( i0, n0, z, pp, dmin, dmin1, dmin2, dn,dnm1, dnm2 )
     !! DLASQ6: computes one dqd (shift equal to zero) transform in
     !! ping-pong form, with protection against underflow and overflow.
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           z( 4_ilp64*n0-pp ) = emin
           return
     end subroutine stdlib_I64_qlasq6
