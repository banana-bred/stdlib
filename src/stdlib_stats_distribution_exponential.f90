...
contains
    ...
    impure function rvs_exp_0_rxdp( ) result(res)
    !
    ! Standard exponential random variate (lambda=1; scale=1/lambda=1)
    !
        real(xdp) :: res, x
        real(xdp), parameter :: r = 7.69711747013104972_xdp
        integer :: jz, iz
        ...
        if( abs( jz ) < ke(iz) ) then
            ...
            L1: do
                if( iz == 0 ) then
                    res = r - log( uni(1.0_xdp) )
                    exit L1
                end if
                x = abs( jz ) * we(iz)
                if(fe(iz) + uni(1.0_xdp) * (fe(iz-1) - fe(iz)) < exp(-x)) then
                    res = x
                    exit L1
                end if
                jz = dist_rand(1_int32)
    ...
    impure elemental function rvs_exp_rxdp(loc, scale) result(res)
    !
    ! Exponential distributed random variate
    !
        real(xdp), intent(in) :: loc, scale
        real(xdp) :: res

        if (scale <= 0._xdp) then
            res = ieee_value(1._xdp, ieee_quiet_nan)
        else
            res = rvs_exp_0_rxdp(  )
            res = res * scale + loc
        end if
    end function rvs_exp_rxdp
    ...
    impure elemental function rvs_exp_cxdp(loc, scale) result(res)
        complex(xdp), intent(in) :: loc, scale
        complex(xdp) :: res
        real(xdp) :: tr, ti

        tr = rvs_exp_rxdp(loc%re, scale%re)
        ti = rvs_exp_rxdp(loc%im, scale%im)
        res = cmplx(tr, ti, kind=xdp)
    end function rvs_exp_cxdp
    ...
    impure function rvs_exp_array_rxdp(loc, scale, array_size) result(res)
        real(xdp), intent(in) :: loc, scale
        integer, intent(in) :: array_size
        real(xdp) :: res(array_size), x, re
        real(xdp), parameter :: r = 7.69711747013104972_xdp
        integer :: jz, iz, i
    ...
    end function rvs_exp_array_rxdp
    ...
    impure function rvs_exp_array_cxdp(loc, scale, array_size) result(res)
        complex(xdp), intent(in) :: loc, scale
        integer, intent(in) :: array_size
        complex(xdp) :: res(array_size)
        integer :: i
        real(xdp) :: tr, ti
        ...
        do i = 1, array_size
            tr = rvs_exp_rxdp(loc%re, scale%re)
            ti = rvs_exp_rxdp(loc%im, scale%im)
            res(i) = cmplx(tr, ti, kind=xdp)
        end do
    end function rvs_exp_array_cxdp
    ...
    elemental function pdf_exp_rxdp(x, loc, scale) result(res)
    !
    ! Exponential Distribution Probability Density Function
    !
        real(xdp), intent(in) :: x, loc, scale
        real(xdp) :: res
    ...
    elemental function pdf_exp_cxdp(x, loc, scale) result(res)
        complex(xdp), intent(in) :: x, loc, scale
        real(xdp) :: res

        res = pdf_exp_rxdp(x%re, loc%re, scale%re)
        res = res * pdf_exp_rxdp(x%im, loc%im, scale%im)
    end function pdf_exp_cxdp
    ...
    elemental function cdf_exp_rxdp(x, loc, scale) result(res)
    !
    ! Exponential Distribution Cumulative Distribution Function
    !
        real(xdp), intent(in) :: x, loc, scale
        ...
        if (scale <= 0._xdp) then
            ...
            res = 1.0_xdp - exp(- (x - loc) / scale)
        end if
    end function cdf_exp_rxdp
    ...
    elemental function cdf_exp_cxdp(x, loc, scale) result(res)
        complex(xdp), intent(in) :: x, loc, scale
        real(xdp) :: res

        res = cdf_exp_rxdp(x%re, loc%re, scale%re)
        res = res * cdf_exp_rxdp(x%im, loc%im, scale%im)
    end function cdf_exp_cxdp
    ...
    impure elemental function rvs_exp_lambda_rxdp(lambda) result(res)
    !
    ! Exponential distributed random variate
    !
        real(xdp), intent(in) :: lambda
        real(xdp) :: res

        res = rvs_exp_rxdp(0._xdp, 1.0_xdp/lambda)
    end function rvs_exp_lambda_rxdp
    ...
    impure elemental function rvs_exp_lambda_cxdp(lambda) result(res)
        complex(xdp), intent(in) :: lambda
        complex(xdp) :: res
        real(xdp) :: tr, ti

        tr = rvs_exp_rxdp(0._xdp, 1.0_xdp/lambda%re)
        ti = rvs_exp_rxdp(0._xdp, 1.0_xdp/lambda%im)
        res = cmplx(tr, ti, kind=xdp)
    end function rvs_exp_lambda_cxdp
    ...
    impure function rvs_exp_array_lambda_rxdp(lambda, array_size) result(res)
        real(xdp), intent(in) :: lambda
        integer, intent(in) :: array_size
        real(xdp) :: res(array_size)

        res = rvs_exp_array_rxdp(0._xdp, 1.0_xdp/lambda, array_size)
    end function rvs_exp_array_lambda_rxdp
    ...
    impure function rvs_exp_array_lambda_cxdp(lambda, array_size) result(res)
        complex(xdp), intent(in) :: lambda
        integer, intent(in) :: array_size
        complex(xdp) :: res(array_size)
        integer :: i
    ...
    end function rvs_exp_array_lambda_cxdp
    ...
    elemental function pdf_exp_lambda_rxdp(x, lambda) result(res)
    !
    ! Exponential Distribution Probability Density Function
    !
        real(xdp), intent(in) :: x, lambda
        real(xdp) :: res

        res = pdf_exp_rxdp(x, 0._xdp, 1.0_xdp/lambda)
    end function pdf_exp_lambda_rxdp
    ...
    elemental function pdf_exp_lambda_cxdp(x, lambda) result(res)
        complex(xdp), intent(in) :: x, lambda
        real(xdp) :: res

        res = pdf_exp_rxdp(x%re, 0._xdp, 1.0_xdp/lambda%re)
        res = res * pdf_exp_rxdp(x%im, 0._xdp, 1.0_xdp/lambda%im)
    end function pdf_exp_lambda_cxdp
    ...
    elemental function cdf_exp_lambda_rxdp(x, lambda) result(res)
    !
    ! Exponential Distribution Cumulative Distribution Function
    !
        real(xdp), intent(in) :: x, lambda
        ...
        res = cdf_exp_rxdp(x, 0._xdp, 1.0_xdp/lambda)
    end function cdf_exp_lambda_rxdp
    ...
    elemental function cdf_exp_lambda_cxdp(x, lambda) result(res)
        complex(xdp), intent(in) :: x, lambda
        real(xdp) :: res

        res = cdf_exp_rxdp(x%re, 0._xdp, 1.0_xdp/lambda%re)
        res = res * cdf_exp_rxdp(x%im, 0._xdp, 1.0_xdp/lambda%im)
    end function cdf_exp_lambda_cxdp
