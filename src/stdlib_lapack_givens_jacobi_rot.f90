submodule(stdlib_lapack_base) stdlib_lapack_givens_jacobi_rot
  ...
  contains
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! february 2021
           use stdlib_blas_constants_xdp, only: zero, half, one, czero, safmax, safmin, rtmin, rtmax
        ! Scalar Arguments 
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        real(xdp) :: abssq
        ! Statement Function Definitions 
        abssq( t ) = real( t,KIND=xdp)**2_ilp + aimag( t )**2_ilp
        ! Executable Statements 
        if( g == czero ) then
           c = one
           s = czero
     ...
     !! If x(i)=0, then c(i)=0 and s(i) is chosen so that r(i) is real.
        ...
        ! =====================================================================
           ...
           abssq( ff ) = real( ff,KIND=xdp)**2_ilp + aimag( ff )**2_ilp
           ! Executable Statements 
           ! if( first ) then
              ! first = .false.
              safmin = stdlib_xlamch( 'S' )
           ...
           loop_60: do i = 1, n
              f = x( ix )
              g = y( iy )
              ! use identical algorithm as in stdlib_ylartg
              scale = max( abs1( f ), abs1( g ) )
              fs = f
              gs = g
              count = 0_ilp
              ...
              if( f2<=max( g2, one )*safmin ) then
                 ...
                 if( f==czero ) then
                    cs = zero
                    r = stdlib_xlapy2( real( g,KIND=xdp), aimag( g ) )
                    ! do complex/real division explicitly with two real
                    ! divisions
                    d = stdlib_xlapy2( real( gs,KIND=xdp), aimag( gs ) )
                    sn = cmplx( real( gs,KIND=xdp) / d, -aimag( gs ) / d,KIND=xdp)
                    go to 50
                 end if
                 f2s = stdlib_xlapy2( real( fs,KIND=xdp), aimag( fs ) )
                 ! g2 and g2s are accurate
                 ! g2 is at least safmin, and g2s is at least safmn2
                 g2s = sqrt( g2 )
                 ! error in cs from underflow in f2s is at most
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        real(xdp) :: abssq
        ! Statement Function Definitions 
        abssq( t ) = real( t,KIND=xdp)**2_ilp64 + aimag( t )**2_ilp64
        ! Executable Statements 
        if( g == czero ) then
           c = one
           s = czero
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        real(qp) :: abssq
        ! Statement Function Definitions 
        abssq( t ) = real( t,KIND=qp)**2_ilp64 + aimag( t )**2_ilp64
        ! Executable Statements 
        if( g == czero ) then
           c = one
           s = czero
     ...
     !! The sign is chosen so that R >= 0.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! if( first ) then
              safmin = stdlib_I64_xlamch( 'S' )
              eps = stdlib_I64_xlamch( 'E' )
              safmn2 = stdlib_I64_xlamch( 'B' )**int( log( safmin / eps ) /log( stdlib_I64_xlamch( 'B' ) )&
                         / two,KIND=ilp64)
              safmx2 = one / safmn2
              ! first = .false.
           ! end if
     ...
     !! The sign is chosen so that R >= 0.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! if( first ) then
              safmin = stdlib_I64_qlamch( 'S' )
              eps = stdlib_I64_qlamch( 'E' )
              safmn2 = stdlib_I64_qlamch( 'B' )**int( log( safmin / eps ) /log( stdlib_I64_qlamch( 'B' ) )&
                         / two,KIND=ilp64)
              safmx2 = one / safmn2
              ! first = .false.
           ! end if
     ...
     !! If x(i)=0, then c(i)=0 and s(i) is chosen so that r(i) is real.
        ...
        ! =====================================================================
           ...
           ! Statement Function Definitions 
           abs1( ff ) = max( abs( real( ff,KIND=xdp) ), abs( aimag( ff ) ) )
           abssq( ff ) = real( ff,KIND=xdp)**2_ilp64 + aimag( ff )**2_ilp64
           ! Executable Statements 
           ! if( first ) then
              ! first = .false.
              safmin = stdlib_I64_xlamch( 'S' )
           ...
           loop_60: do i = 1, n
              f = x( ix )
              g = y( iy )
              ! use identical algorithm as in stdlib_I64_ylartg
              scale = max( abs1( f ), abs1( g ) )
              fs = f
              gs = g
              count = 0_ilp64
              ...
              if( f2<=max( g2, one )*safmin ) then
                 ...
                 if( f==czero ) then
                    cs = zero
                    r = stdlib_I64_xlapy2( real( g,KIND=xdp), aimag( g ) )
                    ! do complex/real division explicitly with two real
                    ! divisions
                    d = stdlib_I64_xlapy2( real( gs,KIND=xdp), aimag( gs ) )
                    sn = cmplx( real( gs,KIND=xdp) / d, -aimag( gs ) / d,KIND=xdp)
                    go to 50
                 end if
                 f2s = stdlib_I64_xlapy2( real( fs,KIND=xdp), aimag( fs ) )
                 ! g2 and g2s are accurate
                 ! g2 is at least safmin, and g2s is at least safmn2
                 g2s = sqrt( g2 )
                 ! error in cs from underflow in f2s is at most
                 ...
                 ! do complex/real division explicitly with 2 real divisions
                 if( abs1( f )>one ) then
                    d = stdlib_I64_xlapy2( real( f,KIND=xdp), aimag( f ) )
                    ff = cmplx( real( f,KIND=xdp) / d, aimag( f ) / d,KIND=xdp)
                 else
                    dr = safmx2*real( f,KIND=xdp)
                    di = safmx2*aimag( f )
                    d = stdlib_I64_xlapy2( dr, di )
                    ff = cmplx( dr / d, di / d,KIND=xdp)
                 end if
                 sn = ff*cmplx( real( gs,KIND=xdp) / g2s, -aimag( gs ) / g2s,KIND=xdp)
                 r = cs*f + sn*g
     ...
     !! If x(i)=0, then c(i)=0 and s(i) is chosen so that r(i) is real.
        ...
        ! =====================================================================
           ...
           ! Statement Function Definitions 
           abs1( ff ) = max( abs( real( ff,KIND=qp) ), abs( aimag( ff ) ) )
           abssq( ff ) = real( ff,KIND=qp)**2_ilp64 + aimag( ff )**2_ilp64
           ! Executable Statements 
           ! if( first ) then
              ! first = .false.
              safmin = stdlib_I64_qlamch( 'S' )
           ...
           loop_60: do i = 1, n
              f = x( ix )
              g = y( iy )
              ! use identical algorithm as in stdlib_I64_wlartg
              scale = max( abs1( f ), abs1( g ) )
              fs = f
              gs = g
              count = 0_ilp64
              ...
              if( f2<=max( g2, one )*safmin ) then
                 ...
                 if( f==czero ) then
                    cs = zero
                    r = stdlib_I64_qlapy2( real( g,KIND=qp), aimag( g ) )
                    ! do complex/real division explicitly with two real
                    ! divisions
                    d = stdlib_I64_qlapy2( real( gs,KIND=qp), aimag( gs ) )
                    sn = cmplx( real( gs,KIND=qp) / d, -aimag( gs ) / d,KIND=qp)
                    go to 50
                 end if
                 f2s = stdlib_I64_qlapy2( real( fs,KIND=qp), aimag( fs ) )
                 ! g2 and g2s are accurate
                 ! g2 is at least safmin, and g2s is at least safmn2
                 g2s = sqrt( g2 )
                 ! error in cs from underflow in f2s is at most
                 ...
                 ! do complex/real division explicitly with 2 real divisions
                 if( abs1( f )>one ) then
                    d = stdlib_I64_qlapy2( real( f,KIND=qp), aimag( f ) )
                    ff = cmplx( real( f,KIND=qp) / d, aimag( f ) / d,KIND=qp)
                 else
                    dr = safmx2*real( f,KIND=qp)
                    di = safmx2*aimag( f )
                    d = stdlib_I64_qlapy2( dr, di )
                    ff = cmplx( dr / d, di / d,KIND=qp)
                 end if
                 sn = ff*cmplx( real( gs,KIND=qp) / g2s, -aimag( gs ) / g2s,KIND=qp)
                 r = cs*f + sn*g
