  ...
  contains
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_xpotrf( uplo, n, a, lda, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xpotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_ypotrf( uplo, n, a, lda, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ypotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_xpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_xlaqsy( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_xpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_xlansy( '1', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xpocon( uplo, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_xporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_ypoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_ylaqhe( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_ypotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylanhe( '1', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_ypocon( uplo, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ypotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_yporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_xpptrf( uplo, n, ap, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xpptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_ypptrf( uplo, n, ap, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ypptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_xppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_xlaqsp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_xpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_xppcon( uplo, n, afp, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_xpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, iwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_yppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_ylaqhp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_ypptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_yppcon( uplo, n, afp, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ypptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_ypprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, rwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_xpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xpbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_ypbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ypbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_xpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_xlaqsb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp )
                    call stdlib_xcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp,afb( kd+1-j+j1, j ), 1_ilp )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_xcopy( j2-j+1, ab( 1_ilp, j ), 1_ilp, afb( 1_ilp, j ), 1_ilp )
                 end do
              end if
              call stdlib_xpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_xlansb( '1', uplo, n, kd, ab, ldab, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, iwork,info )
           ! compute the solution matrix x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_xpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_ypbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_ylaqhb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp )
                    call stdlib_ycopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp,afb( kd+1-j+j1, j ), 1_ilp )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_ycopy( j2-j+1, ab( 1_ilp, j ), 1_ilp, afb( 1_ilp, j ), 1_ilp )
                 end do
              end if
              call stdlib_ypbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylanhb( '1', uplo, n, kd, ab, ldab, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_ypbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, rwork,info )
           ! compute the solution matrix x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ypbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_ypbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**t (or u**t*d*u) factorization of a.
           call stdlib_xpttrf( n, d, e, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_xpttrs( n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**h (or u**h*d*u) factorization of a.
           call stdlib_ypttrf( n, d, e, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_ypttrs( 'LOWER', n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**t (or u**t*d*u) factorization of a.
              call stdlib_xcopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp )call stdlib_xcopy( n-1, e, 1_ilp, ef, 1_ilp )
              call stdlib_xpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_xlanst( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_xptcon( n, df, ef, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_xpttrs( n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_xptrfs( n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr, berr,work, info )
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**h (or u**h*d*u) factorization of a.
              call stdlib_xcopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp )call stdlib_ycopy( n-1, e, 1_ilp, ef, 1_ilp )
              call stdlib_ypttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_ylanht( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_yptcon( n, df, ef, anorm, rcond, rwork, info )
           ! compute the solution vectors x.
           call stdlib_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_ypttrs( 'LOWER', n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_yptrfs( 'LOWER', n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_xlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_xpotrf( uplo, n, a, lda, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xpotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_qpotrf( uplo, n, a, lda, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qpotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_ypotrf( uplo, n, a, lda, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ypotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_wpotrf( uplo, n, a, lda, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wpotrs( uplo, n, nrhs, a, lda, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact .or. equil ) then
              ...
              rcequ = stdlib_lsame( equed, 'Y' )
              smlnum = stdlib_I64_xlamch( 'SAFE MINIMUM' )
              bignum = one / smlnum
           end if
           ! test the input parameters.
           if( .not.nofact .and. .not.equil .and. .not.stdlib_lsame( fact, 'F' ) )then
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_xpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_xlaqsy( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_xpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_xlansy( '1', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xpocon( uplo, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_xporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact .or. equil ) then
              ...
              rcequ = stdlib_lsame( equed, 'Y' )
              smlnum = stdlib_I64_qlamch( 'SAFE MINIMUM' )
              bignum = one / smlnum
           end if
           ! test the input parameters.
           if( .not.nofact .and. .not.equil .and. .not.stdlib_lsame( fact, 'F' ) )then
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_qpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_qlaqsy( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_qpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_qlansy( '1', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qpocon( uplo, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_qporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_ypoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_ylaqhe( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_ypotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylanhe( '1', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ypocon( uplo, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ypotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_yporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_wpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_wlaqhe( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_wpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlanhe( '1', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wpocon( uplo, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_wporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_xpptrf( uplo, n, ap, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xpptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_qpptrf( uplo, n, ap, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qpptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_ypptrf( uplo, n, ap, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ypptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_wpptrf( uplo, n, ap, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wpptrs( uplo, n, nrhs, ap, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_xppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_xlaqsp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_xpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xppcon( uplo, n, afp, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_xpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, iwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_qppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_qlaqsp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_qpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qppcon( uplo, n, afp, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_qpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, iwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_yppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_ylaqhp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_ypptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_yppcon( uplo, n, afp, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ypptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_ypprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, rwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_wppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_wlaqhp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_I64_wpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wppcon( uplo, n, afp, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_wpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, rwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_xpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xpbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_I64_qpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qpbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_ypbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ypbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_I64_wpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wpbtrs( uplo, n, kd, nrhs, ab, ldab, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_xpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_xlaqsb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp64 )
                    call stdlib_I64_xcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp64,afb( kd+1-j+j1, j ), 1_ilp64 )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_I64_xcopy( j2-j+1, ab( 1_ilp64, j ), 1_ilp64, afb( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              call stdlib_I64_xpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_xlansb( '1', uplo, n, kd, ab, ldab, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, iwork,info )
           ! compute the solution matrix x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_xpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_qpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_qlaqsb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp64 )
                    call stdlib_I64_qcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp64,afb( kd+1-j+j1, j ), 1_ilp64 )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_I64_qcopy( j2-j+1, ab( 1_ilp64, j ), 1_ilp64, afb( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              call stdlib_I64_qpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_qlansb( '1', uplo, n, kd, ab, ldab, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, iwork,info )
           ! compute the solution matrix x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_qpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_ypbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_ylaqhb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp64 )
                    call stdlib_I64_ycopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp64,afb( kd+1-j+j1, j ), 1_ilp64 )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_I64_ycopy( j2-j+1, ab( 1_ilp64, j ), 1_ilp64, afb( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              call stdlib_I64_ypbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylanhb( '1', uplo, n, kd, ab, ldab, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ypbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, rwork,info )
           ! compute the solution matrix x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ypbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_ypbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_I64_wpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp64 ) then
                 ! equilibrate the matrix.
                 call stdlib_I64_wlaqhb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp64 )
                    call stdlib_I64_wcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp64,afb( kd+1-j+j1, j ), 1_ilp64 )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_I64_wcopy( j2-j+1, ab( 1_ilp64, j ), 1_ilp64, afb( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              call stdlib_I64_wpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlanhb( '1', uplo, n, kd, ab, ldab, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, rwork,info )
           ! compute the solution matrix x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_I64_wpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**t (or u**t*d*u) factorization of a.
           call stdlib_I64_xpttrf( n, d, e, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_xpttrs( n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**t (or u**t*d*u) factorization of a.
           call stdlib_I64_qpttrf( n, d, e, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_qpttrs( n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**h (or u**h*d*u) factorization of a.
           call stdlib_I64_ypttrf( n, d, e, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ypttrs( 'LOWER', n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! used to solve the system of equations.
        ...
        ! =====================================================================
           ...
           ! compute the l*d*l**h (or u**h*d*u) factorization of a.
           call stdlib_I64_wpttrf( n, d, e, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_wpttrs( 'LOWER', n, nrhs, d, e, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**t (or u**t*d*u) factorization of a.
              call stdlib_I64_xcopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 )call stdlib_I64_xcopy( n-1, e, 1_ilp64, ef, 1_ilp64 )
              call stdlib_I64_xpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_xlanst( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_xptcon( n, df, ef, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_xlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_xpttrs( n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_xptrfs( n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr, berr,work, info )
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**t (or u**t*d*u) factorization of a.
              call stdlib_I64_qcopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 )call stdlib_I64_qcopy( n-1, e, 1_ilp64, ef, 1_ilp64 )
              call stdlib_I64_qpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_qlanst( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_qptcon( n, df, ef, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_qpttrs( n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_qptrfs( n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr, berr,work, info )
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**h (or u**h*d*u) factorization of a.
              call stdlib_I64_xcopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 )call stdlib_I64_ycopy( n-1, e, 1_ilp64, ef, 1_ilp64 )
              call stdlib_I64_ypttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_ylanht( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_yptcon( n, df, ef, anorm, rcond, rwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_ylacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ypttrs( 'LOWER', n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_yptrfs( 'LOWER', n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_xlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**h (or u**h*d*u) factorization of a.
              call stdlib_I64_qcopy( n, d, 1_ilp64, df, 1_ilp64 )
              if( n>1_ilp64 )call stdlib_I64_wcopy( n-1, e, 1_ilp64, ef, 1_ilp64 )
              call stdlib_I64_wpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_wlanht( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_wptcon( n, df, ef, anorm, rcond, rwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_wpttrs( 'LOWER', n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_wptrfs( 'LOWER', n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_qlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
