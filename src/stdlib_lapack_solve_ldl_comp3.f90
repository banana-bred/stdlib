  ...
  contains
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           kase = 0_ilp
           30 continue
           call stdlib_ylacn2( n, work( n+1 ), work, ainvnm, kase, isave )
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_yhetrs( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_ylahef;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_ylahef( uplo, k, nb, kb, a, lda, ipiv, work, n, iinfo )
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_yhetf2( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_ylahef( uplo, n-k+1, nb, kb, a( k, k ), lda, ipiv( k ),work, n, &
                           iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_yhetf2( uplo, n-k+1, a( k, k ), lda, ipiv( k ), iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
     ...
     !! A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              kstep = 1_ilp
              ! copy column k of a to column kw of w and update it
              call stdlib_ycopy( k-1, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              w( k, kw ) = real( a( k, k ),KIND=xdp)
              if( k<n ) then
                 call stdlib_ygemv( 'NO TRANSPOSE', k, n-k, -cone, a( 1_ilp, k+1 ), lda,w( k, kw+1 ), &
                           ldw, cone, w( 1_ilp, kw ), 1_ilp )
              ...
              ! determine both colmax and imax.
              if( k>1_ilp ) then
                 imax = stdlib_iyamax( k-1, w( 1_ilp, kw ), 1_ilp )
                 colmax = cabs1( w( imax, kw ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call stdlib_ycopy( imax-1, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ), 1_ilp )
                    w( imax, kw-1 ) = real( a( imax, imax ),KIND=xdp)
                    call stdlib_ycopy( k-imax, a( imax, imax+1 ), lda,w( imax+1, kw-1 ), 1_ilp )
                              
                    call stdlib_ylacgv( k-imax, w( imax+1, kw-1 ), 1_ilp )
                    if( k<n ) then
                       call stdlib_ygemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp, k+1 ), lda, w( imax,&
                                  kw+1 ), ldw,cone, w( 1_ilp, kw-1 ), 1_ilp )
                    ...
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = imax + stdlib_iyamax( k-imax, w( imax+1, kw-1 ), 1_ilp )
                    rowmax = cabs1( w( jmax, kw-1 ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iyamax( imax-1, w( 1_ilp, kw-1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( w( jmax, kw-1 ) ) )
                    end if
                    ! case(2)
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                    ...
                    ! case(3)
                       ...
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
                       call stdlib_ycopy( k, w( 1_ilp, kw-1 ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
                    ! case(4)
                    else
                       ! interchange rows and columns k-1 and imax, use 2-by-2
                       ! pivot block
                 ...
                 if( kp/=kk ) then
                    ...
                    call stdlib_ycopy( kk-1-kp, a( kp+1, kk ), 1_ilp, a( kp, kp+1 ),lda )
                    call stdlib_ylacgv( kk-1-kp, a( kp, kp+1 ), lda )
                    if( kp>1_ilp )call stdlib_ycopy( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    ! interchange rows kk and kp in last k+1 to n columns of a
                    ! (columns k (or k and k-1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_yswap( n-k, a( kk, k+1 ), lda, a( kp, k+1 ),lda )
                    call stdlib_yswap( n-kk+1, w( kk, kkw ), ldw, w( kp, kkw ),ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column kw of w now holds
                    ! w(kw) = u(k)*d(k),
                    ...
                    ! element d(k,k) from w (potentially saves only one load))
                    call stdlib_ycopy( k, w( 1_ilp, kw ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    if( k>1_ilp ) then
                       ! (note: no need to check if a(k,k) is not zero,
                        ! since that was ensured earlier in pivot search:
                        ! case a(k,k) = 0 falls into 2x2 pivot case(4))
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_ydscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                       ! (2) conjugate column w(kw)
                       call stdlib_ylacgv( k-1, w( 1_ilp, kw ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns kw and kw-1 of w now hold
                    ! ( w(kw-1) w(kw) ) = ( u(k-1) u(k) )*d(k)
                    ...
                    ! (2) conjugate columns w(kw) and w(kw-1)
                    call stdlib_ylacgv( k-1, w( 1_ilp, kw ), 1_ilp )
                    call stdlib_ylacgv( k-2, w( 1_ilp, kw-1 ), 1_ilp )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 do jj = j, j + jb - 1
                    ...
                    call stdlib_ygemv( 'NO TRANSPOSE', jj-j+1, n-k, -cone,a( j, k+1 ), lda, w( jj,&
                               kw+1 ), ldw, cone,a( j, jj ), 1_ilp )
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                 end do
                 ! update the rectangular superdiagonal block
                 call stdlib_ygemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( 1_ilp, k+1 ), &
                           lda, w( j, kw+1 ), ldw,cone, a( 1_ilp, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp
                 if( jp/=jj .and. j<=n )call stdlib_yswap( n-j+1, a( jp, j ), lda, a( jj, j ), &
                           lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( k<n )call stdlib_ycopy( n-k, a( k+1, k ), 1_ilp, w( k+1, k ), 1_ilp )
              call stdlib_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ), lda,w( k, 1_ilp ), ldw,&
                         cone, w( k, k ), 1_ilp )
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_iyamax( n-k, w( k+1, k ), 1_ilp )
                 colmax = cabs1( w( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column k+1 of w and update it
                    call stdlib_ycopy( imax-k, a( imax, k ), lda, w( k, k+1 ), 1_ilp )
                    call stdlib_ylacgv( imax-k, w( k, k+1 ), 1_ilp )
                    w( imax, k+1 ) = real( a( imax, imax ),KIND=xdp)
                    if( imax<n )call stdlib_ycopy( n-imax, a( imax+1, imax ), 1_ilp,w( imax+1, k+1 ), &
                              1_ilp )
                    call stdlib_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ),lda, w( imax, &
                              1_ilp ), ldw, cone, w( k, k+1 ),1_ilp )
                    w( imax, k+1 ) = real( w( imax, k+1 ),KIND=xdp)
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = k - 1_ilp + stdlib_iyamax( imax-k, w( k, k+1 ), 1_ilp )
                    rowmax = cabs1( w( jmax, k+1 ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iyamax( n-imax, w( imax+1, k+1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( w( jmax, k+1 ) ) )
                    end if
                    ! case(2)
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                    ...
                    ! case(3)
                       ...
                       kp = imax
                       ! copy column k+1 of w to column k of w
                       call stdlib_ycopy( n-k+1, w( k, k+1 ), 1_ilp, w( k, k ), 1_ilp )
                    ! case(4)
                    else
                       ! interchange rows and columns k+1 and imax, use 2-by-2
                       ! pivot block
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = real( a( kk, kk ),KIND=xdp)
                    call stdlib_ycopy( kp-kk-1, a( kk+1, kk ), 1_ilp, a( kp, kk+1 ),lda )
                    call stdlib_ylacgv( kp-kk-1, a( kp, kk+1 ), lda )
                    if( kp<n )call stdlib_ycopy( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                              
                    ! interchange rows kk and kp in first k-1 columns of a
                    ! (columns k (or k and k+1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in first kk columns of w.
                    if( k>1_ilp )call stdlib_yswap( k-1, a( kk, 1_ilp ), lda, a( kp, 1_ilp ), lda )
                    call stdlib_yswap( kk, w( kk, 1_ilp ), ldw, w( kp, 1_ilp ), ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k of w now holds
                    ! w(k) = l(k)*d(k),
                    ...
                    ! a( k, k ) = real( w( k, k),KIND=xdp) to separately copy diagonal
                    ! element d(k,k) from w (potentially saves only one load))
                    call stdlib_ycopy( n-k+1, w( k, k ), 1_ilp, a( k, k ), 1_ilp )
                    if( k<n ) then
                       ! (note: no need to check if a(k,k) is not zero,
                        ! since that was ensured earlier in pivot search:
                        ! case a(k,k) = 0 falls into 2x2 pivot case(4))
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_ydscal( n-k, r1, a( k+1, k ), 1_ilp )
                       ! (2) conjugate column w(k)
                       call stdlib_ylacgv( n-k, w( k+1, k ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 of w now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    ! (2) conjugate columns w(k) and w(k+1)
                    call stdlib_ylacgv( n-k, w( k+1, k ), 1_ilp )
                    call stdlib_ylacgv( n-k-1, w( k+2, k+1 ), 1_ilp )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp ) then
              ...
              do j = k, n, nb
                 ...
                 do jj = j, j + jb - 1
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                    call stdlib_ygemv( 'NO TRANSPOSE', j+jb-jj, k-1, -cone,a( jj, 1_ilp ), lda, w( jj,&
                               1_ilp ), ldw, cone,a( jj, jj ), 1_ilp )
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                 end do
                 ! update the rectangular subdiagonal block
                 if( j+jb<=n )call stdlib_ygemm( 'NO TRANSPOSE', 'TRANSPOSE', n-j-jb+1, jb,k-1, -&
                           cone, a( j+jb, 1_ilp ), lda, w( j, 1_ilp ),ldw, cone, a( j+jb, j ), lda )
              end do
              ! put l21 in standard form by partially undoing the interchanges
              ! of rows in columns 1:k-1 looping backwards from k-1 to 1
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp
                 if( jp/=jj .and. j>=1_ilp )call stdlib_yswap( j, a( jp, 1_ilp ), lda, a( jj, 1_ilp ), lda )
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! determine both colmax and imax.
              if( k>1_ilp ) then
                 imax = stdlib_iyamax( k-1, a( 1_ilp, k ), 1_ilp )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_xisnan(absakk) ) then
                 ! column k is zero or underflow, or contains a nan:
                 ! set info and continue
                 if( info==0_ilp )info = k
                 kp = k
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = imax + stdlib_iyamax( k-imax, a( imax, imax+1 ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iyamax( imax-1, a( 1_ilp, imax ), 1_ilp )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_yswap( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                       a( kp, j ) = t
                    ...
                    a( k, k ) = real( a( k, k ),KIND=xdp)
                    if( kstep==2_ilp )a( k-1, k-1 ) = real( a( k-1, k-1 ),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one / real( a( k, k ),KIND=xdp)
                    call stdlib_yher( uplo, k-1, -r1, a( 1_ilp, k ), 1_ilp, a, lda )
                    ! store u(k) in column k
                    call stdlib_ydscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if( k>2_ilp ) then
                       d = stdlib_xlapy2( real( a( k-1, k ),KIND=xdp),aimag( a( k-1, k ) ) )
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_iyamax( n-k, a( k+1, k ), 1_ilp )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_xisnan(absakk) ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = k - 1_ilp + stdlib_iyamax( imax-k, a( imax, k ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iyamax( n-imax, a( imax+1, imax ), 1_ilp )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_yswap( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_yher( uplo, n-k, -r1, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_yswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_ygeru( k-1, nrhs, -cone, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb &
                           )
                 ! multiply by the inverse of the diagonal block.
                 s = real( cone,KIND=xdp) / real( a( k, k ),KIND=xdp)
                 call stdlib_ydscal( nrhs, s, b( k, 1_ilp ), ldb )
                 k = k - 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k-1 )call stdlib_yswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_ygeru( k-2, nrhs, -cone, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb &
                           )
                 call stdlib_ygeru( k-2, nrhs, -cone, a( 1_ilp, k-1 ), 1_ilp, b( k-1, 1_ilp ),ldb, b( 1_ilp, 1_ilp ), &
                           ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k-1, k )
                 akm1 = a( k-1, k-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! stored in column k of a.
                 if( k>1_ilp ) then
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp ) then
                    ...
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k+1, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k+1 )&
                              , 1_ilp, cone, b( k+1, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k+1, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_ygeru( n-k, nrhs, -cone, a( k+1, k ), 1_ilp, b( k, 1_ilp ),ldb, b( &
                           k+1, 1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 s = real( cone,KIND=xdp) / real( a( k, k ),KIND=xdp)
                 call stdlib_ydscal( nrhs, s, b( k, 1_ilp ), ldb )
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_ygeru( n-k-1, nrhs, -cone, a( k+2, k ), 1_ilp, b( k, 1_ilp ),ldb, b( k+2, &
                              1_ilp ), ldb )
                    call stdlib_ygeru( n-k-1, nrhs, -cone, a( k+2, k+1 ), 1_ilp,b( k+1, 1_ilp ), ldb, b( &
                              k+2, 1_ilp ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k+1, k )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k<n ) then
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', n-k, nrhs, -cone,b( k+1, 1_ilp ), ldb, &
                              a( k+1, k ), 1_ilp, cone,b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp ) then
                    call stdlib_ycopy( k-1, a( 1_ilp, k ), 1_ilp, work, 1_ilp )
                    call stdlib_yhemv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k ), 1_ilp )
                              
                    a( k, k ) = a( k, k ) - real( stdlib_ydotc( k-1, work, 1_ilp, a( 1_ilp,k ), 1_ilp ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k>1_ilp ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_ydotc( k-1, work, 1_ilp, a( 1_ilp,k ), 1_ilp ),&
                              KIND=xdp)
                    a( k, k+1 ) = a( k, k+1 ) -stdlib_ydotc( k-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
                              
                    call stdlib_ycopy( k-1, a( 1_ilp, k+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_yhemv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k+1 ), 1_ilp )
                              
                    a( k+1, k+1 ) = a( k+1, k+1 ) -real( stdlib_ydotc( k-1, work, 1_ilp, a( 1_ilp, k+1 ),&
                              1_ilp ),KIND=xdp)
                 end if
                 kstep = 2_ilp
              end if
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the leading
                 ! submatrix a(1:k+1,1:k+1)
                 call stdlib_yswap( kp-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                 do j = kp + 1, k - 1
                    temp = conjg( a( j, k ) )
                    a( j, k ) = conjg( a( kp, j ) )
                    a( kp, j ) = temp
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_ycopy( n-k, a( k+1, k ), 1_ilp, work, 1_ilp )
                    call stdlib_yhemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp, czero, a( k+&
                              1_ilp, k ), 1_ilp )
                    a( k, k ) = a( k, k ) - real( stdlib_ydotc( n-k, work, 1_ilp,a( k+1, k ), 1_ilp ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k<n ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_ydotc( n-k, work, 1_ilp,a( k+1, k ), 1_ilp ),&
                              KIND=xdp)
                    a( k, k-1 ) = a( k, k-1 ) -stdlib_ydotc( n-k, a( k+1, k ), 1_ilp, a( k+1, k-1 ),1_ilp &
                              )
                    call stdlib_ycopy( n-k, a( k+1, k-1 ), 1_ilp, work, 1_ilp )
                    call stdlib_yhemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp, czero, a( k+&
                              1_ilp, k-1 ), 1_ilp )
                    a( k-1, k-1 ) = a( k-1, k-1 ) -real( stdlib_ydotc( n-k, work, 1_ilp, a( k+1, k-1 )&
                              ,1_ilp ),KIND=xdp)
                 end if
                 kstep = 2_ilp
              end if
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the trailing
                 ! submatrix a(k-1:n,k-1:n)
                 if( kp<n )call stdlib_yswap( n-kp, a( kp+1, k ), 1_ilp, a( kp+1, kp ), 1_ilp )
                 do j = k + 1, kp - 1
                    temp = conjg( a( j, k ) )
                    a( j, k ) = conjg( a( kp, j ) )
                    a( kp, j ) = temp
     ...
     !! provides error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! loop until stopping criterion is satisfied.
              ! compute residual r = b - a * x
              call stdlib_ycopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_yhemv( uplo, n, -cone, a, lda, x( 1_ilp, j ), 1_ilp, cone, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_yhetrs( uplo, n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 call stdlib_yaxpy( n, cone, work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              ! is incremented by safe1 if the i-th component of
              ! abs(a)*abs(x) + abs(b) is less than safe2.
              ! use stdlib_ylacn2 to estimate the infinity-norm of the matrix
                 ! inv(a) * diag(w),
              ! where w = abs(r) + nz*eps*( abs(a)*abs(x)+abs(b) )))
              do i = 1, n
                 if( rwork( i )>safe2 ) then
              ...
              kase = 0_ilp
              100 continue
              call stdlib_ylacn2( n, work( n+1 ), work, ferr( j ), kase, isave )
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(a**h).
                    call stdlib_yhetrs( uplo, n, 1_ilp, af, ldaf, ipiv, work, n, info )
     ...
     !! scalings.
        ...
        ! =====================================================================
           ...
           do iter = 1, max_iter
              ...
              call stdlib_ylassq( n, work( n+1 ), 1_ilp, scale, sumsq )
              std = scale * sqrt( sumsq / n )
              if ( std < tol * avg ) goto 999
              do i = 1, n
                 t = cabs1( a( i, i ) )
                 ...
                 c0 = -(t*si)*si + 2_ilp * real( work( i ),KIND=xdp) * si - n*avg
                 d = c1*c1 - 4_ilp*c0*c2
                 if ( d <= 0_ilp ) then
                    info = -1_ilp
                    return
           ...
           999 continue
           smlnum = stdlib_xlamch( 'SAFEMIN' )
           bignum = one / smlnum
           smin = bignum
           smax = zero
           t = one / sqrt( avg )
           base = stdlib_xlamch( 'B' )
           u = one / log( base )
           do i = 1, n
              s( i ) = base ** int( u * log( s( i ) * t ),KIND=ilp)
              smin = min( smin, s( i ) )
     ...
     !! A = L*D*L**H computed by ZHETRF and converted by ZSYCONV.
        ...
        ! =====================================================================
           ...
           if( n==0 .or. nrhs==0 )return
           ! convert a
           call stdlib_ysyconv( uplo, 'C', n, a, lda, ipiv, work, iinfo )
           if( upper ) then
              ! solve a*x = b, where a = u*d*u**h.
             ! p**t * b
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp==-ipiv( k-1 ) )call stdlib_yswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb &
                           )
                 k=k-2
              end if
             end do
        ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
             call stdlib_ytrsm('L','U','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (u \p**t * b) ]
              i=n
              do while ( i >= 1 )
                 if( ipiv(i) > 0_ilp ) then
                   s = real( cone,KIND=xdp) / real( a( i, i ),KIND=xdp)
                   call stdlib_ydscal( nrhs, s, b( i, 1_ilp ), ldb )
                 elseif ( i > 1_ilp) then
                    if ( ipiv(i-1) == ipiv(i) ) then
                       akm1k = work(i)
                       akm1 = a( i-1, i-1 ) / akm1k
            ...
            ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call stdlib_ytrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
             k=1_ilp
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k < n .and. kp==-ipiv( k+1 ) )call stdlib_yswap( nrhs, b( k, 1_ilp ), ldb, b( kp,&
                            1_ilp ), ldb )
                 k=k+2
              endif
             end do
             ...
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and -ipiv(k+1).
                 kp = -ipiv( k+1 )
                 if( kp==-ipiv( k ) )call stdlib_yswap( nrhs, b( k+1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                           
                 k=k+2
              endif
             end do
        ! compute (l \p**t * b) -> b    [ (l \p**t * b) ]
             call stdlib_ytrsm('L','L','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (l \p**t * b) ]
              i=1_ilp
              do while ( i <= n )
                 if( ipiv(i) > 0_ilp ) then
        ...
        ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
             call stdlib_ytrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k>1_ilp .and. kp==-ipiv( k-1 ) )call stdlib_yswap( nrhs, b( k, 1_ilp ), ldb, b( kp, &
                           1_ilp ), ldb )
                 k=k-2
              endif
             end do
           ...
           ! revert a
           call stdlib_ysyconv( uplo, 'R', n, a, lda, ipiv, work, iinfo )
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do k = n, 1, -1
                 kp = abs( ipiv( k ) )
                 if( kp/=k ) then
                    call stdlib_yswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 end if
              end do
              ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
              call stdlib_ytrsm( 'L', 'U', 'N', 'U', n, nrhs, cone, a, lda, b, ldb )
     ...
     !! a hermitian matrix.
        ...
        ! =====================================================================
           ...
           if (upper) then
               ...
               ! first swap
                ! - swap column i1 and i2 from i1 to i1-1
              call stdlib_yswap( i1-1, a(1_ilp,i1), 1_ilp, a(1_ilp,i2), 1_ilp )
                ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap row i1 from i1+1 to i2-1 with col i2 from i1+1 to i2-1
                ! - swap a(i2,i1) and a(i1,i2)
              ...
              do i=i2+1,n
               ...
               ! first swap
                ! - swap row i1 and i2 from 1 to i1-1
              call stdlib_yswap ( i1-1, a(i1,1_ilp), lda, a(i2,1_ilp), lda )
               ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap col i1 from i1+1 to i2-1 with row i2 from i1+1 to i2-1
                ! - swap a(i2,i1) and a(i1,i2)
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_yhptrs( uplo, n, 1_ilp, ap, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! 1-by-1 and 2-by-2 diagonal blocks.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! column k, and colmax is its absolute value
              if( k>1_ilp ) then
                 imax = stdlib_iyamax( k-1, ap( kc ), 1_ilp )
                 colmax = cabs1( ap( kc+imax-1 ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = ( imax-1 )*imax / 2_ilp + 1_ilp
                    if( imax>1_ilp ) then
                       jmax = stdlib_iyamax( imax-1, ap( kpc ), 1_ilp )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_yswap( kp-1, ap( knc ), 1_ilp, ap( kpc ), 1_ilp )
                    kx = kpc + kp - 1_ilp
                    do j = kp + 1, kk - 1
                       kx = kx + j - 1_ilp
                       t = conjg( ap( knc+j-1 ) )
                    ...
                    ap( kc+k-1 ) = real( ap( kc+k-1 ),KIND=xdp)
                    if( kstep==2_ilp )ap( kc-1 ) = real( ap( kc-1 ),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    call stdlib_yhpr( uplo, k-1, -r1, ap( kc ), 1_ilp, ap )
                    ! store u(k) in column k
                    call stdlib_ydscal( k-1, r1, ap( kc ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if( k>2_ilp ) then
                       d = stdlib_xlapy2( real( ap( k-1+( k-1 )*k / 2_ilp ),KIND=xdp),aimag( ap( k-1+( &
                                 k-1 )*k / 2_ilp ) ) )
                       d22 = real( ap( k-1+( k-2 )*( k-1 ) / 2_ilp ),KIND=xdp) / d
                       d11 = real( ap( k+( k-1 )*k / 2_ilp ),KIND=xdp) / d
                       tt = one / ( d11*d22-one )
                       d12 = ap( k-1+( k-1 )*k / 2_ilp ) / d
                       d = tt / d
                       do j = k - 2, 1, -1
                          ...
                          ap( j+( k-1 )*k / 2_ilp ) = wk
                          ap( j+( k-2 )*( k-1 ) / 2_ilp ) = wkm1
                          ap( j+( j-1 )*j / 2_ilp ) = cmplx( real( ap( j+( j-1 )*j / 2_ilp ),KIND=xdp), &
                                    zero,KIND=xdp)
                       end do
                    end if
                 end if
              ...
              ! column k, and colmax is its absolute value
              if( k<n ) then
                 imax = k + stdlib_iyamax( n-k, ap( kc+1 ), 1_ilp )
                 colmax = cabs1( ap( kc+imax-k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = npp - ( n-imax+1 )*( n-imax+2 ) / 2_ilp + 1_ilp
                    if( imax<n ) then
                       jmax = imax + stdlib_iyamax( n-imax, ap( kpc+1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_yswap( n-kp, ap( knc+kp-kk+1 ), 1_ilp, ap( kpc+1 ),1_ilp )
                    ...
                    ap( kc ) = real( ap( kc ),KIND=xdp)
                    if( kstep==2_ilp )ap( knc ) = real( ap( knc ),KIND=xdp)
                 end if
                 ! update the trailing submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if( k<n ) then
                       ...
                       call stdlib_yhpr( uplo, n-k, -r1, ap( kc+1 ), 1_ilp,ap( kc+n-k+1 ) )
                       ! store l(k) in column k
                       call stdlib_ydscal( n-k, r1, ap( kc+1 ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if( k<n-1 ) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = stdlib_xlapy2( real( ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ),KIND=xdp),aimag( &
                                 ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ) ) )
                       d11 = real( ap( k+1+k*( 2_ilp*n-k-1 ) / 2_ilp ),KIND=xdp) / d
                       d22 = real( ap( k+( k-1 )*( 2_ilp*n-k ) / 2_ilp ),KIND=xdp) / d
                       tt = one / ( d11*d22-one )
                       d21 = ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ) / d
                       d = tt / d
                       do j = k + 2, n
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_ygeru( k-1, nrhs, -cone, ap( kc ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_ygeru( k-2, nrhs, -cone, ap( kc ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                           
                 call stdlib_ygeru( k-2, nrhs, -cone, ap( kc-( k-1 ) ), 1_ilp,b( k-1, 1_ilp ), ldb, b( 1_ilp, &
                           1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+k-2 )
                 akm1 = ap( kc-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k>1_ilp ) then
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp ) then
                    ...
                    call stdlib_ylacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k+1, 1_ilp ), ldb )
                    call stdlib_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc+k ),&
                               1_ilp, cone, b( k+1, 1_ilp ), ldb )
                    call stdlib_ylacgv( nrhs, b( k+1, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_ygeru( n-k, nrhs, -cone, ap( kc+1 ), 1_ilp, b( k, 1_ilp ),ldb, b( k+&
                           1_ilp, 1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_ygeru( n-k-1, nrhs, -cone, ap( kc+2 ), 1_ilp, b( k, 1_ilp ),ldb, b( k+2, &
                              1_ilp ), ldb )
                    call stdlib_ygeru( n-k-1, nrhs, -cone, ap( kc+n-k+2 ), 1_ilp,b( k+1, 1_ilp ), ldb, b( &
                              k+2, 1_ilp ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+1 )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp ) then
                    call stdlib_ycopy( k-1, ap( kc ), 1_ilp, work, 1_ilp )
                    call stdlib_yhpmv( uplo, k-1, -cone, ap, work, 1_ilp, czero,ap( kc ), 1_ilp )
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_ydotc( k-1, work, 1_ilp, ap( kc ), 1_ilp ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k>1_ilp ) then
                    ...
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_ydotc( k-1, work, 1_ilp, ap( kc ), 1_ilp ),&
                              KIND=xdp)
                    ap( kcnext+k-1 ) = ap( kcnext+k-1 ) -stdlib_ydotc( k-1, ap( kc ), 1_ilp, ap( &
                              kcnext ),1_ilp )
                    call stdlib_ycopy( k-1, ap( kcnext ), 1_ilp, work, 1_ilp )
                    call stdlib_yhpmv( uplo, k-1, -cone, ap, work, 1_ilp, czero,ap( kcnext ), 1_ilp )
                              
                    ap( kcnext+k ) = ap( kcnext+k ) -real( stdlib_ydotc( k-1, work, 1_ilp, ap( kcnext &
                              ),1_ilp ),KIND=xdp)
                 end if
                 kstep = 2_ilp
                 kcnext = kcnext + k + 1_ilp
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = ( kp-1 )*kp / 2_ilp + 1_ilp
                 call stdlib_yswap( kp-1, ap( kc ), 1_ilp, ap( kpc ), 1_ilp )
                 kx = kpc + kp - 1_ilp
                 do j = kp + 1, k - 1
                    kx = kx + j - 1_ilp
                    temp = conjg( ap( kc+j-1 ) )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_ycopy( n-k, ap( kc+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_yhpmv( uplo, n-k, -cone, ap( kc+n-k+1 ), work, 1_ilp,czero, ap( kc+1 )&
                              , 1_ilp )
                    ap( kc ) = ap( kc ) - real( stdlib_ydotc( n-k, work, 1_ilp,ap( kc+1 ), 1_ilp ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k<n ) then
                    call stdlib_ycopy( n-k, ap( kc+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_yhpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp, czero, ap( &
                              kc+1 ), 1_ilp )
                    ap( kc ) = ap( kc ) - real( stdlib_ydotc( n-k, work, 1_ilp,ap( kc+1 ), 1_ilp ),&
                              KIND=xdp)
                    ap( kcnext+1 ) = ap( kcnext+1 ) -stdlib_ydotc( n-k, ap( kc+1 ), 1_ilp,ap( kcnext+&
                              2_ilp ), 1_ilp )
                    call stdlib_ycopy( n-k, ap( kcnext+2 ), 1_ilp, work, 1_ilp )
                    call stdlib_yhpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp, czero, ap( &
                              kcnext+2 ), 1_ilp )
                    ap( kcnext ) = ap( kcnext ) -real( stdlib_ydotc( n-k, work, 1_ilp, ap( kcnext+2 ),&
                              1_ilp ),KIND=xdp)
                 end if
                 kstep = 2_ilp
                 kcnext = kcnext - ( n-k+3 )
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - ( n-kp+1 )*( n-kp+2 ) / 2_ilp + 1_ilp
                 if( kp<n )call stdlib_yswap( n-kp, ap( kc+kp-k+1 ), 1_ilp, ap( kpc+1 ), 1_ilp )
                 kx = kc + kp - k
                 do j = k + 1, kp - 1
                    kx = kx + n - j + 1_ilp
                    temp = conjg( ap( kc+j-k ) )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           kase = 0_ilp64
           30 continue
           call stdlib_I64_ylacn2( n, work( n+1 ), work, ainvnm, kase, isave )
           if( kase/=0_ilp64 ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_I64_yhetrs( uplo, n, 1_ilp64, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_I64_ylahef;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_I64_ylahef( uplo, k, nb, kb, a, lda, ipiv, work, n, iinfo )
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_I64_yhetf2( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_I64_ylahef( uplo, n-k+1, nb, kb, a( k, k ), lda, ipiv( k ),work, n, &
                           iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_I64_yhetf2( uplo, n-k+1, a( k, k ), lda, ipiv( k ), iinfo )
                 kb = n - k + 1_ilp64
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo + k - 1_ilp64
     ...
     !! A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              kstep = 1_ilp64
              ! copy column k of a to column kw of w and update it
              call stdlib_I64_ycopy( k-1, a( 1_ilp64, k ), 1_ilp64, w( 1_ilp64, kw ), 1_ilp64 )
              w( k, kw ) = real( a( k, k ),KIND=xdp)
              if( k<n ) then
                 call stdlib_I64_ygemv( 'NO TRANSPOSE', k, n-k, -cone, a( 1_ilp64, k+1 ), lda,w( k, kw+1 ), &
                           ldw, cone, w( 1_ilp64, kw ), 1_ilp64 )
                 w( k, kw ) = real( w( k, kw ),KIND=xdp)
              end if
              ! determine rows and columns to be interchanged and whether
              ...
              ! determine both colmax and imax.
              if( k>1_ilp64 ) then
                 imax = stdlib_I64_iyamax( k-1, w( 1_ilp64, kw ), 1_ilp64 )
                 colmax = cabs1( w( imax, kw ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call stdlib_I64_ycopy( imax-1, a( 1_ilp64, imax ), 1_ilp64, w( 1_ilp64, kw-1 ), 1_ilp64 )
                    w( imax, kw-1 ) = real( a( imax, imax ),KIND=xdp)
                    call stdlib_I64_ycopy( k-imax, a( imax, imax+1 ), lda,w( imax+1, kw-1 ), 1_ilp64 )
                              
                    call stdlib_I64_ylacgv( k-imax, w( imax+1, kw-1 ), 1_ilp64 )
                    if( k<n ) then
                       call stdlib_I64_ygemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp64, k+1 ), lda, w( imax,&
                                  kw+1 ), ldw,cone, w( 1_ilp64, kw-1 ), 1_ilp64 )
                       w( imax, kw-1 ) = real( w( imax, kw-1 ),KIND=xdp)
                    end if
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = imax + stdlib_I64_iyamax( k-imax, w( imax+1, kw-1 ), 1_ilp64 )
                    rowmax = cabs1( w( jmax, kw-1 ) )
                    if( imax>1_ilp64 ) then
                       jmax = stdlib_I64_iyamax( imax-1, w( 1_ilp64, kw-1 ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( w( jmax, kw-1 ) ) )
                    end if
                    ! case(2)
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                    ...
                    ! case(3)
                       ...
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
                       call stdlib_I64_ycopy( k, w( 1_ilp64, kw-1 ), 1_ilp64, w( 1_ilp64, kw ), 1_ilp64 )
                    ! case(4)
                    else
                       ! interchange rows and columns k-1 and imax, use 2-by-2
                       ! pivot block
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = real( a( kk, kk ),KIND=xdp)
                    call stdlib_I64_ycopy( kk-1-kp, a( kp+1, kk ), 1_ilp64, a( kp, kp+1 ),lda )
                    call stdlib_I64_ylacgv( kk-1-kp, a( kp, kp+1 ), lda )
                    if( kp>1_ilp64 )call stdlib_I64_ycopy( kp-1, a( 1_ilp64, kk ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    ! interchange rows kk and kp in last k+1 to n columns of a
                    ! (columns k (or k and k-1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_I64_yswap( n-k, a( kk, k+1 ), lda, a( kp, k+1 ),lda )
                    call stdlib_I64_yswap( n-kk+1, w( kk, kkw ), ldw, w( kp, kkw ),ldw )
                 end if
                 if( kstep==1_ilp64 ) then
                    ! 1-by-1 pivot block d(k): column kw of w now holds
                    ! w(kw) = u(k)*d(k),
                    ...
                    ! a( k, k ) = real( w( k, k),KIND=xdp) to separately copy diagonal
                    ! element d(k,k) from w (potentially saves only one load))
                    call stdlib_I64_ycopy( k, w( 1_ilp64, kw ), 1_ilp64, a( 1_ilp64, k ), 1_ilp64 )
                    if( k>1_ilp64 ) then
                       ! (note: no need to check if a(k,k) is not zero,
                        ! since that was ensured earlier in pivot search:
                        ! case a(k,k) = 0 falls into 2x2 pivot case(4))
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_I64_ydscal( k-1, r1, a( 1_ilp64, k ), 1_ilp64 )
                       ! (2) conjugate column w(kw)
                       call stdlib_I64_ylacgv( k-1, w( 1_ilp64, kw ), 1_ilp64 )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns kw and kw-1 of w now hold
                    ! ( w(kw-1) w(kw) ) = ( u(k-1) u(k) )*d(k)
                    ...
                    ! (2) conjugate columns w(kw) and w(kw-1)
                    call stdlib_I64_ylacgv( k-1, w( 1_ilp64, kw ), 1_ilp64 )
                    call stdlib_I64_ylacgv( k-2, w( 1_ilp64, kw-1 ), 1_ilp64 )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp64 ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 do jj = j, j + jb - 1
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                    call stdlib_I64_ygemv( 'NO TRANSPOSE', jj-j+1, n-k, -cone,a( j, k+1 ), lda, w( jj,&
                               kw+1 ), ldw, cone,a( j, jj ), 1_ilp64 )
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                 end do
                 ! update the rectangular superdiagonal block
                 call stdlib_I64_ygemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( 1_ilp64, k+1 ), &
                           lda, w( j, kw+1 ), ldw,cone, a( 1_ilp64, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp64
                 if( jp/=jj .and. j<=n )call stdlib_I64_yswap( n-j+1, a( jp, j ), lda, a( jj, j ), &
                           lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              ! copy column k of a to column k of w and update it
              w( k, k ) = real( a( k, k ),KIND=xdp)
              if( k<n )call stdlib_I64_ycopy( n-k, a( k+1, k ), 1_ilp64, w( k+1, k ), 1_ilp64 )
              call stdlib_I64_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp64 ), lda,w( k, 1_ilp64 ), ldw,&
                         cone, w( k, k ), 1_ilp64 )
              w( k, k ) = real( w( k, k ),KIND=xdp)
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_I64_iyamax( n-k, w( k+1, k ), 1_ilp64 )
                 colmax = cabs1( w( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column k+1 of w and update it
                    call stdlib_I64_ycopy( imax-k, a( imax, k ), lda, w( k, k+1 ), 1_ilp64 )
                    call stdlib_I64_ylacgv( imax-k, w( k, k+1 ), 1_ilp64 )
                    w( imax, k+1 ) = real( a( imax, imax ),KIND=xdp)
                    if( imax<n )call stdlib_I64_ycopy( n-imax, a( imax+1, imax ), 1_ilp64,w( imax+1, k+1 ), &
                              1_ilp64 )
                    call stdlib_I64_ygemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp64 ),lda, w( imax, &
                              1_ilp64 ), ldw, cone, w( k, k+1 ),1_ilp64 )
                    w( imax, k+1 ) = real( w( imax, k+1 ),KIND=xdp)
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = k - 1_ilp64 + stdlib_I64_iyamax( imax-k, w( k, k+1 ), 1_ilp64 )
                    rowmax = cabs1( w( jmax, k+1 ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_I64_iyamax( n-imax, w( imax+1, k+1 ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( w( jmax, k+1 ) ) )
                    end if
                    ! case(2)
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                    ...
                    ! case(3)
                       ...
                       kp = imax
                       ! copy column k+1 of w to column k of w
                       call stdlib_I64_ycopy( n-k+1, w( k, k+1 ), 1_ilp64, w( k, k ), 1_ilp64 )
                    ! case(4)
                    else
                       ! interchange rows and columns k+1 and imax, use 2-by-2
                       ! pivot block
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = real( a( kk, kk ),KIND=xdp)
                    call stdlib_I64_ycopy( kp-kk-1, a( kk+1, kk ), 1_ilp64, a( kp, kk+1 ),lda )
                    call stdlib_I64_ylacgv( kp-kk-1, a( kp, kk+1 ), lda )
                    if( kp<n )call stdlib_I64_ycopy( n-kp, a( kp+1, kk ), 1_ilp64, a( kp+1, kp ), 1_ilp64 )
                              
                    ! interchange rows kk and kp in first k-1 columns of a
                    ! (columns k (or k and k+1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in first kk columns of w.
                    if( k>1_ilp64 )call stdlib_I64_yswap( k-1, a( kk, 1_ilp64 ), lda, a( kp, 1_ilp64 ), lda )
                    call stdlib_I64_yswap( kk, w( kk, 1_ilp64 ), ldw, w( kp, 1_ilp64 ), ldw )
                 end if
                 if( kstep==1_ilp64 ) then
                    ! 1-by-1 pivot block d(k): column k of w now holds
                    ! w(k) = l(k)*d(k),
                    ...
                    ! a( k, k ) = real( w( k, k),KIND=xdp) to separately copy diagonal
                    ! element d(k,k) from w (potentially saves only one load))
                    call stdlib_I64_ycopy( n-k+1, w( k, k ), 1_ilp64, a( k, k ), 1_ilp64 )
                    if( k<n ) then
                       ! (note: no need to check if a(k,k) is not zero,
                        ! since that was ensured earlier in pivot search:
                        ! case a(k,k) = 0 falls into 2x2 pivot case(4))
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_I64_ydscal( n-k, r1, a( k+1, k ), 1_ilp64 )
                       ! (2) conjugate column w(k)
                       call stdlib_I64_ylacgv( n-k, w( k+1, k ), 1_ilp64 )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 of w now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    ! (2) conjugate columns w(k) and w(k+1)
                    call stdlib_I64_ylacgv( n-k, w( k+1, k ), 1_ilp64 )
                    call stdlib_I64_ylacgv( n-k-1, w( k+2, k+1 ), 1_ilp64 )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp64 ) then
              ...
              do j = k, n, nb
                 ...
                 do jj = j, j + jb - 1
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                    call stdlib_I64_ygemv( 'NO TRANSPOSE', j+jb-jj, k-1, -cone,a( jj, 1_ilp64 ), lda, w( jj,&
                               1_ilp64 ), ldw, cone,a( jj, jj ), 1_ilp64 )
                    a( jj, jj ) = real( a( jj, jj ),KIND=xdp)
                 end do
                 ! update the rectangular subdiagonal block
                 if( j+jb<=n )call stdlib_I64_ygemm( 'NO TRANSPOSE', 'TRANSPOSE', n-j-jb+1, jb,k-1, -&
                           cone, a( j+jb, 1_ilp64 ), lda, w( j, 1_ilp64 ),ldw, cone, a( j+jb, j ), lda )
              end do
              ! put l21 in standard form by partially undoing the interchanges
              ! of rows in columns 1:k-1 looping backwards from k-1 to 1
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp64
                 if( jp/=jj .and. j>=1_ilp64 )call stdlib_I64_yswap( j, a( jp, 1_ilp64 ), lda, a( jj, 1_ilp64 ), lda )
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! determine both colmax and imax.
              if( k>1_ilp64 ) then
                 imax = stdlib_I64_iyamax( k-1, a( 1_ilp64, k ), 1_ilp64 )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_I64_xisnan(absakk) ) then
                 ! column k is zero or underflow, or contains a nan:
                 ! set info and continue
                 if( info==0_ilp64 )info = k
                 kp = k
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = imax + stdlib_I64_iyamax( k-imax, a( imax, imax+1 ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax>1_ilp64 ) then
                       jmax = stdlib_I64_iyamax( imax-1, a( 1_ilp64, imax ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_I64_yswap( kp-1, a( 1_ilp64, kk ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                       a( kp, j ) = t
                    ...
                    a( k, k ) = real( a( k, k ),KIND=xdp)
                    if( kstep==2_ilp64 )a( k-1, k-1 ) = real( a( k-1, k-1 ),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp64 ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one / real( a( k, k ),KIND=xdp)
                    call stdlib_I64_yher( uplo, k-1, -r1, a( 1_ilp64, k ), 1_ilp64, a, lda )
                    ! store u(k) in column k
                    call stdlib_I64_ydscal( k-1, r1, a( 1_ilp64, k ), 1_ilp64 )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if( k>2_ilp64 ) then
                       d = stdlib_I64_xlapy2( real( a( k-1, k ),KIND=xdp),aimag( a( k-1, k ) ) )
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_I64_iyamax( n-k, a( k+1, k ), 1_ilp64 )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_I64_xisnan(absakk) ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    jmax = k - 1_ilp64 + stdlib_I64_iyamax( imax-k, a( imax, k ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_I64_iyamax( n-imax, a( imax+1, imax ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_I64_yswap( n-kp, a( kp+1, kk ), 1_ilp64, a( kp+1, kp ), 1_ilp64 )
                 ...
                 if( kstep==1_ilp64 ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one / real( a( k, k ),KIND=xdp)
                       call stdlib_I64_yher( uplo, n-k, -r1, a( k+1, k ), 1_ilp64,a( k+1, k+1 ), lda )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_I64_yswap( nrhs, b( k, 1_ilp64 ), ldb, b( kp, 1_ilp64 ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_I64_ygeru( k-1, nrhs, -cone, a( 1_ilp64, k ), 1_ilp64, b( k, 1_ilp64 ), ldb,b( 1_ilp64, 1_ilp64 ),&
                     & ldb &
                           )
                 ! multiply by the inverse of the diagonal block.
                 s = real( cone,KIND=xdp) / real( a( k, k ),KIND=xdp)
                 call stdlib_I64_ydscal( nrhs, s, b( k, 1_ilp64 ), ldb )
                 k = k - 1_ilp64
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k-1 )call stdlib_I64_yswap( nrhs, b( k-1, 1_ilp64 ), ldb, b( kp, 1_ilp64 ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_I64_ygeru( k-2, nrhs, -cone, a( 1_ilp64, k ), 1_ilp64, b( k, 1_ilp64 ), ldb,b( 1_ilp64, 1_ilp64 ),&
                     & ldb &
                           )
                 call stdlib_I64_ygeru( k-2, nrhs, -cone, a( 1_ilp64, k-1 ), 1_ilp64, b( k-1, 1_ilp64 ),ldb, b( 1_ilp64, 1_ilp64&
                     & ), &
                           ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k-1, k )
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! stored in column k of a.
                 if( k>1_ilp64 ) then
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp64, k ), &
                              1_ilp64, cone, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp64 ) then
                    ...
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp64, k ), &
                              1_ilp64, cone, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k+1, 1_ilp64 ), ldb )
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp64, k+1 )&
                              , 1_ilp64, cone, b( k+1, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k+1, 1_ilp64 ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_I64_ygeru( n-k, nrhs, -cone, a( k+1, k ), 1_ilp64, b( k, 1_ilp64 ),ldb, b( &
                           k+1, 1_ilp64 ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 s = real( cone,KIND=xdp) / real( a( k, k ),KIND=xdp)
                 call stdlib_I64_ydscal( nrhs, s, b( k, 1_ilp64 ), ldb )
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_I64_ygeru( n-k-1, nrhs, -cone, a( k+2, k ), 1_ilp64, b( k, 1_ilp64 ),ldb, b( k+2, &
                              1_ilp64 ), ldb )
                    call stdlib_I64_ygeru( n-k-1, nrhs, -cone, a( k+2, k+1 ), 1_ilp64,b( k+1, 1_ilp64 ), ldb, b( &
                              k+2, 1_ilp64 ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k+1, k )
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 if( k<n ) then
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', n-k, nrhs, -cone,b( k+1, 1_ilp64 ), ldb, &
                              a( k+1, k ), 1_ilp64, cone,b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp64 ) then
                    call stdlib_I64_ycopy( k-1, a( 1_ilp64, k ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhemv( uplo, k-1, -cone, a, lda, work, 1_ilp64, czero,a( 1_ilp64, k ), 1_ilp64 )
                              
                    a( k, k ) = a( k, k ) - real( stdlib_I64_ydotc( k-1, work, 1_ilp64, a( 1_ilp64,k ), 1_ilp64 ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp64
              else
                 ...
                 if( k>1_ilp64 ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_I64_ydotc( k-1, work, 1_ilp64, a( 1_ilp64,k ), 1_ilp64 ),&
                              KIND=xdp)
                    a( k, k+1 ) = a( k, k+1 ) -stdlib_I64_ydotc( k-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, k+1 ), 1_ilp64 )
                              
                    call stdlib_I64_ycopy( k-1, a( 1_ilp64, k+1 ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhemv( uplo, k-1, -cone, a, lda, work, 1_ilp64, czero,a( 1_ilp64, k+1 ), 1_ilp64 )
                              
                    a( k+1, k+1 ) = a( k+1, k+1 ) -real( stdlib_I64_ydotc( k-1, work, 1_ilp64, a( 1_ilp64, k+1 ),&
                              1_ilp64 ),KIND=xdp)
                 end if
                 kstep = 2_ilp64
              end if
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the leading
                 ! submatrix a(1:k+1,1:k+1)
                 call stdlib_I64_yswap( kp-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                 do j = kp + 1, k - 1
                    temp = conjg( a( j, k ) )
                    a( j, k ) = conjg( a( kp, j ) )
                    a( kp, j ) = temp
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_I64_ycopy( n-k, a( k+1, k ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp64, czero, a( k+&
                              1_ilp64, k ), 1_ilp64 )
                    a( k, k ) = a( k, k ) - real( stdlib_I64_ydotc( n-k, work, 1_ilp64,a( k+1, k ), 1_ilp64 ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp64
              else
                 ...
                 if( k<n ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_I64_ydotc( n-k, work, 1_ilp64,a( k+1, k ), 1_ilp64 ),&
                              KIND=xdp)
                    a( k, k-1 ) = a( k, k-1 ) -stdlib_I64_ydotc( n-k, a( k+1, k ), 1_ilp64, a( k+1, k-1 ),1_ilp64 &
                              )
                    call stdlib_I64_ycopy( n-k, a( k+1, k-1 ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp64, czero, a( k+&
                              1_ilp64, k-1 ), 1_ilp64 )
                    a( k-1, k-1 ) = a( k-1, k-1 ) -real( stdlib_I64_ydotc( n-k, work, 1_ilp64, a( k+1, k-1 )&
                              ,1_ilp64 ),KIND=xdp)
                 end if
                 kstep = 2_ilp64
              end if
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the trailing
                 ! submatrix a(k-1:n,k-1:n)
                 if( kp<n )call stdlib_I64_yswap( n-kp, a( kp+1, k ), 1_ilp64, a( kp+1, kp ), 1_ilp64 )
                 do j = k + 1, kp - 1
                    temp = conjg( a( j, k ) )
                    a( j, k ) = conjg( a( kp, j ) )
                    a( kp, j ) = temp
     ...
     !! provides error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! loop until stopping criterion is satisfied.
              ! compute residual r = b - a * x
              call stdlib_I64_ycopy( n, b( 1_ilp64, j ), 1_ilp64, work, 1_ilp64 )
              call stdlib_I64_yhemv( uplo, n, -cone, a, lda, x( 1_ilp64, j ), 1_ilp64, cone, work, 1_ilp64 )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_I64_yhetrs( uplo, n, 1_ilp64, af, ldaf, ipiv, work, n, info )
                 call stdlib_I64_yaxpy( n, cone, work, 1_ilp64, x( 1_ilp64, j ), 1_ilp64 )
                 lstres = berr( j )
                 count = count + 1_ilp64
                 go to 20
              end if
              ...
              kase = 0_ilp64
              100 continue
              call stdlib_I64_ylacn2( n, work( n+1 ), work, ferr( j ), kase, isave )
              if( kase/=0_ilp64 ) then
                 if( kase==1_ilp64 ) then
                    ! multiply by diag(w)*inv(a**h).
                    call stdlib_I64_yhetrs( uplo, n, 1_ilp64, af, ldaf, ipiv, work, n, info )
     ...
     !! scalings.
        ...
        ! =====================================================================
           ...
           do iter = 1, max_iter
              ...
              call stdlib_I64_ylassq( n, work( n+1 ), 1_ilp64, scale, sumsq )
              std = scale * sqrt( sumsq / n )
              if ( std < tol * avg ) goto 999
              do i = 1, n
                 t = cabs1( a( i, i ) )
                 ...
                 c2 = ( n-1 ) * t
                 c1 = ( n-2 ) * ( real( work( i ),KIND=xdp) - t*si )
                 c0 = -(t*si)*si + 2_ilp64 * real( work( i ),KIND=xdp) * si - n*avg
                 d = c1*c1 - 4_ilp64*c0*c2
                 if ( d <= 0_ilp64 ) then
                    info = -1_ilp64
                    return
           ...
           999 continue
           smlnum = stdlib_I64_xlamch( 'SAFEMIN' )
           bignum = one / smlnum
           smin = bignum
           smax = zero
           t = one / sqrt( avg )
           base = stdlib_I64_xlamch( 'B' )
           u = one / log( base )
           do i = 1, n
              s( i ) = base ** int( u * log( s( i ) * t ),KIND=ilp64)
              smin = min( smin, s( i ) )
     ...
     !! A = L*D*L**H computed by ZHETRF and converted by ZSYCONV.
        ...
        ! =====================================================================
           ...
           if( n==0 .or. nrhs==0 )return
           ! convert a
           call stdlib_I64_ysyconv( uplo, 'C', n, a, lda, ipiv, work, iinfo )
           if( upper ) then
              ! solve a*x = b, where a = u*d*u**h.
             ! p**t * b
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp==-ipiv( k-1 ) )call stdlib_I64_yswap( nrhs, b( k-1, 1_ilp64 ), ldb, b( kp, 1_ilp64 ), ldb &
                           )
                 k=k-2
              end if
             end do
        ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
             call stdlib_I64_ytrsm('L','U','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (u \p**t * b) ]
              i=n
              do while ( i >= 1 )
                 if( ipiv(i) > 0_ilp64 ) then
                   s = real( cone,KIND=xdp) / real( a( i, i ),KIND=xdp)
                   call stdlib_I64_ydscal( nrhs, s, b( i, 1_ilp64 ), ldb )
                 elseif ( i > 1_ilp64) then
                    if ( ipiv(i-1) == ipiv(i) ) then
                       akm1k = work(i)
                       akm1 = a( i-1, i-1 ) / akm1k
            ...
            ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call stdlib_I64_ytrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
             k=1_ilp64
             do while ( k <= n )
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k < n .and. kp==-ipiv( k+1 ) )call stdlib_I64_yswap( nrhs, b( k, 1_ilp64 ), ldb, b( kp,&
                            1_ilp64 ), ldb )
                 k=k+2
              endif
             end do
             ...
             do while ( k <= n )
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! interchange rows k and -ipiv(k+1).
                 kp = -ipiv( k+1 )
                 if( kp==-ipiv( k ) )call stdlib_I64_yswap( nrhs, b( k+1, 1_ilp64 ), ldb, b( kp, 1_ilp64 ), ldb )
                           
                 k=k+2
              endif
             end do
        ! compute (l \p**t * b) -> b    [ (l \p**t * b) ]
             call stdlib_I64_ytrsm('L','L','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (l \p**t * b) ]
              i=1_ilp64
              do while ( i <= n )
                 if( ipiv(i) > 0_ilp64 ) then
        ...
        ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
             call stdlib_I64_ytrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k>1_ilp64 .and. kp==-ipiv( k-1 ) )call stdlib_I64_yswap( nrhs, b( k, 1_ilp64 ), ldb, b( kp, &
                           1_ilp64 ), ldb )
                 k=k-2
              endif
             end do
           ...
           ! revert a
           call stdlib_I64_ysyconv( uplo, 'R', n, a, lda, ipiv, work, iinfo )
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do k = n, 1, -1
                 kp = abs( ipiv( k ) )
                 if( kp/=k ) then
                    call stdlib_I64_yswap( nrhs, b( k, 1_ilp64 ), ldb, b( kp, 1_ilp64 ), ldb )
                 end if
              end do
              ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
              call stdlib_I64_ytrsm( 'L', 'U', 'N', 'U', n, nrhs, cone, a, lda, b, ldb )
     ...
     !! a hermitian matrix.
        ...
        ! =====================================================================
           ...
           if (upper) then
               ...
               ! first swap
                ! - swap column i1 and i2 from i1 to i1-1
              call stdlib_I64_yswap( i1-1, a(1_ilp64,i1), 1_ilp64, a(1_ilp64,i2), 1_ilp64 )
                ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap row i1 from i1+1 to i2-1 with col i2 from i1+1 to i2-1
                ! - swap a(i2,i1) and a(i1,i2)
              ...
              do i=i2+1,n
               ...
               ! first swap
                ! - swap row i1 and i2 from 1 to i1-1
              call stdlib_I64_yswap ( i1-1, a(i1,1_ilp64), lda, a(i2,1_ilp64), lda )
               ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap col i1 from i1+1 to i2-1 with row i2 from i1+1 to i2-1
                ! - swap a(i2,i1) and a(i1,i2)
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp64 ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_I64_yhptrs( uplo, n, 1_ilp64, ap, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! 1-by-1 and 2-by-2 diagonal blocks.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! column k, and colmax is its absolute value
              if( k>1_ilp64 ) then
                 imax = stdlib_I64_iyamax( k-1, ap( kc ), 1_ilp64 )
                 colmax = cabs1( ap( kc+imax-1 ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = ( imax-1 )*imax / 2_ilp64 + 1_ilp64
                    if( imax>1_ilp64 ) then
                       jmax = stdlib_I64_iyamax( imax-1, ap( kpc ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_I64_yswap( kp-1, ap( knc ), 1_ilp64, ap( kpc ), 1_ilp64 )
                    kx = kpc + kp - 1_ilp64
                    do j = kp + 1, kk - 1
                       kx = kx + j - 1_ilp64
                       t = conjg( ap( knc+j-1 ) )
                    ...
                    ap( kc+k-1 ) = real( ap( kc+k-1 ),KIND=xdp)
                    if( kstep==2_ilp64 )ap( kc-1 ) = real( ap( kc-1 ),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp64 ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one / real( ap( kc+k-1 ),KIND=xdp)
                    call stdlib_I64_yhpr( uplo, k-1, -r1, ap( kc ), 1_ilp64, ap )
                    ! store u(k) in column k
                    call stdlib_I64_ydscal( k-1, r1, ap( kc ), 1_ilp64 )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if( k>2_ilp64 ) then
                       d = stdlib_I64_xlapy2( real( ap( k-1+( k-1 )*k / 2_ilp64 ),KIND=xdp),aimag( ap( k-1+( &
                                 k-1 )*k / 2_ilp64 ) ) )
                       d22 = real( ap( k-1+( k-2 )*( k-1 ) / 2_ilp64 ),KIND=xdp) / d
                       d11 = real( ap( k+( k-1 )*k / 2_ilp64 ),KIND=xdp) / d
                       tt = one / ( d11*d22-one )
                       d12 = ap( k-1+( k-1 )*k / 2_ilp64 ) / d
                       d = tt / d
                       do j = k - 2, 1, -1
                          ...
                          ap( j+( k-1 )*k / 2_ilp64 ) = wk
                          ap( j+( k-2 )*( k-1 ) / 2_ilp64 ) = wkm1
                          ap( j+( j-1 )*j / 2_ilp64 ) = cmplx( real( ap( j+( j-1 )*j / 2_ilp64 ),KIND=xdp), &
                                    zero,KIND=xdp)
                       end do
                    end if
                 end if
              ...
              ! column k, and colmax is its absolute value
              if( k<n ) then
                 imax = k + stdlib_I64_iyamax( n-k, ap( kc+1 ), 1_ilp64 )
                 colmax = cabs1( ap( kc+imax-k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = npp - ( n-imax+1 )*( n-imax+2 ) / 2_ilp64 + 1_ilp64
                    if( imax<n ) then
                       jmax = imax + stdlib_I64_iyamax( n-imax, ap( kpc+1 ), 1_ilp64 )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_I64_yswap( n-kp, ap( knc+kp-kk+1 ), 1_ilp64, ap( kpc+1 ),1_ilp64 )
                    ...
                    ap( kc ) = real( ap( kc ),KIND=xdp)
                    if( kstep==2_ilp64 )ap( knc ) = real( ap( knc ),KIND=xdp)
                 end if
                 ! update the trailing submatrix
                 if( kstep==1_ilp64 ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one / real( ap( kc ),KIND=xdp)
                       call stdlib_I64_yhpr( uplo, n-k, -r1, ap( kc+1 ), 1_ilp64,ap( kc+n-k+1 ) )
                       ! store l(k) in column k
                       call stdlib_I64_ydscal( n-k, r1, ap( kc+1 ), 1_ilp64 )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if( k<n-1 ) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = stdlib_I64_xlapy2( real( ap( k+1+( k-1 )*( 2_ilp64*n-k ) / 2_ilp64 ),KIND=xdp),aimag( &
                                 ap( k+1+( k-1 )*( 2_ilp64*n-k ) / 2_ilp64 ) ) )
                       d11 = real( ap( k+1+k*( 2_ilp64*n-k-1 ) / 2_ilp64 ),KIND=xdp) / d
                       d22 = real( ap( k+( k-1 )*( 2_ilp64*n-k ) / 2_ilp64 ),KIND=xdp) / d
                       tt = one / ( d11*d22-one )
                       d21 = ap( k+1+( k-1 )*( 2_ilp64*n-k ) / 2_ilp64 ) / d
                       d = tt / d
                       do j = k + 2, n
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_I64_ygeru( k-1, nrhs, -cone, ap( kc ), 1_ilp64, b( k, 1_ilp64 ), ldb,b( 1_ilp64, 1_ilp64 ), ldb )
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_I64_ygeru( k-2, nrhs, -cone, ap( kc ), 1_ilp64, b( k, 1_ilp64 ), ldb,b( 1_ilp64, 1_ilp64 ), ldb )
                           
                 call stdlib_I64_ygeru( k-2, nrhs, -cone, ap( kc-( k-1 ) ), 1_ilp64,b( k-1, 1_ilp64 ), ldb, b( 1_ilp64, &
                           1_ilp64 ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+k-2 )
                 akm1 = ap( kc-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 if( k>1_ilp64 ) then
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc ), &
                              1_ilp64, cone, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp64 ) then
                    ...
                    call stdlib_I64_ylacgv( nrhs, b( k, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k+1, 1_ilp64 ), ldb )
                    call stdlib_I64_ygemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc+k ),&
                               1_ilp64, cone, b( k+1, 1_ilp64 ), ldb )
                    call stdlib_I64_ylacgv( nrhs, b( k+1, 1_ilp64 ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_I64_ygeru( n-k, nrhs, -cone, ap( kc+1 ), 1_ilp64, b( k, 1_ilp64 ),ldb, b( k+&
                           1_ilp64, 1_ilp64 ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 s = real( cone,KIND=xdp) / real( ap( kc ),KIND=xdp)
                 call stdlib_I64_ydscal( nrhs, s, b( k, 1_ilp64 ), ldb )
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_I64_ygeru( n-k-1, nrhs, -cone, ap( kc+2 ), 1_ilp64, b( k, 1_ilp64 ),ldb, b( k+2, &
                              1_ilp64 ), ldb )
                    call stdlib_I64_ygeru( n-k-1, nrhs, -cone, ap( kc+n-k+2 ), 1_ilp64,b( k+1, 1_ilp64 ), ldb, b( &
                              k+2, 1_ilp64 ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+1 )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp64 ) then
                    call stdlib_I64_ycopy( k-1, ap( kc ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhpmv( uplo, k-1, -cone, ap, work, 1_ilp64, czero,ap( kc ), 1_ilp64 )
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_I64_ydotc( k-1, work, 1_ilp64, ap( kc ), 1_ilp64 ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp64
              else
                 ...
                 if( k>1_ilp64 ) then
                    ...
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_I64_ydotc( k-1, work, 1_ilp64, ap( kc ), 1_ilp64 ),&
                              KIND=xdp)
                    ap( kcnext+k-1 ) = ap( kcnext+k-1 ) -stdlib_I64_ydotc( k-1, ap( kc ), 1_ilp64, ap( &
                              kcnext ),1_ilp64 )
                    call stdlib_I64_ycopy( k-1, ap( kcnext ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhpmv( uplo, k-1, -cone, ap, work, 1_ilp64, czero,ap( kcnext ), 1_ilp64 )
                              
                    ap( kcnext+k ) = ap( kcnext+k ) -real( stdlib_I64_ydotc( k-1, work, 1_ilp64, ap( kcnext &
                              ),1_ilp64 ),KIND=xdp)
                 end if
                 kstep = 2_ilp64
                 kcnext = kcnext + k + 1_ilp64
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = ( kp-1 )*kp / 2_ilp64 + 1_ilp64
                 call stdlib_I64_yswap( kp-1, ap( kc ), 1_ilp64, ap( kpc ), 1_ilp64 )
                 kx = kpc + kp - 1_ilp64
                 do j = kp + 1, k - 1
                    kx = kx + j - 1_ilp64
                    temp = conjg( ap( kc+j-1 ) )
              ...
              if( ipiv( k )>0_ilp64 ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_I64_ycopy( n-k, ap( kc+1 ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhpmv( uplo, n-k, -cone, ap( kc+n-k+1 ), work, 1_ilp64,czero, ap( kc+1 )&
                              , 1_ilp64 )
                    ap( kc ) = ap( kc ) - real( stdlib_I64_ydotc( n-k, work, 1_ilp64,ap( kc+1 ), 1_ilp64 ),&
                              KIND=xdp)
                 end if
                 kstep = 1_ilp64
              else
                 ...
                 if( k<n ) then
                    call stdlib_I64_ycopy( n-k, ap( kc+1 ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp64, czero, ap( &
                              kc+1 ), 1_ilp64 )
                    ap( kc ) = ap( kc ) - real( stdlib_I64_ydotc( n-k, work, 1_ilp64,ap( kc+1 ), 1_ilp64 ),&
                              KIND=xdp)
                    ap( kcnext+1 ) = ap( kcnext+1 ) -stdlib_I64_ydotc( n-k, ap( kc+1 ), 1_ilp64,ap( kcnext+&
                              2_ilp64 ), 1_ilp64 )
                    call stdlib_I64_ycopy( n-k, ap( kcnext+2 ), 1_ilp64, work, 1_ilp64 )
                    call stdlib_I64_yhpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp64, czero, ap( &
                              kcnext+2 ), 1_ilp64 )
                    ap( kcnext ) = ap( kcnext ) -real( stdlib_I64_ydotc( n-k, work, 1_ilp64, ap( kcnext+2 ),&
                              1_ilp64 ),KIND=xdp)
                 end if
                 kstep = 2_ilp64
                 kcnext = kcnext - ( n-k+3 )
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - ( n-kp+1 )*( n-kp+2 ) / 2_ilp64 + 1_ilp64
                 if( kp<n )call stdlib_I64_yswap( n-kp, ap( kc+kp-k+1 ), 1_ilp64, ap( kpc+1 ), 1_ilp64 )
                 kx = kc + kp - k
                 do j = k + 1, kp - 1
                    kx = kx + n - j + 1_ilp64
                    temp = conjg( ap( kc+j-k ) )
