  ...
  contains
     ...
     !! ZAXPY: constant times a vector plus a vector.
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i, ix, iy
           if (n<=0) return
           if (stdlib_cabs1(za)==0.0_xdp) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
                 zy(i) = zy(i) + za*zx(i)
     ...
     !! defined in a similar way using INCY.
        ...
        ! =====================================================================
           ...
           ! code for equal, positive, non-unit increments.
              ns = n*incx
              do i = 1,ns,incx
                 stdlib_xsdot = stdlib_xsdot + real(sx(i),KIND=xdp)*real(sy(i),KIND=xdp)
              end do
           else
           ! code for unequal or nonpositive increments.
              kx = 1
              ...
              if (incy<0) ky = 1 + (1-n)*incy
              do i = 1,n
                 stdlib_xsdot = stdlib_xsdot + real(sx(kx),KIND=xdp)*real(sy(ky),KIND=xdp)
                 kx = kx + incx
                 ky = ky + incy
              end do
           end if
     ...
     !! ZDOTC = X^H * Y
        ...
        ! =====================================================================
           ...
           ! Intrinsic Functions 
           intrinsic :: conjg
           ztemp = (0.0_xdp,0.0_xdp)
           stdlib_ydotc = (0.0_xdp,0.0_xdp)
           if (n<=0) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
     ...
     !! See QROTMG for a description of data storage in DPARAM.   
        ...
        ! =====================================================================
           ! Local Scalars 
           real(xdp) :: dflag, dh11, dh12, dh21, dh22, w, z
           integer(ilp) :: i, kx, ky, nsteps
           ! Data Statements 
           dflag = dparam(1)
           if (n<=0 .or. (dflag+two==zero)) return
     ...
     !! used for testing the size of DD1 and DD2. All actual scaling of data is done using GAM.       
        ...
        ! =====================================================================
           ! Local Scalars 
           real(xdp) :: dflag, dh11, dh12, dh21, dh22, dp1, dp2, dq1, dq2, dtemp, du, gam, gamsq, &
                     rgamsq
           ! Intrinsic Functions 
           intrinsic :: abs
           ! Data Statements 
     ...
     !! ZDSCAL: scales a vector by a constant.
        ...
        ! =====================================================================
           ...
           if (incx==1) then
              ! code for increment equal to 1
              do i = 1,n
                 zx(i) = cmplx(da,0.0_xdp,KIND=xdp)*zx(i)
              end do
           else
              ! code for increment not equal to 1
              nincx = n*incx
     ...
     !! defined in a similar way using INCY.
        ...
        ! =====================================================================
           ...
           ! Intrinsic Functions 
           intrinsic :: real
           stdlib_I64_xsdot = zero
           if (n<=0) return
           if (incx==incy .and. incx>0) then
           ! code for equal, positive, non-unit increments.
              ns = n*incx
              do i = 1,ns,incx
                 stdlib_I64_xsdot = stdlib_I64_xsdot + real(sx(i),KIND=xdp)*real(sy(i),KIND=xdp)
              end do
           else
           ! code for unequal or nonpositive increments.
              kx = 1
              ...
              if (incy<0) ky = 1 + (1-n)*incy
              do i = 1,n
                 stdlib_I64_xsdot = stdlib_I64_xsdot + real(sx(kx),KIND=xdp)*real(sy(ky),KIND=xdp)
                 kx = kx + incx
                 ky = ky + incy
              end do
           end if
     ...
     !! defined in a similar way using INCY.
        ...
        ! =====================================================================
           ...
           ! Intrinsic Functions 
           intrinsic :: real
           stdlib_I64_qsdot = zero
           if (n<=0) return
           if (incx==incy .and. incx>0) then
           ! code for equal, positive, non-unit increments.
              ns = n*incx
              do i = 1,ns,incx
                 stdlib_I64_qsdot = stdlib_I64_qsdot + real(sx(i),KIND=qp)*real(sy(i),KIND=qp)
              end do
           else
           ! code for unequal or nonpositive increments.
              kx = 1
              ...
              if (incy<0) ky = 1 + (1-n)*incy
              do i = 1,n
                 stdlib_I64_qsdot = stdlib_I64_qsdot + real(sx(kx),KIND=qp)*real(sy(ky),KIND=qp)
                 kx = kx + incx
                 ky = ky + incy
              end do
           end if
     ...
     !! ZDOTC = X^H * Y
        ...
        ! =====================================================================
           ...
           intrinsic :: conjg
           ztemp = (0.0_xdp,0.0_xdp)
           stdlib_I64_ydotc = (0.0_xdp,0.0_xdp)
           if (n<=0) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
     ...
     !! ZDOTC = X^H * Y
        ...
        ! =====================================================================
           ...
           intrinsic :: conjg
           ztemp = (0.0_qp,0.0_qp)
           stdlib_I64_wdotc = (0.0_qp,0.0_qp)
           if (n<=0) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
     ...
     !! ZDOTU = X^T * Y
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: i, ix, iy
           ztemp = (0.0_xdp,0.0_xdp)
           stdlib_I64_ydotu = (0.0_xdp,0.0_xdp)
           if (n<=0) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
     ...
     !! ZDOTU = X^T * Y
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: i, ix, iy
           ztemp = (0.0_qp,0.0_qp)
           stdlib_I64_wdotu = (0.0_qp,0.0_qp)
           if (n<=0) return
           if (incx==1 .and. incy==1) then
              ! code for both increments equal to 1
              do i = 1,n
     ...
     logical(lk) :: notbig
        ...
        stdlib_I64_xnrm2 = scl*sqrt( sumsq )
        return
     ...
     logical(lk) :: notbig
        ...
        stdlib_I64_qnrm2 = scl*sqrt( sumsq )
        return
     ...
     !! DZNRM2 := sqrt( x**H*x )
        ...
        stdlib_I64_xznrm2 = scl*sqrt( sumsq )
        return
     ...
     !! DZNRM2 := sqrt( x**H*x )
        ...
        stdlib_I64_qznrm2 = scl*sqrt( sumsq )
        return
