...
submodule (stdlib_quadrature) stdlib_quadrature_simps
    ...
    interface simps38
        ...
        module procedure simps38_dx_xdp
        module procedure simps38_x_xdp
    ...
    interface simps38_weights
        ...
        module procedure simps38_weights_xdp
...
contains
    ...
    pure recursive module function simps_dx_xdp(y, dx, even) result(integral)
        ...
        case (0:1)
            integral = 0.0_xdp
        case (2)
            integral = 0.5_xdp*dx*(y(1) + y(2))
        case (3)
            integral = dx/3.0_xdp*(y(1) + 4*y(2) + y(3))
        case (4)
            integral = simps38(y, dx)
        ! case (5) not needed; handled by default
        case (6) ! needs special handling because of averaged 3/8's rule case
            ...
            ! either `even` not present or is zero
            ! equivalent to averaging left and right
            integral = dx/48.0_xdp * (17*(y(1) + y(6)) + 59*(y(2) + y(5)) + 44*(y(3) + y(4)))
        case default
            if (mod(n, 2) == 1) then
                integral = dx/3.0_xdp*(y(1) + 4*sum(y(2:n-1:2)) + 2*sum(y(3:n-2:2)) + y(n))
            else
                if (present(even)) then
                    if (even < 0) then
                        ! 3/8th rule on left
                ...
                ! either `even` not present or is zero
                ! equivalent to averaging left and right
                integral = dx/48.0_xdp * (17*(y(1) + y(n)) + 59*(y(2) + y(n-1)) &
                        + 43*(y(3) + y(n-2)) + 49*(y(4) + y(n-3)) + 48*sum(y(5:n-4)))
            end if
        end select
    ...
    recursive module function simps_x_xdp(y, x, even) result(integral)
        ...
        case (0:1)
            integral = 0.0_xdp
        case (2)
            integral = 0.5_xdp*(x(2) - x(1))*(y(1) + y(2))
        case (3)
            h1 = x(2) - x(1)
            h2 = x(3) - x(2)
            a = (2*h1**2 + h1*h2 - h2**2)/(6*h1)
        ...
        case default
            if (mod(n, 2) == 1) then
                ...
                ! either `even` not present or is zero
                integral = 0.5_xdp * ( simps38(y(1:4), x(1:4)) + simps(y(4:n), x(4:n)) &
                    + simps(y(1:n-3), x(1:n-3)) + simps38(y(n-3:n), x(n-3:n)) )
            end if
        end select
    ...
    pure function simps38_dx_xdp(y, dx) result (integral)
        real(xdp), dimension(4), intent(in) :: y
        real(xdp), intent(in) :: dx
        real(xdp) :: integral

        integral = 3.0_xdp*dx/8.0_xdp * (y(1) + y(4) + 3*(y(2) + y(3)))
    end function simps38_dx_xdp
    ...
    pure function simps38_x_xdp(y, x) result(integral)
        real(xdp), dimension(4), intent(in) :: y
        real(xdp), dimension(4), intent(in) :: x
        real(xdp) :: integral
    ...
    pure function simps38_weights_xdp(x) result(w)
        real(xdp), intent(in) :: x(4)
        real(xdp) :: w(size(x))
        ...
        w(3) = (h2+h3-h1)*(h1+h2+h3)**3/(12*h2*h3*(h1+h2))
        w(4) = (h1+h2+h3)*(3*h3**2 + 2*h2*h3 - 2*h1*h3 - h2**2 + h1**2)/(12*h3*(h2+h3))
    end function simps38_weights_xdp
