...
!! Compute the (Moore-Penrose) pseudo-inverse of a matrix.    
     ...
     module subroutine stdlib_linalg_pseudoinvert_x(a,pinva,rtol,err)
         ...
         type(linalg_state_type) :: err0
         integer(ilp) :: m,n,k,i,j
         real(xdp), parameter :: alpha = 1.0_xdp, beta = 0.0_xdp         
         character, parameter :: H =  'T' 
         ...
         !> Discard singular values
         cutoff = tolerance*maxval(s)
         s = merge(1.0_xdp/s,0.0_xdp,s>cutoff)
     ...
     module function stdlib_linalg_pseudoinverse_x(a,rtol,err) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_x(ap,pinva,rtol,err)
     ...
     module function stdlib_linalg_pinv_x_operator(a) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_x(ap,pinva,err=err)

         if (err%error()) then 
         pinva = ieee_value(1.0_xdp,ieee_quiet_nan)
         endif
     ...
     module subroutine stdlib_linalg_pseudoinvert_y(a,pinva,rtol,err)
         ...
         type(linalg_state_type) :: err0
         integer(ilp) :: m,n,k,i,j
         complex(xdp), parameter :: alpha = 1.0_xdp, beta = 0.0_xdp         
         character, parameter :: H =  'C' 
     ...
     module function stdlib_linalg_pseudoinverse_y(a,rtol,err) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_y(ap,pinva,rtol,err)
     ...
     module function stdlib_linalg_pinv_y_operator(a) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_y(ap,pinva,err=err)

         if (err%error()) then 
         pinva = cmplx(ieee_value(1.0_xdp,ieee_quiet_nan), &
